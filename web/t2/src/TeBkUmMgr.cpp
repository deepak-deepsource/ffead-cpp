...
int TeBkUmMgrWorld::getId() const { return id; }

void TeBkUmMgrWorld::setId(int id) { this->id = id; }

int TeBkUmMgrWorld::getRandomNumber() const { return randomNumber; }
...
int TeBkUmMgrFortune::getId() const { return id; }

void TeBkUmMgrFortune::setId(int id) { this->id = id; }

const std::string &TeBkUmMgrFortune::getMessage() const { return message; }

void TeBkUmMgrFortune::setMessage(const std::string &message) {
  this->message = message;
}

TeBkUmMgrFortune::TeBkUmMgrFortune() { id = 0; }
...
const std::string &TeBkUmMgrMessage::getMessage() const { return message; }
...
void TeBkUmMgrRouter::queries(const char *q, int ql,
                              std::vector<TeBkUmMgrWorld> &wlst) {
  int queryCount = 0;
...
void TeBkUmMgrRouter::updates(const char *q, int ql,
                              std::vector<TeBkUmMgrWorld> &wlst) {
  int queryCount = 0;
  strToNum(q, ql, queryCount);
  if (queryCount < 1)
...
#ifdef INC_SDORM_MONGO
void TeBkUmMgrRouter::updateCacheUtil(void *ctx, int rn,
                                      std::vector<MgRawRes> &data) {
...
void TeBkUmMgrRouter::cachedWorlds(const char *q, int ql,
                                   std::vector<TeBkUmMgrWorld> &wlst) {
  int queryCount = 0;
  strToNum(q, ql, queryCount);
  if (queryCount < 1)
  ...
  try {
    ...
    for (int c = 0; c < queryCount; ++c) {
      ...
      int tmp1 = 0;
      CommonUtils::fastStrToNum(v.substr(fn + 1).c_str(), v.length() - fn - 1,
                                tmp1);
...
#ifdef INC_SDORM_MONGO
void TeBkUmMgrRouter::getContextUtil(void *ctx, int rn,
                                     std::vector<MgRawRes> &data) {
...
bool TeBkUmMgrRouter::route(HttpRequest *req, HttpResponse *res, Writer *sif) {
  ...
  if (StringUtil::endsWith(path, "/plaint")) {
    ...
    JSONSerialize::serializeObjectCont(&msg, wcont_ser, "vector",
                                       res->getContentP());
    res->setContentType(ContentTypes::CONTENT_TYPE_APPLICATION_JSON);
    res->setHTTPResponseStatus(HTTPResponseStatus::Ok);
...
TeBkUmMgrRouter::TeBkUmMgrRouter() {
  ...
  wcont_ser = Serializer::getSerFuncForObjectCont("t2", "TeBkUmMgrWorld",
                                                  "std::vector");
}
...
MongoDBRawDataSourceImpl *TeBkUmMgrRouter::getDb() {
  if (sqli == NULL) {
    sqli = static_cast<MongoDBRawDataSourceImpl *>(
        DataSourceManager::getRawImpl("MongoDB-DSN", "t2"));
  }
  return sqli;
}
#endif
