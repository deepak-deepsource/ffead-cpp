...
bool DefaultOAUTHController::service(HttpRequest *req, HttpResponse *res) {
  ...
  for (it = reqparams.begin(); it != reqparams.end(); it++) {
    if (it->first != "realm" && it->first != "oauth_signature" &&
        it->first != "Method") {
  ...
  if (flag) {
    ...
    char *resst = (char *)CryptoHandler::hmac_sha1((char *)allpars.c_str(),
                                                   (char *)key.c_str(), true);
  ...
  std::string signature =
      CryptoHandler::urlDecode(reqparams["oauth_signature"]);
  ...
  if (flag && resu == signature && signature != "" && resu != "") {
    ...
    if (tokk == "" && req->getFile() == "request.oauth") {
      ...
      oauthsec = CastUtil::lexical_cast<std::string>(rand() % 1000 + 123453) +
                 oauthtok +
                 CastUtil::lexical_cast<std::string>(rand() % 1000 + 12353);
      ...
      for (it = reqparams.begin(); it != reqparams.end(); it++) {
        if (it->first != "realm" &&
            it->first.find("oauth_") == std::string::npos &&
            it->first != "Method") {
      ...
      if (reqparams["oauth_callback"] != "") {
        ...
        std::string oauth_ver =
            oauthtok + CastUtil::lexical_cast<std::string>(rand() % 1000);
        wrf = CryptoHandler::urlDecode(reqparams["oauth_callback"]) +
              "?oauth_verifier=" + oauth_ver + "&" + parsc;
        ofs1.write(wrf.c_str(), wrf.length());
        ofs1.close();
      }
      std::string cont = ("oauth_token=" + oauthtok +
                          "&oauth_token_secret=" + oauthsec + "&") +
                         parsc;
    ...
    } else if (isreqtype && reqparams["oauth_token"] != "" &&
               reqparams["oauth_consumer_key"] != "" &&
      ...
      ofs.close();
      std::string cont =
          ("oauth_token=" + oauthtok + "&oauth_token_secret=" + oauthsec + "&");
      for (it = reqparams.begin(); it != reqparams.end(); it++) {
        if (it->first != "realm" &&
            it->first.find("oauth_") == std::string::npos &&
            it->first != "Method") {
    ...
    } else if (!isreqtype && req->getFile() == "getResource.oauth" &&
               reqparams["file"] != "") {
      std::cout << "resource access granted for " << reqparams["file"]
                << std::endl;
      res->setHTTPResponseStatus(HTTPResponseStatus::Ok);
      res->addHeaderValue(HttpResponse::ContentType,
                          ContentTypes::CONTENT_TYPE_TEXT_PLAIN);
    ...
    return true;
  } else if (req->getFile() == "login.oauth" && reqparams["username"] != "" &&
             reqparams["password"] != "" && reqparams["oauthparms"] != "") {
    ...
    if (!flag) {
      ...
      if (reqparams["oauthparms"] != "") {
        ...
        res->addHeaderValue(HttpResponse::Location,
                            CryptoHandler::urlDecode(reqparams["oauthparms"]) +
                                "&access=true");
        std::cout << "redirecting to callback url" << std::endl;
      } else {
        res->setContent(reqparams["oauthparms"] + "&access=true");
        std::cout << "no callback url specified sending access info in content"
                  << std::endl;
      }
    }
    return true;
    ...
    std::string html = "<html><head></head><body><form name=\"name1\" "
                       "method=\"POST\" action=\"/login.oauth\">";
