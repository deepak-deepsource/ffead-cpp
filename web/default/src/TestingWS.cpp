...
void TestingWS::wsmeth1(int a, std::string b, long c) {
  std::cout << ("WS Method wsmeth1 called with args - " +
                CastUtil::lexical_cast<std::string>(a) + " " + b + " " +
                CastUtil::lexical_cast<std::string>(c))
            << std::endl;
}
...
TestObject TestingWS::wsmeth3(std::string a) {
  ...
  obj.setK(-4);
  obj.setL(-13);
  std::cout << ("WS Method wsmeth3 called with args - " + obj.toString())
            << std::endl;
  return obj;
}
...
com::obj::TestObject TestingWS::wsmeth4(bool bol) {
  ...
  l.push_back("string");
  obj.setL(l);
  std::cout << ("WS Method wsmeth4 called with args - " + obj.toString())
            << std::endl;
  return obj;
}

std::string TestingWS::wsmeth5(TestObject obj) {
  std::cout << ("WS Method wsmeth5 called with args - " + obj.toString())
            << std::endl;
  return obj.toString();
}

long TestingWS::wsmeth6(com::obj::TestObject obj) {
  std::cout << ("WS Method wsmeth6 called with args - " + obj.toString())
            << std::endl;
  return 13;
}
} // namespace test
