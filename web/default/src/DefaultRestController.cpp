...
int DefaultRestController::addNumbers(int a, int b) {
  std::cout << "Processed input request inside DefaultRestController for "
               "addNumbers..."
            << std::endl;
  return a + b;
}
...
double DefaultRestController::power(int base, int exponent) {
  std::cout
      << "Processed input request inside DefaultRestController for power..."
      << std::endl;
...
std::vector<TestMany>
DefaultRestController::testVectorObject(std::vector<TestMany> param) {
  std::cout << "Processed input request inside DefaultRestController for "
               "testVectorObject..."
            << std::endl;
  return param;
...
std::string DefaultRestController::testUploadFile(std::ifstream *ifs,
                                                  std::string param) {
  std::string vals;
  unsigned int siz = 0;
...
std::string DefaultRestController::testUploadFileMulti1(std::ifstream *ifs1,
                                                        std::ifstream *ifs2,
                                                        std::ifstream *ifs3,
                                                        std::string param) {
  std::string vals;
  ...
  vals += "\nField value passed = " + param;
  std::cout << "Processed input request inside DefaultRestController for "
               "testUploadFileMulti1..."
            << std::endl;
  return vals;
}

std::string
DefaultRestController::testUploadFileMulti2(std::vector<std::ifstream *> vifs,
                                            std::string param) {
  std::string vals;
  for (int i = 0; i < (int)vifs.size(); ++i) {
    ...
    vals += "Uploaded File" + CastUtil::lexical_cast<std::string>(i) +
            " Size = " + CastUtil::lexical_cast<std::string>(siz) + "\n";
  }
  vals += "Field value passed = " + param;
  std::cout << "Processed input request inside DefaultRestController for "
               "testUploadFileMulti2..."
            << std::endl;
  return vals;
}

TestSTLs DefaultRestController::sertest(TestSTLs ts) { return ts; }
