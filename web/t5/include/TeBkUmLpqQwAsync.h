...
public:
  ...
  static void tmplJson(std::vector<TeBkUmLpqQwAsyncWorld> &vec,
                       std::string *ou) {
    for (auto el : vec) {
      size_t il = ou->length();
      ou->append(tJc.t);
      ...
      ou->insert(il + tJc.tpos.at(1) + ids.length(),
                 std::to_string(el.randomNumber));
    }
    ou->insert(0, "[");
    ou->at(ou->length() - 1) = ']';
  }
...
#endif
#ifdef HAVE_RAPID_JSON
  static void toJson(std::vector<TeBkUmLpqQwAsyncWorld> &vec,
...
public:
  ...
  TeBkUmLpqQwAsyncRouter(const TeBkUmLpqQwAsyncRouter &a) { sqli = NULL; }
  ...
  std::map<std::string, std::string> l(std::map<std::string, std::string> a1,
...
public:
  ...
  operator=(const TeBkUmLpqQwAsyncRouterPooled &a) {
    return *this;
  }
