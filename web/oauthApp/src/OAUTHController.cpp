...
bool OAUTHController::service(HttpRequest *req, HttpResponse *res) {
  ...
  if (req->getFile() == "login.auth") {
    ...
    data += "oauth_callback=http://" + hostp +
            "/oauthApp/"
            "calledback.auth&oauth_consumer_key=sumeet&oauth_nonce=&oauth_"
            "timestamp=0&oauth_token=&oauth_signature_method=HMAC-SHA1&oauth_"
            "version=1.0&tusername=" +
            reqParams["tusername"] + "&";
    std::string sig =
        "GET&" +
        CryptoHandler::urlEncode("http://" + hostp + "/request.oauth") + "&";
    sig += CryptoHandler::urlEncode(
        "oauth_callback=http://" + hostp +
        "/oauthApp/"
        "calledback.auth&oauth_consumer_key=sumeet&oauth_nonce=&oauth_"
        "signature_method=HMAC-SHA1&oauth_timestamp=0&oauth_token=&oauth_"
        "version=1.0&tusername=" +
        reqParams["tusername"]);

    std::cout << sig << std::endl;

    char *resst = (char *)CryptoHandler::hmac_sha1((char *)sig.c_str(),
                                                   (char *)"sumeet&", true);
    ...
    if (mapsd["oauth_token"] != "" && mapsd["oauth_token_secret"] != "" &&
        mapsd["tusername"] != "") {
      std::string filen = req->getCntxt_root() + "/" + reqParams["tusername"] +
                          "-tokens-secrets";
      std::ofstream ofs(filen.c_str());
      std::string wrf = mapsd["tusername"] + ":" + mapsd["oauth_token"] + ":" +
                        mapsd["oauth_token_secret"] + "\n";
    ...
    FileAuthController fauthu(req->getCntxt_root() + "/" +
                                  reqParams["tusername"] + "-tokens-secrets",
                              ":");
    ...
    if (reqParams["tusername"] != "" && flag && reqParams["access"] == "true") {
      ...
      data += "oauth_consumer_key=sumeet&oauth_nonce=&oauth_timestamp=0&oauth_"
              "token=" +
              tok +
              "&oauth_signature_method=HMAC-SHA1&oauth_version=1.0&tusername=" +
              reqParams["tusername"] + "&";
      std::string sig =
          "GET&" +
          CryptoHandler::urlEncode("http://" + hostp + "/access.oauth") + "&";
      sig += CryptoHandler::urlEncode(
          "oauth_consumer_key=sumeet&oauth_nonce=&oauth_signature_method=HMAC-"
          "SHA1&oauth_timestamp=0&oauth_token=" +
          tok + "&oauth_version=1.0&tusername=" + reqParams["tusername"]);
      ...
      if (mapsd["oauth_token"] != "" && mapsd["oauth_token_secret"] != "" &&
          mapsd["tusername"] != "") {
        std::string filen = req->getCntxt_root() + "/" +
                            reqParams["tusername"] + "-access-secrets";
        std::ofstream ofs(filen.c_str());
        std::string wrf = mapsd["tusername"] + ":" + mapsd["oauth_token"] +
                          ":" + mapsd["oauth_token_secret"] + "\n";
        ofs.write(wrf.c_str(), wrf.length());
        ...
        std::string conte =
            ...
            "src='public/json2.js'></script><script type='text/javascript' "
            "src='public/prototype.js'></script><script type='text/javascript' "
            "src='public/oauth.js'></script></head>";
        conte += "File Name: <input id='resource' type='text'/><input "
                 "type='submit' onclick='getResource(\"resource\",\"" +
                 reqParams["tusername"] + "\")'/></body>";
        conte += "</html>";
    ...
    if (reqParams["tusername"] != "" && flag && reqParams["file"] != "") {
      ...
      std::string sig =
          "GET&" +
          CryptoHandler::urlEncode("http://" + hostp + "/getResource.oauth") +
          "&";
      sig += CryptoHandler::urlEncode(
          "file=" + reqParams["file"] +
          "&oauth_consumer_key=sumeet&oauth_nonce=&oauth_signature_method=HMAC-"
