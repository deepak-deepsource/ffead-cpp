...
bool OAUTH2Controller::service(HttpRequest *req, HttpResponse *res) {
  if (req->getFile() == "login.auth2") {
    if (req->getRequestParams()["username"] != "" &&
        req->getRequestParams()["password"] != "") {
    ...
    data += "response_type=token&client_id=174968395858505&redirect_uri=" +
            CryptoHandler::urlEncode(
                "http://public_ip:port/oauthApp/calledback.auth2");
    ...
    if (req->getRequestParams()["access_token"] != "") {
      ...
      std::string conte =
          ...
          "src='public/oauth2.js'></script></head>";
      conte += "Resource: <input id='resource' type='text'/><input "
               "type='submit' onclick='getResource(\"resource\",\"" +
               req->getRequestParams()["tusername"] + "\")'/></body>";
      conte += "</html>";
    ...
    if (tokse != "" && req->getRequestParams()["error_status"] == "") {
      ...
      std::string data = "GET /me/" + req->getRequestParams()["resource"] +
                         "?access_token=" + tokse;
