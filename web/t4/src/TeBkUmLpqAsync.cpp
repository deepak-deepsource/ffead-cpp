...
TemplateJson TeBkUmLpqAsyncWorld::tJ =
    TemplateJson("{\"id\":,\"randomNumber\":}", {6, 22});
TemplateJson TeBkUmLpqAsyncWorld::tJc =
    TemplateJson("{\"id\":,\"randomNumber\":},", {6, 22});

int TeBkUmLpqAsyncWorld::getId() const { return id; }

void TeBkUmLpqAsyncWorld::setId(int id) { this->id = id; }

int TeBkUmLpqAsyncWorld::getRandomNumber() const { return randomNumber; }
...
int TeBkUmLpqAsyncFortune::getId() const { return id; }

void TeBkUmLpqAsyncFortune::setId(int id) { this->id = id; }
...
TeBkUmLpqAsyncFortune::TeBkUmLpqAsyncFortune(int id, const uint8_t *buf,
                                             size_t len) {
  this->id = id;
  this->message =
...
bool TeBkUmLpqAsyncFortune::operator<(
    const TeBkUmLpqAsyncFortune &other) const {
  return message.compare(other.message) < 0;
}

TemplateJson TeBkUmLpqAsyncMessage::tJ =
...
const std::string &TeBkUmLpqAsyncMessage::getMessage() const { return message; }
...
const std::string TeBkUmLpqAsyncRouter::WORLD_ONE_QUERY =
    "select id,randomnumber from world where id=$1";
const std::string TeBkUmLpqAsyncRouter::WORLD_ALL_QUERY =
    "select id,randomnumber from world";
const std::string TeBkUmLpqAsyncRouter::FORTUNE_ALL_QUERY =
...
void TeBkUmLpqAsyncRouter::dbAsync(Writer *sif) {
  ...
  q->withSelectQuery(WORLD_ONE_QUERY)
      ...
      .withCb0([](void *ctx, PGresult *res) {
        ...
        reqdt->reset();
        TeBkUmLpqAsyncWorld w(ntohl(*((uint32_t *)PQgetvalue(res, 0, 0))),
...
void TeBkUmLpqAsyncRouter::queriesMultiAsync(const char *q, int ql,
                                             Writer *sif) {
  int queryCount = 0;
  CommonUtils::naiveStrToNum(q, ql, queryCount);
  queryCount = std::max(1, std::min(queryCount, 500));
...
void TeBkUmLpqAsyncRouter::updatesMulti(const char *q, int ql,
                                        AsyncUpdatesReq *req) {
  int queryCount = 0;
  CommonUtils::naiveStrToNum(q, ql, queryCount);
  queryCount = std::max(1, std::min(queryCount, 500));
  ...
  areq->withFinalCb(req, [](void *ctx, bool status,
    ...
    if (status) {
      ...
      for (int i = 0; i < queryCount; ++i) {
        ...
        { TeBkUmLpqAsyncWorld& w = req->vec.back(); int newRandomNumber =
...
void TeBkUmLpqAsyncRouter::updatesAsyncb(const char *q, int ql,
                                         AsyncUpdatesReq *req) {
  int queryCount = 0;
  CommonUtils::naiveStrToNum(q, ql, queryCount);
  queryCount = std::max(1, std::min(queryCount, 500));
  ...
  areq->withFinalCb(req, [](void *ctx, bool status,
    ...
    for (int i = 0; i < queryCount; ++i) {
      ...
      PQgetvalue(res, 0, j))); TeBkUmLpqAsyncWorld& w = req->vec.back(); int
...
void TeBkUmLpqAsyncRouter::updatesAsync(const char *q, int ql,
                                        AsyncUpdatesReq *req) {
  int queryCount = 0;
  CommonUtils::naiveStrToNum(q, ql, queryCount);
  queryCount = std::max(1, std::min(queryCount, 500));
...
void TeBkUmLpqAsyncRouter::updateCache() {
  ...
  LibpqAsyncReq *areq = sqli->getAsyncRequest();
  LibpqQuery *q = areq->getQuery();
  q->withSelectQuery(WORLD_ALL_QUERY)
      .withContext(req)
      .withCb3([](void *ctx, bool endofdata, int row, int col, char *value) {
        ...
        if (endofdata) {
          ...
          try {
            for (std::vector<TeBkUmLpqAsyncWorld>::iterator it =
                     req->vec.begin();
                 it != req->vec.end(); ++it) {
              char str[12];
...
void TeBkUmLpqAsyncRouter::cachedWorlds(
    const char *q, int ql, std::vector<TeBkUmLpqAsyncWorld> &wlst) {
  int queryCount = 0;
  CommonUtils::naiveStrToNum(q, ql, queryCount);
  queryCount = std::max(1, std::min(queryCount, 500));
  ...
  q->withSelectQuery(FORTUNE_ALL_QUERY)
      ...
      .withCb0([](void *ctx, PGresult *res) {
        ...
        for (int i = 0; i < rows; i++) {
          flst.push_back(new TeBkUmLpqAsyncFortune(
        ...
        std::sort(
            flst.begin(), flst.end(),
            [](const TeBkUmLpqAsyncFortune *a, const TeBkUmLpqAsyncFortune *b) {
...
void TeBkUmLpqAsyncRouter::routeAsync(HttpRequest *req, HttpResponse *res,
                                      Writer *sif) {
  AsyncReqData *dbreq = (AsyncReqData *)sif->getData();
  route(req, &dbreq->r, sif);
}

bool TeBkUmLpqAsyncRouter::route(HttpRequest *req, HttpResponse *res,
                                 Writer *sif) {
  sif->use();
  if (StringUtil::endsWith(req->getPath(), "/plaint")) {
    ...
    fortunes(sif);
  } else if (StringUtil::endsWith(req->getPath(), "/bupdt") ||
             StringUtil::endsWith(req->getPath(), "/updt")) {
    struct yuarel_param params[1];
    yuarel_parse_query((char *)req->getQueryStr().data(),
                       req->getQueryStr().size(), params, 1);
    AsyncUpdatesReq *ar = new AsyncUpdatesReq;
...
TeBkUmLpqAsyncRouter::TeBkUmLpqAsyncRouter() {
  ...
  wcont_ser = Serializer::getSerFuncForObjectCont("t4", "TeBkUmLpqAsyncWorld",
                                                  "std::vector");
}
...
LibpqDataSourceImpl *TeBkUmLpqAsyncRouter::getDb(int max) {
  if (sqli == NULL) {
    sqli = static_cast<LibpqDataSourceImpl *>(
        DataSourceManager::getRawImpl("PostgreSQL-DSN", "t4"));
  }
  return sqli;
}
  ...
  if (inited) {
    ...
    for (int var = 0; var < maxconns; ++var) {
      pool.push_back(static_cast<LibpqDataSourceImpl *>(
          DataSourceManager::getRawImpl("PostgreSQL-DSN", "t4", true)));
    }
    inited = true;
  }
