...
int TeBkUmFortune::getId() const { return id; }

void TeBkUmFortune::setId(int id) { this->id = id; }

const std::string &TeBkUmFortune::getMessage() const { return message; }

void TeBkUmFortune::setMessage(const std::string &message) {
  this->message = message;
}

TeBkUmFortune::TeBkUmFortune() { id = 0; }
...
const std::string &TeBkUmMessage::getMessage() const { return message; }
...
void TeBkUmRouter::queries(const char *q, int ql,
...
void TeBkUmRouter::updates(const char *q, int ql,
                           std::vector<TeBkUmWorld> &wlst) {
  int queryCount = 0;
  strToNum(q, ql, queryCount);
  if (queryCount < 1)
...
void TeBkUmRouter::cachedWorlds(const char *q, int ql,
                                std::vector<TeBkUmWorld> &wlst) {
  int queryCount = 0;
  strToNum(q, ql, queryCount);
  if (queryCount < 1)
...
void TeBkUmRouter::getContext(HttpRequest *request, Context *context) {
  ...
  try {
    ...
    std::sort(flst->begin(), flst->end(),
              [](const TeBkUmFortune &a, const TeBkUmFortune &b) {
...
TeBkUmRouter::TeBkUmRouter() {
  ...
  wcont_ser =
      Serializer::getSerFuncForObjectCont("t1", "TeBkUmWorld", "std::vector");
}
