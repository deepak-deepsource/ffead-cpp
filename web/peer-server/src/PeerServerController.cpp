...
PeerState::PeerState(const std::string &id, const std::string &token,
                     BaseSocket *sif) {
  this->id = id;
  this->sif = sif;
  this->token = token;
...
PeerServerController::PeerServerController() {
  ...
  if (properties.find("isDistributedMode") != properties.end()) {
    isDistributedMode =
        StringUtil::toLowerCopy(properties.find("isDistributedMode")->second) ==
        "true";
  } else {
...
bool PeerServerController::onOpen(WebSocketData *wreq,
  ...
  if (req != NULL) {
    if (req->getMethod() == "POST" && req->getContent().length() > 0 &&
        req->getHeader(HttpRequest::ContentType).find("application/json") !=
            std::string::npos) {
      JSONElement el;
      ...
      if (tmp != NULL &&
          (token = StringUtil::trimCopy(tmp->getValue())) != "") {
      };
    }
    if (req->getMethod() == "POST" && req->getContent().length() > 0 &&
      ...
      int np = yuarel_parse_query((char *)req->getContent().c_str(),
                                  req->getQueryStr().size(), params, 20);
      for (int i = 0; i < np; i++) {
        if (params[i].key_len == 3 &&
            strncasecmp("key", params[i].key, 3) == 0) {
          key = std::string(params[i].val, params[i].val_len);
        } else if (params[i].key_len == 2 &&
                   strncasecmp("id", params[i].key, 2) == 0) {
          id = std::string(params[i].val, params[i].val_len);
        } else if (params[i].key_len == 5 &&
                   strncasecmp("token", params[i].key, 5) == 0) {
    ...
    if (id == "" || key == "" || token == "") {
      std::string qd =
          CryptoHandler::urlDecode(std::string(req->getQueryStr()));
      struct yuarel_param params[20];
  ...
  std::cout << "WS:Req:Open " << uniqueAddress << " " << id << " " << token
            << std::endl;
  return true;
}
...
void PeerServerController::onClose(std::string uniqueAddress) {
  ...
  if (clients.contains(uniqueAddress)) {
    ...
    std::cout << "WS:Req:Close " << uniqueAddress << " " << st->id << " "
              << st->token << std::endl;
  }
}

bool PeerServerController::onMessage(WebSocketData *req,
  ...
  if (req->getTextData().length() > 0) {
    ...
    if (type != "HEARTBEAT") {
      tmp = el.getNodeP("dst");
      if (tmp != NULL &&
          (dstId = StringUtil::trimCopy(tmp->getValue())) != "") {
      };
      ...
      el.add("src", id);
      std::cout << "WS:Req: " << type << ":For:" << dstId << " "
                << uniqueAddress << " " << st->id << " " << st->token
                << std::endl;
      ...
      if (clientIds.contains(dstId)) {
        ...
        std::cout << "WS:Writing: " << type << ":For:" << dstId << " "
                  << uniqueAddress << " " << st->id << " " << st->token
                  << std::endl;
      } else {
        CacheInterface *cchi = CacheManager::getImpl();
        try {
          cchi->addToQ(msgNodeIdentifier,
                       dstId + "|" + JSONUtil::getDocumentStr(el));
          std::cout << "WS:Queuing: " << type << ":For:" << dstId << " "
                    << uniqueAddress << " " << st->id << " " << st->token
                    << std::endl;
          CacheManager::cleanImpl(cchi);
        } catch (const std::exception &e) {
    ...
    try {
      ...
      while (true) {
        ...
        if (_i->clientIds.contains(dstId)) {
          ...
          std::cout << "WS:PeerHandler:Process:Writing "
                    << " to peer with id " << dstId << " " << uniqueAddress
                    << " " << st->id << " " << st->token << std::endl;
        } else {
          std::cout << "WS:PeerHandler:Process:NoPeerFouond "
                    << " with id " << dstId << std::endl;
        }
      }
...
bool PeerServerController::isWriteControl() { return true; }

bool PeerServerRouter::auth(std::string_view path, HttpResponse *res,
...
bool PeerServerRouter::isAuthorizedKey(const std::string &key,
                                       HttpResponse *res) {
  if (authKey != key) {
...
bool PeerServerRouter::isValidUser(const std::string &id,
...
bool PeerServerRouter::route(HttpRequest *req, HttpResponse *res, Writer *sif) {
  ...
  if (path == "/peer-server/") {
    res->setContent(
        "{\"name\":\"ffead-cpp peerjs server\",\"description\":\"SSE to broker "
        "connections between PeerJS "
        "clients.\",\"website\":\"http://peerjs.com/\"}");
    ...
    res->setContentType(ContentTypes::CONTENT_TYPE_APPLICATION_JSON);
    res->setHTTPResponseStatus(HTTPResponseStatus::Ok);
  } else if (StringUtil::endsWith(path, "/connect") ||
             StringUtil::endsWith(path, "/peerjs")) {
    std::string id, token, key;
    ...
    if (req->getMethod() == "POST" && req->getContent().length() > 0 &&
      ...
      int np = yuarel_parse_query((char *)req->getContent().c_str(),
                                  req->getQueryStr().size(), params, 20);
      for (int i = 0; i < np; i++) {
        if (params[1].key_len == 3 &&
            strncasecmp("key", params[1].key, 3) == 0) {
          key = std::string(params[i].val, params[i].val_len);
        } else if (params[1].key_len == 2 &&
                   strncasecmp("id", params[1].key, 2) == 0) {
          id = std::string(params[i].val, params[i].val_len);
        } else if (params[1].key_len == 5 &&
                   strncasecmp("token", params[1].key, 5) == 0) {
          token = std::string(params[i].val, params[i].val_len);
        }
      }
    }
