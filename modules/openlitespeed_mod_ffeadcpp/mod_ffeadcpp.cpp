...
lsi_config_key_t myParam[] = {
    ...
    {"param3", 0, 0},
    {NULL, 0,
     0} // The last position must have a NULL to indicate end of the array
};

// return 0 for correctly parsing
static int parseList(module_param_info_t *param,
  ...
  ls_objarray_t *pList =
      ls_confparser_line(&confparser, param->val, param->val + param->val_len);

  int count = ls_objarray_getsize(pList);
  assert(count > 0);

  // Comment: case param2, maxParamNum is 2, the line should be param2 [21
  // [22]],
  ...
  g_api->log(NULL, LSI_LOG_INFO,
             "[Module:mod_ffeadcpp] parseList param1 %s end\n",
...
static void *parseConfig(module_param_info_t *param, int param_count,
  ...
  int i;
  std::map<std::string, std::string> *pConfig =
      (std::map<std::string, std::string> *)_initial_config;
  if (!_initial_config) {
...
static void freeConfig(void *_config) { free(_config); }
/*
...
int mainInit(lsi_param_t *rec) {
  ...
  std::map<std::string, std::string> *paramMap =
      (std::map<std::string, std::string> *)g_api->get_config(NULL, &MNAME);
  ...
  ServerInitUtil::bootstrap(serverRootDirectory, logger,
                            SERVER_BACKEND::OPENLITESPEED);
  g_api->log(NULL, LSI_LOG_INFO,
             "[Module:mod_ffeadcpp] mainInit finalizing end\n");
  return write_log("LSI_HKPT_MAIN_INITED");
}

int mainPrefork(lsi_param_t *rec) { return write_log("LSI_HKPT_MAIN_PREFORK"); }
...
int mainExit(lsi_param_t *rec) { return write_log("LSI_HKPT_MAIN_ATEXIT"); }
/*
...
static int httpreqread(lsi_param_t *param) {
  ...
  std::string *mydata = (std::string *)g_api->get_module_data(
      param->session, &MNAME, LSI_DATA_HTTP);
  if (mydata == NULL)
    return LS_FAIL;

  while ((len = g_api->stream_read_next(param, tmpBuf, MAX_BLOCK_BUFSIZE)) >
         0) {
...
static void reqhdrcb(int key_id, const char *key, int key_len,
...
static void reqprmcb(int key_id, const char *key, int key_len,
                     const char *value, int val_len, void *arg) {
...
static int on_read(const lsi_session_t *session) {
  ...
  g_api->log(NULL, LSI_LOG_INFO,
             "[Module:mod_ffeadcpp] on_read content_length %d\n",
             content.length());
  ...
  g_api->log(NULL, LSI_LOG_INFO,
             "[Module:mod_ffeadcpp] on_read http_method %s\n", methods.c_str());
  ...
  HttpRequest *req = new (reqMem)
      HttpRequest(uri, (size_t)ulen, qstr, qlen, (const char *)methods.c_str(),
                  methods.length(), content, 1);

  const char *filt = NULL;
  g_api->foreach (session, LSI_DATA_REQ_HEADER, filt, reqhdrcb, req);

  g_api->log(NULL, LSI_LOG_INFO,
             "[Module:mod_ffeadcpp] on_read ffead-cpp handle start\n");
  HttpResponse respo;
  ServiceTask::handle(req, &respo);
  g_api->log(NULL, LSI_LOG_INFO,
             "[Module:mod_ffeadcpp] on_read ffead-cpp handle end\n");

  std::string furl = req->getUrl();
  if (respo.isDone()) {
    g_api->log(NULL, LSI_LOG_INFO,
               "[Module:mod_ffeadcpp] on_read ffead-cpp done received from "
               "framework\n");

    g_api->log(NULL, LSI_LOG_INFO,
               "[Module:mod_ffeadcpp] on_read set cookies start\n");
    for (int var = 0; var < (int)respo.getCookies().size(); var++) {
      g_api->set_resp_header(session, LSI_RSPHDR_SET_COOKIE, NULL, 0,
                             respo.getCookies().at(var).c_str(),
                             respo.getCookies().at(var).length(),
                             LSI_HEADEROP_SET);
    }
    g_api->log(NULL, LSI_LOG_INFO,
               "[Module:mod_ffeadcpp] on_read set cookies end\n");

    g_api->log(NULL, LSI_LOG_INFO,
               "[Module:mod_ffeadcpp] on_read set headers start\n");
    std::string &data = respo.generateNginxApacheResponse();
    RMap::const_iterator it;
    for (it = respo.getCHeaders().begin(); it != respo.getCHeaders().end();
         ++it) {
      g_api->set_resp_header(session, LSI_RSPHDR_UNKNOWN, it->first.c_str(),
                             it->first.length(), it->first.c_str(),
                             it->first.length(), LSI_HEADEROP_SET);
    }
    g_api->log(NULL, LSI_LOG_INFO,
               "[Module:mod_ffeadcpp] on_read set headers end\n");

    g_api->log(NULL, LSI_LOG_INFO,
               "[Module:mod_ffeadcpp] on_read set code/response start\n");
    /* set the status line */
    ...
    g_api->log(NULL, LSI_LOG_INFO,
               "[Module:mod_ffeadcpp] on_read set code/response end\n");
    ...
    g_api->log(NULL, LSI_LOG_INFO,
               "[Module:mod_ffeadcpp] on_read static file request %s\n",
               furl.c_str());
    struct stat sb;
    off_t off = 0;
    ...
    if (stat(furl.c_str(), &sb) == 0) {
      g_api->log(NULL, LSI_LOG_INFO,
                 "[Module:mod_ffeadcpp] on_read static file found\n");
      sz = sb.st_size;
    } else {
      g_api->log(NULL, LSI_LOG_INFO,
                 "[Module:mod_ffeadcpp] on_read static file not found\n");
    ...
    if (g_api->send_file(session, furl.c_str(), off, sz) < 0) {
      g_api->log(NULL, LSI_LOG_INFO,
                 "[Module:mod_ffeadcpp] on_read static file sendfile issues\n");
...
LSMODULE_EXPORT lsi_module_t MNAME = {
    LSI_MODULE_SIGNATURE, init_module, &myhandler, &myconfighanlder,
    "mod_ffeadcpp v1.3",  serverHooks, {0}};
}
/*
