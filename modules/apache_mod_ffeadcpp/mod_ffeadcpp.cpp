...
static void *create_modffeadcpp_config(apr_pool_t *p, server_rec *s) {
  ...
  newcfg = (ffead_cpp_module_config *)apr_pcalloc(
      p, sizeof(ffead_cpp_module_config));
...
const char *set_modffeadcpp_path(cmd_parms *parms, void *mconfig,
                                 const char *arg) {
  // ffead_cpp_module_config *s_cfg =
  // (ffead_cpp_module_config*)ap_get_module_config(
...
const char *set_modffeadcpp_defpath(cmd_parms *parms, void *mconfig,
                                    const char *arg) {
  // ffead_cpp_module_config *s_cfg =
  // (ffead_cpp_module_config*)ap_get_module_config(
  //		parms->server->module_config, &ffead_cpp_module);
...
static const command_rec mod_ffeadcpp_cmds[] = {
    AP_INIT_TAKE1("FFEAD_CPP_PATH", (cmd_func)set_modffeadcpp_path, NULL,
                  RSRC_CONF, "FFEAD_CPP_PATH, the path to the ffead-server"),
    AP_INIT_TAKE1("DocumentRoot", (cmd_func)set_modffeadcpp_defpath, NULL,
                  RSRC_CONF, "DocumentRoot"),
    {NULL}};
...
static int mod_ffeadcpp_method_handler(request_rec *r) {
  ...
  HttpRequest req((const char *)r->uri, strlen(r->uri), (const char *)r->args,
                  strlen(r->args), (const char *)r->method, strlen(r->method),
                  std::move(content), r->proto_num);
  ...
  for (int i = 0; i < fields->nelts; i++) {
    req.addNginxApacheHeader((const char *)e[i].key, strlen(e[i].key),
                             (const char *)e[i].val, strlen(e[i].val));
  }
  ...
  if (respo.isDone()) {
    ...
    char *ptr = apr_pstrdup(r->pool,
                            respo.getHeader(HttpResponse::ContentType).c_str());
    ...
    RegexUtil::replace(acurl, "[/]+", "/");
    // logger << "static file will be processed by apache " << req->getUrl() <<
    // " " << acurl << std::endl;
...
// module does not require any configuration most of the callback locations have
// been left blank, except for the last one - that one is invoked by the HTTPD
// core so that the module can declare other functions that should be invoked to
// handle various events (like an HTTP request).
...
static int mod_ffeadcp_post_config_hanlder(apr_pool_t *pconf, apr_pool_t *plog,
  ...
  // This code is used to prevent double initialization of the module during
  Apache startup apr_pool_userdata_get(&data, key, s->process->pool); if ( data
  == NULL ) { apr_pool_userdata_set((const void *)1, key, apr_pool_cleanup_null,
  s->process->pool); return OK;
  }*/
...
static void mod_ffeadcp_child_init(apr_pool_t *p, server_rec *s) {
  apr_pool_cleanup_register(p, NULL, mod_ffeadcp_child_uninit,
...
static void mod_ffeadcpp_register_hooks(apr_pool_t *p) {
  ...
  // ap_register_input_filter(myInputFilterName, myInputFilter,
  // NULL,AP_FTYPE_RESOURCE);
  /*ap_register_output_filter(myOutputFilterName, myOutputFilter, NULL,
   * AP_FTYPE_RESOURCE) ;*/
  ap_hook_post_config(mod_ffeadcp_post_config_hanlder, NULL, NULL,
                      APR_HOOK_MIDDLE);
...
// When this function is called by the HTTPD core, it registers a handler that
// should be invoked for all HTTP requests.
...
// Obviously an Apache module will require information about structures, macros
// and functions from Apache's core. These two header files are all that is
// required for this module, but real modules will need to include other header
// files relating to request handling, logging, protocols, etc.
