...
ngx_http_ffeadcpp_module_handler_post_read(ngx_http_request_t *r) {
  ...
  HttpRequest req(
      (const char *)r->uri.data, r->uri.len, (const char *)r->args.data,
      r->args.len, (const char *)r->main->method_name.data,
      r->main->method_name.len, std::move(content), r->http_version);
...
so loop through all of it
  ...
  for (i = 0; /* void */; i++) {
    ...
    req.addNginxApacheHeader((const char *)h[i].key.data, h[i].key.len,
  ...
  if (respo.isDone()) {
    for (int var = 0; var < (int)respo.getCookies().size(); var++) {
      set_custom_header_in_headers_out(r, std::string("Set-Cookie"),
    ...
    if (data.length() > 0) {
      ...
      r->headers_out.content_type.data = ngx_pcalloc(r->pool,
      contType.size()+1); if (r->headers_out.content_type.data == NULL) {
              ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
              return;
      }
      ngx_cpystrn(r->headers_out.content_type.data, contType.c_str(),
      contType.size()+1); r->headers_out.content_type.len = contType.size();*/
...
static ngx_int_t init_module(ngx_cycle_t *cycle) {
  ...
  serverRootDirectory.append((const char *)ffeadcpp_path.data,
                             ffeadcpp_path.len);
