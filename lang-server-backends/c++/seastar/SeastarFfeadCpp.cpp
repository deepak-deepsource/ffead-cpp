...
public:
  ...
  future<std::unique_ptr<reply>> handle(const sstring &path,
    ...
    fcpres = ffead_cpp_handle_c_1(&freq, &scode, &smsg, &smsg_len, &out_url,
                                  &out_url_len, out_headers, &out_headers_len,
                                  &out_body, &out_body_len);
    if (scode > 0) {
      for (int i = 0; i < (int)out_headers_len; i++) {
        rep->add_header(
            sstring(out_headers[i].name, out_headers[i].name_len),
            sstring(out_headers[i].value, out_headers[i].value_len));
      }
