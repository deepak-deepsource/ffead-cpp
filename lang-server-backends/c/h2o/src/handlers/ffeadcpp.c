...
static int ffeadcpp(struct st_h2o_handler_t *self, h2o_req_t *request) {
  ...
  tl_fres = ffead_cpp_handle_c_1(&freq, &scode, &smsg, &smsg_len, &out_url,
                                 &out_url_len, out_headers, &out_headers_len,
                                 &out_body, &out_body_len);
  if (scode > 0) {
    for (int i = 0; i < (int)out_headers_len; i++) {
      if (strncmp(out_headers[i].name, "Connection", 10) == 0 ||
          strncmp(out_headers[i].name, "Content-Length", 14) == 0) {
      } else {
        h2o_set_header_by_str(&request->pool, &request->res.headers,
                              out_headers[i].name, out_headers[i].name_len, 1,
                              out_headers[i].value, out_headers[i].value_len,
                              1);
      }
    }
