...
// For http2 use
// https://github.com/nghttp2/nghttp2/blob/master/examples/asio-sv2.cc For http3
// use https://github.com/litespeedtech/lsquic/blob/master/bin/http_server.c
...
static bool isSSLEnabled = false, isThreadprq = false,
            processforcekilled = false, processgendone = false,
            isCompileEnabled = false;
...
void *CHServer::dynamic_page_monitor(void *arg) {
  ...
  struct stat statbuf;
  std::map<std::string, std::string, std::less<>> dcpsss =
      ConfigurationData::getInstance()->dynamicCppPagesMap;
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 5) {
    ...
    servedAppNames = StringUtil::splitAndReturn<std::vector<std::string>>(
        servingAppNames, ",");
  }

  try {
    return CHServer::entryPoint(vhostNum, isMain, serverRootDirectory, port,
                                ipaddr, servedAppNames);
...
int CHServer::entryPoint(int vhostNum, bool isMain,
...
// int sockfd, new_fd;
  ...
  if (isMain) {
    ...
    serverCntrlFileNm =
        serverRootDirectory + "ffead.cntrl." + CastUtil::fromNumber(vhostNum);
  }
  ...
  RegexUtil::replace(serverRootDirectory, "[/]+", "/");
  ConfigurationData::getInstance()->coreServerProperties.serverRootDirectory =
      serverRootDirectory;
  ...
  ConfigurationData::getInstance()->coreServerProperties.sessionTimeout =
      sessionTimeout;
  ConfigurationData::getInstance()->coreServerProperties.sessatserv =
      sessatserv;
  ConfigurationData::getInstance()->coreServerProperties.isMainServerProcess =
      isMain;
  ConfigurationData::getInstance()->coreServerProperties.sessservdistocache =
      false;

  ConfigurationData::getInstance()->enableCors =
      StringUtil::toLowerCopy(srprps["ENABLE_CRS"]) == "true";
  ConfigurationData::getInstance()->enableSecurity =
      StringUtil::toLowerCopy(srprps["ENABLE_SEC"]) == "true";
  ConfigurationData::getInstance()->enableFilters =
      StringUtil::toLowerCopy(srprps["ENABLE_FLT"]) == "true";
  ConfigurationData::getInstance()->enableControllers =
      StringUtil::toLowerCopy(srprps["ENABLE_CNT"]) == "true";
  ConfigurationData::getInstance()->enableExtControllers =
      StringUtil::toLowerCopy(srprps["ENABLE_EXT_CNT"]) == "true";
  ConfigurationData::getInstance()->enableContMpg =
      StringUtil::toLowerCopy(srprps["ENABLE_CNT_MPG"]) == "true";
  ConfigurationData::getInstance()->enableContPath =
      StringUtil::toLowerCopy(srprps["ENABLE_CNT_PTH"]) == "true";
  ConfigurationData::getInstance()->enableContExt =
      StringUtil::toLowerCopy(srprps["ENABLE_CNT_EXT"]) == "true";
  ConfigurationData::getInstance()->enableContRst =
      StringUtil::toLowerCopy(srprps["ENABLE_CNT_RST"]) == "true";
  ConfigurationData::getInstance()->enableExtra =
      StringUtil::toLowerCopy(srprps["ENABLE_EXT"]) == "true";
  ConfigurationData::getInstance()->enableScripts =
      StringUtil::toLowerCopy(srprps["ENABLE_SCR"]) == "true";
  ConfigurationData::getInstance()->enableSoap =
      StringUtil::toLowerCopy(srprps["ENABLE_SWS"]) == "true";
  ConfigurationData::getInstance()->enableLogging =
      StringUtil::toLowerCopy(srprps["LOGGING_ENABLED"]) == "true";
  ConfigurationData::getInstance()->enableJobs =
      StringUtil::toLowerCopy(srprps["ENABLE_JOBS"]) == "true";
  ConfigurationData::getInstance()->enableStaticResponses =
      StringUtil::toLowerCopy(srprps["ENABLE_STATIC_RESP"]) == "true";
  ...
  SSLHandler::initInstance(
      ConfigurationData::getInstance()->securityProperties);
#endif
  ...
  if (isMain) {
    ...
    if (!libpresent) {
      ...
      if (access(configureFilePath.c_str(), F_OK) != -1) {
        ...
        compres =
            respath + "rundyn-automake_autoconf.sh " + serverRootDirectory;
    ...
    if (!libpresent) {
      ...
      std::string compres =
          respath + "rundyn-automake.sh " + serverRootDirectory + " cmake";
#elif defined(BUILD_XMAKE)
      std::string compres =
          respath + "rundyn-automake.sh " + serverRootDirectory + " xmake";
#elif defined(BUILD_MESON)
      std::string compres =
          respath + "rundyn-automake.sh " + serverRootDirectory + " meson";
#elif defined(BUILD_SCONS)
      std::string compres =
          respath + "rundyn-automake.sh " + serverRootDirectory + " scons";
#elif defined(BUILD_BAZEL)
      std::string compres =
          respath + "rundyn-automake.sh " + serverRootDirectory + " bazel";
#elif defined(BUILD_BUCK2)
      std::string compres =
          respath + "rundyn-automake.sh " + serverRootDirectory + " buck2";
#elif defined(BUILD_SHELLB)
      std::string compres =
          respath + "rundyn-automake.sh " + serverRootDirectory + " shellb";
#else
      logger << "Invalid Build Type specified, only cmake, xmake, meson, "
                "scons, shellb, bazel and buck2 supported...\n"
             << std::endl;
#endif
      std::string output = ScriptHandler::execute(compres, true);
      logger << "Intermediate code generation task\n\n" << std::endl;
    ...
    for (unsigned int var1 = 0;
         var1 < ConfigurationData::getInstance()->componentNames.size();
         var1++) {
      std::string name =
    ...
    /*#ifdef INC_DSTC
            ...
            try {
                    ...
                    {
                            distocachepoolsize =
    CastUtil::toInt(srprps["DISTOCACHE_POOL_SIZE"]);
                    }
            } catch(const std::exception& e) {
    ...
    std::endl;
            ...
            if(!distocache) {
                    logger << ("Session store is set to file store") <<
    std::endl;
            }
    #endif*/
  ...
  if (isMain) {
    ...
    if (StringUtil::toLowerCopy(srprps["PROC_PER_CORE"]) == "true" ||
        StringUtil::toLowerCopy(srprps["PROC_PER_CORE"]) == "yes") {
      ...
      for (int var = 0; var < (int)nthreads; ++var) {
        ...
        if (pid == 0) {
          ...
          std::string lnm =
              "CHServer(VHost-" + CastUtil::fromNumber(var + 1) + ")";
          serverCntrlFileNm = serverRootDirectory + "ffead.cntrl." +
...
#else
        std::string vhostcmd = "./vhost-server.sh " + serverRootDirectory +
                               " \"" + ipaddr + "\" " + port + " \"\" " +
                               CastUtil::fromNumber(var + 1);
        std::string vhostcmdo =
            ScriptHandler::chdirExecute(vhostcmd, serverRootDirectory, true);
      ...
      if (mpmap.find("VHOST_ENTRY") != mpmap.end() &&
          mpmap["VHOST_ENTRY"].size() > 0) {
        std::vector<std::string> vhosts = mpmap["VHOST_ENTRY"];
        for (int vhi = 0; vhi < (int)vhosts.size(); vhi++) {
          std::vector<std::string> vhostprops =
          ...
          if (vhostprops.size() == 3) {
            ...
            if (valid) {
              ...
              std::map<std::string, bool, std::less<>> updatedcontextNames =
                  ConfigurationData::getInstance()->servingContexts;
              std::map<std::string, std::string, std::less<>>
                  updatedaliasNames =
                      ConfigurationData::getInstance()->appAliases;
              if (vhostapps != "") {
                ...
                for (int spni = 0; spni < (int)spns.size(); ++spni) {
                  ...
                  if (vapnm != "" &&
                      ConfigurationData::getInstance()->servingContexts.find(
                          vapnm) != ConfigurationData::getInstance()
                                        ->servingContexts.end()) {
                ...
                for (ucit = updatedcontextNames.begin();
                     ucit != updatedcontextNames.end(); ucit++) {
                  if (ConfigurationData::getInstance()->servingContexts.find(
                          ucit->first) !=
                      ConfigurationData::getInstance()->servingContexts.end()) {
                    ConfigurationData::getInstance()->servingContexts.erase(
                        ucit->first);
                  }
                  if (ConfigurationData::getInstance()->appAliases.find(
                          ucit->first) !=
                      ConfigurationData::getInstance()->appAliases.end()) {
                    ConfigurationData::getInstance()->appAliases.erase(
                        ucit->first);
                  }
                }
              }
              ...
              if (pid == 0) {
                ...
                std::string lnm =
                    "CHServer(VHost-" + CastUtil::fromNumber(vhi + 1) + ")";
                serverCntrlFileNm = serverRootDirectory + "ffead.cntrl." +
                                    CastUtil::fromNumber(vhi + 1);
                serve(vhostport, vhostname, thrdpsiz, serverRootDirectory,
                      srprps, vhi + 1);
                exit(0);
              } else {
                serverCntrlFileNm = serverRootDirectory + "ffead.cntrl." +
...
#else
              std::string vhostcmd = "./vhost-server.sh " +
                                     serverRootDirectory + " " + vhostname +
                                     " " + vhostport + " " + vhostapps + " " +
                                     CastUtil::fromNumber(vhi + 1);
              std::string vhostcmdo = ScriptHandler::chdirExecute(
                  vhostcmd, serverRootDirectory, true);
              logger.info("Starting new Virtual-Host at " +
                          (vhostname + ":" + vhostport));
              logger << vhostcmdo << std::endl;
#endif
            }
        ...
        if (!startedOne) {
          logger << ("No valid Virtual-Hosts found, will run normal server")
                 << std::endl;
        }
      }
    }
...
void CHServer::serve(std::string port, std::string ipaddr, int thrdpsiz,
  ...
  if (ConfigurationData::getInstance()->servingContexts.size() == 0) {
    if (vhostNumber == 0) {
      logger << ("No context to be served by Server (" + ipport + ")")
             << std::endl;
    } else {
      logger << ("No context to be served by Virtual-Host (" + ipport + ")")
             << std::endl;
    }
    return;
  } else {
    if (vhostNumber == 0) {
      ...
      for (it = ConfigurationData::getInstance()->servingContexts.begin();
           it != ConfigurationData::getInstance()->servingContexts.end(); ++it)
        logger << ("Server (" + ipport + ") serves context (" + it->first + ")")
               << std::endl;
    } else {
      std::map<std::string, bool, std::less<>>::iterator it;
      for (it = ConfigurationData::getInstance()->servingContexts.begin();
           it != ConfigurationData::getInstance()->servingContexts.end(); ++it)
        logger << ("Virtual-Host (" + ipport + ") serves context (" +
                   it->first + ")")
               << std::endl;
    }
  }
...
#ifdef USE_IO_URING
  if (!isSinglEVH) {
    logger << "Unable to start server with IO_URING in thread-pooled mode, "
              "please set EVH_SINGLE (Single process mode to true)"
           << std::endl;
    return;
  }
#endif

  std::string cntEnc = StringUtil::toLowerCopy(
      ConfigurationData::getInstance()
          ->coreServerProperties.sprops["CONTENT_ENCODING"]);
  try {
    techunkSiz = CastUtil::toInt(
        ConfigurationData::getInstance()
            ->coreServerProperties.sprops["TRANSFER_ENCODING_CHUNK_SIZE"]);
  } catch (const std::exception &e) {
  }
  try {
    connKeepAlive = CastUtil::toInt(
        ConfigurationData::getInstance()
            ->coreServerProperties.sprops["KEEP_ALIVE_SECONDS"]);
  } catch (const std::exception &e) {
  }
  try {
    maxReqHdrCnt = CastUtil::toInt(
        ConfigurationData::getInstance()
            ->coreServerProperties.sprops["MAX_REQUEST_HEADERS_COUNT"]);
  } catch (const std::exception &e) {
  }
  try {
    maxEntitySize = CastUtil::toInt(
        ConfigurationData::getInstance()
            ->coreServerProperties.sprops["MAX_REQUEST_ENTITY_SIZE"]);
  } catch (const std::exception &e) {
  }

  std::string rHandler = "RequestReaderHandler";
  try {
    rHandler = ConfigurationData::getInstance()
                   ->coreServerProperties.sprops["REQUEST_HANDLER"];
  } catch (const std::exception &e) {
  }

  isrHandler1 = rHandler == "RequestReaderHandler";

  bool isLazyHeaderParsing =
      StringUtil::toLowerCopy(
          ConfigurationData::getInstance()
              ->coreServerProperties.sprops["LAZY_HEADER_PARSE"]) == "true";

  std::string qw = "false";
  try {
    qw = ConfigurationData::getInstance()
             ->coreServerProperties.sprops["QUEUED_WRITES"];
  } catch (const std::exception &e) {
  }
  ...
  ServiceHandler *handler =
      new HttpServiceHandler(cntEnc, &ServiceTask::handleWebsockOpen, nthreads,
                             isSinglEVH, &ServiceTask::handle);

  void *rHandleIns = NULL;

  if (isrHandler1) {
    RequestReaderHandler *reader =
        new RequestReaderHandler(handler, true, isSinglEVH);
    ...
    RequestHandler2 *reader =
        new RequestHandler2(handler, true, isSSLEnabled, &ServiceTask::handle);
  ...
  // socklen_t sin_size;
  SOCKET sockfd =
      Server::createListener(ipaddr, CastUtil::toInt(port), true, isSinglEVH);
  if (sockfd == -1) {
    logger << "Unable to start the server on the specified ip/port..."
           << std::endl;
    return;
  }
  ...
  bool enableJobs =
      StringUtil::toLowerCopy(
          ConfigurationData::getInstance()
              ->coreServerProperties.sprops["ENABLE_JOBS"]) == "true";
#ifdef INC_JOBS
  if (enableJobs) {
    JobScheduler::start();
  }
  ...
  if (isCompileEnabled) {
    Thread *pthread =
        new Thread(&CHServer::dynamic_page_monitor, &serverRootDirectory);
    pthread->execute();
  }
#endif
  ...
  // Sleep for some time so as to make sure all the new child processes are set
  // correctly and all init is complete...
  sleep(5);
  ...
  // std::string lg = "Memory allocations waiting to be freed = " +
  // CastUtil::fromNumber(ConfigurationData::counter); logger <<  lg <<
  // std::endl;
...
BaseSocket *CHServer::createSocketInterface(SOCKET fd) {
  if (!isrHandler1) {
    return new Http11Socket(fd, techunkSiz, connKeepAlive, maxReqHdrCnt,
                            maxEntitySize);
  }
#ifdef HAVE_SSLINC
  SSL *ssl = NULL;
  ...
  if (BaseSecureSocket::init(fd, ssl, io)) {
    ...
    return new Http11Handler(fd, ssl, io, techunkSiz, connKeepAlive,
                             maxReqHdrCnt, maxEntitySize);
  }
#else
  return new Http11Handler(fd, NULL, NULL, techunkSiz, connKeepAlive,
                           maxReqHdrCnt, maxEntitySize);
#endif
}
...
bool CHServer::doRegisterListenerFunc() {
  struct stat buffer;
  return stat(serverCntrlFileNm.c_str(), &buffer) == 0 &&
         CacheManager::isInitCompleted() &&
         DataSourceManager::isInitCompleted() &&
         SearchEngineManager::isInitCompleted();
}
