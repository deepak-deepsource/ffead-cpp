...
void Nghttp2Server::handle(const request &req2, const response &res2,
                           std::string_view meth, const uint8_t *data,
                           std::size_t len) {
  ...
  std::string_view url = std::string_view(req2.uri().raw_path.data(),
                                          req2.uri().raw_path.length());
  std::string_view qv = std::string_view(req2.uri().raw_query.data(),
                                         req2.uri().raw_query.length());
  ...
  if (respo.isDone()) {
    for (int var = 0; var < (int)respo.getCookies().size(); var++) {
      hm.emplace(HttpResponse::SetCookie,
                 header_value{value : respo.getCookies().at(var)});
    }
    ...
    if (access(req.getUrl().c_str(), F_OK) != -1) {
      ...
      if (stat(req.getUrl().c_str(), &stbuf) == 0) {
        header.emplace(HttpResponse::ContentLength,
                       header_value{std::to_string(stbuf.st_size)});
        header.emplace(HttpResponse::LastModified,
...
void Nghttp2Server::runServer(std::string ipaddr, std::string port,
  ...
  server.handle("/", [](const request &req2, const response &res2) {
    std::string_view meth =
        std::string_view(req2.method().data(), req2.method().length());
...
int main(int argc, char *argv[]) {
  ...
  ServerInitUtil::bootstrap(serverRootDirectory, logger,
                            SERVER_BACKEND::EMBEDDED_NGHTTP2);
  ServerInitUtil::init(logger);

  Nghttp2Server::runServer(ipaddr, port, servedAppNames, serverRootDirectory,
                           isSSLEnabled);
