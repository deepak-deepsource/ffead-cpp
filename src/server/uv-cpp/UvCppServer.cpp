...
void UvCppServer::handler(uv::http::Request &req2, uv::http::Response *res) {
  ...
  if (req2.getContent().length() > 0) {
    cont =
        std::string_view(req2.getContent().data(), req2.getContent().length());
  }
  phr_header_fcp headers[100];
  std::map<std::string, std::string>::const_iterator it;
  int hc = 0;
  for (it = req2.getHeaders().begin(); it != req2.getHeaders().end();
       ++it, hc++) {
  ...
  if (respo.isDone()) {
    for (int var = 0; var < (int)respo.getCookies().size(); var++) {
      res->appendHead(std::string(HttpResponse::SetCookie),
                      std::string(respo.getCookies().at(var)));
    }
    ...
    res->appendHead(std::string(HttpResponse::ContentLength),
                    std::to_string(respo.getContent().length()));
    res->swapContent(std::string(respo.getContent()));
    res->setStatus(uv::http::Response::StatusCode(respo.getCode()),
                   respo.getStatusMsg());
  } else {
    if (access(req.getUrl().c_str(), F_OK) != -1) {
      ...
      if (f) {
        ...
        res->appendHead(std::string(HttpResponse::ContentType),
                        std::string(CommonUtils::getMimeType(req.getExt())));
        std::string content = std::string(buffer, length);
        free(buffer);
        res->swapContent(std::move(content));
        res->setStatus(
            uv::http::Response::StatusCode(HTTPResponseStatus::Ok.getCode()),
            HTTPResponseStatus::Ok.getMsg());
      } else {
        res->setStatus(uv::http::Response::StatusCode(
                           HTTPResponseStatus::NotFound.getCode()),
                       HTTPResponseStatus::NotFound.getMsg());
      }
    } else {
      res->setStatus(uv::http::Response::StatusCode(
                         HTTPResponseStatus::NotFound.getCode()),
...
void UvCppServer::runServer(std::string ipaddr, int port,
                            std::vector<std::string> servedAppNames,
                            std::string serverRootDirectory,
                            bool isSSLEnabled) {
  uv::EventLoop loop;
  uv::http::HttpServer::SetBufferMode(
      uv::GlobalConfig::BufferMode::CycleBuffer);
...
int main(int argc, char *argv[]) {
  ...
  ServerInitUtil::bootstrap(serverRootDirectory, logger,
                            SERVER_BACKEND::UV_CPP);
  ServerInitUtil::init(logger);

  UvCppServer::runServer(ipaddr, CastUtil::toInt(port), servedAppNames,
                         serverRootDirectory, isSSLEnabled);
