...
libcuckoo::cuckoohash_map<std::string, CinatraSocketHandler *>
    CinatraServer::websocks;

void CinatraServer::runServer(std::string ip_addr, std::string port,
                              std::vector<std::string> servedAppNames) {
  std::function<void(request &, response &)> ffead_cpp_handler =
      [&](request &r, response &response) {
        if (r.get_content_type() == content_type::websocket) {
          r.on(ws_open, [](request &req) {
            ...
            HttpRequest reqf(hl.first, hl.second, req.raw_url(),
                             req.get_method(), req.minor_version(), req.body());
...
#ifdef CINATRA_ENABLE_SSL
            address =
                StringUtil::toHEX((long long)req.get_raw_conn<cinatra::SSL>());
#else
          ...
          HttpRequest req(hl.first, hl.second, r.raw_url(), r.get_method(),
                          r.minor_version(), r.body());
          HttpResponse respo;
          ...
          if (respo.isDone()) {
            ...
            for (it = respo.getCHeaders().begin();
                 it != respo.getCHeaders().end(); ++it) {
  ...
  server.set_http_handler<GET, PUT, POST, DEL, OPTIONS>("/*",
                                                        ffead_cpp_handler);
  server.run();
}
...
std::string CinatraSocketHandler::getProtocol(void *context) { return ""; }

int CinatraSocketHandler::getType(void *context) { return 0; }

int CinatraSocketHandler::getTimeout() { return -1; }

bool CinatraSocketHandler::readRequest(void *request, void *&context,
                                       int &pending, int &reqPos) {
  return false;
}

bool CinatraSocketHandler::writeResponse(void *req, void *res, void *context,
...
void CinatraSocketHandler::onClose() { h->onClose(getAddress()); }
...
CinatraSocketHandler::~CinatraSocketHandler() { delete h; }
...
std::string CinatraSocketHandler::getAddress() { return address; }
...
bool CinatraSocketHandler::isEmbedded() { return false; }
