...
void LithiumServer::runServer(int port,
                              std::vector<std::string> servedAppNames) {

  std::function<void(li::http_request &, li::http_response &)>
      ffead_cpp_handler = [&](li::http_request &request,
        ...
        HttpRequest req(request.http_ctx.header_map,
                        request.http_ctx.url_.substr(
                            0, request.http_ctx.url_.find_last_not_of(" ") + 1),
                        request.http_ctx.get_parameters_string_, meth,
                        request.http_ctx.http_version_, request.http_ctx.body_);
