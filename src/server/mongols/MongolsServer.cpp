...
void MongolsServer::runServer(std::string ipaddr, int port,
                              std::vector<std::string> servedAppNames,
                              std::string serverRootDirectory,
                              bool isSSLEnabled) {
  auto rqf = [](const mongols::request &) { return true; };
  auto rsf = [](const mongols::request &req2, mongols::response &res) {
    ...
    if (req2.form.find("__body__") != req2.form.end()) {
      cont = std::string_view(req2.form.find("__body__")->second.data(),
                              req2.form.find("__body__")->second.length());
    }
    ...
    std::string_view qv =
        std::string_view(req2.param.data(), req2.param.length());
    ...
    if (respo.isDone()) {
      for (int var = 0; var < (int)respo.getCookies().size(); var++) {
        res.headers.emplace(HttpResponse::SetCookie,
      ...
      if (access(req.getUrl().c_str(), F_OK) != -1) {
        ...
        if (f) {
          ...
          fclose(f);
          res.headers.emplace(HttpResponse::ContentLength,
                              std::to_string(length));
          res.headers.emplace(HttpResponse::ContentType,
...
int main(int argc, char *argv[]) {
  ...
  ServerInitUtil::bootstrap(serverRootDirectory, logger,
                            SERVER_BACKEND::MONGOLS);
  ServerInitUtil::init(logger);

  MongolsServer::runServer(ipaddr, CastUtil::toInt(port), servedAppNames,
