...
libcuckoo::cuckoohash_map<std::string, DrogonSocketHandler *>
    DrogonServer::websocks;

void DrogonWebsockHandler::handleNewMessage(const WebSocketConnectionPtr &wcon,
                                            std::string &&msg,
...
void DrogonWebsockHandler::handleNewConnection(
    const HttpRequestPtr &hreq, const WebSocketConnectionPtr &wcon) {
  ...
  ::HttpRequest req(request->bodyData(), request->bodyLength(),
                    request->getHeaders(), request->getPath(),
                    request->getQuery(), request->getMethodString(), hv);
  WebSocketData wreq;
  WebSocketRespponseData wres;
...
void DrogonWebsockHandler::handleConnectionClosed(
    const WebSocketConnectionPtr &wcon) {
  std::string address = StringUtil::toHEX((long long)wcon.get());
  if (DrogonServer::websocks.contains(address)) {
    DrogonSocketHandler *dsk = DrogonServer::websocks.find(address);
    dsk->onClose();
...
void DrogonHttpHandler::handle(
  ...
  if (respo.isDone()) {
    ...
    drogon::HttpStatusCode st =
        static_cast<drogon::HttpStatusCode>(respo.getCode());
...
void DrogonServer::runServer(std::string ip_addr, std::string port,
...
int main(int argc, char *argv[]) {
  ...
  std::vector<std::string> servedAppNames =
      StringUtil::splitAndReturn<std::vector<std::string>>(servingAppNames,
                                                           ",");
...
std::string DrogonSocketHandler::getProtocol(void *context) { return ""; }

int DrogonSocketHandler::getType(void *context) { return -1; }

int DrogonSocketHandler::getTimeout() { return -1; }

bool DrogonSocketHandler::readRequest(void *request, void *&context,
                                      int &pending, int &reqPos) {
  return false;
}

bool DrogonSocketHandler::writeResponse(void *req, void *res, void *context,
...
void DrogonSocketHandler::onClose() { h->onClose(getAddress()); }
...
void DrogonSocketHandler::sendResponse(std::string str, bool isText) {
  conn->send(str, isText ? WebSocketMessageType::Text
                         : WebSocketMessageType::Binary);
}

std::string DrogonSocketHandler::getAddress() { return address; }

bool DrogonSocketHandler::isClosed() { return conn->disconnected(); }
...
bool DrogonSocketHandler::isEmbedded() { return false; }
