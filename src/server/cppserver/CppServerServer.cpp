...
protected:
  void onReceivedRequest(const CppServer::HTTP::HTTPRequest &request) override {
    ...
    if (respo->isDone()) {
      ...
      for (int var = 0; var < (int)respo->getCookies().size(); var++) {
        cresp.SetHeader(std::string_view(HttpResponse::SetCookie),
                        std::string_view(respo->getCookies().at(var)));
      }
      RMap::const_iterator it;
      for (it = respo->getCHeaders().begin(); it != respo->getCHeaders().end();
           ++it) {
        cresp.SetHeader(std::string_view(it->first),
      ...
      if (access(req.getUrl().c_str(), F_OK) != -1) {
        ...
        if (f) {
          ...
          fclose(f);
          cresp.SetBegin(200);
          cresp.SetHeader(std::string_view(HttpResponse::ContentLength),
                          std::string_view(std::to_string(length)));
          cresp.SetHeader(
              std::string_view(HttpResponse::ContentType),
              std::string_view(CommonUtils::getMimeType(req.getExt())));
  ...
  void onReceivedRequestError(const CppServer::HTTP::HTTPRequest &request,
...
protected:
  std::shared_ptr<CppServer::Asio::TCPSession> CreateSession(
    ...
    return std::make_shared<CppIntSession>(
        std::dynamic_pointer_cast<CppServer::HTTP::HTTPServer>(server));
  }
...
void CppServerServer::runServer(std::string ipaddr, int port,
...
int main(int argc, char *argv[]) {
  ...
  ServerInitUtil::bootstrap(serverRootDirectory, logger,
                            SERVER_BACKEND::CPPSERVER);
  ServerInitUtil::init(logger);

  CppServerServer::runServer(ipaddr, CastUtil::toInt(port), servedAppNames,
