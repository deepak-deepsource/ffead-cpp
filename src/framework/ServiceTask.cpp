...
void ServiceTask::saveSessionDataToFile(const std::string &sessionId,
                                        const std::string &value) {
  std::string lockfil = ConfigurationData::getInstance()
                            ->coreServerProperties.serverRootDirectory +
                        "/tmp/" + sessionId + ".lck";
  ...
  std::string filen = ConfigurationData::getInstance()
                          ->coreServerProperties.serverRootDirectory +
                      "/tmp/" + sessionId + ".sess";
...
ServiceTask::getSessionDataFromFile(const std::string &sessionId) {
  ...
  for (int j = 0; j < (int)results.size() - 1; j++) {
    ...
    // logger << ("Read key/value pair " + results1.at(0) + " = " +
    // valss[results1.at(0)]) << std::endl;
  }
  return valss;
}

#ifdef INC_DSTC
void ServiceTask::saveSessionDataToDistocache(
    const std::string &sessionId,
    std::map<std::string, std::string> &sessAttrs) {
  DistGlobalCache globalMap;
  globalMap.addMap(sessionId, sessAttrs);
}

std::map<std::string, std::string>
ServiceTask::getSessionDataFromDistocache(const std::string &sessionId) {
...
void ServiceTask::storeSessionAttributes(HttpResponse *res, HttpRequest *req,
                                         const long &sessionTimeout,
                                         const bool &sessatserv) {
  bool sessionchanged = !req->hasCookie();
  sessionchanged |= req->getSession()->isDirty();

  if (sessionchanged) {
    std::map<std::string, std::string, std::less<>> vals =
        req->getSession()->getSessionAttributes();
    ...
    /*if(sessatserv)
    {
            if(prevcookid=="")
            {
                    res->addCookie("FFEADID=" + id + ";
    expires="+dformat.format(date)+" GMT; path=/; HttpOnly");
            }
            else
            {
                    id = prevcookid;
    ...
    if (!sessatserv ||
        (sessatserv && !ConfigurationData::getInstance()
                            ->coreServerProperties.sessservdistocache)) {
      for (it = vals.begin(); it != vals.end(); it++) {
        ...
        if (!sessatserv)
          res->addCookie(key + "=" + value + "; expires=" +
                         dformat.format(date) + " GMT; path=/; HttpOnly");
        else {
          values += key + "=" + value + "; ";
        }
      }
...
std::string ServiceTask::getFileContents(const char *fileName, const int &start,
                                         const int &end) {
  std::ifstream myfile;
  std::string all;
...
void ServiceTask::updateContent(HttpRequest *req, HttpResponse *res,
                                const std::string &ext, const int &techunkSiz) {
  ...
  std::ifstream myfile;
  if (locale.find("english") == std::string::npos &&
      (ext == ".html" || ext == ".htm")) {
  ...
  if (req->getMethod() == "HEAD") {
    res->addHeader(HttpResponse::ContentLength,
                   CastUtil::fromNumber(getFileSize(fname.c_str())));
    ...
    if (ifmodsincehdr != "") {
      ...
      if (ifmodsince != NULL) {
        // logger << "IfModifiedSince header = " + ifmodsincehdr + ", date = " +
        // ifmodsince->toString() << std::endl; logger << "Lastmodifieddate
        // value = " + lastmodDate + ", date = " + filemodifieddate.toString()
        // << std::endl; logger << "Date Comparisons = "
        // +CastUtil::fromBool(*ifmodsince>=filemodifieddate)  << std::endl;

        if (isifmodsincvalid && *ifmodsince >= filemodifieddate) {
          res->addHeader(HttpResponse::LastModified, ifmodsincehdr);
          // logger << ("File not modified - IfModifiedSince date = " +
          ...
          // logger << ("File modified - IfModifiedSince date = " +
          // ifmodsincehdr + ", FileModified date = " + lastmodDate) <<
          // std::endl;
          forceLoadFile = true;
        }
    ...
    // logger << ("Content request for " + url + " " + ext + " actual file " +
    // fname) << std::endl;
...
WebSockHandler *ServiceTask::handleWebsockOpen(WebSocketData *req,
  ...
  if (!ConfigurationData::isServingContext(hreq->getCntxt_name())) {
    if (ConfigurationData::getInstance()->appAliases.find(
      ...
      hreq->setCntxt_name(
          std::string_view{ConfigurationData::getInstance()
                               ->appAliases.find(hreq->getCntxt_name())
                               ->second});
  ...
  CommonUtils::setAppName(
      ConfigurationData::getInstance()
          ->servingContextAppNames.find(hreq->getCntxt_name())
          ->second);
  hreq->setCntxt_root(ConfigurationData::getInstance()
                          ->servingContextAppRoots.find(hreq->getCntxt_name())
                          ->second);
  ...
  std::map<std::string, std::string, std::less<>> websockcntMap =
      ConfigurationData::getInstance()
          ->websocketMappingMap.find(req->getCntxt_name())
          ->second;
  std::map<std::string, std::string, std::less<>>::iterator it;
  for (it = websockcntMap.begin(); it != websockcntMap.end(); ++it) {
    if (ConfigurationData::urlMatchesPath(req->getCntxt_name(), it->first,
                                          hreq->getCurl())) {
      if (it->second != "") {
        void *_temp = ConfigurationData::getInstance()->ffeadContext.getBean(
            it->second, req->getCntxt_name());
...
bool ServiceTask::handleAsync(HttpRequest *req, HttpResponse *res,
                              Writer *sif) {
  try {
    ...
    std::map<std::string, Router *, std::less<>>::iterator rit =
        ConfigurationData::getInstance()->servingContextRouters.find(
            req->getCntxt_name());
    if (rit != ConfigurationData::getInstance()->servingContextRouters.end()) {
      CommonUtils::setAppName(
          ConfigurationData::getInstance()
              ->servingContextAppNames.find(req->getCntxt_name())
              ->second);
      Router *router = rit->second;
      req->setCntxt_root(
          ConfigurationData::getInstance()
              ->servingContextAppRoots.find(req->getCntxt_name())
              ->second);
      router->routeAsync(req, res, sif);
    } else {
      rit = ConfigurationData::getInstance()->servingContextRouters.find(
          HttpRequest::DEFAULT_CTX);
      if (rit !=
...
bool ServiceTask::handle(HttpRequest *req, HttpResponse *res, Writer *sif) {
  res->setHTTPResponseStatus(HTTPResponseStatus::NotFound);
  /*After going through the controller the response might be blank, just set the
   * HTTP version*/
  res->update(req);

  try {
    if (ConfigurationData::getInstance()->enableStaticResponses &&
        ConfigurationData::getInstance()->staticResponsesMap.find(
            req->getPath()) !=
            ConfigurationData::getInstance()->staticResponsesMap.end()) {
      StaticResponseData &sr = ConfigurationData::getInstance()
                                   ->staticResponsesMap.find(req->getPath())
                                   ->second;
      res->setContent(sr.r);
    ...
    if (req->getRequestParseStatus() != NULL || req->getMethod().at(0) == 't' ||
        req->getMethod().at(0) == 'T') {
    ...
    if (!ConfigurationData::isServingContext(req->getCntxt_name())) {
      if (ConfigurationData::getInstance()->appAliases.find(
        ...
        req->setCntxt_name(
            std::string_view{ConfigurationData::getInstance()
                                 ->appAliases.find(req->getCntxt_name())
                                 ->second});
        if (ConfigurationData::getInstance()->servingContexts.find(
                req->getCntxt_name()) ==
            ConfigurationData::getInstance()->servingContexts.end()) {
    ...
    CommonUtils::setAppName(
        ConfigurationData::getInstance()
            ->servingContextAppNames.find(req->getCntxt_name())
            ->second);
    req->setCntxt_root(ConfigurationData::getInstance()
                           ->servingContextAppRoots.find(req->getCntxt_name())
                           ->second);
                    ...
                    ConfigurationData::getInstance()->applicationFlowMap.find(req->getCntxt_name())->second)
    {
            ...
            std::string path1;
            void *mkr1 = dlsym(ConfigurationData::getInstance().dlib,
    meth1.c_str()); if(mkr1!=NULL)
            {
    ...
    if (ConfigurationData::getInstance()->enableCors) {
      try {
        isContrl = CORSHandler::handle(
            ConfigurationData::getInstance()->corsConfig, req, res);
    ...
    if (!isContrl && ConfigurationData::getInstance()->enableSecurity) {
      ...
      if (hasSecurity) {
        isContrl =
            SecurityHandler::handle(req, res,
                                    ConfigurationData::getInstance()
                                        ->coreServerProperties.sessionTimeout,
                                    reflector);
        if (isContrl) {
          ext = req->getExt();
        }
    ...
    if (!isContrl && ConfigurationData::getInstance()->enableExtra) {
      isContrl = ExtHandler::handle(
          req, res, ConfigurationData::getInstance()->dlib,
          ConfigurationData::getInstance()->ddlib, ext, reflector);
    }
    if (!res->isDone()) {
      if (!isContrl) {
        bool isWbsvc = false;
        ...
        if (ConfigurationData::getInstance()->enableSoap) {
          std::string wsUrl = "http://" +
                              ConfigurationData::getInstance()
                                  ->coreServerProperties.ip_address +
                              req->getCurl();
          std::string wsName = ConfigurationData::getInstance()
                                   ->wsdlMap.find(req->getCntxt_name())
                                   ->second[wsUrl];
          if (wsName != "") {
            isWbsvc = true;
            if (req->getHeader(HttpRequest::ContentType)
                        .find("application/soap+xml") != std::string::npos ||
                req->getHeader(HttpRequest::ContentType).find("text/xml") !=
                    std::string::npos ||
                req->getHeader(HttpRequest::ContentType)
                        .find("application/xml") != std::string::npos) {
              SoapHandler::handle(
                  req, res, ConfigurationData::getInstance()->dlib, wsName);
            } else {
        ...
        if (!isWbsvc) {
          ...
          if (ConfigurationData::getInstance()->enableScripts) {
            cntrlit = ScriptHandler::handle(
                req, res, ConfigurationData::getInstance()->handoffs, ext);
          }
#endif
          if (!cntrlit) {
            ...
            if (req->getExt().length() > 0) {
              ...
              // if(req->isAgentAcceptsCE() && (cntEncoding=="gzip" ||
              // cntEncoding=="deflate") && req->isNonBinary(mimeType)) {
    ...
    // logger << "Standard exception occurred while processing ServiceTask
    // request " << std::endl;
  }
  return true;
}

void ServiceTask::handleWebSocket(HttpRequest *req, void *dlib, void *ddlib,
                                  BaseSocket *sockUtil) {}
