...
std::string
ApplicationUtil::buildAllApplications(const std::vector<std::string> &files,
  ...
  for (unsigned int var = 0; var < files.size(); ++var) {
    std::string path =
        files.at(var).substr(0, files.at(var).find_last_of("/") + 1);
    std::string appName =
        apps.at(var).substr(0, apps.at(var).find_last_of("/"));
    ...
    if (eles.size() == 0) {
      ...
      meth += "std::string " + appName +
              "checkRules(std::string to,HttpSession session)\n{\nstd::string "
              "curr=session.getAttribute(\"CURR\");\n";
      for (unsigned int var = 0; var < eles.size(); var++) {
        if (eles.at(var).getTagName() == "include") {
        ...
        } else if (eles.at(var).getTagName() == "page" ||
                   eles.at(var).getTagName() == "welcome") {
          meth += ("std::string " + eles.at(var).getAttribute("id") + " = \"" +
                   path + eles.at(var).getAttribute("path") + "\";\n");
          if (eles.at(var).getAttribute("who") != "" &&
              eles.at(var).getAttribute("where") != "") {
            if (eles.at(var).getAttribute("where").find("FILE:") !=
                std::string::npos) {
              std::string fileName = eles.at(var).getAttribute("where");
              StringUtil::replaceFirst(fileName, "FILE:", "");
              fileName = (path + fileName);
              meth += "std::string path;\nif(to==" +
                      eles.at(var).getAttribute("id") + ")\n{";
          ...
          } else if (eles.at(var).getAttribute("onsuccess") != "" &&
                     eles.at(var).getAttribute("onfail") != "") {
          }
        } else if (eles.at(var).getTagName() == "rule") {
          meth += "\nif(to==" + eles.at(var).getAttribute("topage") +
                  " && curr==" + eles.at(var).getAttribute("currpage") + ")\n{";
          ...
          for (unsigned int var1 = 0; var1 < elesc.size(); var1++) {
            if (elesc.at(var1).getTagName() == "execute") {
              ...
              for (unsigned int var2 = 0; var2 < elesce.size(); var2++) {
                meth += (elesce.at(var2).getTagName() + " _" +
                         CastUtil::fromNumber(var2 + 1) +
                         " = CastUtil::lexical_cast<" +
                         elesce.at(var2).getTagName() + ">(\"" +
                         elesce.at(var2).getText() + "\");\n");
                args += ("_" + CastUtil::fromNumber(var2 + 1));
                if (var2 != elesce.size() - 1)
                  args += ",";
              }
              if (elesc.at(var1).getAttribute("rettype") != "" ||
                  elesc.at(var1).getAttribute("rettype") != "void") {
                meth += (elesc.at(var1).getAttribute("rettype") + " " +
                         elesc.at(var1).getAttribute("retname") + " = ");
