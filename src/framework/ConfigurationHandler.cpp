...
void ConfigurationHandler::normalizeUrl(const std::string &appName,
                                        std::string &url) {
  if (url == "")
    return;
...
void ConfigurationHandler::handle(strVec webdirs, const strVec &webdirs1,
                                  const std::string &incpath,
                                  const std::string &rtdcfpath,
  ...
  ref.addMarker(Marker("@CondBarrier", Marker::TYPE_CLASS,
                       MarkerHandler::collectStr(2, "type", "value")));
  ref.addMarker(Marker("@CondBarrier", Marker::TYPE_PROP,
                       MarkerHandler::collectStr(2, "type", "value")));
  ref.addMarker(Marker("@CondBarrier", Marker::TYPE_METH,
                       MarkerHandler::collectStr(2, "type", "value")));

  // Start Rest-full markers
  ref.addMarker(Marker("@RestController", Marker::TYPE_CLASS,
                       MarkerHandler::collectStr(1, "path")));
  ref.addMarker(
      Marker("@GET", Marker::TYPE_METH,
             MarkerHandler::collectStr(5, "statusCode", "path", "icontentType",
                                       "ocontentType", "unmapped"),
             MarkerHandler::collectBool(5, false, true, false, false, false)));
  ref.addMarker(
      Marker("@PUT", Marker::TYPE_METH,
             MarkerHandler::collectStr(5, "statusCode", "path", "icontentType",
                                       "ocontentType", "unmapped"),
             MarkerHandler::collectBool(5, false, true, false, false, false)));
  ref.addMarker(
      Marker("@POST", Marker::TYPE_METH,
             MarkerHandler::collectStr(5, "statusCode", "path", "icontentType",
                                       "ocontentType", "unmapped"),
             MarkerHandler::collectBool(5, false, true, false, false, false)));
  ref.addMarker(
      Marker("@DELETE", Marker::TYPE_METH,
             MarkerHandler::collectStr(5, "statusCode", "path", "icontentType",
                                       "ocontentType", "unmapped"),
             MarkerHandler::collectBool(5, false, true, false, false, false)));
  ref.addMarker(Marker("@PathParam", Marker::TYPE_ARG,
                       MarkerHandler::collectStr(1, "name")));
  ref.addMarker(Marker("@QueryParam", Marker::TYPE_ARG,
                       MarkerHandler::collectStr(1, "name")));
  ref.addMarker(Marker("@FormParam", Marker::TYPE_ARG,
                       MarkerHandler::collectStr(1, "name")));
  ref.addMarker(Marker("@HeaderParam", Marker::TYPE_ARG,
                       MarkerHandler::collectStr(1, "name")));
  ref.addMarker(Marker("@MultipartContent", Marker::TYPE_ARG,
                       MarkerHandler::collectStr(1, "name")));
  ref.addMarker(Marker("@Body", Marker::TYPE_ARG));
  ref.addMarker(Marker("@DefaultValue", Marker::TYPE_ARG,
                       MarkerHandler::collectStr(1, "value")));

  // Start Web-Service markers
  ref.addMarker(Marker("@WebService", Marker::TYPE_CLASS,
                       MarkerHandler::collectStr(2, "location", "namespc"),
                       MarkerHandler::collectBool(2, true, false)));
  ref.addMarker(Marker("@WebServiceMethod", Marker::TYPE_METH,
                       MarkerHandler::collectStr(1, "outName"),
                       MarkerHandler::collectBool(1, false)));

  ref.addMarker(
      Marker("@JobFunction", Marker::TYPE_METH,
             MarkerHandler::collectStr(3, "cron", "name", "scope,singleton"),
             MarkerHandler::collectBool(3, true, true, false)));

  ref.addMarker(Marker("@Secure", Marker::TYPE_CLASS,
                       MarkerHandler::collectStr(2, "role", "providerName")));
  ref.addMarker(Marker("@Secure", Marker::TYPE_METH,
                       MarkerHandler::collectStr(2, "role", "providerName")));
  ref.addMarker(
      Marker("@SecurityProvider", Marker::TYPE_CLASS,
             MarkerHandler::collectStr(7, "providerName", "url", "welcomefile",
                                       "usernamefld", "usernamefrom",
                                       "passwordfld", "passwordfrom"),
             MarkerHandler::collectBool(7, true, true, false, true, true, true,
                                        true)));

  ref.addMarker(
      Marker("@Template", Marker::TYPE_CLASS,
             MarkerHandler::collectStr(3, "path", "file", "scope,singleton"),
             MarkerHandler::collectBool(3, true, true, false)));
  ref.addMarker(Marker("@Dview", Marker::TYPE_CLASS,
                       MarkerHandler::collectStr(2, "path", "scope,singleton"),
                       MarkerHandler::collectBool(2, true, false)));
  ref.addMarker(Marker("@Controller", Marker::TYPE_CLASS,
                       MarkerHandler::collectStr(2, "path", "scope,singleton"),
                       MarkerHandler::collectBool(2, true, false)));
  ref.addMarker(Marker("@AjaxInterface", Marker::TYPE_CLASS,
                       MarkerHandler::collectStr(1, "path")));
  ref.addMarker(Marker("@Filter", Marker::TYPE_CLASS,
                       MarkerHandler::collectStr(3, "type,,in,out,handle",
                                                 "path", "scope,singleton"),
                       MarkerHandler::collectBool(3, true, true, false)));
  ref.addMarker(Marker("@WebSocketHandler", Marker::TYPE_CLASS,
                       MarkerHandler::collectStr(2, "path", "scope,singleton"),
                       MarkerHandler::collectBool(2, true, false)));
  ...
  propMultiMap srprpsMulti =
      pread.getPropertiesMultiMap(respath + "/server.prop");
    ...
    for (int i = 0; i < (int)paths.size(); i++) {
      if (paths.at(i).find("|") != std::string::npos) {
        std::vector<std::string> lst =
            StringUtil::splitAndReturn<std::vector<std::string>>(
                StringUtil::trimCopy(paths.at(i)), "|");
        if (lst.size() < 3)
          continue;
        ...
        if (pth != "" && resp != "") {
          StaticResponseData &sr =
              ConfigurationData::getInstance()->staticResponsesMap[pth];
          sr.r = resp;
          sr.t = ctyp;
        }
      }
  ...
  if (StringUtil::toLowerCopy(srprps["LOGGING_ENABLED"]) != "true") {
    isLoggingEnabled =
        StringUtil::toLowerCopy(srprps["LOGGING_ENABLED"]) == "true";
  }
  ...
  std::map<std::string, std::string, std::less<>> ajintpthMap, tpes, dcps,
      compnts;
  propMap srp;
  ...
  if (SSLHandler::getInstance()->getIsSSL()) {
    ...
    if (sslsec.size() > 0) {
      ConfigurationData::getInstance()->securityProperties.cert_file =
          serverRootDirectory + sslsec["CERTFILE"];
      ConfigurationData::getInstance()->securityProperties.key_file =
          serverRootDirectory + sslsec["KEYFILE"];
      ConfigurationData::getInstance()->securityProperties.dh_file =
          serverRootDirectory + sslsec["DHFILE"];
      ConfigurationData::getInstance()->securityProperties.ca_list =
          serverRootDirectory + sslsec["CA_LIST"];
      ConfigurationData::getInstance()->securityProperties.rand_file =
          serverRootDirectory + sslsec["RANDOM"];
      ConfigurationData::getInstance()->securityProperties.sec_password =
          sslsec["PASSWORD"];
      std::string tempcl = sslsec["CLIENT_SEC_LEVEL"];
      ConfigurationData::getInstance()->securityProperties.srv_auth_prvd =
          sslsec["SRV_AUTH_PRVD"];
      ConfigurationData::getInstance()->securityProperties.srv_auth_mode =
          sslsec["SRV_AUTH_MODE"];
      ConfigurationData::getInstance()->securityProperties.srv_auth_file =
          serverRootDirectory + sslsec["SRV_AUTH_FILE"];
      std::string alpnprotolist = sslsec["ALPN_PROTO_LIST"];
      if (tempcl != "") {
        try {
          ConfigurationData::getInstance()->securityProperties.client_auth =
              CastUtil::toInt(tempcl);
      ...
      try {
        ConfigurationData::getInstance()->securityProperties.isDHParams =
            CastUtil::toBool(sslsec["ISDH_PARAMS"]);
      } catch (const std::exception &e) {
        logger << "Invalid boolean value for isDHParams defined" << std::endl;
      }
      try {
        ConfigurationData::getInstance()->securityProperties.alpnEnabled =
            CastUtil::toBool(sslsec["ALPN_ENABLED"]);
        if (alpnprotolist != "") {
          logger << "ALPN protocols list = " << alpnprotolist << std::endl;
          ConfigurationData::getInstance()->securityProperties.alpnProtoList =
              StringUtil::splitAndReturn<std::vector<std::string>>(
                  alpnprotolist, ",");
        } else {
          ConfigurationData::getInstance()->securityProperties.alpnEnabled =
  ...
  std::map<std::string, std::map<std::string, ClassStructure, std::less<>>,
           std::less<>>
      clsstrucMaps;
  ...
  for (unsigned int var = 0; var < webdirs.size(); var++) {
    ...
    if (access(nbf.c_str(), F_OK) != -1) {
      logger.info("Skipping web directory " + name +
                  " as it is marked as no build");
      continue;
    }
    ...
    rundyncontent += "cp -Rf $FFEAD_CPP_PATH/public/* $FFEAD_CPP_PATH/web/" +
                     name + "/public/\n";
    ...
    for (unsigned int ind = 0; ind < includes.size(); ++ind) {
      std::map<std::string, ClassStructure, std::less<>> clsmap =
          ref.getClassStructures(includes.at(ind), name);
    ...
    ref.generateClassDefinition(allclsmap, tinc, ttypedefs, tclasses, tmethods,
                                topers, name);
    ...
    if (root.getTagName() == "app" && root.getChildElements().size() > 0) {
      ...
      if (root.getAttribute("alias") != "" &&
          StringUtil::trimCopy(root.getAttribute("alias")) != name) {
        ConfigurationData::getInstance()
            ->appAliases[StringUtil::trimCopy(root.getAttribute("alias"))] =
            name;
      }
      if (root.getAttribute("libname") != "" &&
          StringUtil::trimCopy(root.getAttribute("libname")) != name) {
        flibname = StringUtil::trimCopy(root.getAttribute("libname"));
        RegexUtil::replace(flibname, "[^a-zA-Z0-9_\\-]+", "");
      }
      if (root.getAttribute("router") != "" &&
          StringUtil::trimCopy(root.getAttribute("router")) != "") {
        std::string router = StringUtil::trimCopy(root.getAttribute("router"));
        ConfigurationData::getInstance()->servingContextRouterNames[name] =
            router + ";" + name;
      ...
      if (root.getAttribute("properties") != "" &&
          StringUtil::trimCopy(root.getAttribute("properties")) != "") {
        ...
        // Bean bean1(router,"",router,"singleton",false,true,name);
        // ConfigurationData::getInstance()->ffeadContext.addBean(bean1);
        logger << ("Adding Properties for " + name + " :: " + properties)
               << std::endl;
      }
      for (unsigned int apps = 0; apps < eles.size(); apps++) {
          ...
          for (unsigned int cntn = 0; cntn < cntrls.size(); cntn++) {
            if (cntrls.at(cntn).getTagName() == "controller") {
              ...
              if (url != "" && clas != "") {
                ...
                ConfigurationData::getInstance()
                    ->controllerObjectMap[name][url] = clas;
                logger << ("Adding Controller for " + url + " :: " + clas)
                       << std::endl;
              } else {
                ...
                if (to.find("*") != std::string::npos ||
                    to.find("regex(") != std::string::npos) {
                  logger << ("Skipping controller mapping as it contains "
                             "*/regex function")
                         << std::endl;
                  to = "";
                }
                if (from != "" && to != "") {
                  ...
                  ConfigurationData::getInstance()
                      ->mappingObjectMap[name][from] = to;
                  logger << ("Adding Mapping for " + from + " :: " + to)
                         << std::endl;
                }
                ...
                if (to.find("*") != std::string::npos ||
                    to.find("regex(") != std::string::npos) {
                  logger << ("Skipping controller extension mapping as it "
                             "contains */regex function")
                         << std::endl;
                  to = "";
                }
                if (to.find(".") == 0) {
                  logger << ("Skipping controller provide 'to' file extension "
                             "without dot(.)")
                         << std::endl;
                  to = "";
                }
                if (from.find("*") != std::string::npos ||
                    from.find("regex(") != std::string::npos) {
                  logger << ("Skipping controller extension mapping as it "
                             "contains */regex function")
                         << std::endl;
                  from = "";
                ...
                if (from.find(".") == 0) {
                  logger << ("Skipping controller provide 'from' file "
                ...
                if (from != "" && to != "") {
                  ConfigurationData::getInstance()
                      ->mappingextObjectMap[name][from] = to;
                  logger << ("Adding Extension Mapping for " + from +
                             " :: " + to)
                         << std::endl;
                }
              }
        ...
        /*else if(eles.at(apps).getTagName()=="authhandlers")
        {
                ...
                for (unsigned int cntn = 0; cntn < cntrls.size(); cntn++)
                {
                        ...
                        {
                                std::string url =
        cntrls.at(cntn).getAttribute("url"); StringUtil::trim(url); std::string
        provider = cntrls.at(cntn).getAttribute("provider");
                                StringUtil::trim(provider);
                                std::string scope =
        cntrls.at(cntn).getAttribute("scope"); StringUtil::trim(scope);
                                if(url.find("*")!=std::string::npos ||
        url.find("regex(")!=std::string::npos)
                                {
                                        logger << ("Skipping authhandler as
        login url contains *regex function") << std::endl; url = "";
                                }
                                if(url!="" && provider!="")
                                {
                                        ConfigurationData::getInstance()->authHandlerObjectMap[name][url]
        = provider; if(provider!="" &&
        provider.find("class:")!=std::string::npos)
                                        {
                                                std::string clas =
        provider.substr(provider.find(":")+1); Bean
        bean(name+clas,"",clas,scope,false,name);
                                                ConfigurationData::getInstance()->ffeadContext.addBean(bean);
                                        }
                                        logger << ("Adding Authhandler for " +
        (name + url) + " :: " + provider) << std::endl;
                                }
                        }
                }
        }*/
          ...
          for (unsigned int cntn = 0; cntn < cntrls.size(); cntn++) {
            if (cntrls.at(cntn).getTagName() == "filter") {
              ...
              if (clas != "" &&
                  (type == "in" || type == "out" || type == "handle")) {
                normalizeUrl(name, url);
                if (url == "")
                  url = "*.*";
                ConfigurationData::getInstance()
                    ->filterObjectMap[name][url + type]
                    .push_back(clas);
                if (clas != "") {
                  Bean bean(clas, "", clas, scope, false, false, name);
                  ConfigurationData::getInstance()->ffeadContext.addBean(bean);
                }
                logger << ("Adding Filter for " + (name + url + type) +
                           " :: " + clas)
                       << std::endl;
              }
            }
          ...
          for (unsigned int tmpn = 0; tmpn < tmplts.size(); tmpn++) {
            if (tmplts.at(tmpn).getTagName() == "template") {
              std::string clas =
                  StringUtil::trimCopy(tmplts.at(tmpn).getAttribute("class"));
              std::string url =
                  StringUtil::trimCopy(tmplts.at(tmpn).getAttribute("path"));
              std::string file =
                  StringUtil::trimCopy(tmplts.at(tmpn).getAttribute("file"));
              if (url.find("*") == std::string::npos &&
                  url.find("regex(") == std::string::npos) {
                normalizeUrl(name, url);
                ...
                StringUtil::replaceFirst(wbpi,
                                         ConfigurationData::getInstance()
                                             ->coreServerProperties.webPath,
                                         "");
                RegexUtil::replace(wbpi, "[^a-zA-Z0-9_]+", "");
                ConfigurationData::getInstance()
                    ->templateMappingMap[name][url] = clas + ";" + wbpi;
                tpes[fpath] = name;
                ...
                logger << ("Adding Template for " + (url) + " :: " + clas)
                       << std::endl;
              } else {
                logger
                    << ("Skipping Template as path contains */regex function")
                    << std::endl;
              }
            }
          }
          ...
          for (unsigned int tmpn = 0; tmpn < dycpppgs.size(); tmpn++) {
            if (dycpppgs.at(tmpn).getTagName() == "dynamic-cpp-page") {
              std::string url =
                  StringUtil::trimCopy(dycpppgs.at(tmpn).getAttribute("path"));
              std::string file =
                  StringUtil::trimCopy(dycpppgs.at(tmpn).getAttribute("file"));
              if (url.find("*") == std::string::npos &&
                  url.find("regex(") == std::string::npos) {
                normalizeUrl(name, url);
                ...
                if (dcps.find(fpath) != dcps.end()) {
                  ...
                  StringUtil::replaceFirst(wbpi,
                                           ...
                                           "");
                  RegexUtil::replace(wbpi, "[^a-zA-Z0-9_]+", "");
                  ConfigurationData::getInstance()->dcpMappingMap[name][url] =
                      wbpi;
                  logger << ("Adding Dynamic C++ Page for " + (url) +
                             " :: " + fpath)
                         << std::endl;
                } else {
                  logger << ("Could not find Dynamic C++ Page for " + (url) +
                             " :: " + fpath)
                         << std::endl;
                }
              } else {
          ...
          for (unsigned int tmpn = 0; tmpn < tmplts.size(); tmpn++) {
            if (tmplts.at(tmpn).getTagName() == "web-socket-handler") {
              ...
              if (url.find("*") == std::string::npos &&
                ...
                normalizeUrl(name, url);
                ConfigurationData::getInstance()
                    ->websocketMappingMap[name][url] = clas;
                std::string scope = tmplts.at(tmpn).getAttribute("scope");
                if (clas != "") {
                  Bean bean(clas, "", clas, scope, false, false, name);
                  ConfigurationData::getInstance()->ffeadContext.addBean(bean);
                }
                logger << ("Adding WebSocketHandler for " + (url) +
                           " :: " + clas)
                       << std::endl;
              } else {
                logger << ("Skipping WebSocketHandler as path contains */regex "
                           "function")
                       << std::endl;
              }
            }
          ...
          for (unsigned int dn = 0; dn < dvs.size(); dn++) {
            if (dvs.at(dn).getTagName() == "dview") {
              ...
              ConfigurationData::getInstance()->viewMappingMap[name][url] =
                  clas;
              ...
              logger << ("Adding Dynamic View for " + (name + url) +
                         " :: " + clas)
                     << std::endl;
            }
          }
          ...
          for (unsigned int dn = 0; dn < ajintfs.size(); dn++) {
            if (ajintfs.at(dn).getTagName() == "ajax-interface") {
              ...
              if (url.find("*") == std::string::npos &&
                ...
                ConfigurationData::getInstance()->ajaxInterfaceMap[name][url] =
                    clas;
                ...
                afcd.push_back(clas);
                logger << ("Adding Ajax Interface for " + (name + url) +
                           " :: " + clas)
                       << std::endl;
              } else {
                logger << ("Skipping Ajax Interface as path contains */regex "
                           "function")
                       << std::endl;
              }
            }
            ...
            if (cntrls.at(cntn).getTagName() == "restcontroller") {
              ...
              for (unsigned int cntn1 = 0; cntn1 < resfuncs.size(); cntn1++) {
                if (resfuncs.at(cntn1).getTagName() == "restfunction") {
                  ...
                  restfunction.unmapped =
                      StringUtil::toLowerCopy(resfuncs.at(cntn1).getAttribute(
                          "unmapped")) == "true";
                  restfunction.meth = StringUtil::toUpperCopy(
                      resfuncs.at(cntn1).getAttribute("meth"));
                  restfunction.statusCode =
                      resfuncs.at(cntn1).getAttribute("statusCode");
                  ...
                  if (restfunction.statusCode != "") {
                    try {
                      ...
                      logger << "Rest: invalid response statusCode specified, "
                                "defaulting to 200.."
                             << std::endl;
                  ...
                  restfunction.serOpt =
                      SerializeBase::identifySerOption(restfunction.rtype);
                  /*restfunction.baseUrl =
                  resfuncs.at(cntn1).getAttribute("baseUrl");
                  ...
                  restfunction.icontentType =
                      resfuncs.at(cntn1).getAttribute("icontentType");
                  restfunction.ocontentType =
                      resfuncs.at(cntn1).getAttribute("ocontentType");
                  if (restfunction.ocontentType == "") {
                    restfunction.ocontentType =
                        ContentTypes::CONTENT_TYPE_TEXT_PLAIN;
                  }
                  if (StringUtil::toLowerCopy(restfunction.ocontentType)
                          .find(ContentTypes::CONTENT_TYPE_APPLICATION_JSON) ==
                      0) {
                    restfunction.serOpt += 1000;
                  } else if (StringUtil::toLowerCopy(restfunction.ocontentType)
                                 .find(ContentTypes::
                                           CONTENT_TYPE_APPLICATION_XML) == 0) {
                    restfunction.serOpt += 2000;
                  }
                  ElementList resfuncparams =
                      resfuncs.at(cntn1).getChildElements();
                  bool hasBodyParam = false, invalidParam = false;
                  ;
                  for (unsigned int cntn2 = 0; cntn2 < resfuncparams.size();
                       cntn2++) {
                    if (resfuncparams.at(cntn2).getTagName() == "param") {
                      ...
                      param.serOpt =
                          SerializeBase::identifySerOption(param.type);
                      param.from = resfuncparams.at(cntn2).getAttribute("from");
                      param.name = resfuncparams.at(cntn2).getAttribute("name");
                      param.defValue =
                          resfuncparams.at(cntn2).getAttribute("defValue");
                      if (param.from == "body") {
                        hasBodyParam = true;
                      }
                      if (StringUtil::trimCopy(param.type) == "") {
                        invalidParam = true;
                        logger << "Rest: no type specified for param"
                               << std::endl;
                      } else if ((param.type == "filestream" ||
                                  param.type == "vector-of-filestream") &&
                                 param.from != "multipart-content") {

                      } else if (param.type == "vector-of-filestream" &&
                                 param.from == "multipart-content" &&
                                 ...
                                 StringUtil::trimCopy(param.name) == "") {
                        invalidParam = true;
                        logger << "Rest: no name specified for param"
                               << std::endl;
                      } else if (param.from == "body" &&
                                 (restfunction.meth == "GET" ||
                                  restfunction.meth == "OPTIONS" ||
                                  restfunction.meth == "TRACE" ||
                                  restfunction.meth == "HEAD")) {
                        invalidParam = true;
                        logger << ("Rest: skipping param " + param.type +
                                   ", from is body and method is " +
                                   restfunction.meth)
                               << std::endl;
                        ...
                        invalidParam = true;
                        logger << ("Rest: skipping param " + param.type +
                                   ", from is not body and input is a complex "
                                   "type")
                               << std::endl;
                      } else if (param.from == "postparam" &&
                                 (restfunction.meth == "GET" ||
                                  restfunction.meth == "OPTIONS" ||
                                  restfunction.meth == "TRACE" ||
                                  restfunction.meth == "HEAD")) {
                        invalidParam = true;
                        logger << ("Rest: skipping param " + param.type +
                                   ", from is postparam and method is " +
                                   restfunction.meth)
                               << std::endl;
                      }
                      if (!invalidParam) {
                  ...
                  if (hasBodyParam && restfunction.params.size() > 1) {
                    logger << ("Rest: skipping method " + restfunction.meth +
                               " as only one argument allowed of type body")
                           << std::endl;
                    continue;
                  }
                  if (invalidParam) {
                  ...
                  if (clas != "") {
                    if (url.find("*") == std::string::npos) {
                      ...
                      if (restfunction.path != "") {
                        ...
                        ConfigurationData::getInstance()
                            ->rstCntMap[name][restfunction.meth + urlmpp]
                            .push_back(restfunction);
                      } else {
                      ...
                      if (clas != "") {
                        Bean bean(clas, "", clas, scope, false, true, name);
                        ConfigurationData::getInstance()->ffeadContext.addBean(
                            bean);
                      }
                      logger << ("Rest: Adding rest-controller => " + urlmpp +
                                 " , class => " + clas)
                             << std::endl;
                    }
                  }
          ...
          for (unsigned int cntpra = 0; cntpra < allprovs.size(); cntpra++) {
            if (allprovs.at(cntpra).getTagName() == "providers") {
              ...
              for (unsigned int cntpr = 0; cntpr < provs.size(); cntpr++) {
                ...
                if (provName == "") {
                  logger
                      << ("No name provided for security-provider, skipping...")
                      << std::endl;
                  continue;
                }

                if (ConfigurationData::getInstance()->securityObjectMap.find(
                        name) != ConfigurationData::getInstance()
                                     ->securityObjectMap.end() &&
                    ConfigurationData::getInstance()
                            ->securityObjectMap[name]
                            .find(provName) != ConfigurationData::getInstance()
                                                   ->securityObjectMap[name]
                                                   .end()) {
                  logger << ("Security-provider already exists, skipping "
                             "duplicate...")
                         << std::endl;
                  continue;
                }
                ...
                for (unsigned int cntn = 0; cntn < cntrls.size(); cntn++) {
                  if (cntrls.at(cntn).getTagName() == "login-handler") {
                    std::string provider =
                        cntrls.at(cntn).getAttribute("provider");
                    std::string url = cntrls.at(cntn).getAttribute("path");
                    std::string sessionTimeoutV =
                        cntrls.at(cntn).getAttribute("sessionTimeout");
                    std::string scope = cntrls.at(cntn).getAttribute("scope");

                    Security securityObject =
                        ConfigurationData::getInstance()
                            ->securityObjectMap[name][provName];
                    securityObject.loginProvider = provider;
                    normalizeUrl(name, url);
                    securityObject.loginUrl = url;
                    try {
                      securityObject.sessTimeout =
                          CastUtil::toLong(sessionTimeoutV);
                    } catch (const std::exception &e) {
                      securityObject.sessTimeout = 3600;
                      logger << ("Security: Invalid session timeout value "
                                 "defined, defaulting to 1hour/3600sec")
                             << std::endl;
                    }
                    ConfigurationData::getInstance()
                        ->securityObjectMap[name][provName] = securityObject;
                    if (provider != "" &&
                        provider.find("class:") != std::string::npos) {
                      std::string clas =
                          provider.substr(provider.find(":") + 1);
                      Bean bean(clas, "", clas, scope, false, false, name);
                      ConfigurationData::getInstance()->ffeadContext.addBean(
                          bean);
                    }
                    logger << ("Security: Adding Login Handler => " + url +
                               " , provider => " + provider)
                           << std::endl;
                    ...
                    if (securityObject.addAspect(secureAspect)) {
                      ConfigurationData::getInstance()
                          ->securityObjectMap[name][provName] = securityObject;
                      logger << ("Security: Adding Secure Path => " + path +
                                 " , role => " + role)
                             << std::endl;
                    }
                  } else if (cntrls.at(cntn).getTagName() == "welcome") {
                    std::string welcomeFile =
                        cntrls.at(cntn).getAttribute("file");
                    Security securityObject =
                        ConfigurationData::getInstance()
                            ->securityObjectMap[name][provName];
                    ...
                    ConfigurationData::getInstance()
                        ->securityObjectMap[name][provName] = securityObject;
                    logger << ("Security: Adding Welcome file => " +
                               welcomeFile)
                           << std::endl;
                    ...
                    if (ffrom != "reqparam" && ffrom != "postparam" &&
                        ffrom != "header" && fname != "") {
                      Security securityObject =
                          ConfigurationData::getInstance()
                              ->securityObjectMap[name][provName];
          ...
          for (unsigned int cntn = 0; cntn < cntrls.size(); cntn++) {
            if (cntrls.at(cntn).getTagName() == "handoff") {
              ...
              ConfigurationData::getInstance()->handoffs[app + "extension"] =
                  ext;
              handoffVec.push_back("-l" + app + " ");
              logger << ("Adding Handoff for app => " + app + " , ext => " +
                         ext + " , default url => " + def)
                     << std::endl;
            }
          }
          ...
          ConfigurationData::getInstance()->corsConfig =
              CorsConfig(allwdOrigins, allwdMethods, allwdHeaders,
                         exposedHeaders, allwdCredentials, maxAge);
          logger
              << ("CORS Configuartion allow-origins => " +
                  ConfigurationData::getInstance()->corsConfig.allwdOrigins +
                  " , allow-methods => " +
                  ConfigurationData::getInstance()->corsConfig.allwdMethods +
                  " , allow-headers => " +
                  ConfigurationData::getInstance()->corsConfig.allwdHeaders +
                  " , allow-credentials => " +
                  CastUtil::fromBool(ConfigurationData::getInstance()
                                         ->corsConfig.allwdCredentials) +
                  " , expose-headers => " +
                  ConfigurationData::getInstance()->corsConfig.exposedHeaders +
                  " , max-age => " +
                  CastUtil::fromNumber(
                      ConfigurationData::getInstance()->corsConfig.maxAge))
              << std::endl;
        }
#ifdef INC_JOBS
    ...
    ilibs += ("FIND_LIBRARY(HAVE_" + StringUtil::toUpperCopy(applibname) + " " +
              flibname + " HINTS \"${CMAKE_SOURCE_DIR}/../lib\")\n");
    ...
    if (root1.getTagName() == "fview" && root1.getChildElements().size() > 0) {
      ...
      for (unsigned int apps = 0; apps < eles.size(); apps++) {
        if (eles.at(apps).getTagName() == "page") {
          ...
          ConfigurationData::getInstance()->fviewMappingMap[name][fvw] =
              eles.at(apps).getAttribute("class");
          pathvec.push_back(name);
          vecvp.push_back(usrincludes);
          stat.push_back(false);
          normalizeUrl(name, fvw);
          ConfigurationData::getInstance()->ajaxInterfaceMap[name][fvw] =
              eles.at(apps).getAttribute("class");
          ...
          for (int appse = 0; appse < (int)elese.size(); appse++) {
            if (elese.at(appse).getTagName() == "event") {
              nsfns += "\n\"_fview_cntxt_global_js_callback" +
                       CastUtil::fromNumber(appse) +
                       "\" : function(response){" +
                       elese.at(appse).getAttribute("cb") + "},";
              js += "\ndocument.getElementById('" +
                    elese.at(appse).getAttribute("eid") + "').";
              js += elese.at(appse).getAttribute("type") + " = function(){";
              js += eles.at(apps).getAttribute("class") + "." +
                    elese.at(appse).getAttribute("func") + "(";
              std::string args = elese.at(appse).getAttribute("args");
              if (args != "")
                args += ",";
              js += args + "\"_fview_cntxt_global_js_callback" +
                    CastUtil::fromNumber(appse) + "\",\"/" + name + "/" + fvw +
                    "\",_fview_namespace);}";
            } else if (elese.at(appse).getTagName() == "form") {
              // ConfigurationData::getInstance()->fviewFormMap[name][elese.at(appse).getAttribute("name")]
              // = *(elese.at(appse));
              ...
              elese.at(appse).copy(
                  &(ConfigurationData::getInstance()
                        ->fviewFormMap[name]
                                      [elese.at(appse).getAttribute("name")]));
              logger << ("Fview: Adding form => " +
                         elese.at(appse).getAttribute("name") +
                         " , form class => " +
                         elese.at(appse).getAttribute("bean") +
                         " , form controller => " +
                         elese.at(appse).getAttribute("controller"))
                     << std::endl;
            }
          }
          js += "\n}\n\n";
          ...
          AfcUtil::writeTofile(webpubpath +
                                   eles.at(apps).getAttribute("class") + ".js",
                               js, true);

          logger << ("Fview: Adding fview page class => " +
                     eles.at(apps).getAttribute("class") + " , html => " +
                     eles.at(apps).getAttribute("htm"))
                 << std::endl;
        }
      }
    }
    ...
    std::string infjs;
    ajrt += AfcUtil::generateJsInterfacessAll(allclsmap, infjs, ajintpthMap,
                                              afcd, ref);
    ...
    LoggerFactory::init(defpath + "/config/logging.xml", defpath, name,
                        isLoggingEnabled);
  }
  ...
  for (cmpit = compnts.begin(); cmpit != compnts.end(); ++cmpit) {
    ...
    try {
      std::string file =
          gen.generateComponentCU(cmpit->first, cudata, cuheader, curemote,
                                  curemoteheaders, cmpit->second);
      AfcUtil::writeTofile(rtdcfpath + file + ".h", cuheader, true);
      AfcUtil::writeTofile(rtdcfpath + file + ".cpp", cudata, true);
      AfcUtil::writeTofile(rtdcfpath + file + "_Remote.h", curemoteheaders,
                           true);
      ...
      logger << ("Exception occurred during component code generation : ") << ex
             << std::endl;
    }
  }
  ...
  std::string ret =
      ref.generateClassDefinitionsAll(clsstrucMaps, includeRef, webdirs1);
  ...
  confsrcFiles += "ReflectorInterface.cpp ";
#endif
  ret = ref.generateSerDefinitionAll(clsstrucMaps, includeRef, true, objs,
                                     ajaxret, headers, typerefs, webdirs1);
  ...
  handleMarkerConfigurations(clsstrucMaps, allwsdets, stat, vecvp, pathvec,
                             ajintpthMap, tpes, serverRootDirectory, afcd, ajrt,
                             ref);
  ...
  std::vector<WsDetails> allwsdets1 =
      wsu.getWsDetails(webdirs1, serverRootDirectory);
  ...
  std::string cont = TemplateEngine::evaluate(respath+"/rundyn_template.sh",
  cntxt); AfcUtil::writeTofile(respath+"/rundyn.sh", cont, true); cntxt.clear();
...
if (access( configureFilePath.c_str(), F_OK ) == -1 )
  {
          std::string compres = rtdcfpath+"/autotools/autogen.sh
"+serverRootDirectory; std::string output = ScriptHandler::execute(compres,
true); logger << "Set up configure for intermediate libraries\n\n" << std::endl;
  }
...
void ConfigurationHandler::configureDataSources(
  ...
  bool isSinglEVH =
      StringUtil::toLowerCopy(
          ConfigurationData::getInstance()
              ->coreServerProperties.sprops["EVH_SINGLE"]) == "true";
  ...
  if (dbroot.getTagName() == "sdorm") {
    ...
    for (unsigned int dsnu = 0; dsnu < datasrcs.size(); dsnu++) {
      if (datasrcs.at(dsnu).getTagName() == "data-source") {
        ...
        if (!found_default &&
            (datasrcs.at(dsnu).getAttribute("default") == "true" ||
             datasrcs.size() == 1)) {
            ...
            for (unsigned int cns = 0; cns < confs.size(); cns++) {
              if (confs.at(cns).getTagName() == "nodes") {
                ...
                for (unsigned int nn = 0; nn < nodec.size(); nn++) {
                  if (nodec.at(nn).getTagName() != "node")
                    continue;
                  ...
                  for (unsigned int ncc = 0; ncc < nodes.size(); ncc++) {
                    if (nodes.at(ncc).getTagName() == "url") {
                      ...
                      try {
                        cnode.readTimeout =
                            CastUtil::toFloat(nodes.at(ncc).getText());
                      } catch (const std::exception &e) {
                      }
                    } else if (nodes.at(ncc).getTagName() ==
                               "connectionTimeout") {
                      try {
                        cnode.connectionTimeout =
                            CastUtil::toFloat(nodes.at(ncc).getText());
                      } catch (const std::exception &e) {
                      }
                ...
                cprops.addProperty(confs.at(cns).getTagName(),
                                   confs.at(cns).getText());
              }
            }
              ...
              if (tabs.at(dn).getTagName() == "table") {
                ...
                for (unsigned int cn = 0; cn < cols.size(); cn++) {
                  if (cols.at(cn).getTagName() == "hasOne") {
                    ...
                    dsempg.addPropertyColumnMapping(
                        cols.at(cn).getAttribute("obf"),
...
void ConfigurationHandler::configureCaches(const std::string &name,
  ...
  if (dbroot.getTagName() == "caches") {
    ...
    for (unsigned int dsnu = 0; dsnu < datasrcs.size(); dsnu++) {
      if (datasrcs.at(dsnu).getTagName() == "cache") {
        ...
        if (cprops.getName() != "") {
          ConfigurationData::getInstance()
              ->cacheConnProperties[name][cprops.getName()] = cprops;
        }
      }
    }
  }
...
void ConfigurationHandler::destroyCaches() { CacheManager::destroy(); }

void ConfigurationHandler::handleMarkerConfigurations(
    std::map<std::string, std::map<std::string, ClassStructure, std::less<>>,
             std::less<>> &clsstrucMaps,
    std::vector<WsDetails> &wsdvec, std::vector<bool> &stat, strVec &vecvp,
    strVec &pathvec,
    std::map<std::string, std::string, std::less<>> &ajintpthMap,
    std::map<std::string, std::string, std::less<>> &tpes,
    const std::string &serverRootDirectory, strVec &afcd, std::string &ajrt,
    Reflection &ref) {
  Logger logger = LoggerFactory::getLogger("ConfigurationHandler");
  std::map<std::string, std::map<std::string, ClassStructure, std::less<>>,
  ...
  for (it = clsstrucMaps.begin(); it != clsstrucMaps.end(); ++it) {
    ...
    for (itt = it->second.begin(); itt != it->second.end(); ++itt) {
      ...
      if (cs.markers.find("@SecurityProvider") != cs.markers.end()) {
        Marker secProv = cs.markers["@SecurityProvider"].at(
            cs.markers["@SecurityProvider"].size() - 1);

        if (cs.markers["@SecurityProvider"].size() > 1) {
          logger
              << ("Found more than one @SecurityProvider marker, only the last "
                  "defined marker will be considered, ignoring others..")
              << std::endl;
        }
        ...
        std::string sessionTimeoutV =
            secProv.getAttributeValue("sessionTimeout");
        ...
        if ((userfld != "" && passfld == "") ||
            (userfld == "" && passfld != "")) {
          logger << ("Both usernamefld/passwordfld need to be provided "
                     "together, skipping...")
                 << std::endl;
          continue;
        }

        if ((userfld != "" && userfrom != "" && passfrom == "") ||
            (passfld != "" && userfrom == "" && passfrom != "")) {
          logger << ("Both usernamefrom/passwordfrom need to be provided "
                     "together, skipping...")
                 << std::endl;
          continue;
        }

        if (ConfigurationData::getInstance()->securityObjectMap[appName].find(
                provName) != ConfigurationData::getInstance()
                                 ->securityObjectMap[appName]
                                 .end()) {
          logger << ("Provider with name " + provName +
                     " already exists, skipping duplicate...")
                 << std::endl;
          continue;
        }

        Security securityObject = ConfigurationData::getInstance()
                                      ->securityObjectMap[appName][provName];
        ...
        try {
          ...
          securityObject.sessTimeout = 3600;
          logger << ("Security: Invalid session timeout value defined, "
                     "defaulting to 1hour/3600sec")
                 << std::endl;
        }
        ...
        ConfigurationData::getInstance()->securityObjectMap[appName][provName] =
            securityObject;
        Bean bean(clas, "", clas, scope, false, false, appName);
        ConfigurationData::getInstance()->ffeadContext.addBean(bean);
        logger << ("Security: Adding Security Provider => " + url +
                   " , provider => " + provider)
               << std::endl;
      }
    }
    ...
    for (itt = it->second.begin(); itt != it->second.end(); ++itt) {
      ...
      if (cs.markers.find("@RestController") != cs.markers.end()) {
        ...
        Marker webcnt =
            cs.markers["@WebService"].at(cs.markers["@WebService"].size() - 1);
        if (cs.markers["@WebService"].size() > 1) {
          logger << ("Found more than one @WebService marker, only the last "
                     "defined marker will be considered, ignoring others..")
                 << std::endl;
        }
        WsDetails wsd;
        ...
        if (StringUtil::trimCopy(wsd.location) == "") {
          logger << ("No location defined for web-service, skipping...")
                 << std::endl;
          continue;
        }
        ...
        if (cs.markers.find("@Secure") != cs.markers.end()) {
          ...
          if (valid &&
              ConfigurationData::getInstance()->securityObjectMap[appName].find(
                  provName) != ConfigurationData::getInstance()
                                   ->securityObjectMap[appName]
                                   .end()) {
            Security securityObject =
            ...
            if (securityObject.addAspect(aspect)) {
              ...
              ConfigurationData::getInstance()
                  ->securityObjectMap[appName][provName] = securityObject;
              logger << ("Security: Adding Secure Path => " + aspect.path +
                         " , role => " + aspect.role)
                     << std::endl;
            }
          } else if (valid) {
            logger << ("Security Provider " + provName +
                       " not found, skipping...")
                   << std::endl;
          }
        }
        for (int var = 0; var < (int)cs.pubms.size(); ++var) {
          if (cs.pubms.at(var).markers.find("@WebServiceMethod") !=
              cs.pubms.at(var).markers.end()) {
            Marker websrvmth = cs.pubms.at(var).markers["@WebServiceMethod"].at(
                cs.pubms.at(var).markers["@WebServiceMethod"].size() - 1);
            if (cs.pubms.at(var).markers["@WebServiceMethod"].size() > 1) {
              logger << ("Found more than one @WebServiceMethod marker, only "
                         "the last defined marker will be considered, ignoring "
                         "others..")
                     << std::endl;
            }
            outnmmp[cs.pubms.at(var).name] =
                websrvmth.getAttributeValue("outname");
          }
        }
        wsd.outnmmp = outnmmp;
        wsd.appname = appName;
        ...
        for (int var = 0; var < (int)controllers.size(); ++var) {
          ...
          StringUtil::trim(url);
          if (url == "") {
            logger << ("No url defined for Controller, skipping...")
                   << std::endl;
            continue;
          }
          ...
          if (clas != "") {
            normalizeUrl(appName, url);
            ConfigurationData::getInstance()
                ->controllerObjectMap[appName][url] = clas;
            Bean bean(clas, "", clas, scope, false, true, appName);
            ConfigurationData::getInstance()->ffeadContext.addBean(bean);
          }
          logger << ("Adding Controller for " + (appName + url) + " :: " + clas)
                 << std::endl;
        }
        ...
        for (int var = 0; var < (int)ajaxintfs.size(); ++var) {
          ...
          if (StringUtil::trimCopy(url) == "") {
            logger << ("No path defined for AjaxInterface, skipping...")
                   << std::endl;
            continue;
          }
          if (cs.markers.find("@Secure") != cs.markers.end()) {
            ...
            SecureAspect aspect;
            aspect.role =
                cs.markers["@Secure"].at(totl - 1).getAttributeValue("role");
            aspect.path = url;
            normalizeUrl(appName, aspect.path);
            bool valid = true;
            if (StringUtil::trimCopy(aspect.role) == "") {
            ...
            std::string provName =
                cs.markers["@Secure"].at(totl - 1).getAttributeValue(
                    "providerName");
            if (StringUtil::trimCopy(provName) == "") {
              logger << ("No providerName defined for Secure path, skipping...")
                     << std::endl;
          ...
          if (url.find("*") == std::string::npos &&
            ...
            normalizeUrl(appName, url);
            std::string clas = cs.getFullyQualifiedClassName();
            ConfigurationData::getInstance()->ajaxInterfaceMap[appName][url] =
                clas;
            ...
            ajintpthMap[clas] = url;
            afcd.push_back(clas);
            logger << ("Adding Ajax Interface for " + (appName + url) +
                       " :: " + clas)
                   << std::endl;
            isJsObjects = true;
          } else {
            logger
                << ("Skipping Ajax Interface as path contains */regex function")
                << std::endl;
          }
        }
        ...
        for (int var = 0; var < (int)filters.size(); ++var) {
          ...
          if (clas != "" &&
            ...
            if (clas != "") {
              ConfigurationData::getInstance()
                  ->filterObjectMap[appName][url + type]
                  .push_back(clas);
              Bean bean(clas, "", clas, scope, false, false, appName);
              ConfigurationData::getInstance()->ffeadContext.addBean(bean);
            }
            logger << ("Adding Filter for " + (appName + url + type) +
                       " :: " + clas)
                   << std::endl;
          }
        }
        ...
        for (int var = 0; var < (int)templates.size(); ++var) {
          ...
          std::string file =
              StringUtil::trimCopy(templates.at(var).getAttributeValue("file"));
          if (StringUtil::trimCopy(url) == "") {
            logger << ("No path defined for Template, skipping...")
                   << std::endl;
            continue;
          }
          if (url.find("*") == std::string::npos &&
            ...
            StringUtil::replaceFirst(
                wbpi,
                ConfigurationData::getInstance()->coreServerProperties.webPath,
                "");
            RegexUtil::replace(wbpi, "[^a-zA-Z0-9_]+", "");
            ConfigurationData::getInstance()->templateMappingMap[appName][url] =
                clas + ";" + wbpi;
            tpes[fpath] = appName;
            // tpes.push_back(defpath+tmplts.at(tmpn).getAttribute("file"));
            ...
            logger << ("Skipping Template as path contains */regex function")
                   << std::endl;
          }
        }
        ...
        for (int var = 0; var < (int)templates.size(); ++var) {
          std::string clas = cs.getFullyQualifiedClassName();
          std::string url =
              StringUtil::trimCopy(templates.at(var).getAttributeValue("path"));
          if (StringUtil::trimCopy(url) == "") {
            logger << ("No path defined for WebSocketHandler, skipping...")
                   << std::endl;
            continue;
          }
          if (url.find("*") == std::string::npos &&
            ...
            normalizeUrl(appName, url);
            ConfigurationData::getInstance()
                ->websocketMappingMap[appName][url] = clas;
            std::string scope = templates.at(var).getAttributeValue("scope");
            if (clas != "") {
              Bean bean(clas, "", clas, scope, false, false, appName);
              ConfigurationData::getInstance()->ffeadContext.addBean(bean);
            }
            logger << ("Adding WebSocketHandler for " + (url) + " :: " + clas)
                   << std::endl;
          } else {
            logger << ("Skipping WebSocketHandler as path contains */regex "
                       "function")
        ...
        for (int var = 0; var < (int)dviews.size(); ++var) {
          ...
          logger << ("Adding Dynamic View for " + (appName + url) +
                     " :: " + clas)
                 << std::endl;
        }
      }
    ...
    if (isJsObjects) {
      std::string infjs;
      ajrt += AfcUtil::generateJsInterfacessAll(it->second, infjs, ajintpthMap,
                                                afcd, ref);
      ...
      pathvec.clear();
      ajintpthMap.clear();
      std::string webpubpath =
          serverRootDirectory + "/web/" + appName + "/public/";
...
void ConfigurationHandler::handleDataSourceEntities(
    const std::string &appName,
    std::map<std::string, Mapping, std::less<>> &mappings,
    std::map<std::string, ClassStructure, std::less<>> &allclsmap) {
  Logger logger = LoggerFactory::getLogger("ConfigurationHandler");
  ...
  for (itt = allclsmap.begin(); itt != allclsmap.end(); ++itt) {
    ClassStructure cs = itt->second;
    std::string clas = cs.getFullyQualifiedClassName();
    if (cs.markers.find("@Entity") != cs.markers.end() &&
        cs.markers.find("@Table") != cs.markers.end()) {
      ...
      if (entityv.size() > 1) {
        logger << ("Found more than one @Entity marker, only the last defined "
                   "marker will be considered, ignoring others..")
               << std::endl;
      }
      ...
      for (int var = 0; var < (int)csaps.size(); ++var) {
        ...
        for (piit = csaps.at(var).markers.begin();
             piit != csaps.at(var).markers.end(); ++piit) {
          std::vector<Marker> miit = piit->second;
          for (int mi = 0; mi < (int)miit.size(); mi++) {
            // std::cout << miit.at(mi).name << std::endl;
          }
        }
        if (csaps.at(var).markers.find("@Column") !=
            csaps.at(var).markers.end()) {
          if (csaps.at(var).markers["@Column"].size() > 1) {
            logger << ("Found more than one @Column marker, only the last "
                       "defined marker will be considered, ignoring others..")
                   << std::endl;
          }
          Marker colmrk = csaps.at(var).markers["@Column"].at(
              csaps.at(var).markers["@Column"].size() - 1);
          std::string dbf = colmrk.getAttributeValue("dbf");
          if (dbf == "")
            dbf = csaps.at(var).name;
          dsempg.addPropertyColumnMapping(csaps.at(var).name, dbf);
        } else if (csaps.at(var).markers.find("@Id") !=
                   csaps.at(var).markers.end()) {
          if (csaps.at(var).markers["@Id"].size() > 1) {
            logger << ("Found more than one @Id marker, only the last defined "
                       "marker will be considered, ignoring others..")
                   << std::endl;
          }
          if (!idfound) {
            Marker colmrk = csaps.at(var).markers["@Id"].at(
                csaps.at(var).markers["@Id"].size() - 1);
            std::string dbf = colmrk.getAttributeValue("dbf");
            if (dbf == "")
              dbf = csaps.at(var).name;
            dsempg.addPropertyColumnMapping(csaps.at(var).name, dbf);
            ...
            idfound = true;
            if (csaps.at(var).markers.find("@IdGenerate") !=
                csaps.at(var).markers.end()) {
              if (csaps.at(var).markers["@IdGenerate"].size() > 1) {
                logger
                    << ("Found more than one @IdGenerate marker, only the last "
                        "defined marker will be considered, ignoring others..")
                    << std::endl;
              }
              colmrk = csaps.at(var).markers["@IdGenerate"].at(
                  csaps.at(var).markers["@IdGenerate"].size() - 1);
              dsempg.idgendbEntityType =
                  colmrk.getAttributeValue("dbEntityType");
              dsempg.idgendbEntityName =
                  colmrk.getAttributeValue("dbEntityName");
              dsempg.idgentype = colmrk.getAttributeValue("type");
              dsempg.idgenhiValueColumn =
                  colmrk.getAttributeValue("hiValueColumn");
              dsempg.idgenlowValue =
                  CastUtil::toInt(colmrk.getAttributeValue("lowValue"));
              dsempg.idgenentityColumn =
                  colmrk.getAttributeValue("entityColumn");
              dsempg.idgencolumnName = colmrk.getAttributeValue("columnName");
              if (dsempg.idgencolumnName == "")
                dsempg.idgencolumnName = csaps.at(var).name;
            }
          } else {
            logger << ("Multiple Id properties defined, skipping...")
                   << std::endl;
            skipIt = true;
            break;
          }
        ...
        if (csaps.at(var).markers.find("@HasOne") !=
            csaps.at(var).markers.end()) {
          if (csaps.at(var).markers["@HasOne"].size() > 1) {
            logger << ("Found more than one @HasOne marker, only the last "
                       "defined marker will be considered, ignoring others..")
                   << std::endl;
          }
          Marker colmrk = csaps.at(var).markers["@HasOne"].at(
              csaps.at(var).markers["@HasOne"].size() - 1);
          std::string dfk = colmrk.getAttributeValue("dfk");
          std::string dmappedBy = colmrk.getAttributeValue("dmappedBy");
          if (StringUtil::trimCopy(dfk) == "" ||
              StringUtil::trimCopy(dmappedBy) == "") {
            logger << ("No dfk/dmappedBy defined for HasOne marker, both need "
                       "to be defined, skipping...")
                   << std::endl;
            continue;
          }
          DataSourceInternalRelation relation;
          ...
          dsempg.addRelation(relation);
        } else if (csaps.at(var).markers.find("@HasMany") !=
                   csaps.at(var).markers.end()) {
          if (csaps.at(var).markers["@HasMany"].size() > 1) {
            logger << ("Found more than one @HasMany marker, only the last "
                       "defined marker will be considered, ignoring others..")
                   << std::endl;
          }
          Marker colmrk = csaps.at(var).markers["@HasMany"].at(
              csaps.at(var).markers["@HasMany"].size() - 1);
          std::string dfk = colmrk.getAttributeValue("dfk");
          std::string dmappedBy = colmrk.getAttributeValue("dmappedBy");
          if (StringUtil::trimCopy(dfk) == "" ||
              StringUtil::trimCopy(dmappedBy) == "") {
            logger << ("No dfk/dmappedBy defined for HasMany marker, both need "
                       "to be defined, skipping...")
                   << std::endl;
            continue;
          }
          ...
          if (mtype.find("vector<") == std::string::npos ||
              mtype.find("std::vector<") == std::string::npos) {
            logger << ("HasMany association can have only a stl vector "
                       "mapping, skipping...")
                   << std::endl;
            continue;
          }
      ...
      if (!skipIt) {
        if (idfound) {
          if (mappings.find(dataSourceName) != mappings.end() &&
              mappings[dataSourceName].appName == appName) {
            Mapping mp = mappings[dataSourceName];
            mp.addDataSourceEntityMapping(dsempg);
            mappings[dataSourceName] = mp;
            // std::cout << "adding mapping for dsn " << dataSourceName << ",
            // app is " << appName << std::endl;
          } else {
            std::map<std::string, Mapping, std::less<>>::iterator it;
            for (it = mappings.begin(); it != mappings.end(); ++it) {
              if (it->second.appName == appName) {
                // std::cout << "adding mapping for dsn " << it->first << ", app
                // is " << appName << std::endl;
...
Marker ConfigurationHandler::getRestFunctionMarker(
    std::map<std::string, std::vector<Marker>> &markers) {
  Logger logger = LoggerFactory::getLogger("ConfigurationHandler");
  if (markers.find("@GET") != markers.end()) {
    if (markers["@GET"].size() > 1) {
      logger << ("Found more than one @GET marker, only the last defined "
                 "marker will be considered, ignoring others..")
             << std::endl;
    }
    ...
    if (markers["@POST"].size() > 1) {
      logger << ("Found more than one @POST marker, only the last defined "
                 "marker will be considered, ignoring others..")
             << std::endl;
    }
    ...
    if (markers["@PUT"].size() > 1) {
      logger << ("Found more than one @PUT marker, only the last defined "
                 "marker will be considered, ignoring others..")
             << std::endl;
    }
    ...
    if (markers["@DELETE"].size() > 1) {
      logger << ("Found more than one @DELETE marker, only the last defined "
                 "marker will be considered, ignoring others..")
             << std::endl;
    }
...
Marker ConfigurationHandler::getRestFunctionParamMarker(
    std::map<std::string, std::vector<Marker>> &markers) {
  Logger logger = LoggerFactory::getLogger("ConfigurationHandler");
  if (markers.find("@PathParam") != markers.end()) {
    if (markers["@PathParam"].size() > 1) {
      logger << ("Found more than one @PathParam marker, only the last defined "
                 "marker will be considered, ignoring others..")
             << std::endl;
    }
    ...
    if (markers["@QueryParam"].size() > 1) {
      logger << ("Found more than one @QueryParam marker, only the last "
                 "defined marker will be considered, ignoring others..")
             << std::endl;
    }
    ...
    if (markers["@FormParam"].size() > 1) {
      logger << ("Found more than one @FormParam marker, only the last defined "
                 "marker will be considered, ignoring others..")
             << std::endl;
    }
    ...
    if (markers["@HeaderParam"].size() > 1) {
      logger << ("Found more than one @HeaderParam marker, only the last "
                 "defined marker will be considered, ignoring others..")
             << std::endl;
    }
    ...
    if (markers["@Body"].size() > 1) {
      logger << ("Found more than one @Body marker, only the last defined "
                 "marker will be considered, ignoring others..")
             << std::endl;
    }
    ...
    if (markers["@MultipartContent"].size() > 1) {
      logger << ("Found more than one @MultipartContent marker, only the last "
                 "defined marker will be considered, ignoring others..")
             << std::endl;
    }
    return markers["@MultipartContent"].at(markers["@MultipartContent"].size() -
                                           1);
  }
  return Marker();
}

void ConfigurationHandler::handleRestControllerMarker(
    ClassStructure &cs, const std::string &appName) {
  Logger logger = LoggerFactory::getLogger("ConfigurationHandler");
  Marker restcnt = cs.markers["@RestController"].at(
      cs.markers["@RestController"].size() - 1);
  if (cs.markers["@RestController"].size() > 1) {
    logger << ("Found more than one @RestController marker, only the last "
               "defined marker will be considered, ignoring others..")
           << std::endl;
  }
  ...
  if (cs.markers.find("@Secure") != cs.markers.end()) {
    ...
    normalizeUrl(appName, aspect.path);
    if (cs.markers["@Secure"].size() > 1) {
      logger << ("Found more than one @Secure marker, only the last defined "
                 "marker will be considered, ignoring others..")
             << std::endl;
    }
    std::string provName =
        cs.markers["@Secure"].at(totl - 1).getAttributeValue("providerName");
    if (StringUtil::trimCopy(provName) == "") {
      logger << ("No providerName defined for Secure path, skipping...")
             << std::endl;
      valid = false;
    ...
    if (valid &&
      ...
      logger << ("Security Provider " + provName + " not found, skipping...")
             << std::endl;
    }
  }
  ...
  for (int var = 0; var < (int)cs.pubms.size(); ++var) {
    ...
    if (m.getName() != "") {
      ...
      CryptoHandler::deSanitizeHtml(restfunction.rtype);
      restfunction.serOpt =
          SerializeBase::identifySerOption(cs.pubms.at(var).retType);
      ...
      if (restfunction.statusCode != "") {
        try {
          ...
          logger << "Rest: invalid response statusCode specified, defaulting "
                    "to 200.."
                 << std::endl;
          restfunction.statusCode = "200";
        }
      ...
      restfunction.unmapped =
          StringUtil::toLowerCopy(m.getAttributeValue("unmapped")) == "true";
      ...
      if (StringUtil::toLowerCopy(restfunction.ocontentType)
              .find(ContentTypes::CONTENT_TYPE_APPLICATION_JSON) == 0) {
        restfunction.serOpt += 1000;
      } else if (StringUtil::toLowerCopy(restfunction.ocontentType)
                     .find(ContentTypes::CONTENT_TYPE_APPLICATION_XML) == 0) {
        restfunction.serOpt += 2000;
      }

      std::map<int, std::map<std::string, std::vector<Marker>>> argMarkers =
          cs.pubms.at(var).argMarkers;
      ...
      for (pit = argMarkers.begin(); pit != argMarkers.end(); ++pit) {
        ...
        if (param.type.find("std::vector<") == 0 ||
            param.type.find("vector<") == 0 ||
            param.type.find("std::list<") == 0 ||
            param.type.find("list<") == 0 ||
            param.type.find("std::deque<") == 0 ||
            param.type.find("deque<") == 0 ||
            param.type.find("std::queue<") == 0 ||
            param.type.find("queue<") == 0 ||
            param.type.find("std::set<") == 0 || param.type.find("set<") == 0 ||
            param.type.find("std::multiset<") == 0 ||
            param.type.find("multiset<") == 0) {
        ...
        if (pit->second.find("@DefaultValue") != pit->second.end()) {
          ...
          if (defmarkers.size() > 1) {
            logger
                << ("Found more than one @DefaultValue marker, only the last "
                    "defined marker will be considered, ignoring others..")
                << std::endl;
          }
          if (defmarkers.at(defmarkers.size() - 1).getAttributeValue("value") !=
              "") {
            param.defValue =
                defmarkers.at(defmarkers.size() - 1).getAttributeValue("value");
          }
        }
        ...
        if (StringUtil::trimCopy(ttype) == "") {
        ...
        } else if ((ttype == "filestream" || ttype == "vector-of-filestream") &&
                   param.from != "multipart-content") {

        } else if (ttype == "vector-of-filestream" &&
                   param.from == "multipart-content" &&
                   StringUtil::trimCopy(param.name) == "") {
        ...
        } else if (param.from == "body" && (restfunction.meth == "GET" ||
                                            restfunction.meth == "OPTIONS" ||
                                            restfunction.meth == "TRACE" ||
                                            restfunction.meth == "HEAD")) {
          invalidParam = true;
          logger << ("Rest: skipping param " + param.type +
                     ", from is body and method is " + restfunction.meth)
                 << std::endl;
        } else if (!(SerializeBase::isPrimitiveDataType(ttype) ||
                     ttype == "filestream" ||
                     ttype == "vector-of-filestream") &&
                   param.from != "body") {
          invalidParam = true;
          logger << ("Rest: skipping param " + param.type +
                     ", from is not body and input is a complex type")
                 << std::endl;
        } else if (param.from == "postparam" &&
                   (restfunction.meth == "GET" ||
                    restfunction.meth == "OPTIONS" ||
          ...
          invalidParam = true;
          logger << ("Rest: skipping param " + param.type +
                     ", from is postparam and method is " + restfunction.meth)
                 << std::endl;
        }
        if (!invalidParam) {
          restfunction.params.push_back(param);
      ...
      if (clas != "" && url.find("*") == std::string::npos) {
        ...
        if (restfunction.path != "") {
          ...
          restfunction.path = urlmpp;
          ConfigurationData::getInstance()
              ->rstCntMap[appName][restfunction.meth + urlmpp]
              .push_back(restfunction);
        } else {
        ...
        Bean bean(clas, "", clas, scope, false, true, appName);
        ConfigurationData::getInstance()->ffeadContext.addBean(bean);
        if (!isClasSecure && cs.pubms.at(var).markers.find("@Secure") !=
                                 cs.pubms.at(var).markers.end()) {
          int totl = cs.pubms.at(var).markers["@Secure"].size();
          SecureAspect aspect;
          aspect.role = cs.pubms.at(var)
                            .markers["@Secure"]
                            .at(totl - 1)
          ...
          aspect.path = urlmpp;
          normalizeUrl(appName, aspect.path);
          std::string provName = cs.pubms.at(var)
                                     .markers["@Secure"]
                                     .at(totl - 1)
                                     .getAttributeValue("providerName");
          if (StringUtil::trimCopy(provName) == "") {
            logger << ("No providerName defined for Secure path, skipping...")
                   << std::endl;
            valid = false;
...
void ConfigurationHandler::initializeDataSources() {
  ...
  Logger logger = LoggerFactory::getLogger("ConfigurationHandler");
  std::map<std::string, bool, std::less<>> mycntxts =
      ConfigurationData::getInstance()->servingContexts;
  std::map<std::string,
           std::map<std::string, ConnectionProperties, std::less<>>,
           std::less<>>
      sdormConnProperties =
          ConfigurationData::getInstance()->sdormConnProperties;
  std::map<std::string, std::map<std::string, Mapping, std::less<>>,
           std::less<>>
      sdormEntityMappings =
          ConfigurationData::getInstance()->sdormEntityMappings;
  ...
  for (mit = mycntxts.begin(); mit != mycntxts.end(); ++mit) {
    ...
    if (sdormConnProperties.find(mit->first) != sdormConnProperties.end()) {
      ...
      std::map<std::string, ConnectionProperties, std::less<>> allProps =
          sdormConnProperties[mit->first];
      std::map<std::string, Mapping, std::less<>> allMapps =
          sdormEntityMappings[mit->first];
      for (it = allProps.begin(); it != allProps.end(); ++it) {
        logger << "ds is " << it->first << std::endl;
        DataSourceManager::initDSN(
            allProps[it->first], allMapps[it->first],
            &ConfigurationHandler::populateClassBeanInstanceCb);
      }
    }
  }
}

void ConfigurationHandler::configureSearches(const std::string &name,
                                             const std::string &configFile) {
  Logger logger = LoggerFactory::getLogger("ConfigurationHandler");
  SimpleXmlParser parser("Parser");
  ...
  if (dbroot.getTagName() == "searches") {
    ...
    for (unsigned int dsnu = 0; dsnu < datasrcs.size(); dsnu++) {
      if (datasrcs.at(dsnu).getTagName() == "search") {
        ...
        if (cprops.getName() != "") {
          ConfigurationData::getInstance()
              ->searchConnProperties[name][cprops.getName()] = cprops;
        }
      }
    }
  }
...
void ConfigurationHandler::initializeSearches() {
  ...
  std::map<std::string,
           std::map<std::string, ConnectionProperties, std::less<>>,
           std::less<>>
      searchConnProperties =
          ConfigurationData::getInstance()->searchConnProperties;
  std::map<std::string, bool, std::less<>>::iterator mit;
  for (mit = mycntxts.begin(); mit != mycntxts.end(); ++mit) {
    if (searchConnProperties.find(mit->first) != searchConnProperties.end()) {
      std::map<std::string, ConnectionProperties, std::less<>> allProps =
          searchConnProperties[mit->first];
      std::map<std::string, ConnectionProperties, std::less<>>::iterator it;
      for (it = allProps.begin(); it != allProps.end(); ++it) {
        SearchEngineManager::initSearch(
            allProps[it->first], mit->first,
            &ConfigurationHandler::populateClassBeanInstanceCb);
      }
    }
  }
...
void ConfigurationHandler::destroySearches() { SearchEngineManager::destroy(); }

void ConfigurationHandler::populateClassBeanInstanceCb(std::string &clsn,
                                                       std::string appn,
                                                       ClassBeanIns *cbi) {
  cbi->clas = ConfigurationData::getReflector()->getClassInfo(clsn, appn);
  if (cbi->clas->getClassName() != "") {
    void *_temp =
        ConfigurationData::getInstance()->ffeadContext.getBean(clsn, appn);
    if (_temp != NULL) {
      cbi->instance = _temp;
      cbi->cleanUp = false;
      return;
...
void ConfigurationHandler::initializeCaches() {
  ...
  std::map<std::string,
           ...
           std::less<>>
      cacheConnProperties =
          ConfigurationData::getInstance()->cacheConnProperties;
  std::map<std::string, bool, std::less<>>::iterator mit;
  for (mit = mycntxts.begin(); mit != mycntxts.end(); ++mit) {
    if (cacheConnProperties.find(mit->first) != cacheConnProperties.end()) {
      std::map<std::string, ConnectionProperties, std::less<>> allProps =
          cacheConnProperties[mit->first];
      std::map<std::string, ConnectionProperties, std::less<>>::iterator it;
      for (it = allProps.begin(); it != allProps.end(); ++it) {
...
void ConfigurationHandler::initializeWsdls() {
  ...
  std::map<std::string, bool, std::less<>> mycntxts =
      ConfigurationData::getInstance()->servingContexts;
  std::map<std::string, std::vector<WsDetails>, std::less<>>
      webserviceDetailMap =
          ConfigurationData::getInstance()->webserviceDetailMap;
  std::map<std::string, bool, std::less<>>::iterator mit;
  for (mit = mycntxts.begin(); mit != mycntxts.end(); ++mit) {
    if (webserviceDetailMap.find(mit->first) != webserviceDetailMap.end()) {
      ...
      wsu.generateAllWSDL(
          allwsdets,
          ConfigurationData::getInstance()->coreServerProperties.resourcePath,
