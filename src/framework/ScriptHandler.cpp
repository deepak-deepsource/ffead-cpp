...
std::string ScriptHandler::chdirExecute(const std::string &exe,
...
#if !defined(OS_MINGW)
int ScriptHandler::popenRWE(int *rwepipe, const char *exe,
                            const char *const argv[], const std::string &tmpf) {
  int in[2];
  int out[2];
  int err[2];
  int pid;
...
#ifdef INC_SCRH
bool ScriptHandler::handle(HttpRequest *req, HttpResponse *res,
  ...
  if(referer.find("http://")!=std::string::npos)
  {
          ...
          if(appl.find(req->getCntxt_name())==0 &&
  handoffs.find(req->getCntxt_name())!=handoffs.end())
          {
