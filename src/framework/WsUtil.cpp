...
std::string WsUtil::generateAllWSDL(
    const std::vector<WsDetails> &wsdvec, const std::string &resp,
    Reflection &ref,
    std::map<std::string, std::map<std::string, ClassStructure, std::less<>>,
             std::less<>> &clsstrucMaps) {
  std::string ret,
      headers = "#include \"AppDefines.h\"\n#include \"Exception.h\"\n#include "
                "\"string\"\n#include <sstream>\n#include "
  ...
  return (headers + "\n\n" +
          "extern \"C\"\n{\nvoid normalizeNamespaces(Element* ele, std::string "
          "appname, std::string& allnmspcs) { 	ElementList elements = "
          "ele->getChildElements(); 	std::string nmspc = "
          "ele->getAttribute(\"namespace\"); 	if(nmspc!=\"\") 	{ "
          "		std::string nmspcid = "
          "Reflection::getNameSpaceId(nmspc, appname); 		"
          "ele->setTagName(nmspcid+\":\"+ele->getTagName()); 		"
          "ele->removeAttribute(\"namespace\"); 		allnmspcs += "
          "(\" xmlns:\" + nmspcid + \"=\\\"\" + nmspc + \"\\\"\"); 	} "
          "	for(unsigned int i=0;i<elements.size();i++) 	{ 	"
          "	normalizeNamespaces((Element*)&(elements.at(i)), appname, "
          "allnmspcs);		} }" +
          ret + "\n}\n");
}

std::vector<WsDetails>
WsUtil::getWsDetails(const std::vector<std::string> &apps,
  ...
  for (unsigned int var = 0; var < apps.size(); ++var) {
    ...
    for (unsigned int ii = 0; ii < root.getChildElements().size(); ii++) {
      ...
      for (unsigned int i = 0; i < ws->getChildElements().size(); i++) {
        outnmmp[ws->getChildElements().at(i).getTagName()] =
            ws->getChildElements().at(i).getAttribute("outname");
      }
      WsDetails wsd;
      ...
      ConfigurationData::getInstance()
          ->webserviceDetailMap[wsd.appname]
          .push_back(wsd);
    }
  }
  return wsdvec;
...
void WsUtil::handleWebService(
    std::string &ws_funcs, const WsDetails &wsd,
    std::map<std::string, std::map<std::string, ClassStructure, std::less<>>,
             std::less<>> &clsstrucMaps,
    const std::string &resp, std::string &headers, Reflection &ref) {
  ...
  std::map<std::string, ClassStructure, std::less<>> &allclsmap =
      clsstrucMaps[appname];

  ws_inp_out_Map ws_info;
  meth_Map meth_info;
  StringContext gcntxt;
  std::string ws_name, reqr_res_bind, wsdl_msgs, wsdl_ops, wsdl_bind, wsdl,
      wsdl_obj_bind;
  ...
  if (StringUtil::trimCopy(ws_name) == "") {
    // logger << ("No class name defined for web-service, skipping...") <<
    // std::endl;
    return;
  }
  ...
  // logger << ("Web service " + ws_name + " found for appname " + appname) <<
  // std::endl;
  ...
  if (clstruct == NULL) {
    // logger << ("Error generating web-service artifacts, as class "+claz +
    // "not found...") << std::endl;
    return;
  }
  ...
  headers.append("#include \"" +
                 ref.getClassPath(clstruct->getTreatedClassName(true)) +
                 "\"\n");
  ...
  for (unsigned int i = 0; i < info.size(); i++) {
    ...
    if (results.size() < 2)
      continue;
    if (results.size() > 0 &&
        (results.at(0) == it->second.getTreatedClassName(false) ||
    ...
    if (results.at(0).find("vector<") != std::string::npos) {
      ...
      if (vecn == "int" || vecn == "string" || vecn == "short" ||
          vecn == "float" || vecn == "double" || vecn == "bool" ||
          vecn == "long long" || vecn == "long" || vecn == "unsigned int" ||
          vecn == "unsigned short" || vecn == "unsigned long long" ||
          vecn == "unsigned long" || vecn == "char" ||
          vecn == "unsigned char") {
        if (vecn == "long long") {
          vecn = "long";
        ...
        retType =
            "\n<xsd:element minOccurs=\"0\" maxOccurs=\"unbounded\" name=\"" +
            in_out_info["RETURN"] + "\" type=\"xsd:" + vecn + "\"/>";
      } else {
        std::string trgnmspc;
        std::string fqcn =
            ref.getFullyQualifiedClassName(vecn, clstruct->getNamespaces());
        if (trgnms.find(fqcn) == trgnms.end()) {
          xsdobjdef =
              ref.getXSDDefinitions(allclsmap, fqcn, ref, appname, trgnmspc,
                                    allnmspcs, gcntxt["WS_NMSPC"], resp);
        ...
        retType =
            "\n<xsd:element minOccurs=\"0\" maxOccurs=\"unbounded\" name=\"" +
            in_out_info["RETURN"] + "\" type=\"" + trgnmspc + ":" + vecn +
            "\"/>";
      }
    } else {
      if (results.at(0) != "void") {
        if (results.at(0) == "int" || results.at(0) == "string" ||
            results.at(0) == "short" || results.at(0) == "float" ||
            results.at(0) == "double" || results.at(0) == "bool" ||
            results.at(0) == "long long" || results.at(0) == "long" ||
            results.at(0) == "unsigned int" ||
            results.at(0) == "unsigned short" ||
            results.at(0) == "unsigned long long" ||
            results.at(0) == "unsigned long" || results.at(0) == "char" ||
            results.at(0) == "unsigned char") {
          ...
          retType = "\n<xsd:element name=\"" + in_out_info["RETURN"] +
                    "\" type=\"xsd:" + results.at(0) + "\"/>";
        } else {
          std::string type = results.at(0);
          std::string trgnmspc;
          std::string fqcn =
              ref.getFullyQualifiedClassName(type, clstruct->getNamespaces());
          if (trgnms.find(fqcn) == trgnms.end()) {
            xsdobjdef =
                ref.getXSDDefinitions(allclsmap, fqcn, ref, appname, trgnmspc,
                                      allnmspcs, gcntxt["WS_NMSPC"], resp);
    ...
    for (unsigned int j = 0; j < results1.size(); j++) {
      ...
      std::string te;
      ss >> te;
      if (type == "int" || type == "string" || type == "short" ||
          type == "float" || type == "double" || type == "bool" ||
          type == "long long" || type == "long" || type == "unsigned int" ||
          type == "unsigned short" || type == "unsigned long long" ||
          type == "unsigned long" || type == "char" ||
          type == "unsigned char") {
        ...
        if (results2.size() >= 2) {
          ...
          // logger << ("Invalid thing happening " + results1.at(j)) <<
          // std::endl;
        }
        strMap allfs, tyfs;
        ...
        if (type.find("vector<") != std::string::npos && results2.size() == 2) {
          ...
          StringUtil::replaceFirst(vecn, ">", " ");
          StringUtil::trim(vecn);
          if (vecn == "int" || vecn == "short" || vecn == "float" ||
              vecn == "double" || vecn == "bool" || vecn == "long long" ||
              vecn == "long" || vecn == "string" || vecn == "unsigned int" ||
              vecn == "unsigned short" || vecn == "unsigned long long" ||
              vecn == "unsigned long" || vecn == "char" ||
              vecn == "unsigned char") {
            if (vecn == "long long") {
            ...
            inp_params.append("\n<xsd:element minOccurs=\"0\" "
                              "maxOccurs=\"unbounded\" name=\"" +
                              results2.at(1) + "\" type=\"xsd:" + vecn +
                              "\"/>");
          } else {
            std::string trgnmspc;
            std::string fqcn =
            ...
            if (vecn.find("::") != std::string::npos)
              vecn = vecn.substr(vecn.find_last_of("::") + 1);
            // logger << "in result after " + vecn + " " + trgnmspc <<
            // std::endl;
            inp_params.append("\n<xsd:element minOccurs=\"0\" "
                              "maxOccurs=\"unbounded\" name=\"" +
                              results2.at(1) + "\" type=\"" + trgnmspc + ":" +
                              vecn + "\"/>");
          }
  ...
  AfcUtil::writeTofile(
      resp + "../web/" + appname + "/public/" + ws_name + ".wsdl", wsdl, true);
  ...
  for (iter = ws_info.begin(); iter != ws_info.end(); iter++) {
    ...
    for (iter1 = meth.begin(); iter1 != meth.end(); iter1++) {
      ...
      ws_funcs.append("std::string " + appname + me_n + ws_nn +
                      "(Element* _req)\n{\nElement* ele = NULL;\n");
      ...
      for (iter2 = pars.begin(); iter2 != pars.end(); iter2++) {
        if (iter2->first != "RETURN" && iter2->first != "RETURNTYP") {
          ...
          if (iter2->second == "int" || iter2->second == "short" ||
              iter2->second == "double" || iter2->second == "float" ||
              iter2->second == "string" || iter2->second == "bool" ||
              iter2->second == "long" || iter2->second == "long long" ||
              iter2->second == "unsigned int" ||
              iter2->second == "unsigned short" ||
              iter2->second == "unsigned long" ||
              iter2->second == "unsigned long long" ||
              iter2->second == "std::string" || iter2->second == "char" ||
              iter2->second == "unsigned char") {
            ws_funcs.append("ele = _req->getElementByNameIgnoreCase(\"" +
                            argname + "\");\n");
            ws_funcs.append(iter2->second + " " + argname + ";\n");
            ws_funcs.append("if(ele!=NULL)");
            ws_funcs.append(argname + " = CastUtil::lexical_cast<" +
                            iter2->second + ">(ele->getText());\n");
            ...
            if (vecn == "int" || vecn == "short" || vecn == "float" ||
              ...
              ws_funcs.append("ElementList list = _req->getElementsByName(\"" +
                              argname + "\");\n");
              ws_funcs.append("for(int i=0;i<list.size();i++)");
              ws_funcs.append(argname + ".push_back(CastUtil::lexical_cast<" +
                              vecn + ">((&(list.at(i)))->getText()));\n");
            } else {
              std::string serOpt =
              ...
              ws_funcs.append(
                  argname +
                  ".push_back(XMLSerialize::unserialize<std::vector<" + vecn +
                  " > >(list.at(i), " + serOpt + ", \"vector<" + vecn +
                  " >\", \"" + appname + "\"));\n}\n");
            }
          } else {
            std::string serOpt = CastUtil::fromNumber(
                SerializeBase::identifySerOption(iter2->second));
            ws_funcs.append("ele = _req->getElementByNameIgnoreCase(\"" +
                            argname + "\");\n");
            ws_funcs.append(iter2->second + " " + argname + ";\n");
            ws_funcs.append("if(ele!=NULL)");
            ...
            ws_funcs.append("\n" + argname + " = XMLSerialize::unserialize<" +
                            iter2->second + " >(ele, " + serOpt + ", \"" +
                            iter2->second + "\", \"" + appname + "\");\n");
            // ws_funcs.append("\ncout << ele->renderSerialization() <<
            // std::endl;");
            ws_funcs.append("}\n");
          }
          args.append(argname);
      ...
      ws_funcs.append(
          "_retStr = \"<tns:\" + _req->getTagName() + \"Response\";\n");
      // ws_funcs.append("for(it=attl.begin();it!=attl.end();it++)\n");
      // ws_funcs.append("_retStr.append(\" \" + it->first + \"=\\\"\" +
      // it->second + \"\\\" \");\n");
      ws_funcs.append("_retStr.append(\">\");\n");
      if (pars["RETURNTYP"] == "void") {
        ws_funcs.append("_obj." + me_n + "(" + args + ");\n");
      } else if (pars["RETURNTYP"] == "int" || pars["RETURNTYP"] == "short" ||
                 pars["RETURNTYP"] == "long long" ||
                 pars["RETURNTYP"] == "long" ||
                 pars["RETURNTYP"] == "unsigned int" ||
                 pars["RETURNTYP"] == "unsigned short" ||
                 pars["RETURNTYP"] == "unsigned long long" ||
                 pars["RETURNTYP"] == "unsigned long" ||
                 pars["RETURNTYP"] == "char" ||
                 pars["RETURNTYP"] == "unsigned char") {
        ws_funcs.append(pars["RETURNTYP"] + " _retval;\n");
        ws_funcs.append("_retval = _obj." + me_n + "(" + args + ");\n");
        ws_funcs.append("_retStr += \"<tns:" + pars["RETURN"] +
                        ">\"+CastUtil::fromNumber(_retval)+\"</tns:" +
                        pars["RETURN"] + ">\";\n");
      } else if (pars["RETURNTYP"] == "float" ||
                 pars["RETURNTYP"] == "double") {
        ws_funcs.append(pars["RETURNTYP"] + " _retval;\n");
        ws_funcs.append("_retval = _obj." + me_n + "(" + args + ");\n");
        ws_funcs.append("_retStr += \"<tns:" + pars["RETURN"] +
                        ">\"+CastUtil::fromDouble(_retval)+\"</tns:" +
                        pars["RETURN"] + ">\";\n");
      } else if (pars["RETURNTYP"] == "bool") {
        ws_funcs.append(pars["RETURNTYP"] + " _retval;\n");
        ws_funcs.append("_retval = _obj." + me_n + "(" + args + ");\n");
        ws_funcs.append("_retStr += \"<tns:" + pars["RETURN"] +
                        ">\"+CastUtil::fromBool(_retval)+\"</tns:" +
                        pars["RETURN"] + ">\";\n");
      } else if (pars["RETURNTYP"] == "string") {
        ws_funcs.append(pars["RETURNTYP"] + " _retval;\n");
        ws_funcs.append("_retval = _obj." + me_n + "(" + args + ");\n");
        ws_funcs.append("_retStr += \"<tns:" + pars["RETURN"] +
                        ">\"+_retval+\"</tns:" + pars["RETURN"] + ">\";\n");
      } else if (pars["RETURNTYP"] != "") {
        std::string serOpt = CastUtil::fromNumber(
            SerializeBase::identifySerOption(pars["RETURNTYP"]));
        ws_funcs.append(pars["RETURNTYP"] + " _retval;\n");
        ws_funcs.append("_retval = _obj." + me_n + "(" + args + ");\n");
        if (pars["RETURNTYP"].find("vector<") != std::string::npos) {
          ...
          StringUtil::replaceFirst(vecn, ">", " ");
          StringUtil::trim(vecn);
          if (vecn == "int" || vecn == "short" || vecn == "long long" ||
              vecn == "long" || vecn == "unsigned int" ||
              vecn == "unsigned short" || vecn == "unsigned long long" ||
              vecn == "unsigned long" || vecn == "char" ||
              vecn == "unsigned char") {
            ws_funcs.append("for(int i=0;i<_retval.size();i++)");
            ws_funcs.append("_retStr += \"<tns:" + pars["RETURN"] +
                            ">\"+CastUtil::fromNumber(_retval.at(i))+\"</tns:" +
                            pars["RETURN"] + ">\";\n");
          } else if (vecn == "short" || vecn == "float") {
            ws_funcs.append("for(int i=0;i<_retval.size();i++)");
            ws_funcs.append("_retStr += \"<tns:" + pars["RETURN"] +
                            ">\"+CastUtil::fromDouble(_retval.at(i))+\"</tns:" +
                            pars["RETURN"] + ">\";\n");
          } else if (vecn == "bool") {
            ws_funcs.append("for(int i=0;i<_retval.size();i++)");
            ws_funcs.append("_retStr += \"<tns:" + pars["RETURN"] +
                            ">\"+CastUtil::fromBool(_retval.at(i))+\"</tns:" +
                            pars["RETURN"] + ">\";\n");
          } else if (vecn == "string") {
            ws_funcs.append("for(int i=0;i<_retval.size();i++)");
            ws_funcs.append("_retStr += \"<tns:" + pars["RETURN"] +
                            ">\"+_retval.at(i)+\"</tns:" + pars["RETURN"] +
                            ">\";\n");
          } else {
            ...
            ws_funcs.append(
                "Document doc;\nparser.parse(XMLSerialize::serialize<" +
                pars["RETURNTYP"] + " >(&(_retval.at(i))," + serOpt + ",\"" +
                vecn + "\",\"" + appname + "\"), doc);\n");
            ws_funcs.append("normalizeNamespaces(&(doc.getRootElement()),\"" +
                            appname + "\",allnmspcs);\n");
            ws_funcs.append(
                "_retStr += \"<tns:" + pars["RETURN"] +
                ">\"+doc.getRootElement().renderChildren()+\"</tns:" +
                pars["RETURN"] + ">\";\n}\n");
          }
        } else {
          ws_funcs.append("std::string allnmspcs;\n");
          ...
          ws_funcs.append(
              "Document doc;\nparser.parse(XMLSerialize::serialize<" +
              pars["RETURNTYP"] + " >(&_retval," + serOpt + ",\"" +
              pars["RETURNTYP"] + "\",\"" + appname + "\"), doc);\n");
          ws_funcs.append("normalizeNamespaces(&(doc.getRootElement()),\"" +
                          appname + "\",allnmspcs);\n");
          ws_funcs.append("_retStr += \"<tns:" + pars["RETURN"] +
                          ">\"+doc.getRootElement().renderChildren()+\"</tns:" +
      ...
      ws_funcs.append("return "
                      "\"<soap:Fault><faultcode>soap:Server</"
                      "faultcode><faultstring>Exception "
                      "occurred</faultstring></soap:Fault>\";\n}\n");
