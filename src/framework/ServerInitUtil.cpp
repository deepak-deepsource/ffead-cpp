...
void ServerInitUtil::bootstrapIB(std::string serverRootDirectory,
                                 SERVER_BACKEND type) {
  if (type == V_PICO) {
...
void ServerInitUtil::bootstrap(std::string serverRootDirectory, Logger &logger,
                               SERVER_BACKEND type) {
  serverRootDirectory += "/";
  ...
  ConfigurationData::enableFeatures(
      enableCors, enableSecurity, enableFilters, enableControllers,
      enableContMpg, enableContPath, enableContExt, enableContRst, enableExtra,
      enableScripts, enableSoap, enableLogging, enableExtControllers,
      enableJobs, enableStaticResponses);
...
#else
    logger << "Invalid Build Type specified, only autotools, cmake, xmake, "
              "meson, scons, shellb, bazel and buck2 supported...\n"
           << std::endl;
#endif
    std::string output = ScriptHandler::execute(compres, true);
    logger << "Intermediate code generation task\n\n" << std::endl;
...
void ServerInitUtil::initIB() { init(loggerIB); }

void ServerInitUtil::initIB(cb_reg_ext_fd_pv pvregfd, cb_into_pv cb,
