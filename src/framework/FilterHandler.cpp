...
bool FilterHandler::hasFilters(std::string_view cntxtName) {
  std::map<std::string,
           ...
           std::less<>> &filterObjectMap =
      ConfigurationData::getInstance()->filterObjectMap;
  return filterObjectMap.find(cntxtName) != filterObjectMap.end() &&
         filterObjectMap.find(cntxtName)->second.size() > 0;
}

bool FilterHandler::getFilterForPath(std::string_view cntxtName,
  ...
  std::map<std::string, std::vector<std::string>, std::less<>> filterMap =
      filterObjectMap.find(cntxtName)->second;
  std::map<std::string, std::vector<std::string>, std::less<>>::iterator it;
  for (it = filterMap.begin(); it != filterMap.end(); ++it) {
    std::string pathurl =
        it->first.substr(0, it->first.length() - type.length());
    if (StringUtil::endsWith(it->first, type) &&
        ConfigurationData::urlMatchesPath(cntxtName, pathurl, actUrl)) {
      filters = it->second;
      return true;
    }
  }
...
void FilterHandler::handleIn(HttpRequest *req, const std::string &ext,
                             Reflector &reflector) {
  std::vector<std::string> filters;
...
bool FilterHandler::handle(HttpRequest *req, HttpResponse *res,
                           const std::string &ext, Reflector &reflector) {
  bool continue_proc_request = true;
  std::vector<std::string> filters;
  if (getFilterForPath(req->getCntxt_name(), req->getCurl(), filters,
                       "handle")) {
    for (int var = 0; var < (int)filters.size(); ++var) {
      std::string claz = filters.at(var);
      void *_temp = ConfigurationData::getInstance()->ffeadContext.getBean(
      ...
      if (meth.getMethodName() != "") {
        valus.push_back(req);
        valus.push_back(res);
        reflector.invokeMethod<bool>(&continue_proc_request, _temp, meth,
                                     valus);
      }
      ConfigurationData::getInstance()->ffeadContext.release(
          _temp, claz, req->getCntxt_name());
...
void FilterHandler::handleOut(HttpRequest *req, HttpResponse *res,
                              const std::string &ext, Reflector &reflector) {
  std::vector<std::string> filters;
