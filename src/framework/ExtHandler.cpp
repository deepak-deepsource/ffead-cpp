...
bool ExtHandler::handle(HttpRequest *req, HttpResponse *res, void *dlib,
                        void *ddlib, const std::string &ext,
                        Reflector &reflector) {
  std::map<std::string, std::string, std::less<>> *tmplMap =
      &(ConfigurationData::getInstance()
            ->templateMappingMap.find(req->getCntxt_name())
            ->second);
  ...
  if (tmplMap->find(req->getCurl()) != tmplMap->end()) {
    if (ddlib != NULL) {
      bool done = false;
      std::string tpefilename =
          tmplMap->find(req->getCurl())
              ->second.substr(tmplMap->find(req->getCurl())->second.find(";") +
                              1);
      std::string tpeclasname =
          tmplMap->find(req->getCurl())
              ->second.substr(0,
                              tmplMap->find(req->getCurl())->second.find(";"));
      cntrlit = true;
      void *_temp = ConfigurationData::getInstance()->ffeadContext.getBean(
          tpeclasname, req->getCntxt_name());
      if (_temp != NULL) {
        try {
          args argus;
          vals valus;
          ...
          if (meth.getMethodName() != "") {
            ...
            // logger << "Invalid Template handler, no method getContext
            // found..." << std::endl;
        ...
        ConfigurationData::getInstance()->ffeadContext.release(
            _temp, tpeclasname, req->getCntxt_name());
      }
      if (done) {
...
#endif
  std::map<std::string, std::string, std::less<>> *dcpMap =
      &(ConfigurationData::getInstance()
            ->dcpMappingMap.find(req->getCntxt_name())
            ->second);
  std::map<std::string, std::string, std::less<>> *vwMap =
      &(ConfigurationData::getInstance()
            ->viewMappingMap.find(req->getCntxt_name())
            ->second);
  std::map<std::string, std::string, std::less<>> *ajaxIntfMap =
      &(ConfigurationData::getInstance()
            ->ajaxInterfaceMap.find(req->getCntxt_name())
            ->second);
  std::map<std::string, std::string, std::less<>> *fviewMap =
      &(ConfigurationData::getInstance()
            ->fviewMappingMap.find(req->getCntxt_name())
            ->second);
  std::map<std::string, Element, std::less<>> *formMap =
      &(ConfigurationData::getInstance()
            ->fviewFormMap.find(req->getCntxt_name())
            ->second);

  if (ajaxIntfMap->find(req->getCurl()) != ajaxIntfMap->end() &&
      req->getMethod() == "POST" && req->getRequestParam("method") != "") {
    cntrlit = true;
    ...
    if (methName == "") {
      ...
      if (paramSize >= 0) {
        ...
        metn = std::string(req->getCntxt_name()) + "invokeAjaxMethodFor" +
               claz + methName;
        ...
        res->setHTTPResponseStatus(HTTPResponseStatus::InternalServerError);
             ...
             formMap->find(req->getFile()) != formMap->end()) {
    try {
      ...
      res->setHTTPResponseStatus(HTTPResponseStatus::InternalServerError);
             ...
             fviewMap->find(req->getFile()) != fviewMap->end()) {
    ...
    void *_temp = ConfigurationData::getInstance()->ffeadContext.getBean(
        vwMap->find(req->getCurl())->second, req->getCntxt_name());
    if (_temp != NULL) {
      cntrlit = true;
      try {
        args argus;
        ...
        if (meth.getMethodName() != "") {
          ...
          // logger << "Invalid Dynamic View handler, no method getDocument
          // found..." << std::endl;
          res->setHTTPResponseStatus(HTTPResponseStatus::InternalServerError);
        }
      } catch (const std::exception &e) {
      ...
      ConfigurationData::getInstance()->ffeadContext.release(
          _temp, vwMap->find(req->getCurl())->second, req->getCntxt_name());
    } else {
