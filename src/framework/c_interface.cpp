...
void ffead_cpp_bootstrap(const char *srv, size_t srv_len, int type) {
  ServerInitUtil::bootstrapIB(std::string(srv, srv_len),
                              static_cast<SERVER_BACKEND>(type));
}

/*
    This function should be called after ffead_cpp_bootstrap
    It is responsible for initializing configuration
   settings/parameters/singletons instances for ffead-cpp framework
*/
void ffead_cpp_init() { ServerInitUtil::initIB(); }

void ffead_cpp_init_for_pv(cb_reg_ext_fd_pv pvregfd, cb_into_pv cb,
                           cb_into_pv_for_date cdt) {
  ServerInitUtil::initIB(pvregfd, cb, cdt);
}

/*
    This should be called before application exit, responsible for cleaning up
   the ffead-cpp framework
*/
void ffead_cpp_cleanup() { ServerInitUtil::cleanUp(); }
...
void *ffead_cpp_handle_rust_1(const ffead_request *request, int *scode,
                              ...
                              size_t *out_headers_len, const char **out_body,
                              size_t *out_body_len) {
  HttpRequest req((void *)request->headers, request->headers_len,
                  std::string_view{request->path, request->path_len},
                  std::string_view{request->method, request->method_len},
                  request->version,
                  std::string_view{request->body, request->body_len});
...
*/
void *ffead_cpp_handle_rust_swift_1(
...
*/
void *ffead_cpp_handle_c_1(const ffead_request *request, int *scode,
  ...
  if (!respo->isDone()) {
    ...
    *scode = 0;
    *out_body_len = 0;
    respo->addHeader(HttpResponse::ContentType,
                     CommonUtils::getMimeType(req.getExt()));
  } else {
    *out_url_len = 0;
    *scode = respo->getCode();
    ...
    *out_body_len = cnt.length();
    if (cnt.length() > 0) {
      respo->addHeader(HttpResponse::ContentLength,
                       std::to_string((int)cnt.length()));
    }
  }
...
*/
void *ffead_cpp_handle_picov_1(const ffead_request3 *request, int *scode,
                               const char **smsg, size_t *smsg_len,
...
void *ffead_cpp_handle_crystal_js_1(
...
void *ffead_cpp_handle_go_1(
    ...
    const char **out_body, size_t *out_body_len) {
  HttpRequest req(method, method_len, path, path_len, query, query_len,
                  in_headers, in_headers_len, in_body, in_body_len, version);
  HttpResponse *respo = new HttpResponse();
...
void *ffead_cpp_handle_go_2(const char *server_str, size_t server_str_len,
                            ...
                            const char *path, size_t path_len, int version,
                            const char *in_headers, size_t in_headers_len,
                            const char *in_body, size_t in_body_len, int *scode,
                            const char **out_url, size_t *out_url_len,
                            const char **out_mime, size_t *out_mime_len,
                            const char **out_headers, size_t *out_headers_len,
                            const char **out_body, size_t *out_body_len) {
  HttpRequest req(method, method_len, path, path_len, NULL, 0, in_headers,
                  in_headers_len, in_body, in_body_len, version);
  HttpResponse *respo = new HttpResponse();
...
void *ffead_cpp_handle_v(const char *server_str, size_t server_str_len,
                         const char *in_headers, size_t in_headers_len,
                         const char *in_body, size_t in_body_len, int *done,
                         const char **out_url, size_t *out_url_len,
                         const char **out_mime, size_t *out_mime_len,
                         const char **out_headers, size_t *out_headers_len,
                         const char **out_body, size_t *out_body_len) {
...
*/
void *ffead_cpp_handle_java(int *scode, const char **out_url,
                            size_t *out_url_len, const char **out_mime,
                            size_t *out_mime_len, const char **out_body,
                            size_t *out_body_len, const char **out_headers,
                            size_t *out_headers_len, const char *server_str,
                            size_t server_str_len, const char *method,
                            size_t method_len, const char *path,
                            size_t path_len, const char *body, size_t body_len,
                            int version, int headers_len, ...) {
...
void *ffead_cpp_handle_1t(const ffead_request2 *request, int *scode,
  ...
  HttpRequest req((void *)request->headers, request->headers_len,
                  std::string_view{request->path, request->path_len},
                  std::string_view{request->qstr, request->qstr_len},
                  std::string_view{request->method, request->method_len},
                  request->version,
                  std::string_view{request->body, request->body_len});
  HttpResponse *respo = new HttpResponse();
...
void *ffead_cpp_handle_2(const ffead_request1 *request, int *scode,
                         ...
                         phr_header_fcp *out_headers, size_t *out_headers_len,
                         const char **out_body, size_t *out_body_len) {
  HttpRequest req(std::string_view{request->path, request->path_len},
                  std::string_view{request->method, request->method_len},
                  request->version, request->headers, request->headers_len,
...
void *ffead_cpp_handle_3(const char *server_str, size_t server_str_len,
                         ...
                         const char *in_body, size_t in_body_len, int *done,
                         const char **out_url, size_t *out_url_len,
                         bool with_status_line, int *scode, const char **smsg,
                         size_t *smsg_len, const char **out_headers,
                         size_t *out_headers_len, const char **out_body,
                         size_t *out_body_len) {
  HttpRequest req(in_headers, in_headers_len, in_body, in_body_len);
  HttpResponse *respo = new HttpResponse();
...
void ffead_cpp_get_resp_header_str(void *ptr, const char **headers,
                                   size_t *headers_len, const char *server_str,
...
void ffead_cpp_get_resp_header_str_with_statusline(void *ptr,
                                                   const char **headers,
                                                   size_t *headers_len,
...
void ffead_cpp_get_resp_get_body(void *ptr, const char **body,
                                 size_t *body_len) {
  HttpResponse *respo = (HttpResponse *)ptr;
  const std::string &cnt = respo->getContent();
  *body = cnt.c_str();
...
void *ffead_cpp_handle_js_1(const ffead_request *request, int *scode,
                            size_t *out_url_len, size_t *out_headers_len,
                            size_t *out_body_len) {
  HttpRequest req((void *)request->headers, request->headers_len,
                  std::string_view{request->path, request->path_len},
                  std::string_view{request->method, request->method_len},
...
const char *ffead_cpp_handle_js_out_hdr_name(void *res, int pos,
                                             size_t *name_len) {
  HttpResponse *respo = (HttpResponse *)res;
  RMap::const_iterator it = respo->getCHeaders().cbegin();
  *name_len = 0;
...
const char *ffead_cpp_handle_js_out_hdr_value(void *res, int pos,
                                              size_t *value_len) {
  HttpResponse *respo = (HttpResponse *)res;
  RMap::const_iterator it = respo->getCHeaders().cbegin();
  *value_len = 0;
