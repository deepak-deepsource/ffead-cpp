...
typedef std::map<std::string, std::vector<RestFunction>, std::less<>>
    resFuncMap;
...
public:
  ...
  CoreServerProperties(std::string serverRootDirectory,
                       std::string resourcePath, std::string webPath,
                       propMap sprops, long sessionTimeout, bool sessatserv) {
    isMainServerProcess = false;
    sessservdistocache = false;
    sessionFileLockTimeout = 0;
...
enum SERVER_BACKEND {
  NGINX,
  APACHE,
  OPENLITESPEED, // All http server modules [0, 1, 2]
  EMBEDDED,      // The embedded ffead-cpp server engine [3]
  CINATRA,
  LITHIUM,
  DROGON, // All C++ http server engines [4, 5, 6]
  C_LIBREACTOR,
  C_H2O, // All C http server engines [7, 8]
  V_WEB,
  V_PICO, // All V http server engines [9, 10]
  RUST_ACTIX,
  RUST_HYPER,
  RUST_FAF,
  RUST_THRUSTER, // All Rust http server engines [11, 12, 13, 14]
  GO_FASTHTTP,
  GO_ATRUEGO,
  GO_GNET, // All Go http server engines [15, 16, 17]
  CRYSTAL_HTTP,
  CRYSTAL_H2O, // All Crystal http server engines [18, 19]
  JAVA_FIRENIO,
  JAVA_RAPIDOID,
  JAVA_WIZZARDO_HTTP, // All Java http server engines [20, 21, 22]
  SWIFT_NIO,          // All Swift http server engines [23]
  D_HUNT,             // All D http server engines [24]
  EMBEDDED_NGHTTP2,
  C_LSQUICHTTP3, // HTTP2 and HTTP3 server engines [25, 26]
  JS_NODEJS,
  JS_DENO,        // Nodejs and Deno [27, 28]
  C_QUICHE_HTTP3, // Quiche http3 [29]
  SEASTAR,        // Seastar framework [30]
  JULIA_HTTP,     // Julia http server [31]
  MONGOLS,
  CPPSERVER,
  UV_CPP // Mongols, Cppserver, uv-cpp [32, 33, 34]
};
...
class ConfigurationData {
  ...
  std::map<std::string, std::map<std::string, std::string, std::less<>>,
           std::less<>>
      wsdlMap;
  std::map<std::string, std::map<std::string, Element, std::less<>>,
           std::less<>>
      fviewFormMap;
  std::map<std::string, std::map<std::string, std::string, std::less<>>,
           std::less<>>
...
public:
  static void enableFeatures(bool enableCors, bool enableSecurity,
                             bool enableFilters, bool enableControllers,
                             bool enableContMpg, bool enableContPath,
                             bool enableContExt, bool enableContRst,
                             bool enableExtra, bool enableScripts,
                             bool enableSoap, bool enableLogging,
                             bool enableExtControllers, bool enableJobs,
                             bool enableStaticResponses);
  static Reflector *getReflector();
  static int getProcessId();
  static ClassInfo *getClassInfo(const std::string &,
  ...
  static bool urlMatchesPath(std::string_view cntxtName, std::string pathurl,
                             std::string url);
  static void initializeAllSingletonBeans();
  static void clearAllSingletonBeans();
  static void
  setCoreServerProperties(CoreServerProperties coreServerProperties);
