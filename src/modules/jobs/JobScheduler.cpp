...
JobScheduler::JobScheduler() { isStarted = false; }

JobScheduler::~JobScheduler() {}

void JobScheduler::init(const std::string &fileName,
...
void JobScheduler::init(const ElementList &tabs, const std::string &appName) {
  ...
  for (unsigned int dn = 0; dn < tabs.size(); dn++) {
    if (tabs.at(dn).getTagName() != "job-proc") {
      logger
          << "Invalid Element Tag found inside job-procs, should be job-proc..."
          << std::endl;
      continue;
    }
    ...
    if (clas != "" && meth != "" && cron != "" && name != "") {
      ...
      logger << "Cannot add Job Process as some mandatory elements are missing "
  ...
  for (int dn = 0; dn < (int)instance->configs.size(); dn++) {
    ...
    if (clas != "" && method != "" && cron != "" && name != "") {
      ...
      if (claz->getClassName() != "") {
        ...
        logger << "JobScheduler - Got method,class " + meth.getMethodName() +
                      "," + ctor.getName()
               << std::endl;
        ...
        if (meth.getMethodName() != "" && ctor.getName() != "") {
          ...
          logger << "JobScheduler - Got objins,func " << objIns << "," << f
                 << std::endl;
          ...
          if (objIns != NULL && f != NULL) {
            ...
            logger << "Cannot add Job Process as could not initialize class "
                      "instance and method"
                   << std::endl;
          }
        } else {
          logger << "Cannot add Job Process as method was not found or public "
                    "no argument constructor not defined for class"
                 << std::endl;
        }
      } else {
        logger << "Cannot add Job Process as class information was not found"
               << std::endl;
      }
    } else {
      logger << "Cannot add Job Process as some mandatory elements are missing "
        ...
        if (timer.isValid(3, d2.getMonth(), timer.nextRunDate.getMonth())) {
          if (timer.isValid(2, d2.getDay(), timer.nextRunDate.getDay())) {
            if (timer.isValid(1, d2.getHours(), timer.nextRunDate.getHours())) {
              if (timer.isValid(0, d2.getMinutes(),
                                timer.nextRunDate.getMinutes())) {
                ...
                incrementDone =
                    timer.tryIncrement(0, timer.nextRunDate.getMinutes());
                if (!incrementDone) {
                  incrementDone =
                      timer.tryIncrement(1, timer.nextRunDate.getHours());
                }

                if (!incrementDone) {
                  incrementDone =
                      timer.tryIncrement(2, timer.nextRunDate.getDay());
                }

                if (!incrementDone) {
                  incrementDone =
                      timer.tryIncrement(3, timer.nextRunDate.getMonth());
                }
                if (!incrementDone) {
                  incrementDone =
                      timer.tryIncrement(5, timer.nextRunDate.getYear());
                }

                logger << "Running Job Process " + name + " complete"
                       << std::endl;
              }
            }
          }
...
int JobScheduler::JobTask::getTid() { return -1; }
