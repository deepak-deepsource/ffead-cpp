...
CronTimer::CronTimer(const std::string &cronExpression) {
  ...
  std::vector<std::string> cronParts =
      StringUtil::splitAndReturn<std::vector<std::string>>(cronExpression, " ");
  bool valid = true;
        ...
        if (tst == "*") {
          ...
          if (parts.size() == 3) {
            ...
            if (vparts.at(0) < start || vparts.at(1) > end ||
                vparts.at(2) > (vparts.at(1) - vparts.at(0))) {
              valid = false;
            } else {
          ...
          std::vector<std::string> parts =
              StringUtil::splitAndReturn<std::vector<std::string>>(tst, "-");
          int start, end;
          start = starts.at(position);
          end = ends.at(position);
          if (parts.size() == 2) {
...
std::string CronTimer::toString() {
  ...
  for (int var = 0; var < (const int &)cParts.size(); ++var) {
    if (cParts.at(var).all) {
      ...
      int start = cParts.at(var).from;
      int end = cParts.at(var).to;
      builder.append("every " + types.at(var) + " from " +
                     CastUtil::fromNumber(start) + " " +
                     CastUtil::fromNumber(end));
    } else {
      builder.append("every " + CastUtil::fromNumber(cParts.at(var).from) +
                     " " + types.at(var));
    }
...
bool CronTimer::isValid(const int &pos, const int &cvalue, const int &nvalue) {
  ...
  if (pos < 6 && cvalue > 0) {
    if (cParts.at(pos).all) {
      isValid = true;
    } else if (cParts.at(pos).interval == -1 && cParts.at(pos).from == cvalue) {
      isValid = true;
    } else if (cParts.at(pos).interval == -1 && cParts.at(pos).to != -1 &&
               cvalue >= cParts.at(pos).from && cvalue <= cParts.at(pos).to) {
      isValid = true;
    } else if (cParts.at(pos).interval != -1 && cvalue >= cParts.at(pos).from &&
               cvalue <= cParts.at(pos).to &&
               (cvalue - cParts.at(pos).from) % cParts.at(pos).interval == 0) {
      isValid = true;
    } else if (cParts.at(pos).list.find("," + CastUtil::fromNumber(cvalue) +
                                        ",") != std::string::npos) {
      isValid = true;
    }
  }
...
void CronTimer::correctNextRunDateTime(const int &pos) {
  ...
  if (pos > 0) {
    ...
    if (cParts.at(index).interval == -1 && end == -1 && start != -1 &&
        !cParts.at(index).all) {
      end = start;
    }
...
bool CronTimer::tryIncrement(const int &pos, const int &cvalue) {
  ...
  bool isRangeVal = cvalue >= cParts.at(pos).from && cvalue < cParts.at(pos).to;
  ...
  if (cParts.at(pos).interval != -1 &&
      cvalue + intervalValue > cParts.at(pos).to) {
    intervalValue = cParts.at(pos).from - cvalue;
  }
  if (cParts.at(pos).all || cParts.at(pos).interval != -1 || isRangeVal ||
      lstExists) {
    if (pos == 0) {
        ...
        if (isValid(2, d2.getDay(), nextRunDate.getDay())) {
          if (isValid(1, d2.getHours(), nextRunDate.getHours())) {
            if (isValid(0, d2.getMinutes(), nextRunDate.getMinutes())) {
              ...
              std::cout << "Next Run Date = " + nextRunDate.toString()
                        << std::endl;
            }
          }
        }
