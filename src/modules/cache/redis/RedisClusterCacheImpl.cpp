...
long long RedisClusterCacheImpl::increment(const std::string &key,
                                           const int &number) {
  Connection *connection = pool->checkout();
  sw::redis::RedisCluster *r = (sw::redis::RedisCluster *)connection->getConn();
...
long long RedisClusterCacheImpl::decrement(const std::string &key,
                                           const int &number) {
  Connection *connection = pool->checkout();
  sw::redis::RedisCluster *r = (sw::redis::RedisCluster *)connection->getConn();
...
long double RedisClusterCacheImpl::incrementFloat(const std::string &key,
                                                  const double &number) {
  Connection *connection = pool->checkout();
  sw::redis::RedisCluster *r = (sw::redis::RedisCluster *)connection->getConn();
...
long double RedisClusterCacheImpl::decrementFloat(const std::string &key,
                                                  const double &number) {
...
bool RedisClusterCacheImpl::flushAll() { return false; }

bool RedisClusterCacheImpl::set(const std::string &key, GenericObject &value,
                                int expireSeconds) {
  Connection *connection = pool->checkout();
  sw::redis::RedisCluster *r = (sw::redis::RedisCluster *)connection->getConn();
  ...
  r->set(
      ...
      std::chrono::milliseconds(
          (expireSeconds <= 0 ? defaultExpireSeconds : expireSeconds) * 1000),
      sw::redis::UpdateType::ALWAYS);
  return true;
}

bool RedisClusterCacheImpl::add(const std::string &key, GenericObject &value,
                                int expireSeconds) {
  Connection *connection = pool->checkout();
  sw::redis::RedisCluster *r = (sw::redis::RedisCluster *)connection->getConn();
  std::string valueStr = value.getSerilaizedState();
...
bool RedisClusterCacheImpl::replace(const std::string &key,
                                    GenericObject &value, int expireSeconds) {
  Connection *connection = pool->checkout();
  sw::redis::RedisCluster *r = (sw::redis::RedisCluster *)connection->getConn();
  std::string valueStr = value.getSerilaizedState();
  r->set(
...
bool RedisClusterCacheImpl::setRaw(const std::string &key, const char *value,
                                   int expireSeconds) {
  Connection *connection = pool->checkout();
  sw::redis::RedisCluster *r = (sw::redis::RedisCluster *)connection->getConn();
  r->set(
...
bool RedisClusterCacheImpl::addRaw(const std::string &key, const char *value,
                                   int expireSeconds) {
  Connection *connection = pool->checkout();
  sw::redis::RedisCluster *r = (sw::redis::RedisCluster *)connection->getConn();
  r->set(
...
bool RedisClusterCacheImpl::replaceRaw(const std::string &key,
                                       const char *value, int expireSeconds) {
  Connection *connection = pool->checkout();
  sw::redis::RedisCluster *r = (sw::redis::RedisCluster *)connection->getConn();
  r->set(
      key, value,
...
RedisClusterCacheImpl::getValues(const std::vector<std::string> &keys) {
  std::vector<std::string> rv;
  mgetRaw(keys, rv);
  return rv;
}

bool RedisClusterCacheImpl::setRaw(const unsigned long long &key,
...
bool RedisClusterCacheImpl::addRaw(const unsigned long long &key,
                                   const std::string_view &value,
                                   int expireSeconds) {
  Connection *connection = pool->checkout();
  sw::redis::RedisCluster *r = (sw::redis::RedisCluster *)connection->getConn();
...
bool RedisClusterCacheImpl::replaceRaw(const unsigned long long &key,
                                       const std::string_view &value,
                                       int expireSeconds) {
  Connection *connection = pool->checkout();
  sw::redis::RedisCluster *r = (sw::redis::RedisCluster *)connection->getConn();
...
void RedisClusterCacheImpl::getValues(
...
void RedisClusterCacheImpl::mgetRaw(const std::vector<std::string> &skeys,
                                    std::vector<std::string> &rvalues) {
  Connection *connection = pool->checkout();
  sw::redis::RedisCluster *r = (sw::redis::RedisCluster *)connection->getConn();
...
bool RedisClusterCacheImpl::addToQ(const std::string &qname,
...
void *
RedisClusterCacheConnectionPool::newConnection(const bool &isWrite,
...
void RedisClusterCacheConnectionPool::initEnv() {
  ...
  connection_options.host =
      getProperties().getNodes().at(0).getHost(); // Required.
  connection_options.port =
      getProperties()
          .getNodes()
          .at(0)
          .getPort(); // Optional. The default port is 6379.
  // connection_options.password = "auth";   // Optional. No password by
  // default. connection_options.db = 1;  // Optional. Use the 0th database by
  // default.

  // Optional. Timeout before we successfully send request to or receive
  // response from redis. By default, the timeout is 0ms, i.e. never timeout and
  // block until we send or receive successfuly. NOTE: if any command is timed
  // out, we throw a TimeoutError exception.
  // connection_options.socket_timeout =
  // getProperties().getNodes().at(0).getConnectionTimeout();
  connection_options.connect_timeout = std::chrono::milliseconds(
      (int)getProperties().getNodes().at(0).getConnectionTimeout());
...
void RedisClusterCacheConnectionPool::destroy() { delete r; }
#endif
