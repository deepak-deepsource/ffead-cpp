...
long long MemoryCacheImpl::increment(const std::string &key,
                                     const int &number) {
  long long val = -1;
  MemoryCacheConnectionPool *p = (MemoryCacheConnectionPool *)pool;
...
long long MemoryCacheImpl::decrement(const std::string &key,
                                     const int &number) {
  long long val = -1;
  MemoryCacheConnectionPool *p = (MemoryCacheConnectionPool *)pool;
...
long double MemoryCacheImpl::incrementFloat(const std::string &key,
                                            const double &number) {
  long double val = -1.0;
  MemoryCacheConnectionPool *p = (MemoryCacheConnectionPool *)pool;
...
long double MemoryCacheImpl::decrementFloat(const std::string &key,
                                            const double &number) {
  long double val = -1.0;
  MemoryCacheConnectionPool *p = (MemoryCacheConnectionPool *)pool;
...
bool MemoryCacheImpl::flushAll() { return true; }

void MemoryCacheImpl::clean() {
  MemoryCacheConnectionPool *p = (MemoryCacheConnectionPool *)pool;
  while ((int)p->internalMap.size() > p->size) {
    auto last_it = p->lrul.end();
    last_it--;
...
bool MemoryCacheImpl::setInternal(const std::string &key,
                                  const std::string &value,
                                  const int &expireSeconds,
                                  const int &setOrAddOrRep) {
  MemoryCacheConnectionPool *p = (MemoryCacheConnectionPool *)pool;

  if (setOrAddOrRep < 1 || setOrAddOrRep > 3)
    return false;
  ...
  bool flag = false;
  if (setOrAddOrRep == 1 || (setOrAddOrRep == 2 && !exists) ||
      (setOrAddOrRep == 3 && exists)) {
...
bool MemoryCacheImpl::setInternalN(const unsigned long long &key,
                                   const std::string &value,
                                   const int &expireSeconds,
                                   const int &setOrAddOrRep) {
  MemoryCacheConnectionPool *p = (MemoryCacheConnectionPool *)pool;
...
MemoryCacheImpl::getValues(const std::vector<std::string> &keys) {
  std::vector<std::string> rv;
  mgetRaw(keys, rv);
  return rv;
}

void MemoryCacheImpl::mgetRaw(const std::vector<std::string> &keys,
...
bool MemoryCacheImpl::setRaw(const unsigned long long &key,
                             const std::string_view &value, int expireSeconds) {
  bool status = setInternalN(key, std::string(value.data(), value.size()),
                             expireSeconds, 1);
  return status;
}
bool MemoryCacheImpl::addRaw(const unsigned long long &key,
                             const std::string_view &value, int expireSeconds) {
  bool status = setInternalN(key, std::string(value.data(), value.size()),
                             expireSeconds, 2);
  return status;
}
bool MemoryCacheImpl::replaceRaw(const unsigned long long &key,
...
void MemoryCacheImpl::getValues(const std::vector<unsigned long long> &keys,
                                std::vector<std::string> &values) {
  MemoryCacheConnectionPool *p = (MemoryCacheConnectionPool *)pool;
  std::string rval;
  p->lock.lock();
...
bool MemoryCacheImpl::set(const std::string &key, GenericObject &value,
                          int expireSeconds) {
...
bool MemoryCacheImpl::add(const std::string &key, GenericObject &value,
                          int expireSeconds) {
  std::string valueStr = value.getSerilaizedState();
...
bool MemoryCacheImpl::replace(const std::string &key, GenericObject &value,
                              int expireSeconds) {
  std::string valueStr = value.getSerilaizedState();
...
bool MemoryCacheImpl::setRaw(const std::string &key, const char *value,
                             int expireSeconds) {
  bool status = setInternal(key, std::string(value), expireSeconds, 1);
  return status;
}

bool MemoryCacheImpl::addRaw(const std::string &key, const char *value,
                             int expireSeconds) {
  bool status = setInternal(key, std::string(value), expireSeconds, 2);
  return status;
}

bool MemoryCacheImpl::replaceRaw(const std::string &key, const char *value,
                                 int expireSeconds) {
  bool status = setInternal(key, std::string(value), expireSeconds, 3);
  return status;
}

bool MemoryCacheImpl::addToQ(const std::string &qname,
                             const std::string &value) {
  MemoryCacheConnectionPool *p = (MemoryCacheConnectionPool *)pool;
  p->qlock.lock();
...
void *MemoryCacheConnectionPool::newConnection(const bool &isWrite,
