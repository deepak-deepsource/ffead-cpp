...
void CacheManager::init(bool issevh) { isSinglEVH = issevh; }
...
void CacheManager::initCache(const ConnectionProperties &props,
                             const std::string &appNameN, GetClassBeanIns f) {
  ...
  if (StringUtil::toLowerCopy(props.getType()) != "custom") {
    ...
    try {
      ...
      logger.info("Error initializing Cache " + appNameN + "@" +
  ...
  if (props.getProperty("init") != "") {
    ...
    if (v.size() == 2) {
      ...
      try {
        ...
        logger.info("Error during init call for Cache " + appNameN + "@" +
                    props.getName() + " " + std::string(e.what()));
      }
      if (cbi.cleanUp) {
...
void CacheManager::cleanImpl(CacheInterface *ccImpl) {
  if (!isSinglEVH) {
    if (StringUtil::toLowerCopy(ccImpl->pool->getProperties().getType()) ==
        "memory") {
      delete (MemoryCacheImpl *)ccImpl;
    } else if (StringUtil::toLowerCopy(
                   ccImpl->pool->getProperties().getType()) == "memcached") {
#ifdef INC_MEMCACHED
      delete (MemcachedImpl *)ccImpl;
#endif
    } else if (StringUtil::toLowerCopy(
                   ccImpl->pool->getProperties().getType()) == "redis") {
#if defined(HAVE_REDISINC)
      if (StringUtil::toLowerCopy(
              ccImpl->pool->getProperties().getProperty("cluster")) == "true") {
...
CacheInterface *CacheManager::getImpl(std::string name, std::string appName) {
  ...
  if (StringUtil::toLowerCopy(cchMgr->props.getType()) == "memory") {
    ...
    if (StringUtil::toLowerCopy(cchMgr->props.getProperty("cluster")) ==
        "true") {
