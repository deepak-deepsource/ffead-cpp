...
RequestHandler2::RequestHandler2(ServiceHandler *sh, const bool &isMain,
                                 bool isSSLEnabled, httpSockHandle h,
                                 const SOCKET &listenerSock) {
  this->shi = sh;
...
void RequestHandler2::registerSocketInterfaceFactory(
    const SocketInterfaceFactory &f) {
  this->sf = f;
}

RequestHandler2 *RequestHandler2::getInstance() { return _i; }
...
void RequestHandler2::addListenerSocket(doRegisterListener drl,
                                        const SOCKET &listenerSock) {
  ...
  if (drl != NULL) {
    ...
    while (!drl()) {
      Thread::sSleep(1);
      if (counter++ == 60) {
        logger << "Cannot wait more than 60 seconds for cache/database to "
                  "initialize, will forcefully start server now...."
               << std::endl;
        break;
      }
    }
...
bool RequestHandler2::isActive() { return run; }
...
BaseSocket *RequestHandler2::loopEventCb(SelEpolKqEvPrt *ths, BaseSocket *bi,
                                         int type, int fd, char *buf,
                                         size_t len, bool isClosed) {
  RequestHandler2 *ins = static_cast<RequestHandler2 *>(ths->getCtx());
  switch (type) {
  case ACCEPTED: {
...
void *RequestHandler2::handleWrites(void *inp) {
  ...
  while (ins->shi->run) {
    ...
    // Remove the sync block on io_uring as write does not need to be on the
    // uring
...
Http11Socket::Http11Socket(const SOCKET &fd, const int &chunkSize,
                           const int &connKeepAlive, const int &maxReqHdrCnt,
                           const int &maxEntitySize)
    : BaseSecureSocket(fd) {
  isHeadersDone = false;
  bytesToRead = 0;
...
int Http11Socket::getTimeout() { return connKeepAlive; }
    ...
    if (RequestHandler2::_i->hsh(&req, &res, this)) {
      ...
      if (res.isDone()) {
        ...
        if (!res.isContentRemains()) {
          ...
          bool isFirst = true;
          while (res.hasContent &&
                 res.getRemainingContent(req.getUrl(), isFirst, rd._b)) {
            isFirst = false;
          }
        }
      }
