...
#if !defined(USE_EPOLL) && !defined(USE_KQUEUE) && !defined(USE_WIN_IOCP) &&   \
    !defined(USE_EVPORT)
#include <libcuckoo/cuckoohash_map.hh>
#endif

enum io_uring_events { ACCEPT, READ, WRITE, PROV_BUF, INTERRUPT };
...
typedef BaseSocket *(*onEvent)(SelEpolKqEvPrt *ths, BaseSocket *sfd, int type,
...
public:
  ...
  void post_write_2(BaseSocket *sfd, const std::string &data,
  ...
  void initialize(const int &timeout, eventLoopContinue elcCb = NULL,
                  onEvent eCb = NULL);
  void initialize(SOCKET sockfd, const int &timeout,
                  eventLoopContinue elcCb = NULL, onEvent eCb = NULL);
  int getEvents();
  ...
  bool registerRead(BaseSocket *obj, const bool &isListeningSock = false,
                    bool epoll_et = true, bool isNonBlocking = false);
