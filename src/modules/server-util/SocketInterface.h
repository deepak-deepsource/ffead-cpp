...
public:
  ...
  void doneRead();
  virtual bool hasPendingRead() { return false; }
  bool checkSocketWaitForTimeout(const int &writing, const int &seconds,
                                 const int &micros = 0);
  virtual bool flush();
  ...
  virtual int secureWriteDirect(const std::string &d, int off = 0) {
    return -1;
  };
  virtual int secureWriteDirect(const char *d, size_t len, int off = 0) {
    return -1;
  };
...
public:
  ...
  virtual bool readRequest(void *request, void *&context, int &pending,
                           int &reqPos) = 0;
  virtual bool writeResponse(void *req, void *res, void *context,
                             std::string &data, int reqPos) = 0;
