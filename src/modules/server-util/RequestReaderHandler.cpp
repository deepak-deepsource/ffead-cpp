...
RequestReaderHandler::RequestReaderHandler(ServiceHandler *shi,
                                           const bool &isMain, bool isSinglEVH,
...
RequestReaderHandler *RequestReaderHandler::getInstance() { return _i; }
...
void RequestReaderHandler::addListenerSocket(doRegisterListener drl,
                                             const SOCKET &listenerSock) {
...
bool RequestReaderHandler::isActive() { return run; }

void RequestReaderHandler::registerSocketInterfaceFactory(
...
bool RequestReaderHandler::loopContinue(SelEpolKqEvPrt *ths) {
  RequestReaderHandler *ins =
      static_cast<RequestReaderHandler *>(ths->getCtx());
  return ins->isActive();
}

BaseSocket *RequestReaderHandler::loopEventCb(SelEpolKqEvPrt *ths,
                                              BaseSocket *bi, int type, int fd,
                                              char *buf, size_t len,
                                              bool isClosed) {
  RequestReaderHandler *ins =
      static_cast<RequestReaderHandler *>(ths->getCtx());
...
// Deprecated old handler -- can be used if existing event loop needs to be
// overriden
        ...
        if (ins->selector.isListeningDescriptor(descriptor)) {
          ...
          while (true) {
            ...
            SOCKET newSocket = accept4(
                ins->listenerSock, (struct sockaddr *)&(their_addr), &sin_size,
                SSLHandler::getInstance()->getIsSSL() ? 0 : SOCK_NONBLOCK);
#else
            SOCKET newSocket =
                accept4(ins->listenerSock, (struct sockaddr *)&(their_addr),
