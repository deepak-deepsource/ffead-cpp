...
bool BaseSecureSocket::init(const SOCKET &fd, SSL *&ssl, BIO *&io) {
  if (SSLHandler::getInstance()->getIsSSL()) {
    ...
    if (SSLHandler::getInstance()->securityProperties.client_auth == 2 ||
        SSLHandler::getInstance()->securityProperties.client_auth == 1) {
...
bool BaseSocket::isClosed() { return closed; }
...
int SocketInterface::pushResponse(void *request, void *response, void *context,
                                  int reqPos) {
  // Timer to;
  // to.start();
...
bool SocketInterface::allRequestsDone() { return current == reqPos; }
...
bool BaseSocket::writeFile(int fdes, int remain_data) {
  ...
  while ((sendfile(fdes, fd, offset, &sent_bytes1, NULL, 0) == 0) &&
         sent_bytes1 > 0 && (remain_data > 0)) {
    remain_data -= sent_bytes1;
  }
#elif defined(OS_BSD)
  off_t sent_bytes1 = BUFSIZ;
  while ((sendfile(fdes, fd, offset, BUFSIZ, 0, &sent_bytes1, 0) == 0) &&
         sent_bytes1 > 0 && (remain_data > 0)) {
    remain_data -= sent_bytes1;
  }
#elif defined(IS_SENDFILE) && !defined(OS_MINGW) && !defined(CYGWIN)
  int sent_bytes = 0;
  while (((sent_bytes = sendfile(fd, fdes, &offset, BUFSIZ)) > 0) &&
         (remain_data > 0)) {
...
int BaseSocket::getDescriptor() { return fd; }
std::string BaseSocket::getAddress() { return address; }

bool BaseSocket::flush() { return false; }
...
bool BaseSocket::checkSocketWaitForTimeout(const int &writing,
                                           const int &seconds,
                                           const int &micros) {
#ifdef OS_MINGW
  u_long iMode = 1;
...
bool SocketInterface::isHttp2() { return http2; }

bool BaseSocket::isSecure() { return false; }
...
void BaseSocket::doneRead() {
  ...
  eh->post_read(this);
  //}
#elif defined(USE_SELECT) || defined(USE_MINGW_SELECT) || defined(USE_POLL) || \
    defined(USE_DEVPOLL) || defined(USE_EVPORT)
  eh->registerRead(this);
#endif
}

ResponseData::ResponseData() { oft = 0; }
