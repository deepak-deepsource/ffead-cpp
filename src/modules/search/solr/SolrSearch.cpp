...
std::string SolrSearch::COLL_C_PARAMS =
    ",router.name,numShards,shards,replicationFactor,nrtReplicas,tlogReplicas,"
    "pullReplicas,"
    "maxShardsPerNode,createNodeSet,createNodeSet.shuffle,collection."
    "configName,router.field,autoAddReplicas,async,rule,snitch,policy,"
    "waitForFinalState,";
std::string SolrSearch::COLL_M_PARAMS =
    ",maxShardsPerNode,replicationFactor,autoAddReplicas,collection.configName,"
    "rule,snitch,policy,";

SolrSearch::SolrSearch(ConnectionPooler *pool) {
  this->pool = pool;
  bool isZkUrl = StringUtil::toLowerCopy(
                     pool->getProperties().getProperty("isZkUrl")) == "true";
  solrcloud = StringUtil::toLowerCopy(
                  pool->getProperties().getProperty("solrcloud")) == "true";
  ...
  std::vector<std::string> colnames =
      StringUtil::splitAndReturn<std::vector<std::string>>(colls, ",");
    ...
    if (isZkUrl) {
      ...
      for (std::string ent : get_c_res.children()) {
        if (solrState.collections.find(ent) != solrState.collections.end()) {
          ...
          std::copy(get_res.data().begin(), get_res.data().end(),
          ...
          if (doc.HasMember(ent)) {
            ...
            if (o.HasMember("shards")) {
              for (auto &m : o["shards"].GetObject()) {
                SolrCollectionShard &scs =
                    solrState.collections[ent].shards[m.name.GetString()];
                ...
                for (auto &r : m.value["replicas"].GetObject()) {
                  if (r.value.HasMember("leader") &&
                      strncmp(r.value["leader"].GetString(), "true", 4) == 0) {
                    scs.leader = r.value["base_url"].GetString();
                  }
                  scs.nodes.push_back(
                      std::make_tuple(r.value["base_url"].GetString(),
                                      r.value["node_name"].GetString(),
                                      r.value["core"].GetString()));
                }
              }
            }
          }
      ...
      if (rs.getStatusCode() != "200") {
        ...
        if (doc["responseHeader"]["status"].GetInt() > 0) {
          throw std::runtime_error(
              "Error during index op " +
              std::to_string(doc["responseHeader"]["status"].GetInt()));
        }
        if (doc.HasMember("cluster")) {
          ...
          for (auto &co : doc["cluster"]["collections"].GetObject()) {
            ...
            if (solrState.collections.find(ent) !=
                solrState.collections.end()) {
              ...
              if (o.HasMember("shards")) {
                for (auto &m : o["shards"].GetObject()) {
                  ...
                  for (auto &r : m.value["replicas"].GetObject()) {
                    if (r.value.HasMember("leader") &&
                        strncmp(r.value["leader"].GetString(), "true", 4) ==
                            0) {
                      scs.leader = r.value["base_url"].GetString();
                    }
                    scs.nodes.push_back(
...
void SolrSearch::indexOp(IndexQuery &iq) {
  ...
  if (!solrcloud) {
    throw std::runtime_error(
        "Cannot create collection in standalone mode using API");
  } else {
...
void SolrSearch::docOp(DocumentQuery &iq) {
  ...
  std::string uri;
  if (!solrcloud) {
    uri.append("/" + iq.getIndexName() + "/update" +
               iq.getProperties()["____urprefix"] + "?");
  } else {
    uri.append("/api/c/" + iq.getIndexName() + "/update" +
               iq.getProperties()["____urprefix"] + "?");
  }
  ...
  if (rs.getStatusCode() != "200") {
    ...
    if (ct.find("application/json") != std::string::npos) {
      ...
      if (status > 0) {
        std::string msg(doc["error"]["msg"].GetString(),
                        doc["error"]["msg"].GetStringLength());
...
void SolrSearchConnectionPool::initEnv() {
  bool isZkUrl = false;
  bool inited = false;
  bool solrcloud = StringUtil::toLowerCopy(
                       getProperties().getProperty("solrcloud")) == "true";
  ...
  if (solrcloud) {
    ...
    if (isZkUrl && getProperties().getNodes().size() > 1) {
      throw std::runtime_error(
          "Please specify single node with zookeeper ensemble url");
    } else {
      for (const ConnectionNode &cn : getProperties().getNodes()) {
        std::string burl = cn.getBaseUrl();
        StringUtil::trim(burl);
        ...
        if (burl.find("http://") != 0 || burl.find("https://") != 0) {
          throw std::runtime_error(
              "Please specify a valid node with solr cloud http url");
        }
      }
    }
    ...
    for (const ConnectionNode &cn : getProperties().getNodes()) {
      ...
      if (burl.find("http://") != 0 || burl.find("https://") != 0) {
        throw std::runtime_error(
            "Please specify a valid single node with solr http url");
      }
    }
  }
  ...
  if (solrcloud) {
    if (isZkUrl) {
#ifndef ZK_LIB
      throw std::runtime_error("Cannot start solr cloud client without zk lib "
                               "as a zookeeper ensemble url is specified");
#endif
    }
  }
}

void *SolrSearchConnectionPool::newConnection(const bool &isWrite,
...
SolrSearchConnectionPool::~SolrSearchConnectionPool() { destroyPool(); }
