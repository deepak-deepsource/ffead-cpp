...
Message MessageHandler::readMessageFromQ(const std::string &fileName,
                                         const bool &erase) {
  std::ifstream file;
  size_t fileSize;
...
void MessageHandler::writeMessageToQ(const Message &msg,
...
bool MessageHandler::tempUnSubscribe(const std::string &subs,
                                     const std::string &fileName) {
...
Message MessageHandler::readMessageFromT(const std::string &fileName,
...
void MessageHandler::writeMessageToT(const Message &msg,
                                     const std::string &fileName) {
...
void MessageHandler::unSubscribe(const std::string &subs,
                                 std::string fileName) {
  std::string subscribers;
  std::ifstream myfile1;
  myfile1.open(fileName.c_str());
...
void *MessageHandler::service(void *arg) {
  ...
  if (results.find("<") != std::string::npos &&
      results.find(">") != std::string::npos) {
    std::string h = "Received Message----";
    Cont test;
    try {
      Message msg(results);
      std::string fileName = instance->path + msg.getDestination().getName() +
    ...
    std::string h;
    if (results.find("Queue") != std::string::npos ||
        results.find("Topic") != std::string::npos) {
      h = msg.toXml();
      instance->logger << h << std::flush;
    } else
