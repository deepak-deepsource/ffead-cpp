...
std::string MessageUtil::getSubscriber() const { return this->subscriber; }
...
bool MessageUtil::sendMessage(const Message &msg) {
  ...
  if ((rv = getaddrinfo(this->host.c_str(), this->port.c_str(), &hints,
                        &servinfo)) != 0) {
...
Message MessageUtil::receiveMessage() {
  ...
  std::string h = "GET FROM " + this->destination.getName() + ":" +
...
Destination MessageUtil::getDestination() { return this->destination; }
...
void MessageUtil::subscribe(const std::string &subs) {
  if (this->destination.getType() == "Topic") {
    ...
    std::string h = "SUBSCRIBE " + subs + " TO " + this->destination.getName() +
                    ":" + this->destination.getType();
    // bool flag = true;
    if (send(sockfd, h.c_str(), h.length(), 0) == -1) {
      logger << "send failed" << std::flush;
...
void MessageUtil::unSubscribe(const std::string &subs) {
  if (this->destination.getType() == "Topic") {
    ...
    std::string h = "UNSUBSCRIBE " + subs + " TO " +
