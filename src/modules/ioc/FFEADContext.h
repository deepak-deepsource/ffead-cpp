...
public:
  Bean();
  Bean(const std::string &name, const std::string &value,
       const std::string &clas, const std::string &scope, const bool &isInbuilt,
       bool isController, const std::string &appName = "default");
  ~Bean();
};
...
public:
  ...
  void initializeAllSingletonBeans(
      ...
      Reflector *reflector);
  void clearAllSingletonBeans(
      const std::map<std::string, bool, std::less<>> &servingContexts);
  Reflector *getReflector();
  void release(void *instance, const std::string &beanName,
