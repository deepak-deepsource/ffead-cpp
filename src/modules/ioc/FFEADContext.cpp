...
FFEADContext::FFEADContext(const std::string &depFile,
          ...
          for (unsigned int var1 = 0; var1 < eleeles.size(); var1++) {
            ...
            if (ele1->getTagName() == "inject") {
              if (ele1->getAttribute("bean") != "") {
                ...
                bean.scope =
                    StringUtil::toLowerCopy(ele1->getAttribute("scope"));
...
void *FFEADContext::getBean(const std::string &beanName,
                            const std::string &appName) {
...
void FFEADContext::release(void *instance, const std::string &beanName,
                           const std::string &appName) {
  if (beanName != "" && beans.find(appName + beanName) != beans.end()) {
    Bean &bean = beans[appName + beanName];
...
void FFEADContext::clearAllSingletonBeans(
    const std::map<std::string, bool, std::less<>> &servingContexts) {
  if (cleared)
    return;
...
void FFEADContext::initializeAllSingletonBeans(
  ...
  for (beanIter = beans.begin(); beanIter != beans.end(); beanIter++) {
    ...
    if (servingContexts.find(bean.appName) != servingContexts.end() &&
        bean.scope != "prototype" && objects.find(_k) == objects.end()) {
      ...
      logger << ("Initializing Bean [appName = " + bean.appName +
                 ", name = " + bean.name + ", class = " + bean.clas +
                 ", value = 0x" + StringUtil::toHEX((long long)si))
             << std::endl;
    }
  }
}

Reflector *FFEADContext::getReflector() { return reflector; }

Bean::Bean(const std::string &name, const std::string &value,
           const std::string &type, const std::string &scope,
           const bool &isInbuilt, bool isController,
           const std::string &appName) {
