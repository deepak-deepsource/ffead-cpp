...
SimpleStreamXmlWriter::SimpleStreamXmlWriter(std::ostream &os, bool is_format)
    : os(os), tag_open(false), new_line(true), is_format(is_format) {
  if (is_format) {
    os << HEADER;
  }
}
...
std::string SimpleStreamXmlWriter::writerType() { return "stream"; }

std::string SimpleStreamXmlWriter::toString() { return XmlWriter::BLANK; }
...
void SimpleStreamXmlWriter::attribute(const std::string &key,
...
std::string SimpleStringXmlWriter::writerType() { return "string"; }

std::string SimpleStringXmlWriter::toString() { return ib; }
...
void SimpleStringXmlWriter::attribute(const std::string &key,
                                      const std::string &val) {
  os->append(" ");
  os->append(key);
  os->append("=\"");
...
SimpleXmlParser::SimpleXmlParser(const std::string &mode) { this->mode = mode; }
...
void SimpleXmlParser::readXML(std::string xml, const std::string &parent,
                              Element *par) {
  if (xml == "")
    return;
  StringUtil::trim(xml);
  ...
  if (tag.find_first_of(" ") != std::string::npos) {
    ...
    while (tag.find_first_of("=") != std::string::npos &&
           tag.find_first_of("\"") != std::string::npos) {
  ...
  if (xml.find("</" + ta) != std::string::npos)
    ...
    endTag = xml.find("/>");
  if (xml.find("< ") != std::string::npos) {
    std::string errmsg = ("Invalid Start Tag - at position: " +
                          CastUtil::fromNumber((int)xml.find("< ") + 1) + "\n");
    throw XmlParseException(errmsg);
  } else if (xml.find("<\t") != std::string::npos) {
    std::string errmsg =
        ("Invalid Start Tag - at position: " +
         CastUtil::fromNumber((int)xml.find("<\t") + 1) + "\n");
    throw XmlParseException(errmsg);

  } else if (xml.find("</ ") != std::string::npos) {
    std::string errmsg =
        ("Invalid End Tag - at position: " +
         CastUtil::fromNumber((int)xml.find("</ ") + 1) + "\n");
    throw XmlParseException(errmsg);
    ...
    throw XmlParseException(errmsg);
             ...
             xml.find("</" + ta) != std::string::npos) {
    std::string errmsg = ("No Start Tag - for : " + ta + "\n");
    throw XmlParseException(errmsg);
  } else if (xml.find("<" + ta) != std::string::npos && pndTag == 0 &&
             endTag == 0) {
