...
PropFileReader::PropFileReader() { this->mergeSimProps = false; }
      ...
      if (line != "") {
        ...
        if (line.find("=") != std::string::npos) {
          if (line.find("=") != line.length() - 1) {
            std::string v =
                StringUtil::trimCopy(line.substr(line.find("=") + 1));
...
propMultiMap
PropFileReader::getPropertiesMultiMap(const std::string &filepath) {
  propMultiMap all;
  std::string line;
  if (filepath == "") {
    return all;
      ...
      if (line != "") {
        ...
        if (line.find("=") != std::string::npos) {
          if (line.find("=") != line.length() - 1) {
            ...
            all[StringUtil::trimCopy(line.substr(0, line.find("=")))].push_back(
                v);
          }
        }
      }
