...
private:
  ...
  LoggerConfig *config;
  void writeInternal(const std::string &msg, const std::string &mod,
                     const bool &newline);
  template <typename T>
  void writeTemplate(const T &tmsg, const std::string &mod,
                     const bool &newline) {
    if (config == NULL)
      return;
    Date dat;
  ...
  void writeToStream(std::ostream &(*pf)(std::ostream &),
