...
void JSONUtil::array(std::string &json, JSONElement *element) {
  while (json.find("[") != std::string::npos &&
...
void JSONUtil::arrayOrObject(std::string &json, JSONElement *element) {
  while ((json.find("{") != std::string::npos &&
    ...
    if (json.find("{") != std::string::npos &&
        json.find("}") != std::string::npos &&
        (objst < arrst || arrst == (int)std::string::npos)) {
      element->setType(JSONElement::JSON_OBJECT);
      StringUtil::replaceFirst(json, "{", "");
      StringUtil::replaceLast(json, "}", "");
      readJSON(json, false, element);
...
void JSONUtil::readBalancedJSON(std::string &value, std::string &json,
                                const bool &isarray, const size_t &obs) {
  std::string typest = "{";
  std::string typeen = "}";
  if (isarray) {
    typest = "[";
  ...
  if (eee == std::string::npos) {
    throw std::runtime_error("invalid json - unbalanced " + typest +
                             " found at " + CastUtil::fromNumber(obs));
  }
  ...
  while (stcnt != encnt) {
    ...
    if (eee == std::string::npos) {
      throw std::runtime_error("invalid json - balancing " + typeen +
                               " not found after " + CastUtil::fromNumber(eee));
    }
...
void JSONUtil::readJSON(std::string &json, const bool &isarray,
                        JSONElement *par) {
  if (json == "")
    return;
  ...
  if (!isarray) {
    ...
    if (vst == std::string::npos)
      ...
      throw(
          "invalid json - invalid json - invalid std::string before ':' found");
  ...
  if (obs == 0) {
    ...
    if (obs != std::string::npos && env == 0 &&
        (obs < ars || ars == std::string::npos)) {
      readBalancedJSON(value, json, false, obs);
      element.setType(JSONElement::JSON_OBJECT);
    } else if (ars != std::string::npos && env == 0 &&
               (ars < obs || obs == std::string::npos)) {
      readBalancedJSON(value, json, true, ars);
      element.setType(JSONElement::JSON_ARRAY);
    } else if (obs != std::string::npos && obs < env &&
               (obs < ars || ars == std::string::npos)) {
      readBalancedJSON(value, json, false, obs);
      element.setType(JSONElement::JSON_OBJECT);
    } else if (ars != std::string::npos && ars < env &&
               (ars < obs || obs == std::string::npos)) {
      readBalancedJSON(value, json, true, ars);
      element.setType(JSONElement::JSON_ARRAY);
    } else {
  ...
  if (value == "") {
    throw std::runtime_error("invalid json - no value object found for name " +
                             name);
  }
  if (element.getType() != JSONElement::JSON_OBJECT &&
      element.getType() != JSONElement::JSON_ARRAY) {
...
void JSONUtil::validateSetValue(JSONElement *element, const std::string &v) {
  ...
  if (stn != std::string::npos) {
    ...
    if (enn == std::string::npos) {
      throw std::runtime_error("invalid json - invalid string object '" +
    ...
    element->setType(JSONElement::JSON_STRING);
  } else if (StringUtil::toLowerCopy(value) == "true" ||
             StringUtil::toLowerCopy(value) == "false") {
    ...
    try {
      ...
      throw std::runtime_error("invalid json - invalid double value " + value +
    ...
    try {
      ...
      throw std::runtime_error("invalid json - invalid numeric value " + value +
                               " found for name " + element->getName());
    }
