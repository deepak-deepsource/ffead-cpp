...
Logger::Logger() { this->config = NULL; }

Logger::~Logger() { this->config = NULL; }

void Logger::writeInternal(const std::string &msg, const std::string &mod,
                           const bool &newline) {
  if (config == NULL)
    return;
  Date dat;
  ...
  std::string vhnclsn =
      this->className + (config->vhostNumber > 0
                             ? ("-" + CastUtil::fromNumber(config->vhostNumber))
                             : "");
  std::string fmsg = "[" + te + "] (" + vhnclsn + ") <" + mod + "> :" + msg +
                     (newline ? "\n" : "");
  config->lock->lock();
...
void Logger::writeToStream(std::ostream &(*pf)(std::ostream &),
