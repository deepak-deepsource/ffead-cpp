...
public:
  ...
  template <typename T>
  static T getObjectFromSerilaizedState(const std::string &serilaizedState);

  template <typename T> void operator<<(T &t) { set(t); }

  template <typename T> void operator<<(T *t) { set(t); }

  template <class T>
  void set(T *t, const size_t &strlength = std::string::npos) {
    std::string typeName = CastUtil::getClassName(t);
    set((void *)t, typeName, strlength);
  }

  void set(void *t, std::string typeName,
           size_t strlength = std::string::npos) {
    ...
    internalClear();
    if (typeName == "short")
      objVal = new short(*(short *)t);
    else if (typeName == "int")
      objVal = new int(*(int *)t);
    else if (typeName == "long")
      objVal = new long(*(long *)t);
    else if (typeName == "long long")
      objVal = new long long(*(long long *)t);
    else if (typeName == "unsigned short")
      objVal = new unsigned short(*(unsigned short *)t);
    else if (typeName == "unsigned int")
      objVal = new unsigned int(*(unsigned int *)t);
    else if (typeName == "unsigned long")
      objVal = new unsigned long(*(unsigned long *)t);
    else if (typeName == "unsigned long long")
      objVal = new unsigned long long(*(unsigned long long *)t);
    else if (typeName == "bool")
      objVal = new bool(*(bool *)t);
    else if (typeName == "float")
      objVal = new float(*(float *)t);
    else if (typeName == "double")
      objVal = new double(*(double *)t);
    else if (typeName == "long double")
      objVal = new long double(*(long double *)t);
    else if (typeName == "std::string" || typeName == "string")
      objVal = new std::string(*(std::string *)t);
  ...
  template <class T> void set(T t, const size_t &strlength = -1) {
    ...
    if (typeName.at(typeName.length() - 1) == '*') {
      throw std::runtime_error(
          "Cannot handle pointer types use 'set(T* t)' instead...");
    }
    this->typeName = typeName;
    internalClear();
    if (typeName == "short")
  ...
  template <class T> void get(T &t) {
    ...
    if (typeName.at(typeName.length() - 1) == '*') {
      throw std::runtime_error(
          "Cannot handle pointer types use 'getP()' instead...");
    }
    t = *(T *)objVal;
  }
};
...
template <typename T>
inline T GenericObject::getObjectFromSerilaizedState(
    const std::string &serilaizedState) {
  T t;
  std::string typeName = CastUtil::getClassName(t);
  if (typeName.at(typeName.length() - 1) == '*') {
    throw std::runtime_error(
  ...
  if (GenericObject::isPrimitive(typeName))
    t = CastUtil::lexical_cast<T>(serilaizedState);
  else
    t = XMLSerialize::unserialize<T>(serilaizedState, -1);
  return t;
}
