...
LoggerFactory::~LoggerFactory() {
  ...
  for (it = configs.begin(); it != configs.end(); ++it) {
    std::cout << ("Clearing logger config for " + it->second->name)
              << std::endl;
    delete it->second;
  }
  configs.clear();
...
void LoggerFactory::configureDefaultLogger(const std::string &appName) {
  ...
  if (instance != NULL &&
      instance->configs.find(lname) == instance->configs.end()) {
...
void LoggerFactory::init(const std::string &configFile,
                         const std::string &serverRootDirectory,
                         const std::string &appName,
                         const bool &isLoggingEnabledT) {
  if (instance == NULL) {
    instance = new LoggerFactory();
  }
  ...
  if (isLoggingEnabled && root.getTagName() == "loggers" &&
      root.getChildElements().size() > 0) {
    std::cout << "Reading " << configFileEd << " file to configure loggers..."
              << std::endl;
    ...
    for (unsigned int apps = 0; apps < eles.size(); apps++) {
      if (eles.at(apps).getTagName() == "logger") {
        ...
        if (file != "" && fileele->getAttribute("type") != "") {
          std::string ftyp =
              StringUtil::toUpperCopy(fileele->getAttribute("type"));
          if (ftyp == "ABSOLUTE") {
            logdirtype = "ABSOLUTE";
            logDirPath = "";
          }
        ...
        if (mode == "FILE" && file == "") {
          std::cout << "Error configuring Logger, invalid mode and filepath "
                       "defined..."
                    << std::endl;
        } else if (mode == "FILE") {
          ...
          if (ofs.is_open()) {
            ...
            ofs.close();
            std::cout << "Error configuring Logger, invalid filepath defined..."
                      << std::endl;
            continue;
          }
        }

        if (level != Logger::LEVEL_OFF && level != Logger::LEVEL_FATAL &&
            level != Logger::LEVEL_ERROR && level != Logger::LEVEL_WARN &&
            level != Logger::LEVEL_INFO && level != Logger::LEVEL_DEBUG &&
            level != Logger::LEVEL_TRACE) {
          std::cout << "Error configuring Logger, invalid level defined..."
                    << std::endl;
          continue;
        }
        ...
        if (instance->configs.find(loggerName) != instance->configs.end()) {
          std::cout << "Error configuring Logger " << loggerName
                    << ", Logger already defined skipping..." << std::endl;
          continue;
        } else {
          bool foundDup = false;
          std::map<std::string, LoggerConfig *>::iterator it;
          for (it = instance->configs.begin(); it != instance->configs.end();
               ++it) {
            if ((mode == "FILE" && file != "" && it->second->file == file &&
                 it->second->logdirtype == logdirtype) ||
                (mode == "CONSOLE" && name != "DEFAULT")) {
              instance->dupLogNames[loggerName] = level;
              std::cout << "Found duplicate logger configuration for name " +
                               loggerName
                        << std::endl;
              break;
            }
...
Logger LoggerFactory::getLogger(const std::string &className) {
  ...
  for (it = instance->configs.begin(); it != instance->configs.end(); ++it) {
    if (it->second->pattern != "" &&
        RegexUtil::find(className, it->second->pattern) != -1) {
      return Logger(it->second, className);
    }
  }
  std::string appName = CommonUtils::getAppName(std::string(""));
  if (appName != "") {
    if (instance->configs.find(appName + ".DEFAULT") !=
        instance->configs.end()) {
...
Logger LoggerFactory::getLogger(const std::string &loggerName,
  ...
  if (appName != "") {
    ...
    if (instance->configs.find(nloggerName) == instance->configs.end()) {
      if (instance->dupLogNames.find(nloggerName) !=
          instance->dupLogNames.end()) {
        return Logger(instance->configs[nloggerName], className,
                      instance->dupLogNames[nloggerName]);
      }
    } else {
      return Logger(instance->configs[nloggerName], className);
    }
  ...
  if (instance->configs.find(loggerName) == instance->configs.end()) {
    if (instance->dupLogNames.find(loggerName) != instance->dupLogNames.end()) {
      return Logger(instance->configs[loggerName], className,
                    instance->dupLogNames[loggerName]);
    }
