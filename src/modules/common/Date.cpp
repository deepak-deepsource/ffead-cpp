...
tm *Date::getTimeinfo() { return &ti; }

int Date::getDay() const { return day; }

const std::string &Date::getDayAbbr() const { return dayAbbr; }

const std::string &Date::getHourdesignation() const { return hourdesignation; }

int Date::getMonth() const { return month; }

const std::string &Date::getMonthAbbr() const { return monthAbbr; }

const std::string &Date::getMonthName() const { return monthName; }

long long Date::getNanoseconds() const { return nanoseconds; }

int Date::getPmHours() const { return pmHours; }

const std::string &Date::getTimeZone() const { return timeZone; }

float Date::getTimeZoneOffset() const { return timeZoneOffset; }
...
long long Date::getEpochTime() const { return epochTime; }

int Date::getSeconds() const { return seconds; }
...
std::string Date::toString() {
  return dayAbbr + " " + CastUtil::fromNumber(year) + " " + monthAbbr + " " +
         CastUtil::fromNumber(day) + " " + CastUtil::fromNumber(hours) + ":" +
         CastUtil::fromNumber(minutes) + ":" + CastUtil::fromNumber(seconds) +
         "." + CastUtil::fromNumber(nanoseconds) + " " +
...
Date Date::addMonthsGet(long months) {
  ...
  Date d = getDateFromDays(g);
  d.epochTime =
      getSecondsI(d.year, d.month, d.day, d.hours, d.minutes, d.seconds);
  return d;
}
...
unsigned long long Date::getSecondsI(long y, long m, long d, long hh, long mi,
                                     long ss) const {
...
std::string Date::dayInWords(const int d, const bool &sf) {
  switch (d) {
  case 0:
    return sf ? "SUN" : "SUNDAY";
  case 1:
    return sf ? "MON" : "MONDAY";
  case 2:
    return sf ? "TUE" : "TUESDAY";
  case 3:
    return sf ? "WED" : "WEDNESDAY";
  case 4:
    return sf ? "THU" : "THURSDAY";
  case 5:
    return sf ? "FRI" : "FRIDAY";
  case 6:
    return sf ? "SAT" : "SATURDAY";
  }
  return "";
}
...
std::string Date::monthInWords(const int m, const bool &sf) {
  switch (m) {
  case 0:
    return sf ? "JAN" : "JANUARY";
  case 1:
    return sf ? "FEB" : "FEBRUARY";
  case 2:
    return sf ? "MAR" : "MARCH";
  case 3:
    return sf ? "APR" : "APRIL";
  ...
  case 6:
    return sf ? "JUL" : "JULY";
  case 7:
    return sf ? "AUG" : "AUGUST";
  case 8:
    return sf ? "SEP" : "SEPTEMBER";
  case 9:
    return sf ? "OCT" : "OCTOBER";
  case 10:
    return sf ? "NOV" : "NOVEMBER";
  case 11:
    return sf ? "DEC" : "DECEMBER";
  }
  return "";
}
...
std::string Date::getDayName(int dd, const int &mm, const int &yyyy) {
  ...
  if (!validateDate(dd, mm, yyyy))
    return dayName;
  int days = ((yyyy - 1) * 365 + (yyyy - 1) / 4 - (yyyy - 1) / 100 +
              (yyyy - 1) / 400) %
             7; // Daycode for prev year 31st Dec
  switch (mm) {
  case 12:
    dd += 30;
    break;
...
void Date::populateDay() {
  ...
  int mm = month;
  int dd = day;
  dd += ((yyyy - 1) * 365 + (yyyy - 1) / 4 - (yyyy - 1) / 100 +
         (yyyy - 1) / 400) %
        7; // Daycode for prev year 31st Dec
  switch (mm) {
  case 12:
...
Date::Date(const int &yyyy, const std::string &mmm, const int &dd) {
  ...
  if (!validateDate(dd, CastUtil::toInt(mm), yyyy))
...
Date::Date(const int &yy, const std::string &mmm, const int &dd,
           const bool &te) {
  Date d;
  std::string syyyy =
      CastUtil::fromNumber(d.year).substr(0, 2) + CastUtil::fromNumber(yy);
...
void Date::compare(const Date &d, unsigned long long &thisss,
                   unsigned long long &thtsss) const {
  thisss = getSecondsI(this->year, this->month, this->day, this->hours,
