...
void CommonUtils::setDate() { dateStr = get_date(); }
...
void CommonUtils::loadMimeTypes(const std::string &file) {
  if (getInstance()->mimeTypes.size() > 0)
    return;
...
const std::string &CommonUtils::getMimeType(const std::string &extension) {
  if (getInstance()->mimeTypes.find(extension) !=
      getInstance()->mimeTypes.end()) {
...
void CommonUtils::ntb(std::string &result, const unsigned long long &lon,
                      int ind) {
  for (int i = 0; i < ind; i++) {
...
unsigned long long
CommonUtils::charArrayToULongLong(const std::vector<unsigned char> &l) {
  unsigned long long t = 0;
  int ind = l.size();
  for (int i = 0; i < ind; i++) {
    t = (t << 8) + (l.at(i) & 0xff);
...
std::string CommonUtils::ulonglongTocharArray(const unsigned long long &lon,
                                              const int &provind) {
  int ind;
  if (lon < 256) {
    ind = 1;
...
void CommonUtils::printMap(
    const std::map<int, std::map<std::string, std::string>> &mp) {
  std::map<int, std::map<std::string, std::string>>::const_iterator it;
  for (it = mp.begin(); it != mp.end(); ++it) {
    std::cout << it->first << " = " << it->second.begin()->first << ":"
              << it->second.begin()->second << std::endl;
  }
}
...
std::string CommonUtils::xorEncryptDecrypt(const std::string &toEncrypt,
...
void CommonUtils::listFiles(std::vector<std::string> &files,
                            const std::string &cwd, const std::string &suffix,
                            const bool &isAbs) {
  files.clear();
  struct dirent *entry;
  DIR *dir;
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    if (S_ISREG(sb.st_mode) && StringUtil::endsWith(file, suffix)) {
      ...
      if (isAbs && !StringUtil::endsWith(file, "/.") &&
          !StringUtil::endsWith(file, "/..")) {
        file += "/";
        RegexUtil::replace(file, "[/]+", "/");
        files.push_back(file);
...
std::vector<std::string> CommonUtils::getFiles(const std::string &cwd,
...
void CommonUtils::printStats() {
  ...
  std::string a = ("Connections (Sockets: "+CastUtil::fromNumber(cSocks) + ",
  Open Sockets: " + CastUtil::fromNumber(SocketInterface::openSocks)
                  +", Requests: "+CastUtil::fromNumber(cReqs)+", Responses:
  "+CastUtil::fromNumber(cResps)+")\n"); logger.info(a); std::string b = ("E-E
  Total (EL_Pre: "+CastUtil::fromNumber(tsPoll1)+", EL_Wait:
  "+CastUtil::fromNumber(tsPoll)+
                  ", EL_Process: "+CastUtil::fromNumber(tsProcess)+", Sock_Read:
  "+CastUtil::fromNumber(tsReqSockRead)+
                  ", Req_Parse: "+CastUtil::fromNumber(tsReqParse)+", Req_Prep:
  "+CastUtil::fromNumber(tsReqTotal-tsReqSockRead-tsReqPrsSrvc)+
                  ", Service: "+CastUtil::fromNumber(tsService)+", Sock_Write:
  "+CastUtil::fromNumber(tsResSockWrite)+
                  ", Res_Prep:
  "+CastUtil::fromNumber(tsResTotal-tsResSockWrite)+")\n"); logger.info(b);
  if(cReqs>0) {
          std::string c = ("E-E Average (EL_Pre:
  "+CastUtil::fromNumber(tsPoll1/cReqs)+", EL_Wait:
  "+CastUtil::fromNumber(tsPoll/cReqs)+
                          ", EL_Process:
  "+CastUtil::fromNumber(tsProcess/cReqs)+", Sock_Read:
  "+CastUtil::fromNumber(tsReqSockRead/cReqs)+
                          ", Req_Parse:
  "+CastUtil::fromNumber(tsReqParse/cReqs)+", Req_Prep:
  "+CastUtil::fromNumber((tsReqTotal-tsReqSockRead-tsReqPrsSrvc)/cReqs)+
                          ", Service:
  "+CastUtil::fromNumber((tsService)/cReqs)+", Sock_Write:
  "+CastUtil::fromNumber(tsResSockWrite/cReqs)+
                          ", Res_Prep:
  "+CastUtil::fromNumber((tsResTotal-tsResSockWrite)/cReqs)+")\n");
          logger.info(c);
  }
  std::string d = ("Service Total (Pre: "+CastUtil::fromNumber(tsServicePre)+",
  Cors: "+CastUtil::fromNumber(tsServiceCors)+
                  ", Security: "+CastUtil::fromNumber(tsServiceSec) + ", Filter:
  "+CastUtil::fromNumber(tsServiceFlt)+
                  ", Controller: "+CastUtil::fromNumber(tsServiceCnt)+", Ext:
  "+CastUtil::fromNumber(tsServiceExt)+
                  ", Post: "+CastUtil::fromNumber(tsServicePost)+")\n");
  logger.info(d);
  if(cReqs>0) {
          std::string e = ("Service Average (Pre:
  "+CastUtil::fromNumber(tsServicePre/cReqs)+", Cors:
  "+CastUtil::fromNumber(tsServiceCors/cReqs)+
                          ", Security: "+
  CastUtil::fromNumber(tsServiceSec/cReqs) + ", Filter:
  "+CastUtil::fromNumber(tsServiceFlt/cReqs)+
                          ", Controller:
  "+CastUtil::fromNumber(tsServiceCnt/cReqs)+ ", Ext:
  "+CastUtil::fromNumber(tsServiceExt/cReqs)+
                          ", Post:
  "+CastUtil::fromNumber(tsServicePost/cReqs)+")\n"); logger.info(e);
  }
  std::string f = ("Controller Total (Cont_Cond:
  "+CastUtil::fromNumber(tsContMpg)+", Mapg_Cond:
  "+CastUtil::fromNumber(tsContPath)+
                  ", Ext_Cond: "+CastUtil::fromNumber(tsContExt) + ", Cont_Exec:
  "+CastUtil::fromNumber(tsContExec)+
                  ", Rest_Lkp: "+CastUtil::fromNumber(tsContRstLkp)+",
  Rest_CsiLkp: "+CastUtil::fromNumber(tsContRstCsiLkp)+
                  ", Rest_InsLkp: "+CastUtil::fromNumber(tsContRstInsLkp)+",
  Rest_PrsArgs: "+CastUtil::fromNumber(tsContRstPrsArgs)+
                  ", Rest_Exec: "+CastUtil::fromNumber(tsContRstExec)+ ",
  Rest_Ser: "+CastUtil::fromNumber(tsContRstSer)+")\n"); logger.info(f);
  if(cReqs>0) {
          std::string g = ("Controller Average (Cont_Cond:
  "+CastUtil::fromNumber(tsContMpg/cReqs)+
                          ", Mapg_Cond:
  "+CastUtil::fromNumber(tsContPath/cReqs)+", Ext_Cond:
  "+CastUtil::fromNumber(tsContExt/cReqs) +
                          ", Cont_Exec:
  "+CastUtil::fromNumber(tsContExec/cReqs)+", Rest_Lkp:
  "+CastUtil::fromNumber(tsContRstLkp/cReqs)+
                          ", Rest_CsiLkp:
  "+CastUtil::fromNumber(tsContRstCsiLkp/cReqs)+ ", Rest_InsLkp:
  "+CastUtil::fromNumber(tsContRstInsLkp/cReqs)+
                          ", Rest_PrsArgs:
  "+CastUtil::fromNumber(tsContRstPrsArgs/cReqs)+", Rest_Exec:
  "+CastUtil::fromNumber(tsContRstExec/cReqs)+
                          ", Rest_Ser:
  "+CastUtil::fromNumber(tsContRstSer/cReqs)+")\n"); logger.info(g);
  }*/
}
...
std::string CommonUtils::getTpeFnName(const std::string &tpe,
                                      const std::string &appName, bool fast) {
...
void CommonUtils::getDateStr(std::string &resp) { resp.append(dateStr); }

std::string CommonUtils::getDateStr() { return std::string(dateStr); }

const char *CommonUtils::getDateStrP() { return dateStr; }
