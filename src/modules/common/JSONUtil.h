...
class JSONUtil {
  ...
  static void readBalancedJSON(std::string &value, std::string &json,
                               const bool &isarray, const size_t &obs);
  static void readJSON(std::string &json, const bool &isarray,
                       JSONElement *par);
...
public:
  ...
  template <typename T>
  static std::vector<T> *toVectorP(const JSONElement &root) {
