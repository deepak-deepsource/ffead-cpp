...
Bigint::Bigint(const std::string &value) { create(value); }
...
void Bigint::internalAdd(const Bigint &number) {
  ...
  if (parts.size() > 0 && number.parts.size() > 0) {
    ...
    for (int i = 0; i < eqprtssiz; i++) {
      int res = ((int)parts.size() > i ? parts.at(i) : 0) +
                ((int)number.parts.size() > i ? number.parts.at(i) : 0) +
                (carryOver ? 1 : 0);
      if (res > NUM_MAX) {
  ...
  if (parts.size() > 0 && number.parts.size() > 0) {
    ...
    for (int i = 0; i < eqprtssiz; i++) {
      int res =
          ...
          ((int)sparts.size() > i ? sparts.at(i) : 0);
      if (res < 0) {
        res =
            NUM_MAX_THRESHOLD - ((int)sparts.size() > i ? sparts.at(i) : 0) +
            (((int)fparts.size() > i ? fparts.at(i) - (carryOver ? 1 : 0) : 0));
        carryOver = true;
      } else {
        carryOver = false;
...
void Bigint::divide(const Bigint &number) { internalDivide(number, false, 1); }

void Bigint::internalDivide(const Bigint &number, const bool &isDecimal,
                            const int &precision) {
  std::string mstring;
  std::vector<int> mparts;
  std::string fnvalue = toString();
...
int Bigint::decompose(std::string fnvalue, std::string snvalue,
                      const Bigint &number, int recurse, std::string &build,
                      const bool &isDecimal, const int &precision) {
  if (recurse >= precision || fnvalue == BLANK)
    return recurse;
  std::string fntemp;
  ...
  try {
    ...
    recurse = decompose((fbtemp.toString() == ZERO ? "" : fbtemp.toString()) +
                            fnvalue,
                        snvalue, number, recurse, build, isDecimal, precision);
  } catch (const std::exception &e) {
  ...
  if (isPositive == number.isPositive) {
    if (parts.size() == number.parts.size()) {
      if (parts.at(parts.size() - 1) == number.parts.at(parts.size() - 1)) {
        return 0;
      } else if (parts.at(parts.size() - 1) >
                 number.parts.at(parts.size() - 1)) {
        return 1;
      } else {
        return -1;
      }
