...
void MarkerHandler::addMarker(const Marker &m) { validMarkers.push_back(m); }

void MarkerHandler::initMarkers() {
  // Start DsORM markers
  validMarkers.push_back(
      Marker("@Entity", Marker::TYPE_CLASS,
             collectStr(2, "dataSourceName", "isEmbedded,false,true,false"),
             collectBool(2, false, false)));
  validMarkers.push_back(
      Marker("@Table", Marker::TYPE_CLASS, collectStr(1, "name")));
  validMarkers.push_back(
      Marker("@Column", Marker::TYPE_PROP, collectStr(1, "dbf")));
  validMarkers.push_back(
      Marker("@IdGenerate", Marker::TYPE_PROP,
             collectStr(7, "dbEntityType,,table,sequence,identity",
                        "dbEntityName", "type,,increment,hilo", "hiValueColumn",
                        "lowValue,32767", "entityColumn", "columnName")));
  validMarkers.push_back(
      Marker("@Id", Marker::TYPE_PROP, collectStr(1, "dbf")));
  validMarkers.push_back(
      Marker("@HasOne", Marker::TYPE_PROP, collectStr(2, "dmappedBy", "dfk")));
  validMarkers.push_back(
      Marker("@HasMany", Marker::TYPE_PROP, collectStr(2, "dmappedBy", "dfk")));

  validMarkers.push_back(Marker(
      "@Field", Marker::TYPE_PROP,
      MarkerHandler::collectStr(
          7, "type", "name", "index,false,true,false", "store,false,true,false",
          "required,false,true,false", "analyzer", "searchanalyzer"),
      MarkerHandler::collectBool(7, true, true, false, false, false, false,
                                 false)));
  validMarkers.push_back(
      Marker("@Document", Marker::TYPE_CLASS,
             collectStr(5, "name", "type", "shards", "replicas",
...
const std::string &Marker::getName() const { return name; }
...
Marker MarkerHandler::processMarker(std::string markerText, const int &where) {
  ...
  if (markerText.find("#pragma ") != 0) {
    throw std::string("Invalid Marker, all markers should start with the "
                      "#pragma pre-processor directive");
  }
  StringUtil::replaceFirst(markerText, "#pragma ", "");
  std::vector<std::string> fragments =
      StringUtil::splitAndReturn<std::vector<std::string>>(markerText, " ");
  ...
  if (targetMarker.name == "") {
    throw std::runtime_error("Could not find the " +
                             Marker::getTypeName(where) +
                             " type marker with name " + name);
  }
  ...
  if (targetMarker.reqAttrSize > 0 && (int)fragments.size() == 1) {
    throw std::runtime_error("No attributes specified for the marker " +
                             targetMarker.name);
  } else {
      ...
      if (fragments.at(i) != "") {
        ...
        if (value.at(0) != '"' && value.at(value.length() - 1) != '"') {
          throw std::runtime_error(
              "Attribute value for " + attname +
              " should be within double quotes (\"\") for the marker " +
              targetMarker.name);
        }
        if (value != "\"\"") {
    ...
    for (; it != targetMarker.attributes.end(); ++it) {
      if (tvalues.find(it->first) == tvalues.end()) {
        if (it->second) {
          throw std::runtime_error(
              "No value specified for mandatory attribute " + it->first +
              " for the marker " + targetMarker.name);
        }
        logger.info("Ignoring attribute " + it->first + " for marker " +
                    targetMarker.name);
        ...
        if (value == "") {
          throw std::runtime_error("Attribute value for " + it->first +
                                   " cannot be blank for the marker " +
                                   targetMarker.name);
        }
        if (vss.size() > 0) {
          bool valid = false;
          ...
          if (!valid) {
            std::string err = "Attribute " + it->first +
...
const bool Marker::isTypeClass() const { return TYPE_CLASS == type; }

const bool Marker::isTypeProp() const { return TYPE_PROP == type; }

const bool Marker::isTypeMeth() const { return TYPE_METH == type; }

const bool Marker::isTypeArg() const { return TYPE_ARG == type; }
...
Marker::Marker(const std::string &name, const int &type,
  ...
  for (int i = 0; i < (int)attributes.size(); ++i) {
    ...
    std::vector<std::string> fragments =
        StringUtil::splitAndReturn<std::vector<std::string>>(text, ",");
    std::string name = fragments.at(0);
