...
class CastUtil {
  ...
  template <typename T>
  static void primitive(const T &val, const char *fmt, std::string *d) {
...
public:
  ...
  template <typename T> static std::string getClassName(T &t) {
    ...
    std::map<std::string, std::string> *_mangledClassNameMap =
        (std::map<std::string, std::string> *)_mcMap.get();
    std::map<std::string, std::string>::iterator it =
        _mangledClassNameMap->find(sm);
    ...
    if (tn.find("std::__1::") != std::string::npos) {
      ...
      if (tn.find("std::basic_string<char, std::char_traits<char>, "
                  "std::allocator<char> >") != std::string::npos) {
        StringUtil::replaceAll(tn,
  ...
  template <typename T> static T lexical_cast(const std::string &vval) {
    ...
    if (tn == "float") {
      if (vval.at(0) == '0' || vval.find("+0") == 0 || vval.find("-0") == 0) {
        ...
        if (dots > 1) {
          throw std::runtime_error(
              "Conversion exception - std::string to float");
        }
        if (vval.at(0) == '0') {
      ...
      if (vval.at(0) == '0' || vval.find("+0") == 0 || vval.find("-0") == 0) {
        ...
        if (dots > 1) {
          throw std::runtime_error(
              "Conversion exception - std::string to double");
        }
        if (vval.at(0) == '0') {
          if (vval.find_first_not_of(".0") == std::string::npos) {
          } else {
      ...
      if (sscanf(vval.c_str(), "%Lf", &ld) == 1) {
        ...
        throw std::runtime_error(
            "Conversion exception - std::string to long double");
      }
      vt = &ld;
      t = *(T *)vt;
    } else if (tn == "int") {
      if (vval == "0" ||
          (vval.length() > 0 && vval.at(0) == '0' &&
           vval.find_first_not_of("0") == std::string::npos) ||
          (vval.find("+0") == 0 &&
           vval.find_first_not_of("0", 1) == std::string::npos) ||
          (vval.find("-0") == 0 &&
           vval.find_first_not_of("0", 1) == std::string::npos)) {
      } else {
        bool invalid = false;
      ...
      if (vval == "0" ||
        ...
        if (invalid) {
          throw std::runtime_error(
              "Conversion exception - std::string to short");
        }
      }
      vt = &s;
      t = *(T *)vt;
      ...
      if (vval == "0" ||
        ...
        if (invalid) {
          throw std::runtime_error(
              "Conversion exception - std::string to long");
        }
      }
      vt = &l;
      t = *(T *)vt;
      ...
      if (vval == "0" ||
        ...
        if (invalid) {
          throw std::runtime_error(
              "Conversion exception - std::string to unsigned short");
        }
      }
      vt = &us;
      t = *(T *)vt;
      ...
      if (vval == "0" ||
        ...
        if (invalid) {
          throw std::runtime_error(
              "Conversion exception - std::string to unsigned int");
        }
      }
      vt = &ui;
      t = *(T *)vt;
      ...
      if (vval == "0" ||
        ...
        if (invalid) {
          throw std::runtime_error(
              "Conversion exception - std::string to unsigned long");
        }
      }
      vt = &ul;
      t = *(T *)vt;
      ...
      if (sscanf(vval.c_str(), "%lld", &ll) == 1) {
        ...
        throw std::runtime_error(
            "Conversion exception - std::string to long long");
      }
      vt = &ll;
      t = *(T *)vt;
      ...
      if (sscanf(vval.c_str(), "%llu", &ull) == 1) {
        ...
        throw std::runtime_error(
            "Conversion exception - std::string to unsigned long long");
      }
      vt = &ull;
      t = *(T *)vt;
    } else if (tn == "bool") {
      if (StringUtil::toLowerCopy(vval) == "true" ||
          StringUtil::toLowerCopy(vval) == "1")
        b = true;
      else if (StringUtil::toLowerCopy(vval) == "false" ||
               StringUtil::toLowerCopy(vval) == "0")
        b = false;
      else {
  ...
  static short toShort(const std::string &vval) {
    short s = 0;
    char *endptr;
    /*if(vval=="0" || (vval.length()>0 && vval.at(0)=='0' &&
    vval.find_first_not_of("0")==std::string::npos)
                    || (vval.find("+0")==0 && vval.find_first_not_of("0",
    1)==std::string::npos)
                    || (vval.find("-0")==0 && vval.find_first_not_of("0",
    1)==std::string::npos)) { } else {*/
    bool invalid = false;
    if (vval != "0") {
      s = strtol(vval.c_str(), &endptr, 10);
      invalid = (s == 0);
  ...
  static char toChar(const std::string &vval) { return vval.at(0); }
  ...
  template <typename T> static bool isPrimitiveDataType() {
    ...
    if (type == "short" || type == "short int" || type == "signed short" ||
        ...
        type == "string" || type == STD_STRING || type == "char" ||
