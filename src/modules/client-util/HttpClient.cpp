...
void HttpClient::execute(HttpRequest *request, HttpResponse *response,
                         propMap &props) {
#ifdef HAVE_CURLLIB
  ...
  if (request->getContent() != "") {
    // curl_easy_setopt(_h, CURLOPT_POSTFIELDSIZE_LARGE,
    // request->getContent().length()); curl_easy_setopt(_h, CURLOPT_POSTFIELDS,
    // (char *)request->getContent().c_str());
  ...
  std::string _t;
  for (RMap::const_iterator it = request->headers.begin();
  ...
  if (request->headers.find(HttpRequest::UserAgent) != request->headers.end()) {
    curl_easy_setopt(_h, CURLOPT_USERAGENT,
                     request->headers[HttpRequest::UserAgent].c_str());
  } else {
  ...
  if (props["FOLLOWLOCATION"] == "true") {
    ...
    try {
      curl_easy_setopt(_h, CURLOPT_MAXREDIRS,
                       CastUtil::toInt(props["MAXREDIRS"]));
    } catch (const std::exception &e) {
  ...
  if (r != CURLE_OK) {
    ...
    response->setHTTPResponseStatus(HTTPResponseStatus::getStatusByCode(
        std::to_string(static_cast<int>(http_code))));
...
size_t HttpClient::readContent(char *ptr, size_t size, size_t nmemb,
                               void *data) {
  HttpRequest *request = (HttpRequest *)data;
  size_t ret = request->getContent().length();
  if (ret > 0) {
    memcpy(ptr, (char *)request->getContent().c_str(),
...
size_t HttpClient::onHeaders(void *res, size_t len, size_t mb, void *data) {
  ...
  if (hdr.find_first_of(":") != std::string::npos) {
    ...
    response->addHeaderValue(hdr.substr(0, hdr.find_first_of(":")),
