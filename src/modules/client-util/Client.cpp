...
{ fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv)); return false;
    ...
    for(p = servinfo; p != NULL; p = p->ai_next) {
        ...
        if (connect(sockfd, p->ai_addr, p->ai_addrlen) == -1 && (errno !=
EINPROGRESS)) { closesocket(sockfd); perror("client: connect"); connected =
false; continue; } else { connected = true;
        }
        break;
    }
...
std::string Client::getHttpData(const std::string &hdrdelm,
                                const std::string &cntlnhdr, bool connected,
                                std::string &buffer) {
  int cntlen = 0;
  std::string alldat;
  bool isTE = false;
  ...
  if (connected) {
    ...
    if (ps != std::string::npos) {
      std::string cntle =
          alldat.substr(ps + cntlnhdr1.length(), ltemp.find("\r\n", ps));
...
std::string Client::getTextData(const std::string &hdrdelm,
...
std::string Client::getBinaryData(const int &len,
