...
void Reflection::addMarker(const Marker &m) { handler.addMarker(m); }
...
/*vector<std::string> Reflection::list(const std::string& cwd)
{
        ...
        if(chdir(cwd.c_str())!=0)
                return files;
        command = ("find . \\( ! -name . -prune \\) \\( -type f -o -type l \\)
-name '*.h' 2>/dev/null");
        //command = "ls -F1 "+cwd+"|grep '.h'";
        //logger << ("Searching directory " + cwd + " for pattern .h") <<
std::endl; if ((pipe_fp = popen(command.c_str(), "r")) == NULL)
        {
...
void Reflection::collectInfo(std::string data, const std::string &flag,
                             ClassStructure &cls,
                             const std::vector<std::string> &mrktxt) {
...
void Reflection::handleNamespace(
    std::string data, std::string namepsc,
  ...
  if (clsvec.find(namepsc) == clsvec.end() &&
      RegexUtil::find(data, "^[ \t]*#[ \t]*pragma[ \t]*[^`]+`", true) == 0) {
    int spos, epos;
  ...
  } else if (RegexUtil::find(data, "^[ \t]*using[ \t]*namespace[ \t]*[^;]+;") ==
             0) {
    ...
    bool flag = false;
    if (enind != std::string::npos && cbst != std::string::npos &&
        enind > cbst) {
      enind = 0;
    ...
    } else if (enind != std::string::npos &&
               nmdata.find("{") != std::string::npos) {
      flag = true;
    }
    if (flag) {
      int st = data.find("{");
    ...
    if (classN.find(";") != std::string::npos) {
      handleNamespace(data.substr(data.find(";") + 1), namepsc, clsvec,
                      glbnmspcs, pragmas);
    ...
    if (results.size() > 0) {
      for (int y = 0; y < (int)results.size(); y++) {
        ...
        if (1 == (int)bas.size()) {
          ...
          bases.push_back(
              std::make_tuple(bas.at(0) == "public" ? 3 : 2, bas.at(1)));
        }
      }
      baseClassN = claspost;
    ...
    pragmas.clear();
  } else if (data.find(" namespace ") != std::string::npos &&
             nmspcst != (int)data.find(" namespace ")) {
    std::string sdata = data.substr(0, data.find(" namespace "));
    pragmas.clear();
    handleNamespace(sdata, namepsc, clsvec, glbnmspcs, pragmas);
    handleNamespace(data.substr(data.find(" namespace ")), namepsc, clsvec,
                    glbnmspcs, pragmas);
    ...
    handleNamespace(sdata, namepsc, clsvec, glbnmspcs, pragmas);
    handleNamespace(data.substr(data.find(" class ") + 1), namepsc, clsvec,
                    glbnmspcs, pragmas);
    pragmas.clear();
    ...
    std::vector<std::string> parts =
        StringUtil::splitAndReturn<std::vector<std::string>>(data, ";");
    if (parts.size() > 0) {
      ...
      for (int var = 0; var < (int)parts.size(); ++var) {
        ...
        while (RegexUtil::find(partsva, "^[ \t]*#[ \t]*pragma[ \t]*[^`]+`",
                               true) == 0) {
          int spos, epos;
          RegexUtil::find(partsva, "^[ \t]*#[ \t]*pragma[ \t]*[^`]+`", spos,
                          epos, true);
...
std::map<std::string, ClassStructure, std::less<>>
Reflection::getClassStructures(const std::string &className,
  ...
  if (infile.is_open()) {
    ...
    while (getline(infile, data)) {
      ...
      data = std::regex_replace(
          data, std::regex("([ \t]*)#[ \t]*if[ \t]+(.*)"),
          "$1#pragma @CondBarrier type=\"if\" value=\"$2\"\n");
      int plen = data.length();
      data = std::regex_replace(
          data, std::regex("([ \t]*)#[ \t]*ifdef[ \t]+(.*)"),
          "$1#pragma @CondBarrier type=\"ifdef\" value=\"$2\"\n");
      if (data.length() != plen) {
        logger.info(data);
      }
      data = std::regex_replace(
          data, std::regex("([ \t]*)#[ \t]*ifndef[ \t]+(.*)"),
          "$1#pragma @CondBarrier type=\"ifndef\" value=\"$2\"\n");
      data = std::regex_replace(
          data, std::regex("([ \t]*)#[ \t]*elif[ \t]+(.*)"),
          "$1#pragma @CondBarrier type=\"elif\" value=\"$2\"\n");
      // data = std::regex_replace(data, std::regex("([ \t]*)#[ \t]*else(.*)"),
      // "$1#pragma @CondBarrier type=\"else\"\n"); data =
      // std::regex_replace(data, std::regex("([ \t]*)#[ \t]*endif(.*)"),
      // "$1#pragma @CondBarrier type=\"endif\"\n");
    ...
    \t]*\"[^\"]+\"", " "); RegexUtil::replace(data, "[ \t]*#ifndef[
    \t]*[a-zA-Z0-9_]*[ \t]*", " "); RegexUtil::replace(data, "[ \t]*#endif[
    \t]*", " ");*/

    std::vector<std::string> typedefs =
        RegexUtil::search(data, "[ \t]*typedef[ \t]*[^;]+;");
    ...
    for (it = clsvec.begin(); it != clsvec.end(); ++it) {
      ...
      if (it->second.pub.size() > 0) {
        ...
        if (it->second.nmSpc.find("::") != std::string::npos) {
          it->second.nmSpc = it->second.nmSpc.substr(
              0, it->second.nmSpc.find_last_of("::") + 1);
        }
        if (it->second.nmSpc != "" &&
            nmspcIds.find(appName + it->second.nmSpc) == nmspcIds.end()) {
          nmspcIds[appName + it->second.nmSpc] = nmspcIds.size() + 1;
          std::string nmspcvv = it->second.nmSpc;
          StringUtil::replaceAll(nmspcvv, "::", ".");
          nmspcIdVals[Reflection::getNameSpaceId(it->second.nmSpc, appName)] =
              nmspcvv;
        }
      ...
      if (glbnmspcs.find(it->second.nmSpc) != glbnmspcs.end()) {
        copy(glbnmspcs[it->second.nmSpc].begin(),
             glbnmspcs[it->second.nmSpc].end(), std::back_inserter(nnspcs));
      }
      copy(it->second.namespaces.begin(), it->second.namespaces.end(),
           std::back_inserter(nnspcs));
...
Reflection::getAfcObjectData(ClassStructure &classStructure, const bool &object,
                             std::vector<std::string> &privf,
                             bool &isOpForSet) {
  isOpForSet = false;
  ...
  if (classStructure.pub.size() > 0) {
    for (unsigned int i = 0; i < classStructure.pub.size(); i++) {
      if ((tes = classStructure.pub.at(i).find("(")) != std::string::npos &&
          (tes = classStructure.pub.at(i).find(")")) != std::string::npos) {
  ...
  if (classStructure.pri.size() > 0) {
    for (unsigned int i = 0; i < classStructure.pri.size(); i++) {
      if ((tes = classStructure.pri.at(i).find("(")) != std::string::npos &&
          (tes = classStructure.pri.at(i).find(")")) != std::string::npos) {
  ...
  if (classStructure.pro.size() > 0) {
    for (unsigned int i = 0; i < classStructure.pro.size(); i++) {
      if ((tes = classStructure.pro.at(i).find("(")) != std::string::npos &&
          (tes = classStructure.pro.at(i).find(")")) != std::string::npos) {
...
std::string Reflection::generateClassDefinitionsAll(
    std::map<std::string, std::map<std::string, ClassStructure, std::less<>>,
             std::less<>> &clsstrucMaps,
    std::string &includeRef, const std::vector<std::string> &apps) {
  std::string ret = "";
  ...
  for (unsigned int var = 0; var < apps.size(); ++var) {
    ...
    ret += this->generateClassDefinition(
        clsstrucMaps[StringUtil::trimCopy(apps.at(var))], inc, typedefs,
        classes, methods, opers, app);
  }
  /*ret += "ClassInfo Reflector::getClassInfo(std::string className)\n{\n";
  ret += classes;
  ret += "\n\treturn info;\n}\n";
  ret += "void* Reflector::invokeMethod(void* instance,Method method,vals
  values,bool cleanvals)\n{\n"; ret += methods; ret += "\n\treturn
  returnValue;\n}\n"; ret += "bool Reflector::instanceOf(void*
  instance,std::string className)\n{\n"; ret += "Trace tr;\nstd::string cn =
  tr.getClassName(instance);\nif(cn==className)\nreturn true;\nelse\nreturn
  false;\n}\n";*/
  ret += methods + opers + "\n}\n";
  ret = (inc + ret);
  return ret;
...
std::string Reflection::generateClassDefinitions(
    std::map<std::string, ClassStructure, std::less<>> &allclsmap,
    std::string &includesDefs, std::string &typedefs, std::string &classes,
    std::string &methods, std::string &opers, const std::string &app) {
  return "";
}

std::string Reflection::generateClassDefinition(
    std::map<std::string, ClassStructure, std::less<>> &allclsmap,
    std::string &includesDefs, std::string &typedefs, std::string &classes,
    std::string &methods, std::string &opers, const std::string &app) {
  ...
  for (it = allclsmap.begin(); it != allclsmap.end(); ++it) {
    ...
    if (classStructure.markers.find("@IgnoreRef") !=
        classStructure.markers.end()) {
      continue;
    }
    bool ispurecs = false;
    if (methallpvstats.find(classStructure.getTreatedClassName(true)) !=
        methallpvstats.end()) {
      if (methallpvstats.find(classStructure.getTreatedClassName(true))
              ->second) {
        ispurecs = true;
    ...
    includesDefs += "#include \"" +
                    getClassPath(it->second.getTreatedClassName(true)) + "\"\n";
    // classes +=
    // "\tif(className==\""+classStructure.getTreatedClassName(true)+"\")\n\t\treturn
    // get"+classStructure.getTreatedClassName(true)+"();\n";

    // string structinf = "\nstruct struct"+classStructure.classN+"{\n";

    testStr += "void test" + app + "_fcrcif_" +
               classStructure.getTreatedClassName(true) + "()\n{\n";
    testStr += "ClassInfo ci = " + app + "_fcrcif_" +
               classStructure.getTreatedClassName(true) + "();\n";
    testStr +=
        "assert(ci.getClassName()==\"" + classStructure.classN + "\");\n";
    testStr += "assert(ci.getNamespace()==\"" + classStructure.nmSpc + "\");\n";
    testStr += "assert(ci.getBase()==\"" + classStructure.baseClassN + "\");\n";
    testStr += ("\nConstructor ctor;\nMethod me;\nField f;\n");
    testStr += ("args argu;\n");

    refDef += "ClassInfo " + app + "_" +
              classStructure.getTreatedClassName(true) + "()\n{\nClassInfo ci;";
    ...
    for (int y = 0; y < (int)classStructure.bases.size(); y++) {
      refDef += ("\nci.addBase(" +
                 CastUtil::fromNumber(std::get<0>(classStructure.bases.at(y))) +
                 ",\"" + std::get<1>(classStructure.bases.at(y)) + "\");");
    }
    refDef += ("\nConstructor ctor;\nMethod me;\nField f;\n");
    refDef += ("args argu;\n");
    ...
    if (classStructure.pubps.size() > 0 || classStructure.pubms.size() > 0) {
      for (unsigned int i = 0; i < classStructure.pubms.size(); i++) {
        ...
        if (isCB) {
          for (auto mk : ms.markers.find("@CondBarrier")->second) {
            if (mk.getAttributeValue("type") == "if" ||
                mk.getAttributeValue("type") == "ifdef" ||
                mk.getAttributeValue("type") == "ifndef") {
              openCB += "\n#" + mk.getAttributeValue("type") + " " +
                        mk.getAttributeValue("value") + "\n";
              closeCB += "\n#endif\n";
            }
          }
        }
        ...
        if (pubdecl.find("~") != std::string::npos) {
          methods += openCB;
          std::string mmn = app + "_" +
                            classStructure.getTreatedClassName(true) + "_m" +
                            CastUtil::fromNumber(methcounter++);
          methods += "\nvoid " + mmn + "(void* instance)\n{";
          methods += "\n\t" + classStructure.getFullyQualifiedClassName() +
                     " *_obj = (" +
                     classStructure.getFullyQualifiedClassName() +
                     "*)instance;\n\t";
          methods += "delete _obj;";
          ...
          meth = pubdecl;
          StringUtil::replaceFirst(meth, ";", "");
          if (RegexUtil::find(meth, "[ \t]*=[ \t]*delete") != -1 ||
              RegexUtil::find(meth, "[ \t]*=[ \t]*default") != -1) {
            continue;
          }
          ...
          std::string argts =
              meth.substr(meth.find("("), meth.find(")") - meth.find("("));
          ...
          // if(RegexUtil::find(fld, "[ \t]*static[ \t]+")!=-1 ||
          // RegexUtil::find(fld, "[ \t]+static[ \t]+")!=-1)
          //{
          ...
          for (unsigned int j = 0; j < argp.size(); j++) {
            if (argp.at(j) != "" && argp.at(j) != "(") {
              ...
              if (ptindx == argpmtemp.length() - 1 ||
                  (ptindx != std::string::npos && teindx != std::string::npos &&
                   ptindx > teindx) ||
                  (teindx == std::string::npos && ptindx != std::string::npos &&
                   spindx != std::string::npos && ptindx > spindx)) {
                typ = "*";
                argpmtemp = argpmtemp.substr(0, ptindx);
              } else if (rfindx == argpmtemp.length() - 1 ||
                         (rfindx != std::string::npos &&
                          teindx != std::string::npos && rfindx > teindx) ||
                         (teindx == std::string::npos &&
                          rfindx != std::string::npos &&
                          spindx != std::string::npos && rfindx > spindx)) {
                typ = "&";
                StringUtil::replaceLast(argpmtemp, "&", "");
              }
              if (teindx != std::string::npos && spindx != std::string::npos &&
                  teindx > spindx) {
          ...
          bool ispure = false;
          if (methpm.size() > 0 &&
              methpm.at(0).find("virtual") != std::string::npos) {
            methpm[0] =
                StringUtil::replaceFirstCopy(methpm.at(0), "virtual ", "");
            isvirtual = true;
            std::string tmp = pubdecl;
            RegexUtil::replace(tmp, "[ ]+", "");
            // TODO
            ispure = tmp.at(tmp.length() - 2) == '=' &&
          ...
          // for(unsigned int j = 0; j < methpm.size(); j++)
          {
            if (methpm.at(0) == classStructure.getTreatedClassName(false)) {
              // refDef +=
              // ("ctor.setName(\""+classStructure.getTreatedClassName(true)+"\");\n");
              // refDef +=
              // ("me.setMethodName(\""+classStructure.getTreatedClassName(true)+methpm.at(j)+"\");\n");
              // ctor = true;
              methsd += (classStructure.getTreatedClassName(true));
            } else if (meth.find(" operator") == std::string::npos) {
              std::string fqcn = getFullyQualifiedClassName(
                  methpm.at(0), classStructure.namespaces);
              refDef += ("me.setReturnType(\"" + fqcn + "\");\n");
              typedefs += ("typedef " + methpm.at(0) + " ");
              methsd +=
                  (classStructure.getTreatedClassName(true) + methpm.at(1));
              ms.name = methpm.at(1);
            } else {

              // refDef += (classStructure.getFullyQualifiedClassName() +
              // methpm.at(j)+" =
              // &"+classStructure.getTreatedClassName(true)+"::"+methpm.at(j)+";\n");
              typedefs +=
                  ("(" + classStructure.getTreatedClassName(true) + "::*" +
                   classStructure.getTreatedClassName(true) + methpm.at(0));
              methsd +=
                  (classStructure.getTreatedClassName(true) + methpm.at(1));
            }
            /*else if(methpm.at(j)!="")
            {
                    ...
                    valsd += "\t\t"+(methpm.at(j) + " *_" +
            CastUtil::fromNumber(j-1)+" =
            ("+methpm.at(j)+"*)values.at("+CastUtil::fromNumber(j-2)+");");
          ...
          for (unsigned int j = 0; j < argpm.size(); j++) {
            if (tmpltarg && argpm.at(j).find(">") != std::string::npos) {
              std::string ttt =
                  argpmtemp.at(argpmtemp.size() - 1) + "," + argpm.at(j);
              argpmtemp.at(argpmtemp.size() - 1) = ttt;
            } else if (argpm.at(j).find("<") != std::string::npos) {
              argpmtemp.push_back(argpm.at(j));
              tmpltarg = StringUtil::countOccurrences(argpm.at(j), "<") !=
                         StringUtil::countOccurrences(argpm.at(j), ">");
            } else {
              argpmtemp.push_back(argpm.at(j));
            }
          }
          ...
          for (unsigned int j = 0; j < argpm.size(); j++) {
            ...
            if (meth.find(" operator") == std::string::npos) {
              ...
              while (RegexUtil::find(mdecl, "^[ \t]*#[ \t]*pragma[ \t]*[^`]+`",
                                     true) == 0) {
                int spos, epos;
                RegexUtil::find(mdecl, "^[ \t]*#[ \t]*pragma[ \t]*[^`]+`", spos,
                                epos, true);
              ...
              std::string fqcn =
                  getFullyQualifiedClassName(mdecl, classStructure.namespaces);
            ...
            if (type12 == "*") {
              ...
              std::string fqcn =
                  getFullyQualifiedClassName(mdecl, classStructure.namespaces);
              valsd += "\t\t" +
                       (fqcn + " *_" + CastUtil::fromNumber(j) + " = (" + fqcn +
                        "*)values.at(" + CastUtil::fromNumber(j) + ");\n");
            } else if (type12 == "&") {
              StringUtil::replaceLast(mdecl, "&", "");
              std::string fqcn =
                  getFullyQualifiedClassName(mdecl, classStructure.namespaces);
            ...
            if (type12 == "*")
              ...
              valsa += "*_" + CastUtil::fromNumber(j);
            std::string fqcn = getTreatedFullyQualifiedClassName(
                mdecl, classStructure.namespaces);
            valsades += "\t\tif(cleanvals)delete (" + fqcn + "*)values.at(" +
                        CastUtil::fromNumber(j) + ");\n";
            // if(methpm.at(0)!=classStructure.getFullyQualifiedClassName())
            //{
            typedefs += mdecl;
            typdefName += mdecl;
            // std::string fqcn = getTreatedFullyQualifiedClassName(mdecl,
            // classStructure.namespaces);
          ...
          if (meth.find("operator=") != std::string::npos ||
              meth.find("operator<") != std::string::npos ||
              meth.find("operator>") != std::string::npos ||
              meth.find("operator<=") != std::string::npos ||
              meth.find("operator>=") != std::string::npos ||
              meth.find("operator==") != std::string::npos ||
            ...
            if (methpm.at(0).find(" ") != std::string::npos) {
              methpm.at(0) =
                  methpm.at(0).substr(0, methpm.at(0).find_last_of(" "));
            }
            ...
            if (meth.find("<<") != std::string::npos) {
              ...
              std::string omn = app + "_" +
                                classStructure.getTreatedClassName(true) +
                                "_o" + CastUtil::fromNumber(opcounter++);
              opers +=
                  "\nvoid* " + omn +
                  "(void* instance,vals values,bool cleanvals)\n{\n\t" +
                  classStructure.getFullyQualifiedClassName() + " *_obj = (" +
                  classStructure.getFullyQualifiedClassName() + "*)instance;\n";
              if (methpm.at(0) == "void") {
                opers += valsd;
                ...
                opers += "\n\t" + methpm.at(0) + " *_retVal=new " +
                         methpm.at(0) + ";\n";
                opers += valsd;
                opers += "\n\ttry{*_retVal = (*_obj<=" + valsa +
                         ");} catch(...) {delete (" + methpm.at(0) +
                         "*)_retVal;" + valsades + "throw;}\n";
                opers += valsades;
                opers += "\n\treturn _retVal;";
              }
              ...
              if (methpm.at(0) == "void") {
                ...
                opers += "\n\t" + methpm.at(0) + " *_retVal=new " +
                         methpm.at(0) + ";\n";
                opers += valsd;
                opers += "\n\ttry{*_retVal = (!*_obj);} catch(...) {delete (" +
                         methpm.at(0) + "*)_retVal;" + valsades + "throw;}\n";
                opers += valsades;
                opers += "\n\treturn _retVal;";
              }
              refDef += ("\nci.addOperatorRefName(\"!\", \"" + omn + "\");");
            ...
            if (methpm.at(0) != classStructure.getTreatedClassName(false)) {
              ...
              std::string mmn = app + "_" +
                                classStructure.getTreatedClassName(true) +
                                "_m" + CastUtil::fromNumber(methcounter++);
              methods += "\nvoid* " + mmn +
                         "(void* instance,vals values,bool cleanvals)\n{\n\t";
              if (!methstat) {
                methods += classStructure.getFullyQualifiedClassName() +
                           " *_obj = (" +
                           classStructure.getFullyQualifiedClassName() +
                           "*)instance;\n";
              }
              if (methpm.at(0) == "void") {
                ...
                if (methstat) {
                  methods += "\n\ttry{*_retVal = (" +
                             classStructure.getFullyQualifiedClassName() +
                             "::" + methpm.at(1) + "(" + valsa +
                             "));} catch(...) {delete (" + fqcn + "*)_retVal;" +
                             valsades + "throw;}\n";
                } else {
                  methods += "\n\ttry{*_retVal = (_obj->" + methpm.at(1) + "(" +
                             valsa + "));} catch(...) {delete (" + fqcn +
                             "*)_retVal;" + valsades + "throw;}\n";
                }
                methods += valsades;
                methods += "\n\treturn _retVal;";
              ...
              methallpvstats[classStructure.getFullyQualifiedClassName()] |=
                  ispure;
              refDef += ("me.setArgumentTypes(argu);\n");
              refDef += ("me.setFlags(" + CastUtil::fromBool(methstat) + "," +
                         CastUtil::fromBool(isvirtual) + "," +
                         CastUtil::fromBool(ispure) + ");\n");
              refDef += ("argu.clear();\n");
              refDef +=
                  ("if(me.getMethodName()!=\"\")\n{\nci.addMethod(me);\n}\n");
              ...
              testStr += classStructure.getFullyQualifiedClassName() +
                         " _tstinstance;\n";
              testStr += classStructure.getFullyQualifiedClassName() +
                         " _rtstinstance = Reflector::newInstance();\n";
            } else {
              bool skip = false;
              for (std::map<int, std::string>::iterator it =
                       ms.argstypes.begin();
                   it != ms.argstypes.end(); ++it) {
                if (it->second.find(classStructure.getTreatedClassName(true) +
                                    "&") == 0) {
                  refDef += ("argu.clear();\n");
                  skip = true;
                  break;
      ...
      for (unsigned int i = 0; i < classStructure.pubps.size(); i++) {
        ...
        if (fld.find(",") != std::string::npos) {
          ...
          fldp =
              StringUtil::splitAndReturn<std::vector<std::string>>(fldss, ",");
          fldp.insert(fldp.begin(), fldtype);
          // fldp.insert(fldp.begin(), fldsbc.substr(fldsbc.find_last_of("
          // ")+1));
        } else {
        ...
        if (fldp.size() > 1) {
          std::string fqcn =
              getFullyQualifiedClassName(fldp.at(0), classStructure.namespaces);
          if (ptr) {
            fqcn += "*";
          }
          ...
          for (unsigned int j = 1; j < fldp.size(); j++) {
            ...
            refDef += ("f.updateStaticConst(" + CastUtil::fromBool(fldstatic) +
                       ", " + CastUtil::fromBool(fldconst) + ");\n");
            ...
            std::string fmn = app + "_" +
                              classStructure.getTreatedClassName(true) + "_f" +
    ...
    if (classStructure.prips.size() > 0 || classStructure.prims.size() > 0) {
      for (unsigned int i = 0; i < classStructure.prims.size(); i++) {
        ...
        int numargs = argpm.size();
        if (pubdecl.find("~") == std::string::npos &&
            methpm.at(0) == classStructure.getTreatedClassName(false)) {
          if (numargs == 0) {
            ctorisp = true;
          }
        } else {
    ...
    if (!ctorisp) {
      std::string mmn = app + "_" + classStructure.getTreatedClassName(true) +
                        "_m" + CastUtil::fromNumber(methcounter++);
    ...
    /*refDef += ("\nci.addContRefName(0, \""+app+
    "_"+classStructure.getTreatedClassName(true)+"c1\");"); refDef +=
    ("\nci.addContRefName(2, \""+app+
    "_"+classStructure.getTreatedClassName(true)+"c2\");"); refDef +=
    ("\nci.addContRefName(4, \""+app+
    "_"+classStructure.getTreatedClassName(true)+"c3\");"); refDef +=
    ("\nci.addContRefName(6, \""+app+
    "_"+classStructure.getTreatedClassName(true)+"c4\");"); refDef +=
    ("\nci.addContRefName(8, \""+app+
    "_"+classStructure.getTreatedClassName(true)+"c5\");");*/

    refDef += ("\nci.setContRefName(\"" + app + "_" +
               classStructure.getTreatedClassName(true) + "co\");");

    methods +=
        "\nvoid* " + app + "_" + classStructure.getTreatedClassName(true) +
        "co(void* _vec,void* _instance,int pos,std::string contType,int t){";
    if (!ispurecs) {
      methods +=
          "\nif(t==-1)return Reflector::destroyNestedContainer<" +
          classStructure.getFullyQualifiedClassName() + ">(contType, _vec);\n" +
          "\nelse if(t==1)return Reflector::getNewNestedContainer<" +
          classStructure.getFullyQualifiedClassName() + ">(contType);\n" +
          "\nelse if(t==2){int* _obj = new int;\n*_obj = "
          "Reflector::getNestedContainerSize<" +
          classStructure.getFullyQualifiedClassName() +
          ">(contType,_vec);return _obj;}\n" +
          "\nelse if(t==3){Reflector::addValueToNestedContainer<" +
          classStructure.getFullyQualifiedClassName() + ">(contType, *((" +
          classStructure.getFullyQualifiedClassName() +
          "*)_instance),_vec);return NULL;}\n" + "\nelse if(t==4){" +
          classStructure.getFullyQualifiedClassName() + " *_obj = new " +
          classStructure.getFullyQualifiedClassName() +
          ";\n*_obj = Reflector::getValueFromNestedContainer<" +
          classStructure.getFullyQualifiedClassName() +
          ">(contType, _vec, pos);\nreturn _obj;\n}\n" +
          "\nelse if(t==5)return Reflector::getPValueFromNestedContainer<" +
          classStructure.getFullyQualifiedClassName() +
          ">(contType, _vec, pos);\n";
    }
    methods += "return NULL;\n}";
    ...
    /*methods += "\nvoid* " +app+
    "_"+classStructure.getTreatedClassName(true)+"c1(std::string
    contType){\nreturn "
                    +
    "Reflector::getNewNestedContainer<"+classStructure.getFullyQualifiedClassName()+">(contType);\n}";
    methods += "\nint " +app+
    "_"+classStructure.getTreatedClassName(true)+"c2(void* _vec,std::string
    contType){\n"
                    + "return
    Reflector::getNestedContainerSize<"+classStructure.getFullyQualifiedClassName()+">(contType,
    _vec);\n}"; methods += "\nvoid " +app+
    "_"+classStructure.getTreatedClassName(true)+"c3(void* _vec,void*
    _instance,std::string contType){\n"
                    +classStructure.getFullyQualifiedClassName()+" *_obj =
    ("+classStructure.getFullyQualifiedClassName()+"*)_instance;\n"
                    +
    "Reflector::addValueToNestedContainer<"+classStructure.getFullyQualifiedClassName()+">(contType,
    *_obj, _vec);\n}"; methods += "\nvoid* " +app+
    "_"+classStructure.getTreatedClassName(true)+"c4(void* _vec,int
    pos,std::string contType){\n" +classStructure.getFullyQualifiedClassName()+"
    *_obj = new "+classStructure.getFullyQualifiedClassName()+";\n"
                    + "*_obj =
    Reflector::getValueFromNestedContainer<"+classStructure.getFullyQualifiedClassName()+">(contType,
    _vec, pos);\nreturn _obj;\n}\n"; methods += "\nvoid* " +app+
    "_"+classStructure.getTreatedClassName(true)+"c5(void* _vec,int
    pos,std::string contType){"
            + "return
    Reflector::getPValueFromNestedContainer<"+classStructure.getFullyQualifiedClassName()+">(contType,
    _vec, pos);\n}\n";
    */

    if (classStructure.prosetser) {
      /*refDef += ("\nci.addContRefName(1, \""+app+
      "_"+classStructure.getTreatedClassName(true)+"c6\");"); refDef +=
      ("\nci.addContRefName(3, \""+app+
      "_"+classStructure.getTreatedClassName(true)+"c7\");"); refDef +=
      ("\nci.addContRefName(5, \""+app+
      "_"+classStructure.getTreatedClassName(true)+"c8\");"); refDef +=
      ("\nci.addContRefName(7, \""+app+
      "_"+classStructure.getTreatedClassName(true)+"c9\");"); refDef +=
      ("\nci.addContRefName(9, \""+app+
      "_"+classStructure.getTreatedClassName(true)+"c10\");");

      methods += "\nvoid* " +app+
      "_"+classStructure.getTreatedClassName(true)+"c6(std::string
      contType){\nreturn "
                      +
      "Reflector::getNewNestedContainerSV<"+classStructure.getFullyQualifiedClassName()+">(contType);\n}";
      methods += "\nvoid " +app+
      "_"+classStructure.getTreatedClassName(true)+"c7(void* _vec,void*
      _instance,std::string contType){\n"
                      +classStructure.getFullyQualifiedClassName()+" *_obj =
      ("+classStructure.getFullyQualifiedClassName()+"*)_instance;\n"
                      +
      "Reflector::addValueToNestedContainerSV<"+classStructure.getFullyQualifiedClassName()+">(contType,
      *_obj, _vec);\n}"; methods += "\nint " +app+
      "_"+classStructure.getTreatedClassName(true)+"c8(void* _vec,std::string
      contType){\n"
                      + "return
      Reflector::getNestedContainerSizeSV<"+classStructure.getFullyQualifiedClassName()+">(contType,
      _vec);\n}"; methods += "\nvoid* " +app+
      "_"+classStructure.getTreatedClassName(true)+"c9(void* _vec,int
      pos,std::string contType){\n"
                      +classStructure.getFullyQualifiedClassName()+" *_obj = new
      "+classStructure.getFullyQualifiedClassName()+";\n"
                      + "*_obj =
      Reflector::getValueFromNestedContainerSV<"+classStructure.getFullyQualifiedClassName()+">(contType,
      _vec, pos);\nreturn _obj;\n}\n"; methods += "\nvoid* " +app+
      "_"+classStructure.getTreatedClassName(true)+"c10(void* _vec,int
      pos,std::string contType){\n"
                      + "return
      Reflector::getPValueFromNestedContainerSV<"+classStructure.getFullyQualifiedClassName()+">(contType,
      _vec, pos);\n}\n";
      */
      methods += "\nvoid* " + app + "_" +
                 classStructure.getTreatedClassName(true) +
                 "cosv(void* _vec,void* _instance,int pos,std::string "
                 "contType,int t){";
      if (!ispurecs) {
        methods +=
            "\nif(t==0)return Reflector::destroyNestedContainerSV<" +
            classStructure.getFullyQualifiedClassName() +
            ">(contType, _vec);\n" +
            "\nelse if(t==6)return Reflector::getNewNestedContainerSV<" +
            classStructure.getFullyQualifiedClassName() + ">(contType);\n" +
            "\nif(t==7){int* _obj = new int;\n*_obj = "
            "Reflector::getNestedContainerSizeSV<" +
            classStructure.getFullyQualifiedClassName() +
            ">(contType,_vec);return _obj;}\n" +
            "\nif(t==8){Reflector::addValueToNestedContainerSV<" +
            classStructure.getFullyQualifiedClassName() + ">(contType, *((" +
            classStructure.getFullyQualifiedClassName() +
            "*)_instance),_vec);return NULL;}\n" + "\nif(t==9){" +
            classStructure.getFullyQualifiedClassName() + " *_obj = new " +
            classStructure.getFullyQualifiedClassName() +
            ";\n*_obj = Reflector::getValueFromNestedContainerSV<" +
            classStructure.getFullyQualifiedClassName() +
            ">(contType, _vec, pos);\nreturn _obj;\n}\n" +
            "\nif(t==10)return Reflector::getPValueFromNestedContainerSV<" +
            classStructure.getFullyQualifiedClassName() +
            ">(contType, _vec, pos);\n";
      }
      methods += "return NULL;}";
...
std::string Reflection::generateClassDefinition_Old(
    std::map<std::string, ClassStructure, std::less<>> &allclsmap,
    std::string &includesDefs, std::string &typedefs, std::string &classes,
    std::string &methods, std::string &opers, const std::string &app) {
  return "";
...
std::string Reflection::generateSerDefinitionAll(
    std::map<std::string, std::map<std::string, ClassStructure, std::less<>>,
             std::less<>> &clsstrucMaps,
    std::string &includeRef, const bool &isBinary, std::string &objs,
    std::string &ajaxret, std::string &headers, std::string &typerefs,
  ...
  for (unsigned int var = 0; var < apps.size(); ++var) {
    rert1 += this->generateSerDefinitions(
        clsstrucMaps[StringUtil::trimCopy(apps.at(var))], includeRef, typedefs,
        classes, methods, isBinary, objs, ajaxret, headers, typerefs,
        StringUtil::trimCopy(apps.at(var)));
  }
  headers = includeRef;
...
std::string Reflection::generateSerDefinitions(
    std::map<std::string, ClassStructure, std::less<>> &allclsmap,
    std::string &includesDefs, std::string &typedefs, std::string &classes,
    std::string &methods, const bool &isBinary, std::string &objs,
    std::string &ajaxret, std::string &headers, std::string &typerefs,
  ...
  getClassStructures(includes.at(var)); allclsmap.insert(clsmap.begin(),
  clsmap.end());
  }*/
  ret = generateAllSerDefinition(allclsmap, includesDefs, typedefs, classes,
                                 methods, app);
  ...
  for (it = allclsmap.begin(); it != allclsmap.end(); ++it) {
    ...
    std::vector<std::string> minfo =
        getAfcObjectData(it->second, false, pinfo, isOpForSet);
    pinfo.clear();
    std::vector<std::string> info =
        getAfcObjectData(it->second, true, pinfo, isOpForSet);
  }

  /*for (unsigned int var = 0; var < includes.size(); ++var)
  {
          ////logger << "\ngenerating Ser for file" << includes.at(var) << "\n"
  << std::flush; if(invalidcls.find(includes.at(var))==invalidcls.end())
          {

                  std::vector<std::string> pinfo;
                  bool isOpForSet = false;
                  std::vector<std::string> minfo =
  getAfcObjectData(includes.at(var),false,pinfo,isOpForSet); pinfo.clear();
                  std::vector<std::string> info =
  getAfcObjectData(includes.at(var),true,pinfo,isOpForSet);
          }
...
std::string Reflection::generateAllSerDefinition(
    std::map<std::string, ClassStructure, std::less<>> &allclsmap,
    std::string &includesDefs, std::string &typedefs, std::string &classes,
    std::string &methods, const std::string &app) {
  ...
  for (it = allclsmap.begin(); it != allclsmap.end(); ++it) {
    ClassStructure classStructure = it->second;
    if (classStructure.markers.find("@IgnoreSer") !=
        classStructure.markers.end()) {
      continue;
    }
    if (methallpvstats.find(classStructure.getTreatedClassName(true)) !=
    ...
    includesDefs += "#include \"" +
                    getClassPath(it->second.getTreatedClassName(true)) + "\"\n";
    classStructure.prosetser = false;
    classes += "\nstd::string " + ttapp + "serialize" +
               classStructure.getTreatedClassName(true) +
               "(void* obje, SerializeBase* base, void* serobject);\nvoid* " +
               ttapp + "unSerialize" +
               classStructure.getTreatedClassName(true) +
               "(void* intermediateObject, SerializeBase* base);";
    methods += "\nstd::string " + ttapp + "serialize" +
               classStructure.getTreatedClassName(true) +
               "(void* obje, SerializeBase* base, void* serobject)\n{\n" +
               classStructure.getFullyQualifiedClassName() + " *__obj=(" +
               classStructure.getFullyQualifiedClassName() + "*)obje" +
               ";\nbool serref = serobject!=NULL;serobject = "
               "base->getSerializableObject(serobject);\nbase->"
               "startObjectSerialization(serobject, \"" +
               classStructure.getTreatedClassName(true) + "\");\n";
    typedefs += "\nvoid* " + ttapp + "unSerialize" +
                classStructure.getTreatedClassName(true) +
                "(void* intermediateObject, SerializeBase* base)\n{\n";
    typedefs += "if(!base->isValidClassNamespace(intermediateObject, \"" +
                classStructure.getTreatedClassName(false) + "\", \"" +
                classStructure.nmSpc + "\"))\n\treturn NULL;\n" +
                classStructure.getFullyQualifiedClassName() + " *__obj=new " +
                classStructure.getFullyQualifiedClassName() + ";\n";
    // typedefs += "int totsize =
    // base->getContainerSize(intermediateObject);\nfor(int
    // i=0;i<totsize;i++)\n{\n";
    typedefs += "int i = 0;\n{\n";
    ...
    if (classStructure.pri.size() > 0) {
      for (unsigned int i = 0; i < classStructure.pri.size(); i++) {
        if (((tes = classStructure.pri.at(i).find("(")) == std::string::npos &&
             (tes = classStructure.pri.at(i).find(")")) == std::string::npos &&
             classStructure.pri.at(i).find("~") == std::string::npos)) {
          ...
          if (fld.find(",") != std::string::npos) {
            ...
            fldp = StringUtil::splitAndReturn<std::vector<std::string>>(fldss,
                                                                        ",");
            fldp.insert(fldp.begin(),
                        fldsbc.substr(0, fldsbc.find_last_of(" ")));
            fldp.insert(fldp.begin(),
                        fldsbc.substr(fldsbc.find_last_of(" ") + 1));
          } else {
            fldp.push_back(fld.substr(0, fld.find_last_of(" ")));
            fldp.push_back(fld.substr(fld.find_last_of(" ") + 1));
          }
    ...
    if (classStructure.pro.size() > 0) {
      for (unsigned int i = 0; i < classStructure.pro.size(); i++) {
        if (((tes = classStructure.pro.at(i).find("(")) == std::string::npos &&
             (tes = classStructure.pro.at(i).find(")")) == std::string::npos &&
             classStructure.pro.at(i).find("~") == std::string::npos)) {
    ...
    if (classStructure.pub.size() > 0) {
      for (unsigned int i = 0; i < classStructure.pub.size(); i++) {
        if ((tes = classStructure.pub.at(i).find("(")) != std::string::npos &&
            (tes = classStructure.pub.at(i).find(")")) != std::string::npos &&
            classStructure.pub.at(i).find("~") == std::string::npos &&
            fldnames.size() > 0) {
          ...
          if (meth.find("operator=") != std::string::npos ||
            ...
            if (methpm.at(0) != classStructure.getTreatedClassName(false)) {
              for (unsigned int k = 0; k < fldnames.size(); k = k + 2) {
                std::string cam =
                    StringUtil::capitalizedCopy(fldnames.at(k + 1));
                std::string arggk;
                if (argpm.size() == 1) {
                  arggk = argpm.at(0);
                ...
                if ("set" + cam == methpm.at(1) && argpm.size() == 1 &&
                    arggk == fldnames.at(k) && methpm.at(0) == "void") {
                  fldstat[fldnames.at(k + 1)]++;
                } else if ("get" + cam == methpm.at(1) && argpm.size() == 0 &&
                           methhk == fldnames.at(k)) {
                  fldstat[fldnames.at(k + 1)]++;
                }
              }
            }
        ...
        if (((tes = classStructure.pub.at(i).find("(")) == std::string::npos &&
          ...
          for (unsigned int j = 1; j < fldp.size(); j++) {
            ...
            if (isPrimitiveDataType(fldp.at(0))) {
              ...
              std::string serOpt =
                  CastUtil::fromNumber(SerializeBase::identifySerOption(typ));
              if (!ptr) {
                methods += typ + " _objProp" + fldp.at(j) + " = " + "__obj->" +
                           fldp.at(j) + ";\n";
                methods += "base->addObjectPrimitiveProperty(serobject, " +
                           serOpt + ", \"" + fldp.at(j) + "\", \"" + typ +
                           "\", &_objProp" + fldp.at(j) + ");\n" +
                           "base->afterAddObjectProperty(serobject, \"" +
                           fldp.at(j) + "\");\n";
                std::string cam = StringUtil::capitalizedCopy(fldp.at(j));
                typedefs +=
                    "if(base->isValidObjectProperty(intermediateObject, \"" +
                    fldp.at(j) + "\", i))\n{\n" + typ + "* _val = (" + typ +
                    "*)base->getObjectPrimitiveValue(base->getObjectProperty("
                    "intermediateObject, i, \"" +
                    fldp.at(j) + "\"), " + serOpt + ", \"" + typ + "\", \"" +
                    fldp.at(j) + "\");__obj->" + fldp.at(j) +
                    " = *_val;\ndelete _val;\n}\n";
              } else {
                methods += "base->addObjectPrimitiveProperty(serobject, " +
                           serOpt + ", \"" + fldp.at(j) + "\", \"" + typ +
                ...
                typedefs +=
                    ...
                    "intermediateObject, i, \"" +
                    fldp.at(j) + "\"), " + serOpt + ", \"" + typ + "\", \"" +
                    fldp.at(j) + "\");__obj->" + fldp.at(j) + " = _val;\n}\n";
              }
            } else if (fldp.at(0) == "Date") {
              std::string serOpt = CastUtil::fromNumber(
                  SerializeBase::identifySerOption(fldp.at(0)));
              if (!ptr) {
                methods += fldp.at(0) + " _objProp" + fldp.at(j) + " = " +
                           "__obj->" + fldp.at(j) + ";\n";
                methods += ("base->addObjectPrimitiveProperty(serobject, " +
                            serOpt + ", \"" + fldp.at(j) + "\", \"" +
                ...
                typedefs +=
                    ...
                    fldp.at(j) + "\", i))\n{\n" +
                    "Date* _val = "
                    "(Date*)base->getObjectPrimitiveValue(base->"
                    "getObjectProperty(intermediateObject, i, \"" +
                    fldp.at(j) + "\"), " + serOpt + ", \"Date\", \"" +
                    fldp.at(j) + "\");" + "\n__obj->" + fldp.at(j) +
                    " = *_val;\ndelete _val;\n}\n";
              } else {
                methods += ("base->addObjectPrimitiveProperty(serobject, " +
              ...
              if (!ptr) {
                ...
                typedefs +=
                    ...
                    fldp.at(j) + "\", i))\n{\n" +
                    "std::string* _val = "
                    "(std::string*)base->getObjectPrimitiveValue(base->"
                    "getObjectProperty(intermediateObject, i, \"" +
                    fldp.at(j) + "\"), " + serOpt + ", \"std::string\", \"" +
                    fldp.at(j) + "\");" + "\n__obj->" + fldp.at(j) +
                    " = *(BinaryData::unSerilaize(*_val));\ndelete _val;\n}\n";
              } else {
                methods += ("base->addObjectPrimitiveProperty(serobject, " +
                            serOpt + ", \"" + fldp.at(j) + "\", \"" +
                            fldp.at(0) + "\", __obj->" + fldp.at(j) + ");\n" +
            ...
            } else if (fldp.at(0).find("vector") != std::string::npos ||
                       fldp.at(0).find("queue") != std::string::npos ||
                       fldp.at(0).find("deque") != std::string::npos ||
                       fldp.at(0).find("set") != std::string::npos ||
                       fldp.at(0).find("list") != std::string::npos) {
              ...
              std::string fqcn =
                  contType +
                  getFullyQualifiedClassName(stlcnt, classStructure.namespaces);
              if (fqcn.find(">") == std::string::npos) {
                fqcn += ">";
              } else {
                fqcn += " >";
              }
              contType += getFullyQualifiedClassName(
                              stlcnt, classStructure.namespaces) +
                          ",";
              std::string serOpt =
                  CastUtil::fromNumber(SerializeBase::identifySerOption(fqcn));

              if (!ptr) {
                // methods += (fqcn+" __temp_obj_ser"+fldp.at(j)+" =
                // __obj->"+fldp.at(j)+";\n");
                methods += ("base->addObjectProperty(serobject, \"" +
                            fldp.at(j) + "\", \"" + fldp.at(0) + "\");\n" +
                            "base->serializeUnknownBaseInt(&(__obj->" +
                            fldp.at(j) + ")," + serOpt + ",\"" + fqcn +
                            "\",\"" + app + "\",serobject);\n" +
                            "base->afterAddObjectProperty(serobject, \"" +
                            fldp.at(j) + "\");\n");
                std::string cam = StringUtil::capitalizedCopy(fldp.at(j));
                typedefs +=
                    "if(base->isValidObjectProperty(intermediateObject, \"" +
                    fldp.at(j) + "\", i))\n__obj->" + fldp.at(j) + " = " +
                    "SerializeBase::unSerializeKnown<" + fqcn +
                    " >(base->getObjectProperty(" +
                    "intermediateObject, i, \"" + fldp.at(j) + "\")," + serOpt +
                    ",\"" + contType + "\",\"" + app + "\", base);\n";
              } else {
                // methods += (fqcn+"* __temp_obj_ser"+fldp.at(j)+" =
                // __obj->"+fldp.at(j)+";\n");
                methods += ("if(__obj->" + fldp.at(j) +
                            "!=NULL)base->addObjectProperty(serobject, \"" +
                            fldp.at(j) + "\", \"" + fldp.at(0) + "\");\n" +
                            "base->serializeUnknownBaseInt(__obj->" +
                            fldp.at(j) + "," + serOpt + ",\"" + fqcn + "\",\"" +
                            app + "\",serobject);\n" +
                ...
                typedefs +=
                    "if(base->isValidObjectProperty(intermediateObject, \"" +
                    fldp.at(j) + "\", i))\n__obj->" + fldp.at(j) + " = " +
                    "SerializeBase::unSerializeKnownToPointer<" + fqcn +
                    " >(base->getObjectProperty(" +
                    "intermediateObject, i, \"" + fldp.at(j) + "\")," + serOpt +
                    ",\"" + contType + "\",\"" + app + "\", base);\n";
              }
              ...
              std::string fqcn = getFullyQualifiedClassName(
                  fldp.at(0), classStructure.namespaces);
              std::string serOpt =
                  CastUtil::fromNumber(SerializeBase::identifySerOption(fqcn));
              if (!ptr) {
                // methods += (fqcn+" __temp_obj_ser"+fldp.at(j)+" =
                ...
                // methods += (fldp.at(0)+"* __temp_obj_ser"+fldp.at(j)+" =
                // __obj->"+fldp.at(j)+";\n");
                methods += ("if(__obj->" + fldp.at(j) +
                            "!=NULL)base->addObjectProperty(serobject, \"" +
                            fldp.at(j) + "\", \"" + fldp.at(0) + "\");\n" +
          ...
          if (meth.find("operator=") != std::string::npos ||
            ...
            if (methpm.at(0) != classStructure.getTreatedClassName(false)) {
              for (unsigned int k = 0; k < fldnames.size(); k = k + 2) {
                ...
                if ("set" + cam == methpm.at(1) && argpm.size() == 1 &&
                    argpm.at(0) == fldnames.at(k) && methpm.at(0) == "void") {
                  if (isPrimitiveDataType(argpm.at(0))) {
                    argpm.at(0) = getTypeName(argpm.at(0));
                    std::string serOpt = CastUtil::fromNumber(
                        SerializeBase::identifySerOption(argpm.at(0)));
                    if (!ptr)
                      typedefs +=
                          "if(base->isValidObjectProperty(intermediateObject, "
                          "\"" +
                          fldnames.at(k + 1) + "\", i))\n{\n" + argpm.at(0) +
                          "* _val = (" + argpm.at(0) +
                          "*)base->getObjectPrimitiveValue(base->"
                          "getObjectProperty(intermediateObject, i, \"" +
                          fldnames.at(k + 1) + "\"), " + serOpt + ", \"" +
                    ...
                    if (!ptr)
                      ...
                      typedefs +=
                          ...
                          "((BinaryData::unSerilaize(*_val)));\ndelete "
                          "_val;\n}\n";
                  } else if (argpm.at(0).find("vector") != std::string::npos ||
                             argpm.at(0).find("queue") != std::string::npos ||
                             argpm.at(0).find("deque") != std::string::npos ||
                             argpm.at(0).find("set") != std::string::npos ||
                             argpm.at(0).find("list") != std::string::npos ||
                             argpm.at(0).find("multiset") !=
                                 std::string::npos) {
                    ...
                    if (argpm.at(0).find("vector") != std::string::npos) {
                      ...
                      contType = "std::list<";
                      stlcnttyp = "Lis";
                    } else if (argpm.at(0).find("multiset") !=
                               std::string::npos) {
                      contType = "std::multiset<";
                      stlcnttyp = "MulSet";
                    } else {
                    ...
                    contType += getFullyQualifiedClassName(
                                ...
                                ",";
                    std::string serOpt = CastUtil::fromNumber(
                        SerializeBase::identifySerOption(fqcn));

                    if (!ptr) {
                      typedefs += "if(base->isValidObjectProperty("
                                  "intermediateObject, \"" +
                                  fldnames.at(k + 1) + "\", i)){";
                      typedefs +=
                          "\n__obj->set" + cam +
                          "(SerializeBase::unSerializeKnown<" + fqcn + " >" +
                          "(base->getObjectProperty(intermediateObject, i, \"" +
                          fldnames.at(k + 1) + "\")," + serOpt + ",\"" +
                          contType + "\",\"" + app + "\", base));\n";
                      typedefs += "\n}\n";
                    } else {
                    ...
                    std::string fqcn = getFullyQualifiedClassName(
                        argpm.at(0), classStructure.namespaces);
                    std::string serOpt = CastUtil::fromNumber(
                        SerializeBase::identifySerOption(fqcn));
                    if (!ptr)
                      typedefs +=
                          ...
                          "\"" +
                          fldnames.at(k + 1) + "\", i))" + "\n__obj->" +
                          methpm.at(1) + "(SerializeBase::unSerializeKnown<" +
                          fqcn + " >" +
                          "(base->getObjectProperty(intermediateObject, i, \"" +
                          fldnames.at(k + 1) + "\")," + serOpt + ",\"" + fqcn +
                          "\",\"" + app + "\", base));\n";
                    else
                      typedefs +=
                          "if(base->isValidObjectProperty(intermediateObject, "
                          ...
                          "\",\"" + app + "\", base));\n";
                           ...
                           methpm.at(0) == fldnames.at(k)) {
                  if (isPrimitiveDataType(methpm.at(0))) {
                    methpm.at(0) = getTypeName(methpm.at(0));
                    std::string serOpt = CastUtil::fromNumber(
                        SerializeBase::identifySerOption(methpm.at(0)));
                    if (!ptr) {
                      methods += methpm.at(0) + " _objProp" +
                                 fldnames.at(k + 1) + " = " + "__obj->" +
                                 methpm.at(1) + "();\n";
                      methods +=
                          ("base->addObjectPrimitiveProperty(serobject, " +
                           serOpt + ", \"" + fldnames.at(k + 1) + "\", \"" +
                           methpm.at(0) + "\", &_objProp" + fldnames.at(k + 1) +
                           ");\n" +
                           "base->afterAddObjectProperty(serobject, \"" +
                           fldnames.at(k + 1) + "\");\n");
                    } else {
                      methods += ("if(__obj->" + methpm.at(1) +
                                  "()!=NULL)base->addObjectPrimitiveProperty("
                                  "serobject, " +
                                  serOpt + ", \"" + fldnames.at(k + 1) +
                                  "\", \"" + methpm.at(0) + "\", " + "__obj->" +
                                  methpm.at(1) + "());\n" +
                    ...
                    if (!ptr) {
                      ...
                      methods +=
                          ("if(__obj->" + methpm.at(1) + "()!=NULL){\n" +
                           "base->addObjectPrimitiveProperty(serobject, " +
                           serOpt + ", \"" + fldnames.at(k + 1) + "\", \"" +
                           methpm.at(0) + "\", " + "__obj->" + methpm.at(1) +
                           "());\n" +
                           "base->afterAddObjectProperty(serobject, \"" +
                           fldnames.at(k + 1) + "\");\n");
                    }
                  ...
                  } else if (methpm.at(0).find("vector") != std::string::npos ||
                             methpm.at(0).find("queue") != std::string::npos ||
                             methpm.at(0).find("deque") != std::string::npos ||
                             methpm.at(0).find("set") != std::string::npos ||
                             methpm.at(0).find("list") != std::string::npos ||
                             methpm.at(0).find("multiset") !=
                                 std::string::npos) {
                    std::string contType;
                    ...
                    if (methpm.at(0).find("vector") != std::string::npos) {
                      contType = "std::vector<";
                      stlcnttyp = "Vec";
                    } else if (methpm.at(0).find("queue") !=
                               std::string::npos) {
                      contType = "std::queue<";
                      stlcnttyp = "Q";
                    } else if (methpm.at(0).find("deque") !=
                               std::string::npos) {
                      contType = "std::deque<";
                      stlcnttyp = "Dq";
                      ...
                      contType = "std::list<";
                      stlcnttyp = "Lis";
                    } else if (methpm.at(0).find("multiset") !=
                               std::string::npos) {
                      contType = "std::multiset<";
                      stlcnttyp = "MulSet";
                    } else {
                    ...
                    if (!ptr) {
                      methods +=
                          (fqcn + " __temp_obj_ser" + fldnames.at(k + 1) +
                           " = __obj->" + methpm.at(1) + "();\n");
                      methods +=
                          ("base->addObjectProperty(serobject, \"" +
                           fldnames.at(k + 1) + "\", \"" + stlcontwosp +
                           "\");\n" +
                           "base->serializeUnknownBaseInt(&__temp_obj_ser" +
                           fldnames.at(k + 1) + "," + serOpt + ",\"" + fqcn +
                           "\",\"" + app + "\",serobject);\n" +
                           "base->afterAddObjectProperty(serobject,\"" +
                           fldnames.at(k + 1) + "\");\n");
                    ...
                    std::string serOpt = CastUtil::fromNumber(
                        SerializeBase::identifySerOption(fqcn));
                    if (!ptr) {
                      methods += (methpm.at(0) + " __temp_obj_ser" +
                                  fldnames.at(k + 1) + " = __obj->" +
                                  methpm.at(1) + "();\n");
                      methods +=
                          ("base->addObjectProperty(serobject, \"" +
                           fldnames.at(k + 1) + "\", \"" + methpm.at(0) +
                           "\");\n" +
                           "base->serializeUnknownBaseInt(&__temp_obj_ser" +
                           fldnames.at(k + 1) + "," + serOpt + ",\"" + fqcn +
                           "\",\"" + app + "\",serobject);\n" +
    ...
    methods += "base->endObjectSerialization(serobject, \"" +
               classStructure.getTreatedClassName(true) + "\");\n" +
               "std::string ser;if(!serref){ser = "
               "base->fromSerializableObjectToString(serobject);\nbase->"
               "cleanSerializableObject(serobject);}\nreturn ser;\n}\n";
    /*methods += "\nstd::string " +ttapp+ "serialize" +
       classStructure.getTreatedClassName(true) + "Container(void* obje,
       SerializeBase* base, const std::string& _cont, void* object)\n{\nbool
       serref = object!=NULL;object =
       base->getSerializableObject(object);\nbase->startContainerSerialization(object,
       \""+classStructure.getTreatedClassName(true)+"\", _cont);\n"
                    +"int size =
       SerializeBase::getNestedContainerSize<"+classStructure.getFullyQualifiedClassName()+">(_cont,obje);\n"
                    +"for(int i=0;i<size;i++)\n{\n" +
       classStructure.getFullyQualifiedClassName() + " _lo =
       SerializeBase::getValueFromNestedContainer<"+classStructure.getFullyQualifiedClassName()+">(_cont,obje,i);\nbase->addContainerSerializableElement(object,
       "+ttapp+"serialize" +classStructure.getTreatedClassName(true)+"(&_lo,
       base));\nbase->afterAddContainerSerializableElement(object,i,size);\n}\n"
                    +"base->endContainerSerialization(object,
       \""+classStructure.getTreatedClassName(true)+"\",
       _cont);\nif(!serref){std::string ser =
       base->fromSerializableObjectToString(object);\nbase->cleanSerializableObject(object);\nreturn
       ser;\n} else {return CommonUtils::BLANK;}\n";
    */
    methods += "\nstd::string " + ttapp + "serialize" +
               classStructure.getTreatedClassName(true) +
               "Container(void* obje, SerializeBase* base, const std::string& "
               "_cont, void* object)\n{" +
               "return SerializeBase::serializeCont<" +
               classStructure.getFullyQualifiedClassName() + ">(_cont,obje,\"" +
               classStructure.getFullyQualifiedClassName() +
               "\",object,base,\"" + ttapp + "serialize" +
               classStructure.getTreatedClassName(true) + "\");}";
    classes += "\nstd::string " + ttapp + "serialize" +
               classStructure.getTreatedClassName(true) +
               "Container(void* obje, SerializeBase* base, const std::string& "
               "_cont, void* serobject);\n";
    if (classStructure.prosetser) {
      /*methods += "\nstd::string " +ttapp+ "serialize" +
         classStructure.getTreatedClassName(true) + "ContainerSV(void* obje,
         SerializeBase* base, const std::string& _cont, void* object)\n{\nbool
         serref = object!=NULL;object =
         base->getSerializableObject(object);\nbase->startContainerSerialization(object,
         \""+classStructure.getTreatedClassName(true)+"\", _cont);\n"
                      +"int size =
         SerializeBase::getNestedContainerSizeSV<"+classStructure.getFullyQualifiedClassName()+">(_cont,obje);\n"
                      +"for(int i=0;i<size;i++)\n{\n" +
         classStructure.getFullyQualifiedClassName() + " _lo =
         SerializeBase::getValueFromNestedContainerSV<"+classStructure.getFullyQualifiedClassName()+">(_cont,obje,i);\nbase->addContainerSerializableElement(object,
         "+ttapp+"serialize" +classStructure.getTreatedClassName(true)+"(&_lo,
         base));\nbase->afterAddContainerSerializableElement(object,i,size);\n}\n"
                      +"base->endContainerSerialization(object,
         \""+classStructure.getTreatedClassName(true)+"\",
      ...
      */
      methods += "\nstd::string " + ttapp + "serialize" +
                 classStructure.getTreatedClassName(true) +
                 "ContainerSV(void* obje, SerializeBase* base, const "
                 "std::string& _cont, void* object)\n{" +
                 "return SerializeBase::serializeContSV<" +
                 classStructure.getFullyQualifiedClassName() +
                 ">(_cont,obje,\"" +
                 classStructure.getFullyQualifiedClassName() +
                 "\",object,base,\"" + ttapp + "serialize" +
      ...
      classes += "\nstd::string " + ttapp + "serialize" +
                 classStructure.getTreatedClassName(true) +
                 "ContainerSV(void* obje, SerializeBase* base, const "
                 "std::string& _cont, void* serobject);\n";
    }
    ...
    typedefs +=
        "\nvoid* " + ttapp + "unSerialize" +
        classStructure.getTreatedClassName(true) +
        "Container(void* cintobject, SerializeBase* base, const std::string& "
        "_cont){\n" +
        "if(!base->isValidClassNamespace(cintobject, \"" +
        classStructure.getTreatedClassName(false) + "\", \"" +
        classStructure.nmSpc +
        "\", true))\n\treturn NULL;\nvoid* t = "
        "SerializeBase::getNewNestedContainer<" +
        classStructure.getFullyQualifiedClassName() + ">(_cont);\n" +
        ...
        classStructure.getFullyQualifiedClassName() + "*)" + ttapp +
        "unSerialize" + classStructure.getTreatedClassName(true) +
        "(ciintobject, base);\nSerializeBase::addValueToNestedContainer<" +
        classStructure.getFullyQualifiedClassName() +
        ">(_cont,*__val__,t);delete __val__;\n}\n}\nreturn t;\n}\n";
    classes += "\nvoid* " + ttapp + "unSerialize" +
               classStructure.getTreatedClassName(true) +
               "Container(void* cintobject, SerializeBase* base, const "
               "std::string& _cont);\n";
    if (classStructure.prosetser) {
      typedefs +=
          "\nvoid* " + ttapp + "unSerialize" +
          classStructure.getTreatedClassName(true) +
          "ContainerSV(void* cintobject, SerializeBase* base, const "
          "std::string& _cont){\n" +
          "if(!base->isValidClassNamespace(cintobject, \"" +
          classStructure.getTreatedClassName(false) + "\", \"" +
          classStructure.nmSpc +
          "\", true))\n\treturn NULL;\nvoid* t = "
          "SerializeBase::getNewNestedContainerSV<" +
          classStructure.getFullyQualifiedClassName() + ">(_cont);\n" +
          "int totsize = base->getContainerSize(cintobject);\nfor(int "
          "i=0;i<totsize;i++)\n{\n" +
          "void* ciintobject = base->getContainerElement(cintobject, i);\n" +
          ...
          classStructure.getFullyQualifiedClassName() + "*)" + ttapp +
          "unSerialize" + classStructure.getTreatedClassName(true) +
          "(ciintobject, base);\nSerializeBase::addValueToNestedContainerSV<" +
          classStructure.getFullyQualifiedClassName() +
          ">(_cont,*__val__,t);delete __val__;\n}\n}\nreturn t;\n}\n";
      classes += "\nvoid* " + ttapp + "unSerialize" +
                 classStructure.getTreatedClassName(true) +
...
std::string Reflection::generateSerDefinition(
    std::map<std::string, ClassStructure, std::less<>> &allclsmap,
    std::string &includesDefs, std::string &typedefs, std::string &classes,
    std::string &methods, const std::string &app) {
  return "";
...
std::string Reflection::generateSerDefinitionBinary(
    std::map<std::string, ClassStructure, std::less<>> &allclsmap,
    std::string &includesDefs, std::string &typedefs, std::string &classes,
    std::string &methods, const std::string &app) {
  return "";
...
std::string Reflection::getXSDDefinitions(
    std::map<std::string, ClassStructure, std::less<>> &allclsmap,
    const std::string &fqcn, Reflection &ref, const std::string &appname,
    std::string &trgnmspc, std::set<std::string> &allnmspcs,
    const std::string &dfnmspc, const std::string &resp) {
  ...
  if (clstruct == NULL) {
    // logger << ("Error generating web-service xsd definitions, as class "+fqcn
    // + " not found...") << std::endl;
    return "";
  }
  ...
  // name=\""+nmspcid+":"+clstruct->getTreatedClassName(false)+"\">\n");
  ...
  if (clstruct->pri.size() > 0) {
    for (unsigned int i = 0; i < clstruct->pri.size(); i++) {
      if (((tes = clstruct->pri.at(i).find("(")) == std::string::npos &&
           (tes = clstruct->pri.at(i).find(")")) == std::string::npos &&
           clstruct->pri.at(i).find("~") == std::string::npos)) {
  ...
  if (clstruct->pro.size() > 0) {
    for (unsigned int i = 0; i < clstruct->pro.size(); i++) {
      if (((tes = clstruct->pro.at(i).find("(")) == std::string::npos &&
           (tes = clstruct->pro.at(i).find(")")) == std::string::npos &&
           clstruct->pro.at(i).find("~") == std::string::npos)) {
  ...
  for (unsigned int i = 0; i < clstruct->pub.size(); i++) {
    if (((tes = clstruct->pub.at(i).find("(")) == std::string::npos &&
         (tes = clstruct->pub.at(i).find(")")) == std::string::npos &&
         clstruct->pub.at(i).find("~") == std::string::npos)) {
      ...
      if (fldp.size() == 2) {
        ...
        if (isPrimitiveDataType(fldp.at(0))) {
          ...
          std::string fqcn =
              ref.getFullyQualifiedClassName(stlcnt, clstruct->getNamespaces());
          contType +=
              getFullyQualifiedClassName(stlcnt, clstruct->getNamespaces()) +
              ",";
          ...
          if (!ptr) {
            ...
            if (isPrimitiveDataType(fqcn)) {
              ...
              obj_binding.append("<xsd:complexType name=\"ArrayOf" +
                                 clstruct->getTreatedClassName(true) + fqcn +
                                 "\"> <xsd:sequence>\n");
              obj_binding.append(
                  "<xsd:element maxOccurs=\"unbounded\" minOccurs=\"0\" "
                  "name=\"" +
                  nam + "\" nillable=\"true\" type=\"xsd:" + xstyp + "\" />\n");
              ...
              StringUtil::replaceAll(xstyp, "::", "_");
              obj_binding.append("<xsd:complexType name=\"ArrayOf" +
                                 clstruct->getTreatedClassName(true) + xstyp +
                                 "\"> <xsd:sequence>\n");
              obj_binding.append("<xsd:element maxOccurs=\"unbounded\" "
                                 "minOccurs=\"0\" name=\"" +
                                 nam + "\" nillable=\"true\" type=\"" +
                                 namespc + ":" + xstyp + "\" />\n");
              obj_binding.append("</xsd:sequence> </xsd:complexType>\n");
              int_obj_binding.append(
                  getXSDDefinitions(allclsmap, fqcn, ref, appname, tmpnmspc,
                                    allnmspcs, dfnmspc, resp));
            }
          } else {
          }
        } else {
          std::string namespc;
          std::string nam = fldp.at(1);
          std::string fqcn = ref.getFullyQualifiedClassName(
              fldp.at(0), clstruct->getNamespaces());
          if (!ptr) {
            std::string clnms = fqcn;
            if (fqcn.find("::") != std::string::npos) {
              clnms = fqcn.substr(0, fqcn.find_last_of("::") + 2);
            ...
            std::string xstyp = fqcn;
            StringUtil::replaceAll(xstyp, "::", "_");
            obj_binding.append("\n<xsd:element name=\"" + nam + "\" type=\"" +
                               namespc + ":" + xstyp + "\"/>\n");
            int_obj_binding.append(getXSDDefinitions(allclsmap, fqcn, ref,
                                                     appname, tmpnmspc,
                                                     allnmspcs, dfnmspc, resp));
          } else {
          }
            ...
            if ("set" + cam == methpm.at(1) && argpm.size() == 1 &&
              ...
              if (isPrimitiveDataType(argpm.at(0))) {
                ...
                if (!ptr) {
                         ...
                         argpm.at(0).find("set") != std::string::npos ||
                         argpm.at(0).find("list") != std::string::npos ||
                         argpm.at(0).find("multiset") != std::string::npos) {
                std::string stlcnt = argpm.at(0);
                std::string stltyp = argpm.at(0);
                std::string contType;
                StringUtil::replaceFirst(stltyp, "std::", "");
                ...
                if (!ptr) {
                  if (isPrimitiveDataType(fqcn)) {
                    ...
                    obj_binding.append(
                        "<xsd:element maxOccurs=\"unbounded\" minOccurs=\"0\" "
                        "name=\"" +
                        xstyp + "\" nillable=\"true\" type=\"xsd:" + fqcn +
                        "\" />\n");
                    obj_binding.append("</xsd:sequence> </xsd:complexType>\n");
                  } else {
                    std::string clnms = fqcn;
                    ...
                    obj_binding.append("<xsd:element maxOccurs=\"unbounded\" "
                                       "minOccurs=\"0\" name=\"" +
                                       xstyp + "\" nillable=\"true\" type=\"" +
                                       namespc + ":" + xstyp + "\" />\n");
                    obj_binding.append("</xsd:sequence> </xsd:complexType>\n");
                    int_obj_binding.append(
                        getXSDDefinitions(allclsmap, fqcn, ref, appname,
                                          tmpnmspc, allnmspcs, dfnmspc, resp));
                  }
                } else {
                }
              } else {
                std::string namespc;
                std::string fqcn = ref.getFullyQualifiedClassName(
                    argpm.at(0), clstruct->getNamespaces());
                if (!ptr) {
                  std::string clnms = fqcn;
                  if (fqcn.find("::") != std::string::npos) {
                    clnms = fqcn.substr(0, fqcn.find_last_of("::") + 2);
...
bool Reflection::isPrimitiveDataType(std::string type) {
  ...
  if (type == "short" || type == "short int" || type == "signed short" ||
      type == "signed short int" || type == "unsigned short" ||
      type == "unsigned short int" || type == "signed" || type == "int" ||
      type == "signed int" || type == "unsigned" || type == "unsigned int" ||
      type == "long" || type == "long int" || type == "signed long" ||
      type == "signed long int" || type == "unsigned long" ||
      type == "unsigned long int" || type == "long long" ||
      type == "long long int" || type == "signed long long" ||
      type == "signed long long int" || type == "unsigned long long" ||
      type == "unsigned long long int" || type == "long double" ||
      type == "bool" || type == "float" || type == "double" ||
      type == "string" || type == "std::string" || type == "char" ||
      type == "signed char" || type == "unsigned char" || type == "wchar_t") {
