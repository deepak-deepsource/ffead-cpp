...
public:
  ...
  static TemplatePtr getTemplateFunc(const std::string &appName,
...
public:
  ...
  GetMeth getMethodInstance(const Method &method) {
    return method.getIns() != NULL
               ? method.getIns()
               : (GetMeth)dlsym(dlib, method.getRefName().c_str());
  }
  template <class T>
  T invokeMethod(void *instance, const Method &method, const vals &values,
                 const bool &cleanVals = false) {
    T obj;
  ...
  template <class T>
  void invokeMethod(T *t, void *instance, const Method &method,
                    const vals &values, const bool &cleanVals = false) {
    GetMeth f = getMethodInstance(method);
    if (f != NULL) {
      if (method.getReturnType() != "void") {
        void *rt = f(instance, values, cleanVals);
  ...
  void destroy(int serOpt, void *instance, std::string cs,
               const std::string &app = "");
  void destroy(int serOpt, void *instance, std::string cs, std::string_view);
  void *invokeMethodGVP(void *instance, const Method &method,
                        const vals &values, const bool &cleanVals = false) {
    void *obj = NULL;
    GetMeth f = getMethodInstance(method);
    if (f != NULL) {
  ...
  template <class T>
  T newInstance(const Constructor &ctor, const vals &values,
                const bool &cleanVals = false) {
    T obj;
    NewInst f = ctor.getIns() != NULL
                    ? ctor.getIns()
                    : (NewInst)dlsym(dlib, ctor.getRefName().c_str());
    if (f != NULL) {
  ...
  template <class T>
  T newInstance(const Constructor &ctor, const bool &cleanVals = false) {
    vals values;
  ...
  void *invokeMethodUnknownReturn(void *instance, const Method &method,
                                  const vals &values,
                                  const bool &cleanVals = false) {
    void *obj = NULL;
    GetMeth f = getMethodInstance(method);
  ...
  template <class T> T getField(void *instance, const Field &field) {
    T t;
    GetFld f = field.getIns() != NULL
                   ? field.getIns()
                   : (GetFld)dlsym(dlib, field.getRefName().c_str());
    if (f != NULL) {
      T *fld = (T *)f(instance);
      t = *fld;
      delete fld;
  ...
  void *execOperator(void *instance, const std::string &operato,
                     const vals &values, const std::string &cs,
                     const bool &cleanVals = false,
                     const std::string &app = "") {
  ...
  void *getNewContainer(const std::string &cs, const std::string &contType,
                        const std::string &app = "") {
    ClassInfo *ci = getClassInfo(cs, app);
    void *obj = NULL;
  ...
  void destroyContainer(void *vec, const std::string &cs,
  ...
  int getContainerSize(void *vec, const std::string &cs,
                       const std::string &contType,
                       const std::string &app = "") {
    ClassInfo *ci = getClassInfo(cs, app);
    int size = -1;
  ...
  void addToContainer(void *vec, void *instance, const std::string &cs,
                      const std::string &contType,
                      const std::string &app = "") {
    ClassInfo *ci = getClassInfo(cs, app);
    std::string methodname = ci->getContRefName();
  ...
  void *getContainerElementValueAt(void *vec, const int &pos,
                                   const std::string &cs,
                                   const std::string &contType,
                                   const std::string &app = "") {
    ClassInfo *ci = getClassInfo(cs, app);
  ...
  void *getContainerElementAt(void *vec, const int &pos, const std::string &cs,
                              const std::string &contType,
                              const std::string &app = "") {
    ClassInfo *ci = getClassInfo(cs, app);
    void *obj = NULL;
  ...
  template <typename T>
  static void *destroyNestedContainer(const std::string &container,
                                      void *instance) {
    if (container == "std::vector") {
  ...
  template <typename T>
  static void *destroyNestedContainerSV(const std::string &container,
                                        void *instance) {
  ...
  template <typename T>
  static void *getPValueFromNestedContainer(const std::string &container,
  ...
  template <typename T>
  static void *getPValueFromNestedContainerSV(const std::string &container,
                                              void *cont, const int &pos) {
    if (container.find("std::set") == 0) {
      typedef typename std::set<T>::iterator iterator_type;
      iterator_type it;
