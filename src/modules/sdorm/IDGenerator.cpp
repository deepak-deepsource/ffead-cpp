...
void IDGenerator::init(DataSourceEntityMapping &dsemp,
                       const bool &forceReinit) {
  if (dsemp.getIdgentype() == "hilo" || dsemp.getIdgentype() == "multihilo") {
    if (!forceReinit && dsemp.getIdgentype() == "hilo" &&
        HiloIdGenerator::isInitialized()) {
      return;
    } else if (!forceReinit && dsemp.getIdgentype() == "multihilo" &&
               HiloIdGenerator::isInitialized(dsemp.getClassName())) {
      return;
    }
    ...
    if (hiValue.isNumber()) {
      ...
      if (dsemp.getIdgentype() == "multihilo")
        HiloIdGenerator::init(dsemp.getClassName(), hival,
                              dsemp.getIdgenlowValue(), forceReinit);
      else
