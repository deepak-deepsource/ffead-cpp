...
protected:
  ...
  virtual void *getDbEntityForBulkInsert(void *entity,
                                         const std::string &clasName,
                                         std::string &error) = 0;
  virtual bool executeInsertBulk(Query &query, std::vector<void *> entities,
                                 std::vector<void *> dbEntities) = 0;
  virtual bool executeUpdateBulk(Query &query, std::vector<void *> entities,
                                 std::vector<void *> dbEntities) = 0;
...
public:
  ...
  virtual std::vector<std::map<std::string, GenericObject>>
  execute(Query &query) = 0;
  virtual bool executeUpdate(Query &query) = 0;
  virtual std::vector<std::map<std::string, GenericObject>>
  execute(QueryBuilder &qb) = 0;
...
template <class T> inline bool DataSourceInterface::insert(T &t) {
  std::string clasName = CastUtil::getClassName(t);
  Query q(BLANK, clasName);
  return executeInsertInternal(q, &t);
}

template <class T> inline bool DataSourceInterface::update(T &t) {
  std::string clasName = CastUtil::getClassName(t);
  Query q(BLANK, clasName);
...
DataSourceInterface::bulkInsert(std::vector<T> &vecT) {
  T t;
  std::string clasName = CastUtil::getClassName(t);
...
template <class T>
inline std::map<int, std::string>
DataSourceInterface::bulkUpdate(std::vector<T> &vecT) {
  T t;
  std::string clasName = CastUtil::getClassName(t);
  // ClassInfo* clas = reflector->getClassInfo(clasName, appName);
  std::vector<void *> dbEntities;
...
template <class T> inline T DataSourceInterface::get(GenericObject &id) {
  T t;
  std::string clasName = CastUtil::getClassName(t);
  ...
  QueryBuilder qb;
  qb.fromClass(clasName, BLANK)
      .condition()
      .where(idColName, QueryOperator::EQUALS, id)
      .end();
...
template <class T> inline std::vector<T> DataSourceInterface::getAll() {
  T t;
  std::string clasName = CastUtil::getClassName(t);
...
inline std::vector<T> DataSourceInterface::getList(Query &query) {
  ...
  std::vector<T> vecT;
  std::string clasName = CastUtil::getClassName(t);
  if (query.getClassName() != clasName)
    return vecT;
  void *vect = executeQuery(query, true);
  if (vect != NULL) {
    vecT = *(std::vector<T> *)vect;
...
template <class T> inline T DataSourceInterface::get(Query &query) {
  T t;
  std::string clasName = CastUtil::getClassName(t);
  if (query.getClassName() != clasName)
    return t;
...
template <class T> inline T DataSourceInterface::get(QueryBuilder &qb) {
  T t;
  std::string clasName = CastUtil::getClassName(t);
  if (qb.getClassName() != clasName)
    return t;
...
template <class T> inline void DataSourceInterface::empty() {
  T t;
  std::string clasName = CastUtil::getClassName(t);
  return empty(clasName);
}

template <class T> inline long DataSourceInterface::getNumRows() {
  T t;
  std::string clasName = CastUtil::getClassName(t);
  return getNumRows(clasName);
}

template <class T> inline bool DataSourceInterface::remove(GenericObject &id) {
  T t;
  std::string clasName = CastUtil::getClassName(t);
