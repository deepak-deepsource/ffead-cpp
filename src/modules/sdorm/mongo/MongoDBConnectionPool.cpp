...
MongoDBConnectionPool::MongoDBConnectionPool(
  ...
  isUnixDomainSocket =
      StringUtil::toLowerCopy(mp["isUnixDomainSocket"]) == "true";
...
void MongoDBConnectionPool::initEnv() {
  ...
  retval = mongoc_client_command_simple(tclient, "admin", command, NULL, &reply,
                                        &error);
  if (!retval) {
...
void *MongoDBConnectionPool::newConnection(const bool &isWrite,
...
MongoDBConnectionPool::~MongoDBConnectionPool() { destroyPool(); }
