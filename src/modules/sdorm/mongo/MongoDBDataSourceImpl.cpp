...
DSType MongoDBDataSourceImpl::getType() { return SD_ORM_MONGO; }

std::string MongoDBDataSourceImpl::initializeDMLQueryParts(
    Query &cquery, bson_t **data, bson_t **query, std::string &operationName) {
  std::string qs = cquery.getQuery();
  StringUtil::trim(qs);
  std::string collectionName;
  if (qs.find("db.") != std::string::npos &&
      qs.find("(") != std::string::npos && qs.at(qs.length() - 1) == ')') {
    ...
    if (queryStrPart.at(0) != '{' ||
        queryStrPart.at(queryStrPart.length() - 1) != '}')
      throw std::runtime_error("Invalid JSON query parts specified");

    if (operationName == "insert") {
      bson_error_t err;
      *data = bson_new_from_json((const uint8_t *)queryStrPart.c_str(),
                                 queryStrPart.length(), &err);
    } else {
      bson_error_t err;
      bson_t *queryParts = bson_new_from_json(
          (const uint8_t *)queryStrPart.c_str(), queryStrPart.length(), &err);
      bson_iter_t i;

      bson_iter_init(&i, queryParts);
      bool found = bson_iter_find(&i, "$data");
      if (found) {
        *data = bson_new_from_data(bson_iter_value(&i)->value.v_doc.data,
                                   bson_iter_value(&i)->value.v_doc.data_len);
      }

      bson_iter_init(&i, queryParts);
      found = bson_iter_find(&i, "$query");
      if (found) {
        *query = bson_new_from_data(bson_iter_value(&i)->value.v_doc.data,
                                    bson_iter_value(&i)->value.v_doc.data_len);
      }
    }
  }
...
std::string
MongoDBDataSourceImpl::initializeQueryParts(Query &cquery, bson_t **querySpec,
                                            bson_t **fields,
  ...
  *fields = NULL;
  ...
  if (qs.find("db.") != std::string::npos &&
    ...
    if (t) {
      *fields = bson_new_from_data(bson_iter_value(&i)->value.v_doc.data,
                                   bson_iter_value(&i)->value.v_doc.data_len);
    }
  }
  return collectionName;
...
QueryComponent *
MongoDBDataSourceImpl::getQueryComponent(const std::vector<Condition> &conds) {
    ...
    if (conds.at(i).getClause() == QueryClause::NONE) {
      if (currentSubQuery->undecided)
        ...
        if (currentSubQuery->tempClauses.size() > 0) {
          for (int var = 0; var < (int)currentSubQuery->tempClauses.size();
               ++var) {
            currentSubQuery->andClauses.push_back(
                currentSubQuery->tempClauses.at(var));
          }
        ...
        if (currentSubQuery->tempClauses.size() > 0) {
          for (int var = 0; var < (int)currentSubQuery->tempClauses.size();
               ++var) {
            currentSubQuery->orClauses.push_back(
                currentSubQuery->tempClauses.at(var));
          }
          currentSubQuery->tempClauses.clear();
        }
      ...
      if (!currentSubQuery->undecided &&
          currentSubQuery->tempChildren.size() > 0) {
        for (int var = 0; var < (int)currentSubQuery->tempChildren.size();
             ++var) {
          if (currentSubQuery->isAnd) {
            currentSubQuery->andChildren.push_back(
                currentSubQuery->tempChildren.at(var));
          } else {
            currentSubQuery->orChildren.push_back(
                currentSubQuery->tempChildren.at(var));
          }
        }
...
std::map<std::string, std::map<std::string, Condition>>
MongoDBDataSourceImpl::toMap(std::vector<Condition> &conds) {
...
bson_t *
MongoDBDataSourceImpl::createSubMongoQuery(std::vector<Condition> &conds) {
  ...
  for (mit = cols.begin(); mit != cols.end(); ++mit) {
    ...
    for (cit = condMap.begin(); cit != condMap.end(); ++cit) {
      if (cit->first == "$in" || cit->first == "$nin") {
        bson_t child;
        bson_t childa;
        bson_append_array_begin(&child, cit->first.c_str(), cit->first.length(),
                                &childa);
...
void MongoDBDataSourceImpl::appendGenericObject(bson_t *b,
...
void *MongoDBDataSourceImpl::getResults(const std::string &collectionName,
                                        Query &query, bson_t *querySpec,
                                        bson_t *fields, const bool &isObj,
  ...
  if (!isCountQuery) {
    cursor =
        mongoc_collection_find(collection, MONGOC_QUERY_NONE, query.getStart(),
                               query.getCount(), 0, querySpec, fields, NULL);
    const bson_t *doc;
    while (mongoc_cursor_next(cursor, &doc)) {
      if (isObj) {
        ...
        ((std::vector<std::map<std::string, GenericObject>> *)result)
            ->push_back(row);
      }
    }
    bson_error_t error;
    ...
    bson_error_t er;
    result = new long(mongoc_collection_count(collection, MONGOC_QUERY_NONE,
                                              querySpec, query.getStart(),
...
void *MongoDBDataSourceImpl::getResults(const std::string &collectionName,
  ...
  cursor = mongoc_collection_find(collection, MONGOC_QUERY_NONE, qb.getStart(),
                                  qb.getCount(), 0, querySpec, fields, NULL);
  const bson_t *doc;
  while (mongoc_cursor_next(cursor, &doc)) {
    if (isObj) {
      void *ob = getObject((bson_t *)doc, NULL, 0, clasName);
...
std::string MongoDBDataSourceImpl::getQueryForRelationship(
    const std::string &column, const std::string &type, void *val) {
  std::string qstr = "{\"" + column + "\":";
  if (type == "string" || type == "std::string") {
    qstr += "\"" + (*(std::string *)val) + "\"}";
  } else if (type == "short" || type == "int" || type == "unsigned short" ||
             type == "unsigned int" || type == "long" ||
             type == "unsigned long" || type == "long long" || type == "char" ||
             type == "unsigned char") {
...
void MongoDBDataSourceImpl::storeProperty(ClassInfo *clas, void *t, void *colV,
                                          const Field &fe) {
  if (colV != NULL) {
...
void MongoDBDataSourceImpl::executePreTable(DataSourceEntityMapping &dsemp,
                                            GenericObject &idv) {
...
void MongoDBDataSourceImpl::executePostTable(DataSourceEntityMapping &dsemp,
                                             GenericObject &idv) {
  if (dsemp.getIdgendbEntityType() == "table") {
    std::string query;
    StringContext params;
    ...
    if (dsemp.getIdgentype() == "multihilo") {
      params["entity_column"] = dsemp.getIdgenentityColumn();
      params["entity_name"] = dsemp.getTableName();
      query = std::string("db.${idgen_tabname}.update({\"$query\": "
                          "{\"${entity_column}\": \"${entity_name}\"}, ") +
              std::string("\"$data\": {\"$inc\": {\"${idgen_colname}\": 1}}})");
    } else {
      query = "db.${idgen_tabname}.update({\"$data\": {\"$inc\": "
              "{\"${idgen_colname}\": 1}}})";
    }
    query = TemplateEngine::evaluate(query, params);
...
void MongoDBDataSourceImpl::executeSequence(DataSourceEntityMapping &dsemp,
                                            GenericObject &idv) {}

void MongoDBDataSourceImpl::executeIdentity(DataSourceEntityMapping &dsemp,
                                            GenericObject &idv) {}

std::vector<std::map<std::string, GenericObject>>
MongoDBDataSourceImpl::execute(Query &query) {
...
MongoDBDataSourceImpl::MongoDBDataSourceImpl(ConnectionPooler *pool,
                                             Mapping *mapping) {
  this->pool = pool;
...
void MongoDBDataSourceImpl::executeCustom(DataSourceEntityMapping &dsemp,
...
void MongoDBDataSourceImpl::getBSONObjectFromObject(
    const std::string &clasName, void *object, bson_t *b,
    const bool &isIdBsonAppend) {
  ...
  for (clsprpmapit = clsprpmap.begin(); clsprpmapit != clsprpmap.end();
       ++clsprpmapit) {
    ...
    if (pf.getType() == "short") {
      ...
      std::vector<short> *val =
          (std::vector<short> *)reflector->invokeMethodGVP(object, meth, valus);
      bson_t child;
      ...
      delete val;
    } else if (pf.getType().find("std::vector<unsigned short,") !=
               std::string::npos) {
      std::vector<unsigned short> *val =
          (std::vector<unsigned short> *)reflector->invokeMethodGVP(
              object, meth, valus);
      bson_t child;
      bson_append_array_begin(b, col.c_str(), col.length(), &child);
      for (int var = 0; var < (int)val->size(); ++var) {
      ...
      std::vector<int> *val =
          (std::vector<int> *)reflector->invokeMethodGVP(object, meth, valus);
      bson_t child;
      bson_append_array_begin(b, col.c_str(), col.length(), &child);
      for (int var = 0; var < (int)val->size(); ++var) {
        bson_append_int32(b, "", 0, val->at(var));
      ...
      bson_append_array_end(b, &child);
      delete val;
    } else if (pf.getType().find("std::vector<unsigned int,") !=
               std::string::npos) {
      std::vector<unsigned int> *val =
          (std::vector<unsigned int> *)reflector->invokeMethodGVP(object, meth,
                                                                  valus);
      bson_t child;
      bson_append_array_begin(b, col.c_str(), col.length(), &child);
      for (int var = 0; var < (int)val->size(); ++var) {
      ...
      std::vector<long> *val =
          (std::vector<long> *)reflector->invokeMethodGVP(object, meth, valus);
      bson_t child;
      bson_append_array_begin(b, col.c_str(), col.length(), &child);
      for (int var = 0; var < (int)val->size(); ++var) {
      ...
      bson_append_array_end(b, &child);
      delete val;
    } else if (pf.getType().find("std::vector<unsigned long,") !=
               std::string::npos) {
      std::vector<unsigned long> *val =
          (std::vector<unsigned long> *)reflector->invokeMethodGVP(object, meth,
                                                                   valus);
      bson_t child;
      bson_append_array_begin(b, col.c_str(), col.length(), &child);
      for (int var = 0; var < (int)val->size(); ++var) {
      ...
      bson_append_array_end(b, &child);
      delete val;
    } else if (pf.getType().find("std::vector<long long,") !=
               std::string::npos) {
      std::vector<long long> *val =
          (std::vector<long long> *)reflector->invokeMethodGVP(object, meth,
                                                               valus);
      bson_t child;
      bson_append_array_begin(b, col.c_str(), col.length(), &child);
      for (int var = 0; var < (int)val->size(); ++var) {
      ...
      std::vector<bool> *val =
          (std::vector<bool> *)reflector->invokeMethodGVP(object, meth, valus);
      bson_t child;
      bson_append_array_begin(b, col.c_str(), col.length(), &child);
      for (int var = 0; var < (int)val->size(); ++var) {
      ...
      std::vector<float> *val =
          (std::vector<float> *)reflector->invokeMethodGVP(object, meth, valus);
      bson_t child;
      bson_append_array_begin(b, col.c_str(), col.length(), &child);
      for (int var = 0; var < (int)val->size(); ++var) {
      ...
      std::vector<double> *val =
          (std::vector<double> *)reflector->invokeMethodGVP(object, meth,
                                                            valus);
      bson_t child;
      bson_append_array_begin(b, col.c_str(), col.length(), &child);
      for (int var = 0; var < (int)val->size(); ++var) {
      ...
      bson_append_array_end(b, &child);
      delete val;
    } else if (pf.getType().find("std::vector<std::string,") !=
                   std::string::npos ||
               pf.getType().find("std::vector<std::string,") !=
                   std::string::npos) {
      std::vector<std::string> *val =
          (std::vector<std::string> *)reflector->invokeMethodGVP(object, meth,
                                                                 valus);
      bson_t child;
      bson_append_array_begin(b, col.c_str(), col.length(), &child);
      for (int var = 0; var < (int)val->size(); ++var) {
      ...
      if (val != NULL) {
        ...
        bson_append_array_begin(b, col.c_str(), col.length(), &child);
        int contSize =
            reflector->getContainerSize(val, vtyp, "std::vector", appName);
        for (int var = 0; var < contSize; ++var) {
          void *contEle = reflector->getContainerElementAt(
              val, var, vtyp, "std::vector", appName);
...
void *MongoDBDataSourceImpl::getObject(bson_t *data, uint8_t *buf, uint32_t len,
  ...
  while (bson_iter_next(&i)) {
    ...
    case BSON_TYPE_DOCUMENT: {
      void *ob =
          getObject(NULL, bson_iter_value(&i)->value.v_doc.data,
                    bson_iter_value(&i)->value.v_doc.data_len, fe.getType());
      ...
      if (te == "char") {
        ...
        if (veci != NULL) {
          while (bson_iter_next(&ii)) {
            // bson_type_t t = bson_iter_type( &ii );
            void *ob =
                getObject(NULL, bson_iter_value(&ii)->value.v_doc.data,
          ...
          logger << ("Invalid class definition found while deserializing " +
                     clasName + " - invalid class " + te)
                 << std::endl;
        }
      }
      bson_destroy(d);
...
void MongoDBDataSourceImpl::getMapOfProperties(
    bson_t *data, std::map<std::string, GenericObject> *map) {
  bson_iter_t i;
  std::string key;
  char oidhex[25];
  bson_iter_init(&i, data);
...
bool MongoDBDataSourceImpl::startTransaction() { return false; }

bool MongoDBDataSourceImpl::commit() { return false; }

bool MongoDBDataSourceImpl::rollback() { return false; }
...
bool MongoDBDataSourceImpl::executeUpdate(Query &query) {
  ...
  std::string operationName;
  std::string collectionName =
      initializeDMLQueryParts(query, &data, &q, operationName);
  if (q == NULL) {
    q = bson_new();
  }
  if (operationName == "insert") {
    ...
    mongoc_collection_t *collection = _collection(collectionName.c_str());
    bson_error_t er;
    bool fl = mongoc_collection_insert(collection, MONGOC_INSERT_NONE, data,
                                       NULL, &er);
    if (data == NULL)
      bson_destroy(data);
    _release(collection);
    ...
    mongoc_collection_t *collection = _collection(collectionName.c_str());
    bson_error_t er;
    bool fl = mongoc_collection_update(collection, MONGOC_UPDATE_NONE, q, data,
                                       NULL, &er);
    if (data == NULL)
      bson_destroy(data);
    bson_destroy(q);
    ...
    mongoc_collection_t *collection = _collection(collectionName.c_str());
    bson_error_t er;
    bool fl = mongoc_collection_update(collection, MONGOC_UPDATE_MULTI_UPDATE,
                                       q, data, NULL, &er);
    if (data == NULL)
      bson_destroy(data);
    bson_destroy(q);
    ...
    bson_error_t er;
    bool fl =
        mongoc_collection_remove(collection, MONGOC_REMOVE_NONE, q, NULL, &er);
    bson_destroy(q);
    _release(collection);
    return fl;
  }
...
std::vector<std::map<std::string, GenericObject>>
MongoDBDataSourceImpl::execute(QueryBuilder &qb) {
...
bool MongoDBDataSourceImpl::executeInsert(Query &query, void *entity) {
  ...
  DataSourceEntityMapping &dsemp =
      mapping->getDataSourceEntityMapping(query.getClassName());
  ...
  if (isIdFound) {
    ...
    mongoc_collection_t *collection = _collection(collectionName.c_str());
    bson_error_t er;
    fl = mongoc_collection_insert(collection, MONGOC_INSERT_NONE, data, NULL,
                                  &er);
    _release(collection);
  }
  bson_destroy(data);
...
bool MongoDBDataSourceImpl::isGetDbEntityForBulkInsert() { return true; }

void *MongoDBDataSourceImpl::getDbEntityForBulkInsert(
    void *entity, const std::string &clasName, std::string &error) {
  bson_t *b = bson_new();
  getBSONObjectFromObject(clasName, entity, b, true);
  return b;
}

bool MongoDBDataSourceImpl::executeInsertBulk(Query &query,
...
bool MongoDBDataSourceImpl::executeUpdateBulk(Query &query,
                                              std::vector<void *> entities,
                                              std::vector<void *> dbEntities) {
  std::string collectionName = mapping->getTableForClass(query.getClassName());
  _conn();
...
bool MongoDBDataSourceImpl::remove(const std::string &clasName,
                                   GenericObject &id) {
  ...
  std::string qstr =
      std::string("db." + collectionName + ".remove({\"$query\": {\"_id\":") +
      idValue + std::string("}})");
  Query q(qstr, clasName);
...
void *MongoDBDataSourceImpl::executeQuery(Query &cquery, const bool &isObj) {
  ...
  if (!isObj) {
    collectionName =
        initializeQueryParts(cquery, &querySpec, &fields, operationName);
    ...
    DataSourceEntityMapping &dsemp =
        mapping->getDataSourceEntityMapping(cquery.getClassName());
    collectionName = dsemp.getTableName();
  }
  return getResults(collectionName, cquery, querySpec, fields, isObj,
                    isCountQuery);
}
    ...
    for (it = cols.begin(); it != cols.end(); ++it) {
      if (!isClassProps) {
        ...
        if (dsemp.getColumnForProperty(it->first) != "") {
          bson_append_int32(fields,
                            dsemp.getColumnForProperty(it->first).c_str(),
...
mongoc_collection_t *MongoDBDataSourceImpl::_collection(const char *collName) {
  if (collection == NULL) {
    return mongoc_client_get_collection(
        (mongoc_client_t *)conn->getConn(),
        conn->getNode().getDatabaseName().c_str(), collName);
  } else {
    return collection;
  }
}
...
bool MongoDBDataSourceImpl::startSession(void *details) {
  ...
  if (mcd != NULL && *mcd != "") {
    collection = mongoc_client_get_collection(
        (mongoc_client_t *)conn->getConn(),
        conn->getNode().getDatabaseName().c_str(), mcd->c_str());
    if (collection == NULL) {
