...
DSType MongoDBRawDataSourceImpl::getType() { return SD_RAW_MONGO; }
...
void MongoDBRawConnectionPool::initEnv() {
  ...
  if (isUnixDomainSocket) {
    if (props.getNodes().at(0).getUsername() != "" &&
    ...
    if (props.getNodes().at(0).getConnectionTimeout() > 0) {
      connTimeOut = CastUtil::fromNumber(
          (long)getProperties().getNodes().at(0).getConnectionTimeout());
    }
  } else {
    ...
    for (int var = 0; var < (int)props.getNodes().size(); ++var) {
      ...
      if (props.getNodes().at(var).getUsername() != "" &&
          props.getNodes().at(var).getPassword() != "") {
        connectionString += (props.getNodes().at(var).getUsername() + ":" +
                             props.getNodes().at(var).getPassword() + "@");
      }
      connectionString +=
          props.getNodes().at(var).getHost() + ":" + CastUtil::fromNumber(port);

      if (connTimeOut == "" &&
          props.getNodes().at(var).getConnectionTimeout() > 0) {
        connTimeOut = CastUtil::fromNumber(
    ...
    if (connectionString.at(connectionString.length() - 1) == ',') {
      connectionString =
          connectionString.substr(0, connectionString.length() - 1);
    }
...
void *MongoDBRawConnectionPool::newConnection(const bool &isWrite,
...
bool MongoDBRawDataSourceImpl::begin(const std::string &collName) {
  ...
  coll = mongoc_client_get_collection(conn, pool->dbName.c_str(),
                                      collName.c_str());
  return coll != NULL;
}
...
void MongoDBRawDataSourceImpl::executeQuery(const std::string &query, void *ctx,
                                            MgRawResFunc cb, int start,
                                            int count) {
  bson_error_t error;
  bson_t *qb = bson_new_from_json((const uint8_t *)query.c_str(),
                                  query.length(), &error);
  executeQuery(qb, ctx, cb, start, count);
  bson_destroy(qb);
}

void MongoDBRawDataSourceImpl::executeQuery(const bson_t *query, void *ctx,
                                            MgRawResFunc cb, int start,
                                            int count) {
  bool qi = false;
  if (query == NULL) {
  ...
  mongoc_cursor_t *cursor = mongoc_collection_find(
      coll, MONGOC_QUERY_NONE, start, count, 0, query, NULL, NULL);
  const bson_t *doc;
  bson_iter_t i;
...
bool MongoDBRawDataSourceImpl::executeUpdateQuery(const std::string &query,
                                                  const std::string &data,
                                                  MongoDBRawOper oper) {
  bson_error_t error;
  bson_t *qb = query.length() > 0
                   ? bson_new_from_json((const uint8_t *)query.c_str(),
                                        query.length(), &error)
                   : bson_new();
  bson_t *db = bson_new_from_json((const uint8_t *)query.c_str(),
                                  query.length(), &error);
...
bool MongoDBRawDataSourceImpl::executeUpdateQuery(const bson_t *query,
                                                  const bson_t *data,
                                                  MongoDBRawOper oper) {
  bson_error_t er;
  switch (oper) {
  ...
  case UPDATE:
    return mongoc_collection_update(coll, MONGOC_UPDATE_NONE, query, data, NULL,
                                    &er);
  case SAVE:
    return mongoc_collection_save(coll, data, NULL, &er);
  case UPDATE_MULTI:
    return mongoc_collection_update(coll, MONGOC_UPDATE_MULTI_UPDATE, query,
                                    data, NULL, &er);
  case REMOVE:
...
bool MongoDBRawDataSourceImpl::startBulk(const std::string &collName) {
  if (bulk != NULL) {
    throw std::runtime_error(
        "Already in a bulk session, bulk session still open");
  }
  begin(collName);
...
bool MongoDBRawDataSourceImpl::executeUpdateBulkQuery(int len,
                                                      MgRawBlkFunc cb) {
  if (len > 0) {
    mongoc_bulk_operation_t *bulk =
        mongoc_collection_create_bulk_operation(coll, true, NULL);
    for (int i = 0; i < len; i++) {
