...
enum MongoDBRawOper { INSERT, SAVE, UPDATE, UPDATE_MULTI, REMOVE };
...
public:
  ...
  void executeQuery(const bson_t *query, void *ctx, MgRawResFunc cb,
                    int start = 0, int count = 0);
  void executeQuery(const std::string &query, void *ctx, MgRawResFunc cb,
                    int start = 0, int count = 0);
  bool executeUpdateQuery(const std::string &query, const std::string &data,
                          MongoDBRawOper oper);
  bool executeUpdateQuery(const bson_t *query, const bson_t *data,
                          MongoDBRawOper oper);
