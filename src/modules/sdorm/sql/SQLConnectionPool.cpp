...
void *SQLConnectionPool::newConnection(const bool &isWrite,
  ...
  if (node.getConnectionTimeout() > 0) {
    ...
    // SQLSetConnectAttr(conn, SQL_ATTR_CONNECTION_TIMEOUT,
    // (SQLUINTEGER*)&seconds, 0); SQLSetConnectAttr(conn, SQL_LOGIN_TIMEOUT,
    // (SQLPOINTER)&seconds, 0);
  }
  // 3. Connect to the datasource "MySQL-test"
  V_OD_erg = SQLConnect(
      conn, (SQLCHAR *)node.getDsn().c_str(), node.getDsn().length(),
  ...
  if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO)) {
    ...
    SQLGetDiagRec(SQL_HANDLE_DBC, conn, 1, V_OD_stat, &V_OD_err_s, V_OD_msg,
                  100, &V_OD_mlen);
...
SQLConnectionPool::~SQLConnectionPool() { destroyPool(); }
