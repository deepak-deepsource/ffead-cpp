...
typedef void (*LipqCbFuncF)(void *ctx, bool status,
                            std::vector<PGresult *> *results,
                            const std::string &query, int counter);
#else
typedef void (*LipqCbFunc0)(void *ctx, void *res);
typedef void (*LipqCbFuncF)(void *ctx, bool status, void *results,
                            const std::string &query, int counter);
#endif

typedef void (*LipqCbFunc1)(void *ctx, bool endofdata, int row, int col,
                            char *name, char *value, int vlen);
typedef void (*LipqCbFunc2)(void *ctx, bool endofdata, int row, int col,
                            char *value, int vlen);
typedef void (*LipqCbFunc3)(void *ctx, bool endofdata, int row, int col,
                            char *value);

typedef void (*LipqCbFunc4)(void *ctx, int row, int col, char *name,
                            char *value, int vlen);
...
public:
  ...
  template <typename Func1> LibpqQuery &withCb0(Func1 cb) {
    this->cb0 = cb;
    this->cbType = 0;
    return *this;
  }
  template <typename Func1> LibpqQuery &withCb1(Func1 cb) {
    this->cb1 = cb;
    this->cbType = 1;
    return *this;
  }
  template <typename Func2> LibpqQuery &withCb2(Func2 cb) {
    this->cb2 = cb;
    this->cbType = 2;
    return *this;
  }
  template <typename Func3> LibpqQuery &withCb3(Func3 cb) {
    this->cb3 = cb;
    this->cbType = 3;
    return *this;
  }
  template <typename Func4> LibpqQuery &withCb4(Func4 cb) {
    this->cb4 = cb;
    this->cbType = 4;
    return *this;
  }
  template <typename Func5> LibpqQuery &withCb5(Func5 cb) {
    this->cb5 = cb;
    this->cbType = 5;
    return *this;
  }
  template <typename Func6> LibpqQuery &withCb6(Func6 cb) {
    this->cb6 = cb;
    this->cbType = 6;
    return *this;
  }
  template <typename FuncF> LibpqQuery &withFinalCb(FuncF fcb) {
    this->fcb = fcb;
    return *this;
  }
  LibpqQuery();
...
public:
  ...
  LibpqAsyncReq();
  LibpqQuery *getQuery();
  template <typename FuncF> LibpqAsyncReq &withFinalCb(void *ctx, FuncF fcb) {
    this->ctx = ctx;
    this->fcb = fcb;
    return *this;
  }
...
class FpgWire : public PgReadTask, public BaseSocket {
  ...
  void writeChar(char num, std::string &sendBuf) { sendBuf.push_back(num); }
  void handleResponse() {
    // std::cout << "buffer.size " << buffer.length() << " pos " << pos <<
    // std::endl;
    if (buffer.length() == 0) {
    ...
    case FpgRes::AuthenticationOk: {
      ...
      if (is == 0) {
        ...
        if (is == 3) {
          ...
          rp.push_back(buffer[pos++]);
          rp.push_back(buffer[pos++]);
          rp = "md5" + CryptoHandler::md5((unsigned char *)rp.data(),
                                          (unsigned int)rp.size());
          rp.push_back('\0');
          req.push_back(FpgReq::Password);
          CommonUtils::ntb(req, 4 + rp.length(), 4);
          req.append(rp);
    ...
    case FpgRes::ParameterStatus: {
      ...
      std::vector<std::string> pv =
          StringUtil::splitAndReturn<std::vector<std::string>>(pvstr, nc);
    ...
    case FpgRes::BackendKeyData: {
      ...
      int pi = readInt32();
      int sec = readInt32();
      // std::cout << "Cancellation Key Data , process and secret are " << pi <<
      // "," << sec << std::endl;
      break;
    }
    case FpgRes::ReadyForQuery: {
      ...
      state = bstat[0]; // I - Idle, T - In Trx, E - In Failed Trx, Q - In
                        // Query(custom)
      break;
    }
    case FpgRes::RowDescription: {
      ...
      for (int i = 0; i < cc; ++i) {
        ...
        c.indx = readInt16(); // if column belongs to table, attribute number of
                              // column
    ...
    case FpgRes::DataRow: {
      ...
      case 1: {
        for (int j = 0; j < cols; ++j) {
          ...
          q->cb1(q->ctx, false, rowNum++, j, (char *)currentMD[j].name.c_str(),
                 (char *)readString(length).c_str(), length);
        }
        break;
      }
      case 2: {
        for (int j = 0; j < cols; ++j) {
          int length = readInt32();
          q->cb2(q->ctx, false, rowNum++, j, (char *)readString(length).c_str(),
                 length);
        }
        break;
      }
      case 3: {
        for (int j = 0; j < cols; ++j) {
          int length = readInt32();
          q->cb3(q->ctx, false, rowNum++, j,
                 (char *)readString(length).c_str());
        }
        break;
      }
      case 4: {
        for (int j = 0; j < cols; ++j) {
          int length = readInt32();
          q->cb4(q->ctx, rowNum++, j, (char *)currentMD[j].name.c_str(),
                 (char *)readString(length).c_str(), length);
        }
        break;
      }
      case 5: {
        for (int j = 0; j < cols; ++j) {
          int length = readInt32();
          q->cb5(q->ctx, rowNum++, j, (char *)readString(length).c_str(),
                 length);
        }
        break;
      }
  ...
  bool sendParse(LibpqQuery &q) {
    ...
    writeInt16(0, sbuff);
    /*if(nParams>=0) {
            for(std::list<LibpqParam>::iterator it=q.pvals.begin(); it !=
    q.pvals.end(); ++it) { writeInt32(0, sbuff);
            }
    }*/
    if (send(fd, sbuff.c_str(), sbuff.length(), 0) <= 0) {
      perror("Can't send simple query packet to postgres");
  ...
  bool sendBind(LibpqQuery &q) {
    ...
    std::map<std::string, std::string>::iterator it;
    writeInt32(4 + 1 + 1 + q.ps.length() + 2 + 2 * nParams + 2 + 4 * nParams +
                   pvlen + 2,
               sbuff);
    writeString("", sbuff);
    writeString(q.ps, sbuff);
...
public:
  FpgWire(SocketInterface *sif, bool isAsync = false)
      : PgReadTask(sif), pos(0), state('0'), pstat(false), bstat(false),
        rowNum(-1), querystatus(false) {
    this->isAsync = isAsync;
  }
  bool connect(std::string url, bool isAsync) {
    std::vector<std::string> parts =
        StringUtil::splitAndReturn<std::vector<std::string>>(url, " ");
    std::string host, database, user, password;
    int port = 5432;
    for (std::string part : parts) {
      std::vector<std::string> kv =
          StringUtil::splitAndReturn<std::vector<std::string>>(part, "=");
      if (StringUtil::toLowerCopy(StringUtil::trimCopy(kv.at(0))) == "host") {
        host = StringUtil::trimCopy(kv.at(1));
      } else if (StringUtil::toLowerCopy(StringUtil::trimCopy(kv.at(0))) ==
                 "user") {
  ...
  bool connect(std::string host, int port, bool isAsync, std::string database,
               std::string user, std::string password) {
    std::string rp = password + user;
    upmd5 =
        CryptoHandler::md5((unsigned char *)rp.data(), (unsigned int)rp.size());
...
public:
  ...
  void postAsync(LibpqAsyncReq *vitem,
                 int numQ); // post async request with n number of multi queries
  // Asynchronous mode operations, NOT THREAD SAFE
