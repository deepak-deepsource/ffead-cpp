...
DSType LibpqDataSourceImpl::getType() { return SD_RAW_SQLPG; }

LibpqDataSourceImpl::LibpqDataSourceImpl(const std::string &url, bool isAsync,
                                         bool isBatch, bool isWire) {
  this->url = url;
#ifdef HAVE_LIBPQ
  conn = NULL;
#else
  if (!isWire) {
    fprintf(stderr, "No libpq found and isWire is set to false, will not "
                    "proceed further\n");
    exit(0);
  }
#endif
...
bool LibpqDataSourceImpl::init() {
  if (isWire) {
    ...
    this->fd = wire->fd;
    if (isAsync) {
      bool rHandler = RequestReaderHandler::getInstance() != NULL ||
                      RequestHandler2::getInstance() != NULL ||
                      Writer::isPicoEvAsyncBackendMode;
      if (rHandler) {
        ...
        if (Writer::isPicoEvAsyncBackendMode) {
          ...
          RequestReaderHandler::getInstance()->selector.registerRead(
              this, false, false, true);
        } else {
          RequestHandler2::getInstance()->selector.registerRead(this, false,
                                                                false, true);
        }
#endif
      } else {
  ...
  if (isAsync) {
    ...
    if (rHandler) {
      ...
      if (Writer::isPicoEvAsyncBackendMode) {
        ...
        RequestReaderHandler::getInstance()->selector.registerRead(this, false,
                                                                   false, true);
      } else {
        RequestHandler2::getInstance()->selector.registerRead(this, false,
                                                              false, true);
    ...
    if (q->isPrepared && !q->prepared) {
      ...
      if (PQresultStatus(res) != PGRES_TUPLES_OK) {
        ...
        case 1: {
          ...
          for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; ++j) {
              q->cb1(q->ctx, (i == rows - 1 && j == cols - 1), i, j,
        ...
        case 2: {
          ...
          for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; ++j) {
              q->cb2(q->ctx, (i == rows - 1 && j == cols - 1), i, j,
        ...
        case 3: {
          ...
          for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; ++j) {
              q->cb3(q->ctx, (i == rows - 1 && j == cols - 1), i, j,
                     PQgetvalue(res, i, j));
            }
          }
          PQclear(res);
        ...
        case 4: {
          ...
          for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; ++j) {
              q->cb4(q->ctx, i, j, PQfname(res, j), PQgetvalue(res, i, j),
                     PQgetlength(res, i, j));
            }
          }
          PQclear(res);
        ...
        case 5: {
          ...
          for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; ++j) {
              q->cb5(q->ctx, i, j, PQgetvalue(res, i, j),
                     PQgetlength(res, i, j));
            }
          }
          PQclear(res);
    ...
    if (ritem->q.size() > 0) {
      ...
      if (q->isPrepared) {
        if ((it = ths->prepStmtMap.find(q->query)) == ths->prepStmtMap.end()) {
          ...
          ritem->cnt++;
          ths->prepStmtMap[q->query] =
              CastUtil::fromNumber(ths->prepStmtMap.size() + 1);
          int qs = PQsendPrepare(ths->conn, ths->prepStmtMap[q->query].c_str(),
                                 q->query.c_str(), psize, NULL);
      ...
      if (q->prepared) {
        if (psize == 0) {
          qs = PQsendQueryPrepared(ths->conn, it->second.c_str(), psize, NULL,
                                   NULL, NULL, 1);
        } else {
          ...
          int var = 0;
          for (std::list<LibpqParam>::iterator it = q->pvals.begin();
               it != q->pvals.end(); ++it, var++) {
            if (it->t == 1) { // short
          ...
          qs = PQsendQueryPrepared(ths->conn, it->second.c_str(), psize,
                                   paramValues, paramLengths, paramBinary, 1);
        }
      } else {
        if (psize == 0) {
          qs = PQsendQueryParams(ths->conn, q->query.c_str(), psize, NULL, NULL,
                                 NULL, NULL, 1);
        } else {
          const char *paramValues[psize];
          int paramLengths[psize];
          ...
          qs = PQsendQueryParams(ths->conn, q->query.c_str(), psize, NULL,
                                 paramValues, paramLengths, paramBinary, 1);
        }
      }
...
PGresult *LibpqDataSourceImpl::executeSync(LibpqQuery *q) {
  ...
  if (q->isPrepared) {
    ...
    if ((it = prepStmtMap.find(q->query)) == prepStmtMap.end()) {
      it = prepStmtMap
               .insert(std::pair<std::string, std::string>(
                   q->query, CastUtil::fromNumber(prepStmtMap.size() + 1)))
               .first;
      // prepStmtMap[q->query] = CastUtil::fromNumber(prepStmtMap.size()+1);
      // it = prepStmtMap.find(q->query);
      PGresult *res = PQprepare(conn, prepStmtMap[q->query].c_str(),
                                q->query.c_str(), paramSize, NULL);
      if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        fprintf(stderr, "PREPARE failed: %s\n", PQerrorMessage(conn));
        PQclear(res);
        return NULL;
    ...
    if (paramSize == 0) {
      PGresult *res =
          PQexecPrepared(conn, it->second.c_str(), 0, NULL, NULL, NULL, 1);
      return res;
    } else {
      ...
      PGresult *res = PQexecPrepared(conn, it->second.c_str(), paramSize,
                                     paramValues, paramLengths, paramBinary, 1);
      return res;
    }
  } else {
    if (paramSize > 0) {
      ...
      PGresult *res = PQexecParams(conn, q->query.c_str(), paramSize, NULL,
                                   paramValues, paramLengths, paramBinary, 1);
      return res;
    } else {
      PGresult *res =
          PQexecParams(conn, q->query.c_str(), 0, NULL, NULL, NULL, NULL, 1);
      return res;
    }
  }
}
...
void LibpqDataSourceImpl::executeQuery(LibpqQuery *q) {
  ...
  case 1: {
    ...
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; ++j) {
        q->cb1(q->ctx, (i == rows - 1 && j == cols - 1), i, j, PQfname(res, j),
               PQgetvalue(res, i, j), PQgetlength(res, i, j));
      }
    }
    break;
...
void PgBatchReadTask::run() {
  ...
  while (true) {
    ...
    if (PQisBusy(ths->conn) == 1) {
      /*if (!PQconsumeInput(ths->conn)) {
              fprintf(stderr, "Failed to consume pg input: %s\n",
      PQerrorMessage(ths->conn)); throw std::runtime_error("Invalid connection
      state");
      }
      if(PQisBusy(ths->conn)==1) {
              return;
  ...
  if (sendBatch && lQ.size() > 0) {
    ...
    if (numQueriesInBatch > 0) {
      ...
      if (PQbatchSendQueue(ths->conn) == 0) {
        ...
        // fprintf(stdout, "processPending:PQbatchSendQueue from submit %d,
        // Batch Query Q size %d\n", numQueriesInBatch, ths->Q.size());
        // PQfinish(ths->conn);
        // return;
      }
      PQflush(ths->conn);
  ...
  if (nitem != NULL) {
    if (!queueEntries && sendBatch) {
      ...
      if (PQbatchSendQueue(ths->conn) == 0) {
        ...
        // fprintf(stdout, "submit:PQbatchSendQueue from submit %d, Batch Query
        // Q size %d\n", numQueriesInBatch, ths->Q.size());
      }
      PQflush(ths->conn);
      queueEntries = true;
...
void PgBatchReadTask::batchQueries(LibpqAsyncReq *nitem,
                                   int &numQueriesInBatch) {
#ifdef HAVE_LIBPQ
  LibpqDataSourceImpl *ths = (LibpqDataSourceImpl *)sif;
...
LibpqQuery &LibpqQuery::withSelectQuery(const std::string &query,
                                        bool isPrepared) {
  this->query = query;
  this->isSelect = true;
...
LibpqQuery &LibpqQuery::withUpdateQuery(const std::string &query,
                                        bool isPrepared) {
  this->query = query;
  this->isSelect = false;
  this->isPrepared = this->isMulti ? false : isPrepared;
...
void LibpqAsyncReq::pop() { q.pop_front(); }
