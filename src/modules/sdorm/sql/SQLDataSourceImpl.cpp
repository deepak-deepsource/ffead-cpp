...
DSType SQLDataSourceImpl::getType() { return SD_ORM_SQL; }
...
Query SQLDataSourceImpl::fromQueryBuilder(QueryBuilder &qb) {
  ...
  if (qb.getGroup().getConditions().getConds().size() > 0) {
    qs.append(" HAVING ");
    for (int var = 0;
         var < (int)qb.getGroup().getConditions().getConds().size(); ++var) {
...
void SQLDataSourceImpl::executePreTable(DataSourceEntityMapping &dsemp,
                                        GenericObject &idv) {
  long long id = -1;
  if (dsemp.getIdgendbEntityType() == "table") {
    ...
    if (dsemp.getIdgentype() == "hilo" || dsemp.getIdgentype() == "multihilo") {
      ...
      if (dsemp.getIdgentype() == "multihilo") {
        ...
        query = DialectHelper::getSQLString(
            dialect, DialectHelper::IDGEN_TABLESELMHL_QUERY, params);
        q.getPropPosVaues()[1].set(dsemp.getTableName());
      } else {
        query = DialectHelper::getSQLString(
            dialect, DialectHelper::IDGEN_TABLESEL_QUERY, params);
      }
    } else {
...
void SQLDataSourceImpl::executePostTable(DataSourceEntityMapping &dsemp,
                                         GenericObject &idv) {
  if (dsemp.getIdgendbEntityType() == "table") {
    std::string query;
    Query q;
    if (dsemp.getIdgentype() == "hilo" || dsemp.getIdgentype() == "multihilo") {
      ...
      if (dsemp.getIdgentype() == "multihilo") {
        params["entity_column"] = dsemp.getIdgenentityColumn();
        query = DialectHelper::getSQLString(
            dialect, DialectHelper::IDGEN_TABLEUPDMHL_QUERY, params);
        q.getPropPosVaues()[3].set(dsemp.getTableName());
      } else {
        query = DialectHelper::getSQLString(
            dialect, DialectHelper::IDGEN_TABLEUPD_QUERY, params);
      }
    } else {
...
void SQLDataSourceImpl::executeSequence(DataSourceEntityMapping &dsemp,
                                        GenericObject &idv) {
  long long id = -1;
  if (dsemp.getIdgendbEntityType() == "sequence") {
    StringContext params;
    params["seq_name"] = dsemp.getIdgendbEntityName();
    std::string query = DialectHelper::getSQLString(
        dialect, DialectHelper::IDGEN_SEQUENCE_QUERY, params);
...
void SQLDataSourceImpl::executeIdentity(DataSourceEntityMapping &dsemp,
                                        GenericObject &idv) {
  long long id = -1;
...
SQLDataSourceImpl::SQLDataSourceImpl(ConnectionPooler *pool, Mapping *mapping) {
  ...
  std::map<std::string, std::string> props =
      pool->getProperties().getProperties();
...
SQLDataSourceImpl::~SQLDataSourceImpl() { closeConn(); }

void SQLDataSourceImpl::executeCustom(DataSourceEntityMapping &dsemp,
...
void *SQLDataSourceImpl::getElements(Query &q, SQLSMALLINT &V_OD_colanz,
                                     SQLLEN &V_OD_rowanz) {
  std::vector<std::string> cols;
  return getElements(cols, q, V_OD_colanz, V_OD_rowanz);
}

void *SQLDataSourceImpl::getElements(const std::vector<std::string> &cols,
                                     Query &q, SQLSMALLINT &V_OD_colanz,
                                     SQLLEN &V_OD_rowanz) {
#ifdef HAVE_LIBODBC
  ...
  DataSourceEntityMapping &dsemp =
      mapping->getDataSourceEntityMapping(clasName);
  ...
  for (int k = 0; k < V_OD_rowanz; k++) {
    ...
    for (int i = 1; i <= V_OD_colanz; i++) {
      ...
      std::string fieldName =
          this->mapping->getPropertyForColumn(thisTableName, columnName);
        ...
        for (int var1 = 0; var1 < (int)relv.size(); ++var1) {
          ...
          if (relation.getType() == 1 || relation.getType() == 2) {
            ...
            std::string tableName1 =
                this->mapping->getTableForClass(relation.getClsName());
            fieldName =
                this->mapping->getPropertyForColumn(tableName1, columnName);
            if (fieldName != "" && tableName1 == thisTableName) {
              thisTableName = tableName1;
              args argus1;
              ClassInfo *clas1 =
                  reflector->getClassInfo(relation.getClsName(), appName);
              if (relation.getType() == 1) {
                ...
                if (rel2Vecmap.find(fldVal) == rel2Vecmap.end()) {
                  void *dvecT = reflector->getNewContainer(
                      relation.getClsName(), "std::vector", appName);
    ...
    if (fldVal != "" && rel2reltype[fldVal] == 2 &&
        rel2Vecmap.find(fldVal) != rel2Vecmap.end() &&
        fldvalrel2clsmap.find(fldVal) != fldvalrel2clsmap.end()) {
      reflector->addToContainer(rel2Vecmap[fldVal], relVecEle,
                                fldvalrel2clsmap[fldVal], "std::vector",
                                appName);
      delete relVecEle;
    }
    rel2Insmap[fldVal] = t;
  ...
  if (fldvalrel2clsmap.size() > 0) {
    ...
    for (fldvalrel2clsmapit = fldvalrel2clsmap.begin();
         fldvalrel2clsmapit != fldvalrel2clsmap.end(); ++fldvalrel2clsmapit) {
      ...
      if (rvect != NULL) {
        ...
        std::string methname =
            "set" + StringUtil::capitalizedCopy(fldnmrel2clsmap[flv]);
        Method meth = clas->getMethod(methname, argus);
        reflector->invokeMethodGVP(rel2Insmap[flv], meth, valus);
        reflector->addToContainer(vecT, rel2Insmap[flv], clasName,
                                  "std::vector", appName);
        reflector->destroy(rel2Insmap[flv], clasName, appName);
        if (rel2reltype[flv] == 2)
          reflector->destroyContainer(rvect, fldvalrel2clsmapit->second,
                                      "std::vector", appName);
        else
...
void *SQLDataSourceImpl::getElements(SQLSMALLINT &V_OD_colanz,
                                     SQLLEN &V_OD_rowanz) {
#ifdef HAVE_LIBODBC
  int V_OD_erg; // result of functions
  SQLCHAR colName[256];
  ...
  std::vector<std::map<std::string, GenericObject>> *vecT =
      new std::vector<std::map<std::string, GenericObject>>;
  ...
  for (int k = 0; k < V_OD_rowanz; k++) {
    ...
    for (int i = 1; i <= V_OD_colanz; i++) {
      V_OD_erg =
          SQLDescribeCol(V_OD_hstmt, i, colName, 255, &colNameLen, &dataType,
                         &columnSize, &numDecimalDigits, &allowsNullValues);
...
int SQLDataSourceImpl::storeProperty(ClassInfo *clas, void *t, int var,
  ...
  case SQL_SMALLINT: {
    unsigned short us;
    ret = SQLGetData(V_OD_hstmt, var + 1, SQL_C_USHORT, &us, sizeof(us),
                     &indicator);
  ...
  case SQL_INTEGER: {
    unsigned int us;
    ret = SQLGetData(V_OD_hstmt, var + 1, SQL_C_ULONG, &us, sizeof(us),
                     &indicator);
    fldVal = CastUtil::fromNumber(us);
    dn = us;
    break;
  ...
  case SQL_TINYINT: {
    unsigned char us;
    ret = SQLGetData(V_OD_hstmt, var + 1, SQL_C_UTINYINT, &us, sizeof(us),
                     &indicator);
    fldVal = CastUtil::fromNumber(us);
    dn = us;
    break;
  ...
  case SQL_BIGINT: {
    ...
    ret = SQLGetData(V_OD_hstmt, var + 1, SQL_C_UBIGINT, &us, sizeof(us),
                     &indicator);
    fldVal = CastUtil::fromNumber(us);
    dn = us;
    break;
  ...
  case SQL_REAL: {
    ret = SQLGetData(V_OD_hstmt, var + 1, SQL_C_FLOAT, &df, sizeof(df),
                     &indicator);
  ...
  case SQL_FLOAT:
  case SQL_DOUBLE: {
    ret = SQLGetData(V_OD_hstmt, var + 1, SQL_C_DOUBLE, &dd, sizeof(dd),
                     &indicator);
  ...
  case SQL_BIT: {
    ret =
        SQLGetData(V_OD_hstmt, var + 1, SQL_C_BIT, &db, sizeof(db), &indicator);
  ...
  case SQL_DATE: {
    DATE_STRUCT ds;
    ret = SQLGetData(V_OD_hstmt, var + 1, SQL_C_TYPE_DATE, &ds, sizeof(ds),
                     &indicator);
  ...
  case SQL_TIME: {
    TIME_STRUCT ts;
    ret = SQLGetData(V_OD_hstmt, var + 1, SQL_C_TYPE_TIME, &ts, sizeof(ts),
                     &indicator);
  ...
  case SQL_TIMESTAMP: {
    TIMESTAMP_STRUCT ts;
    ret = SQLGetData(V_OD_hstmt, var + 1, SQL_C_TYPE_TIMESTAMP, &ts, sizeof(ts),
                     &indicator);
  ...
  case SQL_WLONGVARCHAR: {
    ...
    while ((ret = SQLGetData(V_OD_hstmt, var + 1, SQL_C_BINARY, buf,
                             sizeof(buf), &indicator)) != SQL_NO_DATA) {
      numBytes =
          (indicator > 1024) || (indicator == SQL_NO_TOTAL) ? 1024 : indicator;
...
void SQLDataSourceImpl::storePropertyInt(ClassInfo *clas, void *t, void *col,
                                         const Field &fe, int &var) {
  if (col != NULL) {
    args argus;
    ...
    valus.push_back(col);
    std::string methname =
        "set" + StringUtil::capitalizedCopy(fe.getFieldName());
    Method meth = clas->getMethod(methname, argus);
...
int SQLDataSourceImpl::getProperty(
    const int &dataType, const int &columnSize,
    std::map<std::string, GenericObject> &colValMap, const std::string &colName,
    const int &var) {
#ifdef HAVE_LIBODBC
  // SQLRETURN ret;
  SQLLEN indicator;
  ...
  case SQL_BIT: {
    bool col;
    /*ret = */ SQLGetData(V_OD_hstmt, var + 1, SQL_C_BIT, &col, sizeof(col),
                          &indicator);
    colValMap[colName].set(col);
  } break;
  case SQL_REAL: {
    float col;
    /*ret = */ SQLGetData(V_OD_hstmt, var + 1, SQL_C_FLOAT, &col, sizeof(col),
                          &indicator);
    colValMap[colName].set(col);
  } break;
  case SQL_DECIMAL:
  case SQL_DOUBLE: {
    double col;
    /*ret = */ SQLGetData(V_OD_hstmt, var + 1, SQL_C_DOUBLE, &col, sizeof(col),
                          &indicator);
    colValMap[colName].set(col);
  } break;
  case SQL_VARCHAR:
  ...
  default: {
    char buf[24];
    std::string temp;
    /*ret = */ SQLGetData(V_OD_hstmt, var + 1, SQL_C_CHAR, buf, sizeof(buf),
                          &indicator);
    temp.append(buf);
    if (indicator > (long)24) {
      int len = indicator - 24;
      char buf1[len];
      /*ret = */ SQLGetData(V_OD_hstmt, var + 1, SQL_C_CHAR, buf1, sizeof(buf1),
                            &indicator);
      temp.append(buf1);
    }
  ...
  case SQL_BINARY: {
    ...
    BinaryData temp;
    /*ret = */ SQLGetData(V_OD_hstmt, var + 1, SQL_C_BINARY, buf, sizeof(buf),
                          &indicator);
    if (indicator > (long)24) {
      int len = indicator - 24;
      unsigned char buf1[len];
      /*ret = */ SQLGetData(V_OD_hstmt, var + 1, SQL_C_BINARY, buf1,
  ...
  case SQL_INTEGER: {
    if (columnSize > 10) {
      long col;
      /*ret = */ SQLGetData(V_OD_hstmt, var + 1, SQL_C_LONG, &col, sizeof(col),
                            &indicator);
      colValMap[colName].set(col);
    } else {
      int col;
...
void *SQLDataSourceImpl::executeQueryObject(Query &cquery) {
  ...
  for (int var1 = 0; var1 < (int)relv.size(); var1++) {
    ...
    if (relation.getType() == 1 || relation.getType() == 2) {
      ...
      if (tableName1 != "") {
        query += ",";
        DataSourceEntityMapping &rdsemp =
            mapping->getDataSourceEntityMapping(relation.getClsName());
        ...
        for (it = tabcolmap.begin(); it != tabcolmap.end(); it++) {
          ...
          if (var++ != fldsiz)
            query += ",";
          cquery.aliasedColumns.push_back(relation.getClsName() + "." +
                                          columnName);
        }
        reltabs +=
            "left outer join " + tableName1 + " " + relation.getClsName() + " ";
...
void SQLDataSourceImpl::bindQueryParams(Query &query) {
    ...
    while (counter-- > 0 && queryString.find(":") != std::string::npos) {
      for (ite = qparams.begin(); ite != qparams.end(); ++ite) {
        if (queryString.find(":") != std::string::npos &&
            queryString.find(":" + ite->first) != queryString.find(":")) {
          propPosVauesT[posst++] = ite->second;
          queryString = queryString.substr(0, queryString.find(":")) + "?" +
                        queryString.substr(queryString.find(":" + ite->first) +
                                           ite->first.length() + 1);
        }
      }
    }
  ...
  while (totalParams-- > 0) {
    if (propPosVaues->find(par) == propPosVaues->end())
      throw("No parameter value found for position " +
            CastUtil::fromNumber(par));

    GenericObject *paramValue = &(propPosVaues->find(par)->second);
    if (paramValue->getTypeName() == "short") {
      short *sv;
      paramValue->get(sv);
      V_OD_erg = SQLBindParameter(V_OD_hstmt, par, SQL_PARAM_INPUT, SQL_C_SHORT,
                                  SQL_SMALLINT, 0, 0, sv, 20, NULL);
      ...
      paramValue->get(sv);
      V_OD_erg =
          SQLBindParameter(V_OD_hstmt, par, SQL_PARAM_INPUT, SQL_C_USHORT,
                           SQL_SMALLINT, 0, 0, sv, 20, NULL);
      if (!SQL_SUCCEEDED(V_OD_erg)) {
        showError("SQLBindParameter", V_OD_hstmt, SQL_HANDLE_STMT);
        close();
      }
    } else if (paramValue->getTypeName() == "int" ||
               paramValue->getTypeName() == "long") {
      long *sv;
      paramValue->get(sv);
      V_OD_erg = SQLBindParameter(V_OD_hstmt, par, SQL_PARAM_INPUT, SQL_C_LONG,
                                  SQL_INTEGER, 0, 0, sv, 20, NULL);
      if (!SQL_SUCCEEDED(V_OD_erg)) {
        showError("SQLBindParameter", V_OD_hstmt, SQL_HANDLE_STMT);
        close();
      }
    } else if (paramValue->getTypeName() == "unsigned int" ||
               paramValue->getTypeName() == "unsigned long") {
      unsigned long *sv;
      paramValue->get(sv);
      V_OD_erg = SQLBindParameter(V_OD_hstmt, par, SQL_PARAM_INPUT, SQL_C_ULONG,
                                  SQL_INTEGER, 0, 0, sv, 20, NULL);
      if (!SQL_SUCCEEDED(V_OD_erg)) {
        showError("SQLBindParameter", V_OD_hstmt, SQL_HANDLE_STMT);
        close();
               ...
               paramValue->getTypeName() == "unsigned long long") {
      long long *sv;
      paramValue->get(sv);
      V_OD_erg = SQLBindParameter(V_OD_hstmt, par, SQL_PARAM_INPUT, SQL_C_ULONG,
                                  SQL_INTEGER, 0, 0, sv, 20, NULL);
      ...
      V_OD_erg = SQLBindParameter(V_OD_hstmt, par, SQL_PARAM_INPUT,
                                  SQL_C_DOUBLE, SQL_DOUBLE, 0, 0, sv, 20, NULL);
      if (!SQL_SUCCEEDED(V_OD_erg)) {
        showError("SQLBindParameter", V_OD_hstmt, SQL_HANDLE_STMT);
        close();
      }
      ...
      float *sv;
      paramValue->get(sv);
      V_OD_erg = SQLBindParameter(V_OD_hstmt, par, SQL_PARAM_INPUT, SQL_C_FLOAT,
                                  SQL_REAL, 0, 0, sv, 20, NULL);
      if (!SQL_SUCCEEDED(V_OD_erg)) {
        showError("SQLBindParameter", V_OD_hstmt, SQL_HANDLE_STMT);
        close();
      }
    } else if (paramValue->getTypeName() == "std::string" ||
               paramValue->getTypeName() == "string") {
      std::string *parm;
      paramValue->get(parm);
      V_OD_erg = SQLBindParameter(V_OD_hstmt, par, SQL_PARAM_INPUT, SQL_C_CHAR,
                                  SQL_LONGVARCHAR, 0, 0, (void *)parm->c_str(),
                                  parm->length(), NULL);
      if (!SQL_SUCCEEDED(V_OD_erg)) {
        showError("SQLBindParameter", V_OD_hstmt, SQL_HANDLE_STMT);
        close();
...
void SQLDataSourceImpl::showError(const char *fn, const SQLHANDLE &handle,
...
bool SQLDataSourceImpl::startTransaction() {
#ifdef HAVE_LIBODBC
  if (!DialectHelper::isTransactionSupported(dialect))
    return false;
  ...
  SQLRETURN sqlreturn = SQLSetConnectAttr(V_OD_hdbc, SQL_ATTR_AUTOCOMMIT,
...
bool SQLDataSourceImpl::commit() {
  ...
  SQLSetConnectAttr(V_OD_hdbc, SQL_ATTR_AUTOCOMMIT, (void *)SQL_AUTOCOMMIT_ON,
                    0);
  close();
  return flagc;
#endif
...
void SQLDataSourceImpl::procedureCall(const std::string &procName) {
  ...
  bool flagc = allocateStmt(true);
  if (!flagc)
    throw std::runtime_error("Error getting Database connection");
  int V_OD_erg; // result of functions
  SQLCHAR *query;
  ...
  for (it = ntmap.begin(); it != ntmap.end(); ++it) {
    if (it->second == "IN" || it->second == "in") {
      quer += "?";
    } else if (it->second == "OUT" || it->second == "out" ||
               it->second == "INOUT" || it->second == "inout") {
      quer += ("@" + it->first);
      outq = true;
  ...
  for (it = ntmap.begin(); it != ntmap.end(); ++it) {
    if (it->second == "INOUT" || it->second == "inout") {
      if (params[it->first].getTypeName() == "int") {
        ...
        V_OD_erg =
            SQLBindParameter(V_OD_hstmt, par++, SQL_PARAM_INPUT_OUTPUT,
                             SQL_C_LONG, SQL_INTEGER, 0, 0, &sv, 20, NULL);
        if (!SQL_SUCCEEDED(V_OD_erg)) {
          showError("SQLBindParameter", V_OD_hstmt, SQL_HANDLE_STMT);
          close();
  ...
  for (it = ntmap.begin(); it != ntmap.end(); ++it) {
    ...
    if (it->second == "IN" || it->second == "in") {
      if (params[it->first].getTypeName() == "int") {
        ...
        params[it->first].get(sv);
        V_OD_erg =
            SQLBindParameter(V_OD_hstmt, par, SQL_PARAM_INPUT, SQL_C_LONG,
                             SQL_INTEGER, 0, 0, &sv, 20, NULL);
        if (!SQL_SUCCEEDED(V_OD_erg)) {
          showError("SQLBindParameter", V_OD_hstmt, SQL_HANDLE_STMT);
          close();
        ...
        params[it->first].get(sv);
        V_OD_erg =
            SQLBindParameter(V_OD_hstmt, par, SQL_PARAM_INPUT, SQL_C_SHORT,
                             SQL_SMALLINT, 0, 0, &sv, 20, NULL);
        if (!SQL_SUCCEEDED(V_OD_erg)) {
          showError("SQLBindParameter", V_OD_hstmt, SQL_HANDLE_STMT);
          close();
          throw std::runtime_error("Error Binding parameter");
                 ...
                 params[it->first].getTypeName() == "string") {
        std::string sv;
        params[it->first].get(sv);
        V_OD_erg = SQLBindParameter(
            V_OD_hstmt, par, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR, 0, 0,
            (SQLPOINTER)sv.c_str(), sv.length(), &hotelInd);
        if (!SQL_SUCCEEDED(V_OD_erg)) {
          showError("SQLBindParameter", V_OD_hstmt, SQL_HANDLE_STMT);
          close();
          throw std::runtime_error("Error Binding parameter");
...
std::vector<std::map<std::string, GenericObject>>
SQLDataSourceImpl::execute(Query &query) {
  std::vector<std::map<std::string, GenericObject>> tv;
  void *temp = executeQueryInternal(query, false);
  if (temp != NULL) {
    if (!query.isUpdate()) {
...
std::vector<std::map<std::string, GenericObject>>
SQLDataSourceImpl::execute(QueryBuilder &qb) {
...
bool SQLDataSourceImpl::executeInsert(Query &cquery, void *entity) {
  ...
  for (it = fields.begin(); it != fields.end(); ++it, counter++) {
    ...
    if (fld.getType() == "short" || fld.getType() == "unsigned short" ||
        ...
        fld.getType() == "float" || fld.getType() == "double" ||
        fld.getType() == "string" || fld.getType() == "std::string" ||
        fld.getType() == "bool" || fld.getType() == "long long" ||
        fld.getType() == "unsigned long long" || fld.getType() == "Date" ||
  ...
  /*if(remFields.size()>0) {
          for (int i=0;i<(int)remFields.size();i++) {
                  for (int j=0;j<(int)relv.size();j++) {
                          ...
                          if(relation.getField()==remFields.at(i).getFieldName()
  && (relation.getType()==1 || relation.getType()==2))
                          {
                                  args argus;
                                  std::vector<void *> valus;
                                  std::string methname =
  "get"+StringUtil::capitalizedCopy(relation.getField()); Method meth =
  clas->getMethod(methname,argus); std::string clasName = relation.getClsName();
                                  Query iq("", clasName);
                                  std::vector<std::string> icols;
                                  void* ival =
  reflector->invokeMethodGVP(entity,meth,valus,appName);
                                  if(relation.getType()==1)
                                  {
                                          //ffl &= executeInsert(iq, icols,
  ival);
                                  }
                                  else
                                  {
...
bool SQLDataSourceImpl::isGetDbEntityForBulkInsert() { return false; }

void *SQLDataSourceImpl::getDbEntityForBulkInsert(void *entity,
                                                  const std::string &clasName,
                                                  std::string &error) {
  return NULL;
}

bool SQLDataSourceImpl::executeInsertBulk(Query &query,
...
bool SQLDataSourceImpl::executeUpdateBulk(Query &query,
                                          std::vector<void *> entities,
                                          std::vector<void *> dbEntities) {
  for (unsigned int k = 0; k < entities.size(); k++) {
    query.getColumnBindings().clear();
...
void *SQLDataSourceImpl::executeQueryInternal(Query &query, const bool &isObj) {
  ...
  if (!query.isUpdate()) {
    if (query.getStart() > 0 && query.getCount() > 0) {
      ...
      query.setQuery(DialectHelper::getSQLString(
          dialect, DialectHelper::PAGINATION_OFFSET_SQL, query.getQuery(),
          params));
    } else if (query.getCount() > 0) {
      StringContext params;
      params["count"] = CastUtil::fromNumber(query.getCount());
      query.setQuery(DialectHelper::getSQLString(
          dialect, DialectHelper::PAGINATION_NO_OFFSET_SQL, query.getQuery(),
          params));
    }
  }
  ...
  if (query.getPropPosVaues().size() > 0 &&
      query.getPropNameVaues().size() > 0) {
  ...
  std::string queryString = query.getQuery();
  if (StringUtil::toLowerCopy(query.getQuery()).find(" where ") ==
          std::string::npos &&
      query.getColumnBindings().size() > 0) {
    queryString += " where ";
    ...
    int counter = 1;
    Parameters::iterator ite;
    for (ite = query.getColumnBindings().begin();
         ite != query.getColumnBindings().end(); ++ite) {
      position++;
  ...
  if (StringUtil::toLowerCopy(pool->getProperties().getProperty("logsql")) ==
      "true") {
    ...
    while (totalParams-- > 0) {
      if (query.getPropPosVaues().find(par) != query.getPropPosVaues().end()) {
        out += (" (" +
                query.getPropPosVaues().find(par)->second.getSerilaizedState() +
                ")");
      }
      par++;
    }
  ...
  if (query.isUpdate()) {
    ...
    V_OD_erg =
        SQLPrepare(V_OD_hstmt, (SQLCHAR *)query.getQuery().c_str(), SQL_NTS);
    if (!SQL_SUCCEEDED(V_OD_erg)) {
