...
// Copied from
// https://github.com/OSEHRA-Sandbox/gtm-bindings/blob/master/Source/GTM.cpp
...
#define RELEASE_FUNCTION_DESCRIPTOR(functionname) delete[] functionname##_cstr;
...
GTM::GTM() {
  ...
  if (NULL == getenv("GTMCI"))
    putenv((char *)"GTMCI=/home/sumeet/gtm_access/gtm_access.ci");
  //
  // Since GT.M changes the terminal attributes, save the attributes of stderr,
  // stdin and stdout in order to restore them before exit
  //
...
std::string GTM::getJsonString(const std::string &globalName) {
  ...
  CALLGTM(gtm_cip(&(this->gtmtoJSON), globalName.c_str(), &(p_value),
                  &errorMessage));
...
std::string GTM::order(const std::string &globalName) {
  ...
  p_value.address = static_cast<gtm_char_t *>(valueOfGlobal);
  p_value.length = maxValueLength;
  CALLGTM(gtm_cip(&(this->gtmorder), globalName.c_str(), &(p_value),
                  &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
...
std::string GTM::query(const std::string &globalName) {
  ...
  p_value.address = static_cast<gtm_char_t *>(valueOfGlobal);
  p_value.length = maxValueLength;
  CALLGTM(gtm_cip(&(this->gtmquery), globalName.c_str(), &(p_value),
                  &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
...
void GTM::executeInTransaction(const std::string &globalName) {
  ...
  errorMessage[0] = '\0';
  CALLGTM(
      gtm_cip(&(this->gtmxecuteintrans), globalName.c_str(), &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
  }
...
std::string GTM::getString(const std::string &globalName) {
  ...
  p_value.length = maxValueLength;
  CALLGTM(
      gtm_cip(&(this->gtmgetS), globalName.c_str(), &(p_value), &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
  }
...
int GTM::getInt(const std::string &globalName) {
  ...
  gtm_int_t p_value;
  CALLGTM(
      gtm_cip(&(this->gtmgetI), globalName.c_str(), &(p_value), &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
  }
...
unsigned int GTM::getUInt(const std::string &globalName) {
  ...
  errorMessage[0] = '\0';
  gtm_uint_t p_value;
  CALLGTM(gtm_cip(&(this->gtmgetUI), globalName.c_str(), &(p_value),
                  &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
...
long GTM::getLong(const std::string &globalName) {
  ...
  gtm_long_t p_value;
  CALLGTM(
      gtm_cip(&(this->gtmgetL), globalName.c_str(), &(p_value), &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
  }
...
unsigned long GTM::getULong(const std::string &globalName) {
  ...
  errorMessage[0] = '\0';
  gtm_ulong_t p_value;
  CALLGTM(gtm_cip(&(this->gtmgetUL), globalName.c_str(), &(p_value),
                  &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
...
float GTM::getFloat(const std::string &globalName) {
  ...
  gtm_float_t p_value;
  CALLGTM(
      gtm_cip(&(this->gtmgetF), globalName.c_str(), &(p_value), &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
  }
...
double GTM::getDouble(const std::string &globalName) {
  ...
  CALLGTM(
      gtm_cip(&(this->gtmgetD), globalName.c_str(), &(p_value), &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
  }
...
void GTM::set(const std::string &globalName, const std::string &globalValue) {
  ...
  CALLGTM(
      gtm_cip(&(this->gtmsetS), globalName.c_str(), &(p_value), &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
  }
...
void GTM::set(const std::string &globalName, const int &globalValue) {
  gtm_char_t errorMessage[maxMessageLength];
  errorMessage[0] = '\0';
  CALLGTM(gtm_cip(&(this->gtmsetI), globalName.c_str(), &(globalValue),
                  &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
...
void GTM::set(const std::string &globalName, const unsigned int &globalValue) {
  gtm_char_t errorMessage[maxMessageLength];
  errorMessage[0] = '\0';
  CALLGTM(gtm_cip(&(this->gtmsetUI), globalName.c_str(), &(globalValue),
                  &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
...
void GTM::set(const std::string &globalName, const long &globalValue) {
  gtm_char_t errorMessage[maxMessageLength];
  errorMessage[0] = '\0';
  CALLGTM(gtm_cip(&(this->gtmsetL), globalName.c_str(), &(globalValue),
                  &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
...
void GTM::set(const std::string &globalName, const unsigned long &globalValue) {
  gtm_char_t errorMessage[maxMessageLength];
  errorMessage[0] = '\0';
  CALLGTM(gtm_cip(&(this->gtmsetUL), globalName.c_str(), &(globalValue),
                  &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
...
void GTM::set(const std::string &globalName, const float &globalValue) {
  gtm_char_t errorMessage[maxMessageLength];
  errorMessage[0] = '\0';
  CALLGTM(gtm_cip(&(this->gtmsetF), globalName.c_str(), &(globalValue),
                  &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
...
void GTM::set(const std::string &globalName, const double &globalValue) {
  gtm_char_t errorMessage[maxMessageLength];
  errorMessage[0] = '\0';
  CALLGTM(gtm_cip(&(this->gtmsetD), globalName.c_str(), &(globalValue),
                  &errorMessage));
  if (strlen(errorMessage) != 0) {
    GTMException excp(errorMessage);
    throw(excp);
