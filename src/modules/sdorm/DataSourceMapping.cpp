...
const std::string &DataSourceInternalRelation::getDfk() const { return dfk; }
...
int DataSourceInternalRelation::getType() const { return type; }

void DataSourceInternalRelation::setType(const int &type) { this->type = type; }
...
const std::string &Mapping::getAppName() const { return appName; }

void Mapping::addDataSourceEntityMapping(const DataSourceEntityMapping &dsemp) {
  if (dsemp.getClassName() != "" &&
      dseMap.find(dsemp.getClassName()) == dseMap.end()) {
...
std::string Mapping::getPropertyForColumn(const std::string &tableName,
  ...
  if (tableClassMapping.find(tableName) != tableClassMapping.end()) {
    ...
    if (dsemp.columnPropertyMappingCS.find(columnName) !=
        dsemp.columnPropertyMappingCS.end()) {
      return dsemp.columnPropertyMappingCS[columnName];
    }
    std::string cn = StringUtil::toLowerCopy(columnName);
    if (dsemp.columnPropertyMapping.find(cn) !=
        dsemp.columnPropertyMapping.end()) {
...
void DataSourceEntityMapping::setIdPropertyName(
    const std::string &idPropertyName) {
  this->idPropertyName = idPropertyName;
}

bool DataSourceEntityMapping::isEmbedded() const { return embedded; }
...
void DataSourceEntityMapping::addPropertyColumnMapping(
    const std::string &property, const std::string &column) {
  if (property != "" && column != "") {
...
void DataSourceEntityMapping::addRelation(
    const DataSourceInternalRelation &relation) {
  relations.push_back(relation);
}

std::string
DataSourceEntityMapping::getColumnForProperty(const std::string &property) {
...
std::string
DataSourceEntityMapping::getPropertyForColumn(const std::string &column) {
...
std::vector<DataSourceInternalRelation>
DataSourceEntityMapping::getRelations() {
  return relations;
}

DataSourceEntityMapping &
Mapping::getDataSourceEntityMapping(const std::string &clas) {
  if (dseMap.find(clas) != dseMap.end()) {
    return dseMap[clas];
  }
  return __c;
...
bool DataSourceEntityMapping::isIdGenerate() const { return idGenerate; }

int DataSourceEntityMapping::getIdgenlowValue() const { return idgenlowValue; }
