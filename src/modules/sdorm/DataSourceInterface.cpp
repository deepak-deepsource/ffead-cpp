...
void DataSourceInterface::assignId(DataSourceEntityMapping &dsemp,
                                   ClassInfo *clas, void *entity) {
  GenericObject idv;
  next(dsemp, idv);
  if (!idv.isNull()) {
    Field fld = clas->getField(dsemp.getIdPropertyName());
    std::vector<void *> valus;
    if (GenericObject::isNumber32(idv.getTypeName()) &&
        GenericObject::isNumber(fld.getType())) {
      long *id = new long;
      idv.get(id);
      valus.push_back(id);
    } else if (GenericObject::isNumber64(idv.getTypeName()) &&
               GenericObject::isNumber(fld.getType())) {
      long long *id = new long long;
      idv.get(id);
      valus.push_back(id);
    } else if (GenericObject::isFPN(idv.getTypeName()) &&
               GenericObject::isFPN(fld.getType())) {
      long double *id = new long double;
      idv.get(id);
      valus.push_back(id);
    } else if (GenericObject::isString(idv.getTypeName()) &&
               GenericObject::isString(fld.getType())) {
      std::string *id = new std::string;
      idv.set(id);
      valus.push_back(id);
    } else {
      throw std::runtime_error(
          "Data-Source-Object/Entity types do not match for id property" +
          dsemp.getClassName() + ":" + dsemp.getIdPropertyName());
    }

    args argus;
    argus.push_back(fld.getType());
    std::string methname =
        "set" + StringUtil::capitalizedCopy(fld.getFieldName());
