...
DSType ScyllaDBRawDataSourceImpl::getType() { return SD_RAW_CASS; }
...
bool ScyllaDBRawDataSourceImpl::init() {
  ...
  // CassAuthenticatorCallbacks auth_callbacks = { on_auth_initial,
  // on_auth_challenge, on_auth_success, on_auth_cleanup };
  // cass_cluster_set_contact_points(cluster,
  // props->getNodes().at(0).getHost().c_str());
  // cass_cluster_set_authenticator_callbacks(cluster, &auth_callbacks, NULL,
  // &(props->getNodes().at(0)));
...
void ScyllaDBRawDataSourceImpl::setArgs(CassStatement *statement,
                                        std::list<LibpqParam> &pvals) {
  int var = 0;
  for (std::list<LibpqParam>::iterator it = pvals.begin(); it != pvals.end();
       ++it, var++) {
    switch (it->t) {
    case 1: {
    ...
    case 5: {
      ...
      /*case 6: {
              cass_statement_bind_string_n(statement, var, it->st.c_str(),
      it->st.length()); break;
      }*/
    }
  }
