...
public:
  template <typename Func1> ScyllaQuery &withCb(Func1 cb) {
    this->cb = cb;
    return *this;
  }
...
public:
  ...
  template <typename Func1> void executeQueryAsync(ScyllaQuery &sq, void *arg) {
  ...
  template <typename Func1>
  void executeBatchedQuery(ScyllaQuery &sq, Func1 cb, void *arg,
                           std::vector<std::list<LibpqParam>> &arglst) {
