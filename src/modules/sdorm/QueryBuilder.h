...
public:
  ...
  static QueryClause NONE, LOGICAL_GRP_OPEN, LOGICAL_GRP_CLOSE, AND, OR;
...
public:
  int get() const { return val; }
  static QueryOperator NONE, EQUALS, NOT_EQUALS, IN_, NOT_IN, BETWEEN,
...
public:
  int get() const { return val; }
  static JoinType INNER, LEFT, RIGHT, FULL, NATURAL, CROSS, LEFT_OUTER,
...
public:
  ...
  LogicalGroup &where(const std::string &lhs, const QueryOperator &oper,
                      const GenericObject &rhs);
  LogicalGroup &where(const std::string &lhs, const QueryOperator &oper,
                      const GenericObject &rhs, const GenericObject &rhs1);
  LogicalGroup &where(const std::string &lhs, const QueryOperator &oper,
  ...
  template <class T>
  LogicalGroup &where(const std::string &lhs, const QueryOperator &oper,
                      const T &rhs, const T &rhs1);
  template <class T>
  LogicalGroup &where(const std::string &lhs, const QueryOperator &oper,
                      const std::vector<T> &rhsList);
...
public:
  JoinClause();
  virtual ~JoinClause();
  LogicalGroup &on(const std::string &lhs, const QueryOperator &oper,
                   const GenericObject &rhs);
  QueryBuilder &end();
...
public:
  ...
  JoinClause &joinTable(const JoinType &jt, const std::string &name,
                        const std::string &alias);
  JoinClause &joinClass(const JoinType &jt, const std::string &name,
                        const std::string &alias);
...
template <class T>
inline LogicalGroup &LogicalGroup::where(const std::string &lhs,
