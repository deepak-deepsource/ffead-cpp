...
HttpServiceHandler::HttpServiceHandler(const std::string &cntEncoding,
                                       const webSockHandle &f,
...
int HttpServiceTask::getTid() { return handlerRequest.sif->tid; }

void HttpServiceTask::setTid(int tid) { handlerRequest.sif->tid = tid; }
...
void HttpServiceHandler::handleService(void *request, SocketInterface *sif,
  ...
  // task->handlerRequest.protType =
  // sif->getProtocol(context).find("HTTP")==0?1:2;
  submitTask(task);
}
...
int HttpReadTask::getTid() { return sif->tid; }

void HttpReadTask::setTid(int tid) { sif->tid = tid; }
...
void HttpReadTask::run() {
  ...
  while (pending > 0 &&
         sif->readRequest(hr.request, hr.context, pending, hr.reqPos)) {
    // task->handlerRequest.protType =
    // sif->getProtocol(hr.context).find("HTTP")==0?1:2;
    task->run();
  }
  // to.end();
...
HttpServiceTask::HttpServiceTask() { service = NULL; }
...
std::string HttpServiceTask::getCntEncoding() { return service->cntEncoding; }
  ...
  if (handlerRequest.getProtType() == 1) {
    ...
    if (req->httpVers < 2 && req->isUpgrade()) {
      if (req->isHeaderValue(HttpRequest::Upgrade, "websocket", true) &&
          ...
          (req->hasHeaderValuePart(HttpRequest::SecWebSocketVersion, "7",
                                   false) ||
           req->hasHeaderValuePart(HttpRequest::SecWebSocketVersion, "8",
                                   false) ||
           req->hasHeaderValuePart(HttpRequest::SecWebSocketVersion, "13",
                                   false))) {
        std::string seckey = req->getHeader(HttpRequest::SecWebSocketKey);
        std::string servseckey =
        ...
        res->setDone(true);
#ifdef HAVE_SSLINC
        Http11WebSocketHandler *hws = new Http11WebSocketHandler(
            handlerRequest.getSif()->fd, handlerRequest.getSif()->ssl,
            handlerRequest.getSif()->io, req->getUrl(), true);
#else
        Http11WebSocketHandler *hws = new Http11WebSocketHandler(
            handlerRequest.getSif()->fd, NULL, NULL, req->getUrl(), true);
#endif
        WebSocketData wreq;
        ...
        if (hws->h == NULL) {
          ...
          int ret = handlerRequest.getSif()->pushResponse(
              handlerRequest.getRequest(), handlerRequest.response,
              handlerRequest.getContext(), handlerRequest.reqPos);
          if (ret == 0) {
          ...
          if (!handlerRequest.getSif()->isClosed()) {
            if (!wres.isEmpty()) {
              for (int var = 0; var < (int)wres.getMore().size(); ++var) {
                if (!handlerRequest.getSif()->isClosed() &&
                    wres.getMore()[var].hasData()) {
                  ResponseData rd;
                  handlerRequest.getSif()->writeResponse(
                      &wreq, &wres.getMore()[var], NULL, rd._b, -1);
      ...
      } else if (req->hasHeaderValuePart(HttpRequest::Connection,
...
#ifdef HAVE_SSLINC
        handlerRequest.getSif()->addHandler(new Http2Handler(
            handlerRequest.getSif()->fd, handlerRequest.getSif()->ssl,
            handlerRequest.getSif()->io, true, prev->getWebpath(),
            http2settings));
#else
        handlerRequest.getSif()->addHandler(
            new Http2Handler(handlerRequest.getSif()->fd, NULL, NULL, true,
                             prev->getWebpath(), http2settings));
#endif
      } else {
    ...
    WebSocketRespponseData *response =
        (WebSocketRespponseData *)handlerRequest.getResponse();
    response->reset();
    if (!hws->h->onMessage(request, response,
                           handlerRequest.getSif()->getAddress())) {
      // No Op
    } else if (!hws->h->isWriteControl() &&
               !handlerRequest.getSif()->isClosed()) {
      if (!response->isEmpty()) {
        for (int var = 0; var < (int)response->getMore().size(); ++var) {
          if (!handlerRequest.getSif()->isClosed() &&
              response->getMore()[var].hasData()) {
            ResponseData rd;
            handlerRequest.getSif()->writeResponse(
                request, &response->getMore()[var], NULL, rd._b, -1);
            handlerRequest.getSif()->writeTo(&rd);
          }
        }
      }
...
int HttpWriteTask::getTid() { return sif->tid; }

void HttpWriteTask::setTid(int tid) { sif->tid = tid; }
