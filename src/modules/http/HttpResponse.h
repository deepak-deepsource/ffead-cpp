...
public:
  static void init();
  static std::string AccessControlAllowOrigin, AccessControlAllowHeaders,
      AccessControlAllowCredentials, AccessControlAllowMethods,
      AccessControlMaxAge, AcceptRanges, Age, Allow, CacheControl, Connection,
      ContentEncoding, ContentLanguage, ContentLength, ContentLocation,
      ContentMD5, ContentDisposition, ContentRange, ContentType, DateHeader,
      ETag, Expires, LastModified, Link, Location, P3P, Pragma,
      ProxyAuthenticate, Refresh, RetryAfter, Server, SetCookie, Status,
      StrictTransportSecurity, Trailer, TransferEncoding, Vary, Via, Warning,
      WWWAuthenticate, Upgrade, SecWebSocketAccept, SecWebSocketVersion, AltSvc;
  HttpResponse();
  ...
  void generateResponse(const std::string &httpMethod, HttpRequest *req,
                        std::string &data, const bool &appendHeaders = true);
  std::string generateResponse(const bool &appendHeaders = true);
  void generateResponse(HttpRequest *req, std::string &data,
  ...
  const std::string &getHeadersStr(const std::string &server, bool status_line,
                                   bool with_content, bool with_serverline);
  ...
  HttpResponse &generateHeadResponse(std::string &resp, float httpVers,
                                     bool conn_clos);
  HttpResponse &generateHeadResponseMinimal(std::string &resp,
                                            std::string &contentType,
                                            int content_length = -1,
                                            bool conn_clos = false);
  HttpResponse &generateHeadResponse(std::string &resp,
                                     std::string &contentType,
                                     int content_length = -1);
  HttpResponse &generateHeadResponse(std::string &resp,
                                     std::string &contentType, float httpVers,
                                     bool conn_clos, int content_length = -1);
  void update(HttpRequest *req);

private:
  static RiMap HDRS_SW_CODES;
  static std::map<std::string, std::tuple<std::string, int, int, std::string,
                                          std::string, std::string>>
      HDR_DATA_BY_CNT;
  static const std::string HDR_SRV, HDR_SEP, HDR_SEPT, HDR_END, HDR_CORS_ALW,
      HDR_FIN, CONN_CLOSE, CONN_KAL;
  int fd;
  bool done;
  float httpVers;
  bool conn_clos;
  ...
  std::string getContent(const char *fileName, const int &start = -1,
                         const int &end = -1);
  bool isContentRemains();
  bool getRemainingContent(const std::string &fname, const bool &isFirst,
                           std::string &data);
