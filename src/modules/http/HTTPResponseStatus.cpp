...
void HTTPResponseStatus::init() {
  if (statuses.size() == 0) {
    ...
    for (i = statuses.begin(); i != statuses.end(); ++i) {
      http10slines[i->first] = "HTTP/1.0 " + CastUtil::fromNumber(i->first) +
                               " " + i->second.msg + "\r\n";
      http11slines[i->first] = "HTTP/1.1 " + CastUtil::fromNumber(i->first) +
                               " " + i->second.msg + "\r\n";
    }
  }
}

const std::string &HTTPResponseStatus::getResponseLine(int code,
                                                       float version) {
  if (version < 1.1) {
...
void HTTPResponseStatus::getResponseLine(int code, float version,
                                         std::string &b) {
  if (version < 1.1) {
...
HTTPResponseStatus &
HTTPResponseStatus::getStatusByCode(const std::string &code) {
...
HTTPResponseStatus
    HTTPResponseStatus::NonAuthInfo(203, "Non-authoritative information");
...
HTTPResponseStatus HTTPResponseStatus::MovedPermanently(303,
                                                        "Moved Permanently");
...
HTTPResponseStatus HTTPResponseStatus::InvalidMethod(
    405,
    "HTTP verb used to access this page is not allowed (method not allowed)");
HTTPResponseStatus HTTPResponseStatus::InvalidMime(
    406, "Client browser does not accept the MIME type of the requested page");
HTTPResponseStatus
    HTTPResponseStatus::ProxyAuthRequired(407, "Proxy authentication required");
HTTPResponseStatus
    HTTPResponseStatus::PreconditionFailed(412, "Precondition failed");
HTTPResponseStatus
    HTTPResponseStatus::ReqEntityLarge(413, "Request entity too large");
HTTPResponseStatus HTTPResponseStatus::ReqUrlLarge(414, "Request-URL too long");
HTTPResponseStatus
    HTTPResponseStatus::UnsupportedMedia(415, "Unsupported media type");
HTTPResponseStatus
    HTTPResponseStatus::InvalidReqRange(416, "Requested range not satisfiable");
...
HTTPResponseStatus
    HTTPResponseStatus::InternalServerError(500, "Internal server error");
HTTPResponseStatus HTTPResponseStatus::InvalidHeaderConf(
    501, "Header values specify a configuration that is not implemented");
HTTPResponseStatus HTTPResponseStatus::BadGateway(502, "Bad Gateway");
HTTPResponseStatus
    HTTPResponseStatus::ServiceUnavailable(503, "Service unavailable");
HTTPResponseStatus HTTPResponseStatus::GatewayTimeout(504, "Gateway timeout");
HTTPResponseStatus
    HTTPResponseStatus::HttpVersionNotSupported(505,
...
HTTPResponseStatus::HTTPResponseStatus(const int &code,
...
int HTTPResponseStatus::getCode() const { return code; }

std::string HTTPResponseStatus::getSCode() const { return scode; }

const std::string &HTTPResponseStatus::getMsg() { return msg; }
