...
class Http2HPACKHeaderTable {
  ...
  static std::map<std::string, std::map<int, Bits>>
      HUFFMAN_LK_STRINDX_NUMINDX_TABLE;
  static std::map<std::string, std::map<int, int>>
...
public:
  ...
  int getIndexByNameAndValue(const std::string &name, const std::string &value,
                             const bool &isRequest);
  int getIndexByName(const std::string &name, const bool &isRequest);
  std::map<std::string, std::string>
  getNameAndValueByIndex(const int &index, const bool &isRequest);
...
public:
  ...
  bool decipherHuffmanValue(const int &bitnum, std::bitset<8> obvm,
                            std::bitset<8> &bvm, std::string &out,
                            std::bitset<8> &prev, int &last, uint32_t &indx,
                            std::string &key, std::string &value, int &totbits,
                            int &cub);
  long decodeNumber(const std::string &data, const int &prefixSize,
                    std::bitset<8> bits, size_t &index);
  ...
  Http2HPACKContext();
  std::string
  encode(const std::map<std::string, std::string, std::less<>> &headers);
