...
unsigned int Http2AlternativeServicesFrame::getMaxAge() const { return maxAge; }

const std::string &Http2AlternativeServicesFrame::getOrigin() const {
  return origin;
}

uint16_t Http2AlternativeServicesFrame::getPort() const { return port; }
...
Http2AlternativeServicesFrame::Http2AlternativeServicesFrame(
    const std::string &data, Http2FrameHeader &aheader) {
  header = aheader;
  header.type = 10;
