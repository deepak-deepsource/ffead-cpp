...
bool Http2HeadersFrame::isExclusive() const { return exclusive; }
...
const std::string &Http2HeadersFrame::getPadding() const { return padding; }

unsigned char Http2HeadersFrame::getPadLength() const { return padLength; }

int Http2HeadersFrame::getStreamDependency() const { return streamDependency; }

Http2HeadersFrame::Http2HeadersFrame(std::string data,
  ...
  if (header.isPriority()) {
    ...
    streamDependency =
        (int)CommonUtils::charArrayToULongLong(data.substr(stind, 4));
    stind += 4;
...
unsigned char Http2HeadersFrame::getWeight() const { return weight; }
