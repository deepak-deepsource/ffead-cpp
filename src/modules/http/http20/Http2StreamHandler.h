...
class Http2StreamHandler {
  ...
  void closeConnection(const int &lastStreamIdentifier,
                       Http2ReadWriteUtil *handler);
  bool isWebSocketRequest();
  void *handleWebSocketRequest(Http2HPACKContext *context, Http2Frame *frame,
                               Http2ReadWriteUtil *handler,
                               std::map<uint16_t, uint32_t> &settings);
  void sendPushPromiseFrames(Http2HPACKContext *context, Http2Frame *frame,
                             Http2ReadWriteUtil *handler,
                             std::map<uint16_t, uint32_t> &settings);
...
public:
  Http2StreamHandler();
  Http2StreamHandler(Http2HPACKContext *context, const int &streamIdentifier,
                     const std::string &webpath);
  virtual ~Http2StreamHandler();
  bool handle(Http2Frame *frame, const int &precedingStreamId,
              std::map<uint16_t, uint32_t> &settings,
              Http2ReadWriteUtil *handler, std::map<int, bool> &frameAcks,
              void *&requestObj);
};
