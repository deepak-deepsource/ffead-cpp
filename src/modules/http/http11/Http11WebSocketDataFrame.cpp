...
void Http11WebSocketDataFrame::setApplicationData(
    const std::string &applicationData) {
...
bool Http11WebSocketDataFrame::isFin() const { return fin; }

bool Http11WebSocketDataFrame::isMask() const { return mask; }

uint64_t Http11WebSocketDataFrame::getMaskingKey() const { return maskingKey; }

short Http11WebSocketDataFrame::getOpcode() const { return opcode; }

uint8_t Http11WebSocketDataFrame::getPayloadLength() const {
  return payloadLength;
}

bool Http11WebSocketDataFrame::isRsv1() const { return rsv1; }

bool Http11WebSocketDataFrame::isRsv2() const { return rsv2; }

bool Http11WebSocketDataFrame::isRsv3() const { return rsv3; }
...
void Http11WebSocketDataFrame::getFrameData(std::string &finaldata) {
  ...
  if (this->payloadLength == 126) {
    finaldata.append(
        CommonUtils::ulonglongTocharArray(this->extendedPayloadLength, 2));
  } else if (this->payloadLength == 127) {
    finaldata.append(
        CommonUtils::ulonglongTocharArray(this->extendedPayloadLength, 8));
  }
  if (this->mask) {
    finaldata.append(CommonUtils::ulonglongTocharArray(this->maskingKey, 4));
    this->applicationData =
        CommonUtils::xorEncryptDecrypt(this->applicationData, this->maskingKey);
  }
...
std::string WebSocketData::getTextData() const { return textData; }

std::string WebSocketData::getBinaryData() const { return binaryData; }

std::string WebSocketData::getUrl() const { return url; }

std::string WebSocketData::getCntxt_name() const { return cnxtName; }
...
std::vector<WebSocketData> &WebSocketRespponseData::getMore() { return more; }

bool WebSocketRespponseData::isEmpty() { return more.size() == 0; }

WebSocketRespponseData::WebSocketRespponseData() {}

void WebSocketRespponseData::reset() { more.clear(); }

WebSocketRespponseData::~WebSocketRespponseData() {}

int Http11WebSocketDataFrame::getFramePdu(WebSocketData *wres,
                                          std::string &data, bool copyData) {
