...
#ifdef HAVE_SSLINC
Http11WebSocketHandler::Http11WebSocketHandler(const SOCKET &fd, void *ssl,
                                               void *io, const std::string &url,
                                               const bool &isServer)
...
std::string Http11WebSocketHandler::getUrl() { return this->url; }
...
int Http11WebSocketHandler::getType(void *context) { return 2; }

int Http11WebSocketHandler::getTimeout() { return -1; }
  ...
  if (buffer.length() >= 2) {
    ...
    if (buffer.length() > 4 && frame->payloadLength == 126) {
      frame->extendedPayloadLength =
          CommonUtils::charArrayToULongLong(buffer.substr(2, 2));
      dataLength = frame->extendedPayloadLength;
      count += 2;
    } else if (buffer.length() > 10 && frame->payloadLength == 127) {
      frame->extendedPayloadLength =
          CommonUtils::charArrayToULongLong(buffer.substr(2, 8));
      dataLength = frame->extendedPayloadLength;
      count += 8;
    ...
    if ((frame->mask && buffer.length() >= (dataLength + 4 + count)) ||
        buffer.length() >= (dataLength + count)) {
      if (frame->mask) {
        frame->maskingKey =
            CommonUtils::charArrayToULongLong(buffer.substr(count, 4));
        count += 4;
      }
      frame->applicationData = buffer.substr(count, dataLength);
      if (frame->mask) {
        frame->applicationData = CommonUtils::xorEncryptDecrypt(
            frame->applicationData, frame->maskingKey);
      }
...
bool Http11WebSocketHandler::processFrame(Http11WebSocketDataFrame *frame,
                                          WebSocketData *request, bool &isReq) {
  if (frame->opcode == 8) {
    // closed = true;
    return true;
...
bool Http11WebSocketHandler::readRequest(void *request, void *&context,
...
bool Http11WebSocketHandler::isEmbedded() { return true; }
