...
HttpResponseParser::HttpResponseParser(const std::string &vecstr,
    ...
    for (unsigned int i = 0; i < vec.size(); i++) {
      ...
      if (i == 0) {
        ...
        if (httpst.at(0).find("HTTP") == std::string::npos) {
          ...
          response.setHTTPResponseStatus(
              HTTPResponseStatus::getStatusByCode(httpst.at(1)));
        }
      } else if (vec.at(i).find_first_of(":") != std::string::npos &&
                 !contStarts) {
        if (vec.at(i).find(":") == vec.at(i).find(": ")) {
          temp.push_back(StringUtil::toLowerCopy(
              vec.at(i).substr(0, vec.at(i).find_first_of(": "))));
...
std::string HttpResponseParser::getContent() { return content; }
