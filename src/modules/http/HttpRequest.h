...
public:
  ...
  enum { PREFLIGHT, CORS, OTHER };
  static void init(bool isLazyHeaderParsing = false);
  void normalizeUrl();
  static const std::string DEFAULT_CTX, BLANK;
  static std::string Accept, AcceptCharset, AcceptEncoding, AcceptLanguage,
      AcceptDatetime, AccessControlRequestHeaders, AccessControlRequestMethod,
      Authorization, CacheControl, Connection, Cookie, ContentLength,
      ContentMD5, ContentType, Date, Expect, From, Host, IfMatch,
      IfModifiedSince, IfNoneMatch, IfRange, TransferEncoding,
      IfUnmodifiedSince, MaxForwards, Origin, Pragma, ProxyAuthorization, Range,
      Referer, TE, Upgrade, UserAgent, Via, Warning, SecWebSocketKey,
      SecWebSocketVersion, SecWebSocketAccept, SecWebSocketProtocol,
      SecWebSocketExtensions, AltUsed, Http2Settings;
  HttpRequest();
  HttpRequest(const std::string &);

  HttpRequest(std::string &&data, int *content_length);
  HttpRequest(const char *pp, size_t pl, const char *qp, size_t ql,
              const char *mp, size_t ml, const std::string &content,
              unsigned int hv);
  HttpRequest(const char *mp, size_t ml, const char *pp, size_t pl,
              const char *qp, size_t ql, const char *hp, size_t hl,
              const char *bp, size_t bl, int hv);
  HttpRequest(const char *mp, size_t ml, const char *pp, size_t pl,
              const char *bp, size_t bl, int hv);
  HttpRequest(void *headers_list, size_t num_headers, std::string_view rawUrl,
              std::string_view qv, std::string_view method, int hv,
              std::string_view content);
  HttpRequest(void *headers_list, size_t num_headers, std::string_view rawUrl,
              std::string_view method, int hv, std::string_view content);
  HttpRequest(std::unordered_map<std::string_view, std::string_view> header_map,
              std::string_view url, std::string_view qv,
              std::string_view method, std::string_view hv,
              std::string_view content);
  HttpRequest(const char *cnt, size_t cntlen,
              const std::unordered_map<std::string, std::string> &header_map,
              const std::string &url, const std::string &queryv,
              const char *method, int hv);
  HttpRequest(const char *headers, size_t headers_len, const char *body,
              size_t body_len);
  HttpRequest(std::string_view rawUrl, std::string_view method, int hv,
              const char *headers, size_t headers_len, const char *body,
              size_t body_len);
  ...
  void addNginxApacheHeader(const char *kp, size_t kl, const char *vp,
                            size_t vl);
  std::string toString();
#ifdef INC_SCRH
  ...
  bool isClose();
  bool isKeepAlive();
  bool isHeaderValue(std::string header, const std::string &value,
                     const bool &ignoreCase = true);
  bool hasHeaderValuePart(std::string header, std::string valuePart,
                          const bool &ignoreCase = true);
  std::vector<std::vector<int>> getRanges(std::vector<std::string> &rangesVec);
  std::string getContent_tfile();
  void addMultipartFormContent(const std::string &key,
