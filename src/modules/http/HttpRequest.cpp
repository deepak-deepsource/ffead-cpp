...
std::string HttpRequest::VALID_REQUEST_HEADERS =
    ...
    "content-type,date,expect,from,host,if-match,if-modified-since,if-none-"
    "match,if-range,if-unmodified-since,max-forwards,origin,pragma,proxy-"
    "authorization,range,referer,te,upgrade,user-agent,via,warning,transfer-"
    "encoding,sec-websocket-key,sec-websocket-version,sec-websocket-accept,sec-"
    "websocket-protocol,sec-websocket-extensions,alt-used,http2-settings,";
...
std::string HttpRequest::AccessControlRequestHeaders =
    "Access-Control-Request-Headers";
std::string HttpRequest::AccessControlRequestMethod =
...
void HttpRequest::init(bool isLazyHeaderParsingT) {
  ...
  std::string t =
      VALID_REQUEST_HEADERS.substr(1, VALID_REQUEST_HEADERS.length() - 1);
...
void HttpRequest::getAuthParams(std::string str) {
  ...
  if (strcasecmp(authMethod.c_str(), "basic") == 0) {
    ...
    for (unsigned int i = 0; i < tempv.size(); i++) {
      ...
      if (temr.length() > 1 && (temr.at(0) == '"' || temr.at(0) == '\'') &&
          (temr.at(temr.length() - 1) == '"' ||
           temr.at(temr.length() - 1) == '\'')) {
...
void HttpRequest::getOauthParams(std::string str) {
  ...
  for (unsigned int i = 0; i < tempv.size(); i++) {
    ...
    authinfo[tempvv.at(0)] = temr;
    authorderinf[authorderinf.size() + 1] =
        CryptoHandler::urlDecode(tempvv.at(0));
  }
}

void HttpRequest::addNginxApacheHeader(const char *kp, size_t kl,
...
HttpRequest::HttpRequest(const char *pp, size_t pl, const char *qp, size_t ql,
                         const char *mp, size_t ml, const std::string &data,
                         unsigned int hv) {
...
HttpRequest::HttpRequest(const char *mp, size_t ml, const char *pp, size_t pl,
                         const char *qp, size_t ql, const char *hp, size_t hl,
                         const char *bp, size_t bl, int hv) {
  pathv = std::string_view{pp, pl};
  queryv = ql > 0 ? std::string_view{qp, ql} : std::string_view(BLANK);
  methodv = std::string_view{mp, ml};
  num_headers = 100;
...
HttpRequest::HttpRequest(
    const char *cnt, size_t cntlen,
    const std::unordered_map<std::string, std::string> &header_map,
    const std::string &url, const std::string &query, const char *method,
    int hv) {
  cntxt_root = NULL;
  resp = NULL;
  cookie = false;
  ...
  this->num_headers = 0;
  for (it = header_map.begin(); it != header_map.end();
       ++it, this->num_headers++) {
...
HttpRequest::HttpRequest(
    ...
    std::string_view url, std::string_view qv, std::string_view method,
    std::string_view hv, std::string_view cnt) {
  cntxt_root = NULL;
  resp = NULL;
  cookie = false;
  httpVers = 0;
...
HttpRequest::HttpRequest(void *thdrlist, size_t num_headers,
                         std::string_view rawUrl, std::string_view method,
                         int hv, std::string_view cnt) {
  cntxt_root = NULL;
  resp = NULL;
  cookie = false;
  httpVers = 0;
...
HttpRequest::HttpRequest(void *thdrlist, size_t num_headers,
                         std::string_view rawUrl, std::string_view qv,
                         std::string_view method, int hv,
                         std::string_view cnt) {
  cntxt_root = NULL;
  resp = NULL;
  cookie = false;
...
HttpRequest::HttpRequest(std::string_view rawUrl, std::string_view method,
                         int hv, const char *headers, size_t headers_len,
  ...
  int temp;
  num_headers = 100;
  parser_pos = phr_parse_headers_fcp(headers, headers_len, headers_list,
                                     &num_headers, 0, &temp);
  httpVers = 1 + (float)hv / 10;
  httpVersion = hv == 1 ? "HTTP/1.1" : "HTTP/1.0";
  this->methodv = method;
...
HttpRequest::HttpRequest(const char *headers, size_t headers_len,
                         const char *body, size_t body_len) {
  cntxt_root = NULL;
  resp = NULL;
  cookie = false;
  ...
  if (headers_len > 0) {
    ...
    int content_length;
    num_headers = 100;
    if ((parser_pos = phr_parse_request_fcp(
             headers, headers_len, (const char **)&method, &method_len,
             (const char **)&path, &path_len, &minor_version, headers_list,
             &num_headers, 0, &content_length)) < 0) {
...
HttpRequest::HttpRequest(std::string &&data, int *content_length)
  ...
  if (headers_data.length() > 0) {
    ...
    if ((parser_pos = phr_parse_request_fcp(
             headers_data.c_str(), headers_data.length(),
             (const char **)&method, &method_len, (const char **)&path,
             &path_len, &minor_version, headers_list, &num_headers, 0,
             content_length)) < 0) {
      status = &HTTPResponseStatus::BadRequest;
    } else {
      methodv = std::string_view{method, method_len};
...
bool HttpRequest::parseHeaders(int *content_length) {
  if (headers_data.length() > 0 && httpVers > 0) {
    int r;
    const char *buf = headers_data.c_str() + parser_pos,
               *buf_end = headers_data.c_str() + headers_data.length();
    return parse_headers_fcp(buf, buf_end, headers_list, &num_headers,
                             num_headers, &r, content_length) != NULL;
  }
  return false;
}
  ...
  if (headers_data.length() > 0) {
    ...
    if ((parser_pos =
             phr_parse_request_fcp(
                 ...
                 (const char **)&method, &method_len, (const char **)&path,
                 &path_len, &minor_version, headers_list, &num_headers, 0,
                 content_length, isLazyHeaderParsing) < 0)) {
      status = &HTTPResponseStatus::BadRequest;
    } else {
      methodv = std::string_view{method, method_len};
      pathv = std::string_view{path, path_len};
...
const std::string_view &HttpRequest::getPath() { return pathv; }

const std::string_view &HttpRequest::getQueryStr() { return queryv; }
...
void HttpRequest::updateFromContentStr() {
  ...
  if (this->getHeader(ContentType).find("application/x-www-form-urlencoded") ==
      0) {
    strVec params;
    ...
    for (unsigned j = 0; j < params.size(); j++) {
      ...
      if (param.size() == 2) {
        ...
        StringUtil::replaceFirst(attN, " ", BLANK);
        if (attN.find("[") != std::string::npos &&
            attN.find("]") != std::string::npos) {
  ...
  } else if (this->getContent() != BLANK &&
             this->getContent_boundary() != BLANK) {
    ...
    while (contemp != BLANK) {
      ...
      if (bcontends) {
        ...
        if (content.getName() == BLANK) {
          ...
          if (attN.find("[") != std::string::npos &&
            ...
            addMultipartFormContent(attN.substr(0, attN.find("[") + 1) +
                     ...
                     requestParamsF.find(attN) != requestParamsF.end()) {
            if (requestParamsF.find(attN) != requestParamsF.end()) {
              ...
              addMultipartFormContent(
                  attN + "[" + CastUtil::fromNumber(indices[attN]) + "]",
                  content);
            }
        ...
        if (temp == "\r" || temp == BLANK) {
          ...
          hdrs.push_back(temp);
          if (StringUtil::toLowerCopy(temp).find("content-disposition: ") !=
                  std::string::npos &&
              StringUtil::toLowerCopy(temp).find("filename") !=
                  std::string::npos) {
            filen = this->getContent_boundary();
            StringUtil::replaceAll(filen, "-", BLANK);
            filen = this->getCntxt_root() + "/temp/" + filen +
                    CastUtil::fromNumber(Timer::getCurrentTime());
...
void HttpRequest::buildRequest(std::string key, std::string value) {
  ...
  case 44: {
    ...
    std::string versionStr = StringUtil::replaceFirstCopy(
        StringUtil::toLowerCopy(value), "http/", BLANK);
  ...
  case 47: {
    ...
    if (vemp.size() < 3) {
      ...
      std::string versionStr = StringUtil::replaceFirstCopy(
          StringUtil::toLowerCopy(vemp.at(2)), "http/", BLANK);
      StringUtil::trim(versionStr);
      float version = -1;
      try {
        version = CastUtil::toFloat(versionStr);
...
std::string HttpRequest::toString() {
  ...
  if (num_headers > 0) {
    for (int i = 0; i < (int)num_headers; ++i) {
      ret +=
          "\n" + std::string(headers_list[i].name, headers_list[i].name_len) +
          ": " + std::string(headers_list[i].value, headers_list[i].value_len);
    }
  } else {
    RMap::iterator it;
  ...
  if (this->requestParams.size() > 0) {
    RMap::iterator iter;
    for (iter = this->requestParams.begin(); iter != this->requestParams.end();
         iter++) {
  ...
  if (this->requestParamsF.size() > 0) {
    FMap::iterator iter;
    for (iter = this->requestParamsF.begin();
         iter != this->requestParamsF.end(); ++iter) {
  ...
  ret += "\nRequest Parameters " +
         vals; // CastUtil::fromNumber(this->getRequestParams().size());
  return ret;
}

HttpRequest::~HttpRequest() {}

const std::string_view &HttpRequest::getMethod() const { return methodv; }

void HttpRequest::setMethod(const std::string &method) {
  // this->method = method;
}

HttpSession *HttpRequest::getSession() { return &(this->session); }

std::string HttpRequest::getUrl() const { return this->url; }

void HttpRequest::setCurl(std::string curl) { this->curl = curl; }

std::string HttpRequest::getCurl() const { return this->curl; }

float HttpRequest::getHttpVers() const { return this->httpVers; }
...
std::string HttpRequest::getContent() const { return content; }

void HttpRequest::setContent(std::string &data) { content = data; }

void HttpRequest::setContent(std::string &&data) { content = data; }

RMap HttpRequest::getRequestParams() const { return requestParams; }
...
void HttpRequest::setRequestParam(const std::string &key,
                                  const std::string &value) {
  this->requestParams[key] = value;
}

std::string HttpRequest::getCntxt_root() const { return *cntxt_root; }
...
std::string HttpRequest::getFile() const { return file; }
...
std::string HttpRequest::getActUrl() const { return actUrl; }
...
void HttpRequest::setActUrl(const std::string &actUrl) {
  ...
  if (au != BLANK) {
    ...
    if (actUrlParts.size() > 1 ||
        (actUrlParts.size() == 1 &&
         actUrlParts[0].find(".") == std::string::npos)) {
      cntxt_name = actUrlParts[0];
    }
    if (actUrlParts.size() > 0 &&
        actUrlParts[actUrlParts.size() - 1].find(".") != std::string::npos) {
...
void HttpRequest::normalizeUrl() {
  ...
  if (cntxt_name != BLANK && actUrlParts.size() > 0 &&
      actUrlParts.at(0) != cntxt_name) {
  ...
  if (queryv.size() > 0) {
    ...
    for (int i = 0; i < num_params; i++) {
      queryParams[std::string(params[i].key, params[i].key_len)] =
          std::string(params[i].val, params[i].val_len);
    }
  }
}
...
RMap HttpRequest::getAuthinfo() const { return authinfo; }
...
std::string HttpRequest::toPHPVariablesString(const std::string &def) {
  ...
  ret += "';\n$_SERVER['HTTP_CONTENT_BOUNDARY'] = '" +
         this->getContent_boundary() + "';\n";
  if (this->queryParams.size() > 0) {
    RMap::iterator iter;
    for (iter = this->queryParams.begin(); iter != this->queryParams.end();
         iter++) {
      if (iter->first.find("[") != std::string::npos &&
          iter->first.find("]") != std::string::npos) {
        ret +=
            "\nif(!isset($_GET['" +
            iter->first.substr(0, iter->first.find("[")) + "']))\n{\n$_GET['" +
            iter->first.substr(0, iter->first.find("[")) + "']=array();\n}\n";
  ...
  if (this->requestParamsF.size() > 0) {
    ...
    for (iter = this->requestParamsF.begin();
      ...
      if (iter->first.find("[") != std::string::npos &&
        ...
        ret += "\n$_FILES['" + iter->first.substr(0, iter->first.find("[")) +
               "']" + iter->first.substr(iter->first.find("[")) +
               "['name'] = '" + dat.getFileName() + "';";
        ret += "\n$_FILES['" + iter->first.substr(0, iter->first.find("[")) +
               "']" + iter->first.substr(iter->first.find("[")) +
               "['type'] = '" + dat.getHeader("Content-Type") + "';";
        ret += "\n$_FILES['" + iter->first.substr(0, iter->first.find("[")) +
               "']" + iter->first.substr(iter->first.find("[")) +
               "['size'] = " + CastUtil::fromNumber(dat.getContent().length()) +
               ";";
        ret += "\n$_FILES['" + iter->first.substr(0, iter->first.find("[")) +
               "']" + iter->first.substr(iter->first.find("[")) +
               "['tmp_name'] = '" + dat.getTempFileName() + "';";
        ret += "\n$_FILES['" + iter->first.substr(0, iter->first.find("[")) +
               "']" + iter->first.substr(iter->first.find("[")) +
               "['error'] = 0;";
      } else {
        ret += "\nif(!isset($_FILES['" + iter->first + "']))\n{\n$_FILES['" +
               iter->first + "']=array();\n}\n";
        ret += "\n$_FILES['" + iter->first + "']['name'] = '" +
               dat.getFileName() + "';";
        ret += "\n$_FILES['" + iter->first + "']['type'] = '" +
               dat.getHeader("Content-Type") + "';";
        ret += "\n$_FILES['" + iter->first + "']['size'] = " +
               CastUtil::fromNumber(dat.getContent().length()) + ";";
        ret += "\n$_FILES['" + iter->first + "']['tmp_name'] = '" +
               dat.getTempFileName() + "';";
    ...
    for (iter = this->requestParamsF.begin();
      ...
      if (iter->first.find("[") != std::string::npos &&
        ...
        ret += "\nif(!FILES['" + iter->first.substr(0, iter->first.find("[")) +
               "'].has_key?('" + key + "'))" + "\nFILES['" +
               iter->first.substr(0, iter->first.find("[")) + "']['" + key +
               "']={}\nend";
        ...
        ret += "\nFILES['" + iter->first + "'] = {}";
        ret += "\nFILES['" + iter->first + "']['name'] = '" +
               dat.getFileName() + "'";
        ret += "\nFILES['" + iter->first + "']['type'] = '" +
               dat.getHeader("Content-Type") + "'";
        ret += "\nFILES['" + iter->first + "']['size'] = " +
               CastUtil::fromNumber(dat.getContent().length());
        ret += "\nFILES['" + iter->first + "']['tmp_name'] = '" +
               dat.getTempFileName() + "'";
...
RMap HttpRequest::getAllParams() {
  ...
  std::map<std::string, std::string, std::less<>> reqparams =
      this->getRequestParams();
  std::map<std::string, std::string, std::less<>> qryparams =
      this->getQueryParams();
...
RMap HttpRequest::getQueryParams() const { return queryParams; }

void HttpRequest::setQueryParams(const RMap &queryParams) {
  this->queryParams = queryParams;
}

void HttpRequest::setQueryParam(const std::string &name,
                                const std::string &value) {
...
RMap HttpRequest::getCookieInfo() const { return cookieattrs; }
...
std::string HttpRequest::getHeader(std::string_view key) {
  if (num_headers > 0) {
    for (int i = 0; i < (int)num_headers; ++i) {
      if (headers_list[i].name_len == key.length() &&
          strncasecmp(headers_list[i].name, key.data(),
                      headers_list[i].name_len) == 0) {
...
RMap HttpRequest::getHeaders() { return headers; }

bool HttpRequest::isCorsRequest() { return corsRequest; }

int HttpRequest::getCORSRequestType() {
  if (isCorsRequest()) {
    if (methodv.length() == 7 &&
        strncasecmp("options", methodv.data(), 7) == 0 &&
        hasHeader(AccessControlRequestMethod)) {
...
void HttpRequest::addHeaderValue(std::string header, const std::string &value) {
  if (header.length() > 0) {
    if (HDRS_SW_CODES.find(header) != HDRS_SW_CODES.end()) {
      ...
      // std::cout << ("Non standard Header std::string " + header) <<
      // std::endl;
...
HttpRequest::parseHeaderValue(std::string headerValue) {
  RegexUtil::replace(headerValue, "\\s*,\\s*|\\s+", ",");
  return StringUtil::splitAndReturn<std::vector<std::string>>(headerValue, ",");
}

const std::string HttpRequest::VALID_METHODS =
    ",get,post,options,delete,head,put,trace,";
bool HttpRequest::isValidHttpMethod(const std::string &method) {
  ...
  return strcasecmp(method.c_str(), "get") ||
         ...
         strcasecmp(method.c_str(), "options") ||
         strcasecmp(method.c_str(), "delete") ||
         strcasecmp(method.c_str(), "head") ||
         strcasecmp(method.c_str(), "put") ||
...
bool HttpRequest::isValidHttpMethod() {
  ...
  return strcasecmp(lmeth.c_str(), "get") ||
         strcasecmp(lmeth.c_str(), "post") ||
         strcasecmp(lmeth.c_str(), "options") ||
         strcasecmp(lmeth.c_str(), "delete") ||
         strcasecmp(lmeth.c_str(), "head") ||
         strcasecmp(lmeth.c_str(), "put") || strcasecmp(lmeth.c_str(), "trace");
}

bool HttpRequest::isAgentAcceptsCE() {
  std::string lmeth = StringUtil::toLowerCopy(getHeader(AcceptEncoding));
  return lmeth.find("gzip") != std::string::npos ||
         lmeth.find("deflate") != std::string::npos;
}

bool HttpRequest::isHeaderValue(std::string header, const std::string &value,
                                const bool &ignoreCase) {
  if (num_headers > 0) {
    for (int i = 0; i < (int)num_headers; ++i) {
      if (header.length() == headers_list[i].name_len &&
          strncasecmp(headers_list[i].name, header.c_str(),
                      headers_list[i].name_len) == 0) {
        return strncasecmp(headers_list[i].value, value.c_str(),
                           headers_list[i].value_len) == 0;
      }
    }
    return false;
  }
...
bool HttpRequest::isClose() {
  if (num_headers > 0) {
    for (int i = 0; i < (int)num_headers; ++i) {
      if (HttpRequest::Connection.length() == headers_list[i].name_len &&
          strncasecmp(headers_list[i].name, HttpRequest::Connection.c_str(),
                      headers_list[i].name_len) == 0) {
        ...
        break;*/
        return strncasecmp(headers_list[i].value, "close",
                           headers_list[i].value_len) == 0;
      }
    }
    return false;
  ...
  if (num_headers > 0) {
    for (int i = 0; i < (int)num_headers; ++i) {
      if (HttpRequest::Connection.length() == headers_list[i].name_len &&
        ...
        break;*/
        return strncasecmp(headers_list[i].value, "keep-alive",
                           headers_list[i].value_len) == 0;
      }
    }
    return false;
  ...
  if (num_headers > 0) {
    for (int i = 0; i < (int)num_headers; ++i) {
      if (HttpRequest::Connection.length() == headers_list[i].name_len &&
        ...
        break;*/
        return strncasecmp(headers_list[i].value, "upgrade",
                           headers_list[i].value_len) == 0;
      }
    }
    return false;
...
bool HttpRequest::hasHeaderValuePart(std::string header, std::string valuePart,
                                     const bool &ignoreCase) {
  std::string hvalue;
  if (num_headers > 0) {
    for (int i = 0; i < (int)num_headers; ++i) {
  ...
  if (hvalue == valuePart ||
      (ignoreCase &&
       StringUtil::toLowerCopy(hvalue) == StringUtil::toLowerCopy(valuePart))) {
    return true;
  } else if (ignoreCase) {
    StringUtil::toLower(hvalue);
    StringUtil::toLower(valuePart);
    std::vector<std::string> hvec =
        StringUtil::splitAndReturn<std::vector<std::string>>(hvalue, ",");
...
HttpRequest::getRanges(std::vector<std::string> &rangesVec) {
...
std::string HttpRequest::getContent_tfile() { return content_tfile; }

void HttpRequest::addMultipartFormContent(const std::string &key,
...
HTTPResponseStatus *HttpRequest::getRequestParseStatus() { return status; }

void HttpRequest::setSessionID(const std::string &sessionID) {
  this->sessionID = sessionID;
}

std::string HttpRequest::getSessionID() const { return sessionID; }

std::vector<MultipartContent>
HttpRequest::getMultiPartFileList(const std::string &name) {
  std::vector<MultipartContent> filevec;
  FMap::iterator it;
  if (name != BLANK) {
    for (it = this->requestParamsF.begin(); it != this->requestParamsF.end();
         ++it) {
      if (it->second.getTempFileName() != BLANK &&
          (it->second.getName() == name ||
           it->second.getName().find(name + "[") == 0)) {
...
std::string HttpRequest::getPassword() const { return password; }

std::string HttpRequest::getUserName() const { return userName; }

std::string HttpRequest::getAuthMethod() const { return authMethod; }

void HttpRequest::setPassword(std::string v) { this->password = v; }

void HttpRequest::setUserName(std::string v) { this->userName = v; }

void HttpRequest::setAuthMethod(std::string v) { this->authMethod = v; }
...
std::string HttpRequest::toPluginString() {
  ...
  RMap::iterator it;
  for (it = this->requestParams.begin(); it != this->requestParams.end();
       ++it) {
    text += it->first + "\n";
  ...
  FMap::iterator fit;
  for (fit = this->requestParamsF.begin(); fit != this->requestParamsF.end();
       ++fit) {
...
std::string HttpRequest::getExt() const { return ext; }
