...
int DistoCacheServiceHandler::getLength(const std::string &header,
                                        const int &size) {
...
bool DistoCacheServiceHandler::validQuery(const std::vector<std::string> &parts,
                                          const int &size,
...
void DistoCacheServiceHandler::run() {
  ...
  while (true) {
    ...
    if (lengthm > 10000000) {
      ...
      logger << "WARNING>> Invalid data or someone trying to attack the cache "
                "or maybe a simple telnet attempt...ignore..."
             << std::endl;
      return;
    }
    ...
    try {
      ...
      if (query != NULL && query->getPackets().size() > 0) {
        ...
        if (validQuery(parts, 3, CacheMap::ALLOCATE)) {
        ...
        } else if (validQuery(parts, 3, CacheMap::PUSH_FRONT,
                              CacheMap::COLL_ENTRY)) {
          if (query->getPackets().size() == 2) {
            std::string value = query->getPackets().at(1)->getValue();
        ...
        } else if (validQuery(parts, 3, CacheMap::PUSH_BACK,
                              CacheMap::COLL_ENTRY)) {
          if (query->getPackets().size() == 2) {
            std::string value = query->getPackets().at(1)->getValue();
        ...
        } else if (validQuery(parts, 4, CacheMap::INSERT,
                              CacheMap::COLL_ENTRY)) {
          int position = -1;
          try {
        ...
        } else if (validQuery(parts, 5, CacheMap::INSERT,
                              CacheMap::COLL_ENTRY)) {
          ...
          try {
            ...
            if (position >= 0 && position < size) {
              if (query->getPackets().size() == 2) {
                std::string value = query->getPackets().at(1)->getValue();
                CacheMap::setMapEntryValueByPosition(parts.at(2), position,
                                                     value);
              } else {
                responseMsg = CacheMap::ERR_NOVALUESPEC;
              }
        ...
        } else if (validQuery(parts, 4, CacheMap::GETBYPOS,
                              CacheMap::MAP_ENTRY)) {
          int position = -1;
          try {
            position = CastUtil::toInt(parts.at(3));
            ...
            if (position >= 0 && position < size) {
              responseValue =
                  CacheMap::getMapEntryValueByPosition(parts.at(2), position);
            } else if (size <= position) {
              responseMsg = CacheMap::ERR_INDGRTCONTSIZ;
            } else {
              responseMsg = CacheMap::ERR_NEGATIVEPOS;
          ...
          try {
            ...
            if (position >= 0 && position < size) {
              responseValue =
                  CacheMap::getCollectionEntryAt(parts.at(2), position);
            } else if (size <= position) {
              responseMsg = CacheMap::ERR_INDGRTCONTSIZ;
            } else {
              responseMsg = CacheMap::ERR_NEGATIVEPOS;
        ...
        } else if (validQuery(parts, 3, CacheMap::GET_FRONT,
                              CacheMap::COLL_ENTRY)) {
          responseValue = CacheMap::getFrontValue(parts.at(2));
        } else if (validQuery(parts, 3, CacheMap::GET_BACK,
                              CacheMap::COLL_ENTRY)) {
          responseValue = CacheMap::getBackValue(parts.at(2));
        } else if (validQuery(parts, 3, CacheMap::POPGET,
                              CacheMap::COLL_ENTRY)) {
          responseValue = CacheMap::popGetValueQueue(parts.at(2));
        } else if (validQuery(parts, 3, CacheMap::POP_FRONTGET,
                              CacheMap::COLL_ENTRY)) {
          responseValue = CacheMap::popGetFrontValue(parts.at(2));
        } else if (validQuery(parts, 3, CacheMap::POP_BACKGET,
                              CacheMap::COLL_ENTRY)) {
          responseValue = CacheMap::popGetBackValue(parts.at(2));
        } else if (validQuery(parts, 3, CacheMap::REMOVE,
                              CacheMap::OBJ_ENTRY)) {
          CacheMap::removeObjectEntry(parts.at(2));
        } else if (validQuery(parts, 4, CacheMap::REMOVE,
                              CacheMap::MAP_ENTRY)) {
        ...
        } else if (validQuery(parts, 3, CacheMap::POP_FRONT,
                              CacheMap::COLL_ENTRY)) {
          CacheMap::popFrontValue(parts.at(2));
        } else if (validQuery(parts, 3, CacheMap::POP_BACK,
                              CacheMap::COLL_ENTRY)) {
