...
std::string CacheMap::MAP_ENTRY = "mapentry",
            CacheMap::COLL_ENTRY = "collentry", CacheMap::REMOVE = "remove",
            CacheMap::SIZE = "size", CacheMap::CLEAR = "clear",
            CacheMap::IS_EMPTY = "isempty", CacheMap::ALLOCATE = "allocate",
            CacheMap::DEALLOCATE = "deallocate", CacheMap::MAP = "map",
            CacheMap::SET = "set", CacheMap::LIST = "list",
            CacheMap::VECTOR = "vector", CacheMap::QUEUE = "queue",
            CacheMap::DEQUE = "deque", CacheMap::ADD = "add",
            CacheMap::GET = "get", CacheMap::GET_FRONT = "getfront",
            CacheMap::GET_BACK = "getback", CacheMap::POP = "pop",
            CacheMap::POP_BACK = "popback", CacheMap::POP_FRONT = "popfront",
            CacheMap::PUSH_FRONT = "pushfront",
            CacheMap::PUSH_BACK = "pushback", CacheMap::INSERT = "insert",
            CacheMap::OBJ_ENTRY = "objentry", CacheMap::GETBYPOS = "getbypos",
            CacheMap::POPGET = "popget", CacheMap::POP_FRONTGET = "popfrontget",
            CacheMap::POP_BACKGET = "popbackget";

std::string CacheMap::ERR_NOELEMENTS = "No Elements in container",
            CacheMap::ERR_NOKEYCACHEMAP =
                "Unable to find value for Key specified",
            CacheMap::ERR_INVCONTAINER = "Invalid container specified",
            CacheMap::ERR_OPNOTSUPP = "Operation not supported on container",
            CacheMap::ERR_INDGRTCONTSIZ = "Index greater than container size",
            CacheMap::ERR_NOTAMAPCONT = "Not a map container",
            CacheMap::ERR_NOVALUEFOUND = "No value found for key",
            CacheMap::ERR_ALLOCENTEXISTS = "Entry already exists",
            CacheMap::ERR_NOVALUESPEC = "No value specified",
            CacheMap::ERR_NEGATIVEPOS = "Position value is less than 0",
            CacheMap::ERR_POSNOTNUM = "Position value is not a number",
            CacheMap::ERR_NEGATIVEREP = "Repetition value is less than 0",
            CacheMap::ERR_REPNOTNUM = "Repetition value is not a number",
            CacheMap::SUCCESS = "SUCCESS";
...
void CacheMap::addObjectEntry(const std::string &key,
...
void CacheMap::addMapEntry(const std::string &cacheKey, const std::string &key,
                           const std::string &value) {
  if (checkExistance(cacheKey, MAP)) {
    instance->mutex.lock();
    std::map<std::string, std::string> *valueMap =
        (std::map<std::string, std::string> *)instance->cacheMap[cacheKey];
    instance->mutex.unlock();
...
void CacheMap::removeMapEntry(const std::string &cacheKey,
...
void CacheMap::addCollectionEntry(const std::string &cacheKey,
                                  const std::string &value) {
  ...
  if (type != "") {
    if (type == VECTOR) {
      ...
      std::vector<std::string> *valueVector =
          (std::vector<std::string> *)instance->cacheMap[cacheKey];
      instance->mutex.unlock();
      ...
      instance->mutex.lock();
      std::list<std::string> *valueVector =
          (std::list<std::string> *)instance->cacheMap[cacheKey];
      instance->mutex.unlock();
      ...
      instance->mutex.lock();
      std::set<std::string> *valueVector =
          (std::set<std::string> *)instance->cacheMap[cacheKey];
      instance->mutex.unlock();
      ...
      instance->mutex.lock();
      std::queue<std::string> *valueVector =
          (std::queue<std::string> *)instance->cacheMap[cacheKey];
      instance->mutex.unlock();
      ...
      instance->mutex.lock();
      std::deque<std::string> *valueVector =
          (std::deque<std::string> *)instance->cacheMap[cacheKey];
      instance->mutex.unlock();
...
std::string CacheMap::getMapEntryValue(const std::string &cacheKey,
                                       const std::string &key) {
  std::string value;
  if (checkExistance(cacheKey, MAP)) {
    instance->mutex.lock();
...
std::string CacheMap::getMapEntryValueByPosition(const std::string &cacheKey,
                                                 const int &position) {
  std::string value;
  if (checkExistance(cacheKey, MAP)) {
    instance->mutex.lock();
...
void CacheMap::setMapEntryValueByPosition(const std::string &cacheKey,
                                          const int &position,
                                          const std::string &value) {
  if (checkExistance(cacheKey, MAP)) {
    instance->mutex.lock();
...
void CacheMap::setCollectionEntryAt(const std::string &cacheKey,
                                    const int &position,
                                    const std::string &value) {
  bool error = false;
  std::string type = checkExistanceAndGetType(cacheKey);
  if (type != "") {
    if (type == VECTOR) {
      ...
      instance->mutex.lock();
      std::deque<std::string> *valueDeque =
          (std::deque<std::string> *)instance->cacheMap[cacheKey];
      instance->mutex.unlock();
...
std::string CacheMap::getCollectionEntryAt(const std::string &cacheKey,
                                           const int &position) {
  std::string value;
  bool error = false;
  std::string type = checkExistanceAndGetType(cacheKey);
...
void CacheMap::removeCollectionEntryAt(const std::string &cacheKey,
                                       const int &position) {
  std::string type = checkExistanceAndGetType(cacheKey);
  bool error = false;
  if (type != "") {
...
void CacheMap::insert(const std::string &cacheKey, const std::string &value,
                      const int &position) {
  std::string type = checkExistanceAndGetType(cacheKey);
  bool error = false;
  if (type != "") {
...
void CacheMap::insert(const std::string &cacheKey, const std::string &value,
                      const int &position, const int &repeat) {
  std::string type = checkExistanceAndGetType(cacheKey);
  bool error = false;
  if (type != "") {
    if (type == VECTOR) {
...
void CacheMap::pushBackValue(const std::string &cacheKey,
                             const std::string &value) {
  std::string type = checkExistanceAndGetType(cacheKey);
  if (type != "") {
    if (type == LIST) {
...
void CacheMap::pushFrontValue(const std::string &cacheKey,
                              const std::string &value) {
  std::string type = checkExistanceAndGetType(cacheKey);
  if (type != "") {
    if (type == LIST) {
...
bool CacheMap::checkExistance(const std::string &cacheKey,
  ...
  flag = instance->cacheMap.find(cacheKey) != instance->cacheMap.end() &&
         (type == "" ||
          (type != "" &&
           instance->valueTypes.find(cacheKey) != instance->valueTypes.end() &&
           instance->valueTypes[cacheKey] == type));
  instance->mutex.unlock();
  return flag;
}
