...
public:
  ...
  void insert(const std::pair<K, V> &entry) {
    ...
    cl->addMapEntry(cacheKey, CastUtil::lexical_cast<std::string>(entry.first),
                    serValue);
  }
  const V operator[](const K &key) const {
    std::string serValue = cl->getMapEntryValue(
        cacheKey, CastUtil::lexical_cast<std::string>(key));
  ...
  size_t size() { return cl->size(cacheKey); }
  bool isEmpty() { return cl->isEmpty(cacheKey); }
  void clear() { cl->clear(cacheKey); }
  void free() { cl->deallocate(cacheKey); }
  virtual ~DistMap() {
  ...
  public:
    ...
    V get() {
      if (position >= 0) {
        std::string serValue =
            cl->getMapEntryValueByPosition(cacheKey, position);
        return BinarySerialize::unserialize<V>(serValue, -1);
      } else {
    ...
    bool operator!=(const iterator &b) { return position != b.position; }
  };
