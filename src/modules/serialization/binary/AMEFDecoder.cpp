...
AMEFObject *AMEFDecoder::decodeB(const std::string &buffer,
                                 const bool &considerLength) {
  if (buffer.length() == 0)
    return NULL;
...
/*AMEFObject* AMEFDecoder::decodeBNew(const std::string& buffer, const bool&
considerLength)
{
...
void AMEFDecoder::decodeObjectName(const std::string &buffer,
...
/*AMEFObject* AMEFDecoder::decodeSinglePacketB(const std::string& buffer, bool
ignoreName, AMEFObject* jDBObject)
{
        ...
        else if(type<AMEFObject::OBJECT_TYPE) // String/Double/Date Types
        {
                ...
                if(!ignoreName)
                {
                        ...
                        ++jDBObject->position;
                int lengthm = AMEFObject::charArrayToInt(buffer,
jDBObject->position, 2); jDBObject->position += 2; std::string value =
buffer.substr(jDBObject->position, lengthm); jDBObject->setValue(value);
                jDBObject->position += lengthm;
        }
        else if(type==AMEFObject::OBJECT_TYPE) // Object Types
        {
                ...
                if(!ignoreName)
                {
                        ...
                        ++jDBObject->position;
                jDBObject->blength = 4 + jDBObject->position +
AMEFObject::charArrayToInt(buffer, jDBObject->position, 4); jDBObject->position
+= 4; while(jDBObject->position<jDBObject->blength)
                {
...
AMEFObject *AMEFDecoder::decodeSinglePacketBInternal(const std::string &buffer,
                                                     AMEFObject *jDBObject) {
  char type = (char)buffer[jDBObject->position];
  if (type < AMEFObject::INT_TYPE) // All NULL Types
  {
  ...
  } else if (type == AMEFObject::OBJECT_TYPE) // Object Types
  {
    ...
    decodeObjectName(buffer, jDBObject);
    jDBObject->blength =
        4 + jDBObject->position +
        AMEFObject::charArrayToInt(buffer, jDBObject->position, 4);
    jDBObject->position += 4;
...
AMEFObject *AMEFDecoder::decodeSinglePacketB(const std::string &buffer,
                                             AMEFObject *jDBObject) {
  char type = (char)buffer[jDBObject->position];
...
std::string AMEFDecoder::updatePacket(const std::string &orig,
                                      AMEFObject *jDBObject, int index) {
  std::string np;
