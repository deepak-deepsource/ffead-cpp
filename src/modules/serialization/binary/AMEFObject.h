...
class AMEFObject {
  /*The type of the Object can be std::string, number, date, boolean, character
   * or any complex object*/
  char type;

  /*The name of the Object if required can be used to represent object
   * properties*/
  std::string name;
...
public:
  static int charArrayToInt(const std::string &l, const int &off,
                            const int &ind);
  static std::string intTocharArray(const int &l, const int &ind);
  static std::string ulonglongTocharArray(const unsigned long long &lon,
                                          int ind = -1);
  ...
  static void ulonglongTocharArrayF(const unsigned long long &lon,
                                    std::string &result, int ind = -1) {
    if (ind == -1) {
  ...
  /*template<class T> T getObjectValue()
  {
          ...
          if(desiredType=="short" || desiredType=="short int" ||
  desiredType=="signed short" || desiredType=="signed short int"
                          || desiredType=="unsigned short" ||
  desiredType=="unsigned short int"
                          || desiredType=="signed" || desiredType=="int" ||
  desiredType=="signed int"
                          || desiredType=="unsigned" || desiredType=="unsigned
  int" || desiredType=="long"
                          || desiredType=="long int" || desiredType=="signed
  long" || desiredType=="signed long int"
                          || desiredType=="unsigned long" ||
  desiredType=="unsigned long int"
                          || desiredType=="long long" || desiredType=="long long
  int" || desiredType=="signed long long"
                          || desiredType=="signed long long int" ||
  desiredType=="unsigned long long"
                          || desiredType=="unsigned long long int")
          {
                  if((type=='f' || type=='x' || type=='e' || type=='l'
                                          || type=='n' || type=='w' || type=='r'
  || type=='i') && value.length()<=8)
                  {
                  }
                  else
                  {
  ...
  requested");
                  ...
                  {
                          unsigned short v = (unsigned
  short)charArrayToULongLong(value); t = v;
                  }
  ...
  int)charArrayToULongLong(value); t = v;
                  }
                  ...
                  {
                          unsigned long v = (unsigned
  long)charArrayToULongLong(value); t = v;
                  }
  ...
  long)charArrayToULongLong(value); t = v;
          ...
          else if(desiredType=="long double" || desiredType=="float" ||
  desiredType=="double"
                          || desiredType=="string" ||
  desiredType=="std::string")
          {
