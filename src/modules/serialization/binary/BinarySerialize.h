...
public:
  ...
  static std::string serialize(T &t, int serOpt,
    ...
    return _handleAllSerialization(className, &t, appName, &_i, NULL, NULL,
                                   NULL, NULL);
  }
  template <class T>
  ...
  static std::string serializeMap(std::map<K, V> &mp,
                                  const std::string &appName = "") {
  ...
  static std::map<K, V> unSerializeToMap(const std::string &serStr,
    ...
    if (root != NULL) {
      for (int var = 0; var < (int)root->getPackets().size(); var++) {
        V val =
            unserialize<V>(root->getPackets().at(var)->getValueStr(), appName);
        K key =
            CastUtil::lexical_cast<K>(root->getPackets().at(var)->getNameStr());
        mp[key] = val;
      }
    }
    return mp;
  ...
  template <class K, class V>
  static std::multimap<K, V>
  unSerializeToMultiMap(const std::string &serStr,
                        const std::string &appName = "") {
    std::multimap<K, V> mp;
    AMEFDecoder dec;
    AMEFObject object;
  ...
  template <class T>
  static T unserialize(AMEFObject *serObject, int serOpt,
                       const std::string &appName = "") {
    BinarySerialize serialize;
    T t;
    std::string className = CastUtil::getClassName(t);
  ...
  template <class T>
  static T *unserializeToPointer(const std::string &objXml, int serOpt,
                                 const std::string &appName = "") {
    BinarySerialize serialize;
    T *t;
    std::string className = CastUtil::getClassName(t);
  ...
  template <class T>
  static T *unserializeToPointer(AMEFObject *serObject, int serOpt,
                                 const std::string &appName = "") {
    BinarySerialize serialize;
    T *t;
    std::string className = CastUtil::getClassName(t);
