...
std::string AMEFObject::ulonglongTocharArray(const unsigned long long &lon,
                                             int ind) {
  if (ind == -1) {
    if (lon < 256) {
      ind = 1;
...
std::string
AMEFObject::ulonglongTocharArrayWithLI(const unsigned long long &lon) {
  int ind;
  if (lon < 256) {
    ind = 1;
  } else if (lon < 65536) {
...
int AMEFObject::charArrayToInt(const std::string &l, const int &off,
                               const int &ind) {
  int t = 0;
  for (int i = 0; i < ind; i++) {
...
void AMEFObject::clear() { this->packets.clear(); }
...
void AMEFObject::addPacket(const unsigned long long &lon,
...
void AMEFObject::addPacket(AMEFObject *packet) { packets.push_back(packet); }
...
bool AMEFObject::isLong(const char &type) { return isNumber(type); }
...
bool AMEFObject::isBoolean() { return isBoolean(type); }
...
bool AMEFObject::isNumber() { return isNumber(type); }
...
bool AMEFObject::isDate() { return isDate(type); }

char *AMEFObject::getName() { return (char *)name.c_str(); }
std::string AMEFObject::getNameStr() { return name; }
void AMEFObject::setName(const std::string &name) { this->name = name; }

std::vector<AMEFObject *> AMEFObject::getPackets() { return packets; }
void AMEFObject::setPackets(std::vector<AMEFObject *> packets) {
  this->packets = packets;
}

char AMEFObject::getType() { return type; }
void AMEFObject::setType(const char &type) { this->type = type; }

std::string AMEFObject::getValue() { return value; }

std::string AMEFObject::getValueStr() { return value; }
void AMEFObject::pushChar(const char &v) { this->value.push_back(v); }
...
void AMEFObject::setValue(const std::string &value) { this->value = value; }
...
std::string AMEFObject::tostring() { return displayObject("\t"); }
...
void AMEFObject::addStaticPacket(AMEFObject *obj) { packets.push_back(obj); }
