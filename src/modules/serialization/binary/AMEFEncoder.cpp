...
std::string AMEFEncoder::encodeSinglePacketB(AMEFObject *packet,
                                             const bool &ignoreName) {
  std::string buffer, vbuffer;
  if (!ignoreName) {
    buffer.append(
        AMEFObject::ulonglongTocharArray(packet->getNameStr().length()));
  ...
  if (packet->getType() >= AMEFObject::DATE_TYPE &&
      packet->getType() < AMEFObject::OBJECT_TYPE) {
...
std::string AMEFEncoder::encodeSinglePacketBNew(AMEFObject *packet,
                                                const bool &ignoreName) {
  std::string buffer, vbuffer, pbuffer;
  if (!ignoreName) {
    buffer.append(
        AMEFObject::ulonglongTocharArray(packet->getNameStr().length(), 1));
    buffer.append(packet->getNameStr());
  }
  ...
  if (packet->getType() >= AMEFObject::DATE_TYPE &&
    ...
    buffer.append(AMEFObject::ulonglongTocharArray(
        vbuffer.length() + pbuffer.length() + 4, 4));
  }
  buffer.append(vbuffer);
