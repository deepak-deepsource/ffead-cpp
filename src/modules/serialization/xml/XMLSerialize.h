...
public:
  ...
  template <class T>
  static std::string serialize(void *t, int serOpt,
                               const std::string &className,
                               const std::string &appName = "") {
    if (serOpt == -1)
      serOpt = identifySerOption(className);
  ...
  static std::string serializeMap(const std::map<K, V> &mp,
                                  const std::string &appName = "") {
    std::map<K, V> mpt = mp;
    K k;
  ...
  template <class K, class V>
  static std::map<K, V> unSerializeToMap(const std::string &serStr,
                                         const std::string &appName = "") {
    std::map<K, V> mp;
    ...
    try {
      ...
      if (message.getTagName() != "" &&
          message.getTagName().find("map-") != std::string::npos &&
        ...
        for (int var = 0; var < (int)message.getChildElements().size(); var++) {
          ...
          if (entry->getTagName() == "entry") {
            ...
            if (key->getTagName() != "" && value->getTagName() != "" &&
                key->getChildElements().size() > 0 &&
  ...
  template <class K, class V>
  static std::string serializeMultimap(std::multimap<K, V> &mp,
                                       const std::string &appName = "") {
  ...
  template <class K, class V>
  static std::map<K, V> unSerializeToMultiMap(const std::string &serStr,
                                              const std::string &appName = "") {
    std::multimap<K, V> mp;
    SimpleXmlParser parser("Parser");
    try {
      ...
      Element message = doc.getRootElement();
      if (message.getTagName() != "" &&
          message.getTagName().find("multimap-") != std::string::npos &&
          message.getChildElements().size() > 0) {
        std::string maptype = message.getTagName();
        std::string keytype = maptype.substr(maptype.find("-") + 1);
        if (keytype.find("-") == std::string::npos)
  ...
  template <class T>
  static T unserialize(const std::string &objXml, int serOpt,
                       const std::string &appName = "") {
    ...
    if (serOpt == -1)
      serOpt = identifySerOption(className);
    T *tp = (T *)_handleAllUnSerialization(objXml, serOpt, className, appName,
  ...
  template <class T>
  static T unserialize(Element *element, int serOpt,
                       const std::string &appName = "") {
    XMLSerialize serialize;
    T t;
    std::string className = CastUtil::getClassName(t);
    if (serOpt == -1)
      serOpt = identifySerOption(className);
    T *tp = (T *)_handleAllUnSerialization("", serOpt, className, appName,
                                           &serialize, false, element);
    if (tp != NULL) {
      t = *(T *)tp;
      delete ((T *)tp);
    }
  ...
  template <class T>
  static T *unserializeToPointer(std::string objXml, int serOpt,
                                 const std::string &appName = "") {
    XMLSerialize serialize;
    T *t;
    std::string className = CastUtil::getClassName(t);
    if (serOpt == -1)
      serOpt = identifySerOption(className);
    return (T *)_handleAllUnSerialization(objXml, serOpt, className, appName,
                                          &serialize, false, NULL);
  }
  template <class T>
  static T *unserializeToPointer(Element *element, int serOpt,
                                 const std::string &appName = "") {
    XMLSerialize serialize;
    T *t;
    std::string className = CastUtil::getClassName(t);
    if (serOpt == -1)
      serOpt = identifySerOption(className);
    return (T *)_handleAllUnSerialization("", serOpt, className, appName,
                                          &serialize, false, element);
  }
