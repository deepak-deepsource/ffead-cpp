...
std::string SSLCommon::ciphers =
    std::string("ECDHE-RSA-AES128-GCM-SHA256:ECDHE-ECDSA-AES128-GCM-SHA256:"
                "ECDHE-RSA-") +
    std::string("AES256-GCM-SHA384:ECDHE-ECDSA-AES256-GCM-SHA384:DHE-RSA-"
                "AES128-GCM-SHA256:") +
    std::string("DHE-DSS-AES128-GCM-SHA256:kEDH+AESGCM:ECDHE-RSA-AES128-SHA256:"
                "ECDHE-ECDSA-") +
    std::string("AES128-SHA256:ECDHE-RSA-AES128-SHA:ECDHE-ECDSA-AES128-SHA:"
                "ECDHE-RSA-") +
    std::string("AES256-SHA384:ECDHE-ECDSA-AES256-SHA384:ECDHE-RSA-AES256-SHA:"
                "ECDHE-ECDSA-") +
    std::string("AES256-SHA:DHE-RSA-AES128-SHA256:DHE-RSA-AES128-SHA:DHE-DSS-"
                "AES128-SHA256:") +
    std::string("DHE-RSA-AES256-SHA256:DHE-DSS-AES256-SHA:DHE-RSA-AES256-SHA:!"
                "aNULL:!eNULL:") +
...
void SSLCommon::load_dh_params(SSL_CTX *ctx, char *file) {
  ...
  if ((bio = BIO_new_file(file, "r")) == NULL) {
    std::cout << "Couldn't open DH file, defaulting to setting DH to single use"
              << std::endl;
 ...
 * actually calloc.
 */
void *SSLCommon::zeroingMalloc(size_t howmuch) { return calloc(1, howmuch); }
...
void *ssl_malloc(size_t size, const char * /*filename */, int /*lineno*/) {
  return calloc(1, size);
}

void *ssl_realloc(void *ptr, size_t size, const char * /*filename*/,
                  int /*lineno*/) {
  return realloc(ptr, size);
}

void ssl_free(void *ptr, const char * /*filename*/, int /*lineno*/) {
  free(ptr);
}
...
SSL_CTX *SSLCommon::initialize_ctx(const bool &isServer) {
  ...
  if (isServer) {
    meth = (SSL_METHOD *)
        SSLv23_server_method(); /* create new server-method instance */
  } else {
    meth = (SSL_METHOD *)
        SSLv23_client_method(); /* create new client-method instance */
  }
...
void SSLCommon::loadCerts(SSL_CTX *ctx, char *certFile, char *keyFile,
                          const std::string &caList, const bool &ldcrts) {
  if (ldcrts) {
