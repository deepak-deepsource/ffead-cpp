  ...
  try {
    ...
    if (methInfo.find("lang=\"c++\"") != std::string::npos ||
      ...
      if (message.getAttributes().size() < 3) {
        throw MethodInvokerException(
            "name,class and lang are mandatory attributes\n", retValue);
      ...
      for (unsigned var = 0; var < argts.size(); var++) {
        ...
        if (arg->getTagName() != "argument" || arg->getAttribute("type") == "")
          throw MethodInvokerException(
              "every argument tag should have a name and type attribute\n",
              retValue);
        ...
        if (arg->getAttribute("type") == "int") {
          ...
          *vt = arg->getText();
          ...
          int serOpt =
              SerializeBase::identifySerOption(arg->getAttribute("type"));
          value =
              ser.unSerializeUnknown(objxml, serOpt, arg->getAttribute("type"));
        }
      ...
      if (clas->getClassName() == "") {
        throw MethodInvokerException(
            "class does not exist or is not in the library path\n", retValue);
      }
      Method meth = clas->getMethod(methodName, argus);
      if (meth.getMethodName() == "") {
        throw MethodInvokerException(
            "method does not exist for the class or the class does not exist "
            "in the library path\n",
            retValue);
      } else {
        args argus;
