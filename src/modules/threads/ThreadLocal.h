...
template <typename T> inline ThreadLocal::ThreadLocal(const T &t) {
  init();
...
template <typename T> inline ThreadLocal::ThreadLocal(T *t) {
  init();
  if (pthread_getspecific(_key) == NULL) {
    pthread_setspecific(_key, t);
  }
...
template <typename T> inline T ThreadLocal::get() {
  void *tem = this->get();
  if (tem) {
    T *t = (T *)tem;
    return *t;
...
template <typename T> inline T *ThreadLocal::getPointer() {
  return (T *)this->get();
}
