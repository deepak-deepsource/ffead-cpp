...
ThreadPool::ThreadPool() { initPointers(); }
...
ThreadPool::ThreadPool(const int &maxThreads, const int &lowp,
                       const int &highp) {
  if (lowp > highp)
    throw std::runtime_error(
        "Low Priority should be less than Highest Priority");
  initPointers();
  this->maxThreads = maxThreads;
  this->lowp = lowp;
  this->highp = highp;
...
ThreadPool::ThreadPool(const int &maxThreads,
                       const bool &allowScheduledTasks /* = false*/) {
  initPointers();
  this->lowp = -1;
  this->highp = -1;
  this->maxThreads = maxThreads;
...
void ThreadPool::submit(Task *task) {
  ...
  while (!currentThread.compare_exchange_weak(oldValue, newValue,
...
void ThreadPool::schedule(FutureTask *task, const long long &tunit,
                          const int &type) {
  if (this->allowScheduledTasks) {
    task->tunit = tunit;
    task->type = type;
