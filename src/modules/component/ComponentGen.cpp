...
std::string ComponentGen::generateComponentCU(const std::string &fileName,
                                              std::string &cudata,
                                              std::string &cuheader,
                                              std::string &curemote,
                                              std::string &curemoteheaders,
  ...
  if (srprps.size() > 2) {
    try {
      ...
      curemoteheaders +=
          "class " + file + "_Remote : public RemoteComponent \n{\n";
      ...
      cudata +=
          ("this->setAjaxAvail(" + convertfrom(srprps["@AJAX_AVAIL"]) + ");\n");

      cudata +=
          ("this->setWebsAvail(" + convertfrom(srprps["@WEBS_AVAIL"]) + ");\n");

      cudata += ("this->setMethinvAvail(" + convertfrom(srprps["@MINV_AVAIL"]) +
                 ");\n");

      cudata +=
          ("this->setAutoTranx(" + convertfrom(srprps["@AUTO_TRANS"]) + ");\n");

      cudata +=
          ("this->setSession(" + convertfrom(srprps["@SESSION"]) + ");\n");
      ...
      cudata +=
          ("this->setAuthMode(" + convertfrom(srprps["@AUTH_ALL"]) + ");\n");
      ...
      cudata +=
          ("this->setDbconpoolSize(" + srprps["@DB_CONN_POOL_NUM"] + ");\n");
      ...
      while (srprps["@SERVICE" + CastUtil::fromNumber(cnt)] != "") {
          ...
          if ((sp != std::string::npos && sp < cl) || cl == std::string::npos) {
            ...
            if (temtem.find("@BLOCK_USERS") != std::string::npos ||
              ...
              throw std::runtime_error(
                  "Invalid start of property within another property");
            } else {
              Cont1 alwd;
        ...
        std::string srvargs, srvmcarg, srvmcargremote,
            srvmcargremotevec = "std::vector<GenericObject> _vec;\n";
            ...
            if (temtem.find("@BLOCK_USERS") != std::string::npos ||
              ...
              if (temtem == "void") {
                ...
                for (unsigned int var = 0; var < alwd.size(); ++var) {
                  if (alwd.at(var) != "" &&
                      alwd.at(var).find("*") == std::string::npos &&
                      alwd.at(var).find("&") == std::string::npos) {
                    // srvargs += alwd.at(var) +
                    // "*_"+CastUtil::fromNumber(var+1)+" =
                    // ("+alwd.at(var)+"*)args.at("+CastUtil::fromNumber(var)+");\n";
                    srvmcarg += "_" + CastUtil::fromNumber(var + 1) + "";
                    srvargs +=
                        alwd.at(var) + " _" + CastUtil::fromNumber(var + 1);
                    srvmcargremote +=
                        alwd.at(var) + " &_" + CastUtil::fromNumber(var + 1);
                    srvmcargremotevec += "GenericObject __" +
                                         CastUtil::fromNumber(var + 1) + ";\n";
                    srvmcargremotevec += "__" + CastUtil::fromNumber(var + 1) +
                                         " << _" +
                                         CastUtil::fromNumber(var + 1) + ";\n";
                    srvmcargremotevec += "_vec.push_back(__" +
                                         CastUtil::fromNumber(var + 1) + ");\n";
                    if (var != alwd.size() - 1) {
                      srvmcarg += ",";
                      srvmcargremote += ",";
        ...
        if (srvret != "" && srvret != "void")
          ...
          servs += "}\n\n";
        curemoteheaders +=
            srvret + " " + srvname + "(" + srvmcargremote + ");\n";
        rsrvs += srvret + " " + file + "_Remote::" + srvname + "(" +
                 srvmcargremote + ")\n{\n";
        rsrvs += srvmcargremotevec;
        if (srvret != "" && srvret != "void")
          rsrvs += "return ";
        rsrvs += "*(" + srvret + "*)this->invoke(\"" + srvname + "\",_vec,\"" +
                 srvret + "\");\n";
        rsrvs += "}\n";
      }
      curemoteheaders += "};\n";
      ...
      logger <<
      "\n\n\n\n\n\n------------------------------------------------------------------\n\n\n\n\n\n"
      << std::flush; logger << cudata << std::flush; logger <<
      "\n\n\n\n\n\n------------------------------------------------------------------\n\n\n\n\n\n"
      << std::flush; logger << curemoteheaders << std::flush; logger <<
      "\n\n\n\n\n\n------------------------------------------------------------------\n\n\n\n\n\n"
      << std::flush; logger << curemote << std::flush;*/
