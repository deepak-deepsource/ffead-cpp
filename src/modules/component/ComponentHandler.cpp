...
void *ComponentHandler::service(void *arg) {
  ...
  try {
    ...
    if (methInfo.find("lang=\"c++\"") != std::string::npos ||
        methInfo.find("lang='c++'") != std::string::npos) {
      Document doc;
      ...
      if (message.getAttributes().size() < 4) {
        throw ComponentHandlerException(
            "name,beanName,returnType and lang are mandatory attributes\n",
            retValue);
      ...
      if (message.getChildElements().size() != 1) {
        throw ComponentHandlerException(
            "message tag should have only one child tag\n", retValue);
      } else if (message.getChildElements().at(0).getTagName() != "args") {
        throw ComponentHandlerException(
            "message tag should have an args child tag\n", retValue);
      }
      XMLSerialize ser;
      Reflector reflector;
      args argus;
      ...
      for (unsigned var = 0; var < argts.size(); var++) {
        ...
        if (argts.at(var).getTagName() != "argument") {
          throw ComponentHandlerException(
              "Invalid Tag, only argument tag allowed\n", retValue);

        } else if (argts.at(var).getAttribute("type") == "") {
          throw ComponentHandlerException(
              "every argument tag should have a type attribute\n", retValue);
        }
        if (argts.at(var).getText() == "" &&
            argts.at(var).getChildElements().size() == 0) {
          throw ComponentHandlerException("argument value missing\n", retValue);
        }
        if (argts.at(var).getAttribute("type") != "") {
          std::string objxml =
              argts.at(var).render(); // @suppress("Invalid arguments")
          std::string objClassName = argts.at(var).getTagName();
          int serOpt = SerializeBase::identifySerOption(
              argts.at(var).getAttribute("type"));
          value = ser.unSerializeUnknown(objxml, serOpt,
                                         argts.at(var).getAttribute("type"));
        }
      ...
      if (clas->getClassName() == "") {
        throw ComponentHandlerException(
            "bean does not exist or is not regsitered\n", retValue);
      }
      Method meth = clas->getMethod(methodName, argus);
      if (meth.getMethodName() == "") {
        throw ComponentHandlerException(
            "service does not exist for the bean or the bean does not exist or "
            "is not regsitered\n\n",
            retValue);
        ...
        if (returnType == "void" || returnType == "") {
          ...
          if (returnType == "int") {
            ...
            retValue =
                ("<return:int>" + CastUtil::fromNumber(retv) + "</return:int>");
          } else if (returnType == "long") {
            long retv;
            reflector.invokeMethod<long>(&retv, _temp, meth, valus, true);
            retValue = ("<return:long>" + CastUtil::fromNumber(retv) +
                        "</return:long>");
          } else if (returnType == "long long") {
            long long retv;
            reflector.invokeMethod<long long>(&retv, _temp, meth, valus, true);
            retValue = ("<return:longlong>" + CastUtil::fromNumber(retv) +
                        "</return:longlong>");
          } else if (returnType == "float") {
            float retv;
            reflector.invokeMethod<float>(&retv, _temp, meth, valus, true);
            retValue = ("<return:float>" + CastUtil::fromFloat(retv) +
                        "</return:float>");
          } else if (returnType == "double") {
            double retv;
            reflector.invokeMethod<double>(&retv, _temp, meth, valus, true);
            retValue = ("<return:double>" + CastUtil::fromDouble(retv) +
                        "</return:double>");
          } else if (returnType == "string") {
            std::string retv;
            reflector.invokeMethod<std::string>(&retv, _temp, meth, valus,
                                                true);
            retValue = ("<return:string>" + retv + "</return:string>");
          } else if (returnType != "") {
            void *retobj =
                reflector.invokeMethodUnknownReturn(_temp, meth, valus, true);
            int serOpt = SerializeBase::identifySerOption(returnType);
            std::string oxml =
                ser.serializeUnknown(retobj, serOpt, returnType, NULL);
            retValue = ("<return:" + returnType + ">" + oxml +
                        "</return:" + returnType + ">");
          }
        }
