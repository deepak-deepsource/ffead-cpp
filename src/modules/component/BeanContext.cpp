...
BeanContext RemoteComponentInt::getContext() { return this->cntxt; }
...
BeanContext::~BeanContext() {}
std::string BeanContext::getHost() const { return host; }

void BeanContext::setHost(const std::string &host) { this->host = host; }

int BeanContext::getPort() const { return port; }

void BeanContext::setPort(const int &port) { this->port = port; }
...
void *BeanContext::invoke(const std::string &name,
                          std::vector<GenericObject> args,
                          const std::string &bname, const std::string &rettyp) {
  void *retval = NULL;
  if (client.isConnected()) {
    XMLSerialize ser;
    std::string argus;
    if (args.size() > 0) {
      for (unsigned int var = 0; var < args.size(); ++var) {
        ...
        argus += "<argument type=\"" + rettyp + "\">" +
                 ser.serializeUnknown(args.at(var).getPointer(), serOpt, rettyp,
                                      NULL) +
                 "</argument>";
      }
    }
    std::string call = "<service name=\"" + name + "\" beanName=\"" + bname +
                       "\" lang=\"c++\" returnType=\"" + rettyp + "\"><args>" +
                       argus + "</args></service>";
