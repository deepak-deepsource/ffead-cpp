...
public:
  ...
  static std::string registerService(
      const std::string &name,
      const Service
          &service); // will return a unique identification for this service
  static bool unRegisterService(
      const std::string &id); // unregister will require the unique id
  static std::vector<std::string>
  getServices(const std::string &); // return a list of available services
