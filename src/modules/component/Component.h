...
class Component {
  void *invokeServiceInternal(const std::string &serviceName,
...
public:
  ...
  void setDetails(); // Details about the file/db or ldap connecn and parameters
                     // ConnectionSettings
  ...
  bool getAjaxAvail() const { return ajaxAvail; }

  void setAjaxAvail(const bool &ajaxAvail) { this->ajaxAvail = ajaxAvail; }

  bool getWebsAvail() const { return websAvail; }

  void setWebsAvail(const bool &websAvail) { this->websAvail = websAvail; }

  bool getMethinvAvail() const { return methinvAvail; }

  void setMethinvAvail(const bool &methinvAvail) {
    this->methinvAvail = methinvAvail;
  }

  bool getAutoTranx() const { return autoTranx; }

  void setAutoTranx(const bool &autoTranx) { this->autoTranx = autoTranx; }

  bool getThrdPerReq() const { return thrdPerReq; }

  void setThrdPerReq(const bool &thrdPerReq) { this->thrdPerReq = thrdPerReq; }

  int getThrdPoolSize() const { return thrdPoolSize; }

  void setThrdPoolSize(const int &thrdPoolSize) {
    this->thrdPoolSize = thrdPoolSize;
  }

  int getDbconpoolSize() const { return dbconpoolSize; }

  void setDbconpoolSize(const int &dbconpoolSize) {
    this->dbconpoolSize = dbconpoolSize;
  }

  std::string getAuthFrom() const { return authFrom; }

  void setAuthFrom(const std::string &authFrom) { this->authFrom = authFrom; }

  ConnectionSettings getAuthDets() const { return authDets; }

  void setAuthDets(const ConnectionSettings &authDets) {
    this->authDets = authDets;
  }

  bool getAuthMode() const { return authMode; }

  void setAuthMode(const bool &authMode) { this->authMode = authMode; }

  std::vector<std::string> getProtocols() const { return protocols; }

  void setProtocols(const std::vector<std::string> &protocols) {
    this->protocols = protocols;
  }

  std::vector<std::string> getAllwList() const { return allwList; }

  void setAllwList(const std::vector<std::string> &allwList) {
    this->allwList = allwList;
  }

  std::vector<std::string> getBlkdList() const { return blkdList; }

  void setBlkdList(const std::vector<std::string> &blkdList) {
    this->blkdList = blkdList;
  }

  bool getSession() const { return session; }

  void setSession(const bool &session) { this->session = session; }
