...
std::string execTest(strVecVec testCases, int var, std::string hdr) {
  ...
  if (testCases[var].size() >= 4) {
    ...
    if (testCases[var][0] == "N" || testCases[var][0] == "n") {
      std::cout << "Request " << counter << " " << request << " was Skipped"
                << std::endl;
      skipped++;
      return retHdrVal;
    }
    ...
    data += " HTTP/1.1\r\nHost: " + ip + ":" +
            CastUtil::lexical_cast<std::string>(port) +
            "\r\nUser-Agent: Program\r\n";
    ...
    if (content.length() > 0) {
      data += "content-length: " +
              CastUtil::lexical_cast<std::string>((int)content.length()) +
              "\r\n";
    }
    if (cookies != "") {
      data += "cookie: " + cookies + "\r\n";
    ...
    if (!client->connectionNB(ip, port)) {
      std::cout << "Unable to connect to server at " << ip << ":" << port
                << std::endl;
      delete client;
      return retHdrVal;
    }
    ...
    while (!client->isReady(0)) {
      ...
      c += 100;
      if (c > 5000) {
        std::cout << "Timedout waiting for response after 5 seconds"
                  << std::endl;
        exit(0);
      }
    }
    ...
    if (res.getStatusCode() == responseCode) {
      if (respCntType != "") {
        if (res.getHeader("content-type").find(respCntType) == 0) {
          ss.clear();
          ss = "Test " + CastUtil::lexical_cast<std::string>(counter) + " " +
               request + " was Successfull, Response Time = " +
               CastUtil::lexical_cast<std::string>(millis) + "ms" +
               debugContentValue;
          passedFlag = true;
        } else {
          ss.clear();
          ss = "Test " + CastUtil::lexical_cast<std::string>(counter) + " " +
               request + " Failed, Response Time = " +
               CastUtil::lexical_cast<std::string>(millis) + "ms" +
               ", Expected ContentType = " + respCntType +
               ", Actual ContentType = " + res.getHeader("content-type");
          passedFlag = false;
        }
        done = true;
      }
      if (!done) {
        ...
        if (file != "") {
          ...
          if (myfile.is_open() && cntlen != "" &&
              myfile.tellg() == CastUtil::lexical_cast<int>(cntlen)) {
            ss.clear();
            ss = "Test " + CastUtil::lexical_cast<std::string>(counter) + " " +
                 request + " was Successfull, Response Time = " +
                 CastUtil::lexical_cast<std::string>(millis) + "ms" +
            ...
            ss.clear();
            ss = "Test " + CastUtil::lexical_cast<std::string>(counter) + " " +
                 request + ", Invalid Content Length, Response Time = " +
                 CastUtil::lexical_cast<std::string>(millis) + "ms" +
                 debugContentValue;
            passedFlag = false;
          }
        } else if ((file == "" && fileCntlen == "") ||
                   (fileCntlen != "" && fileCntlen == cntlen)) {
          ss.clear();
          ss = "Test " + CastUtil::lexical_cast<std::string>(counter) + " " +
               request + " was Successfull, Response Time = " +
               CastUtil::lexical_cast<std::string>(millis) + "ms" +
      ...
      ss = "Test " + CastUtil::lexical_cast<std::string>(counter) + " " +
           request + " Failed, Response Time = " +
           CastUtil::lexical_cast<std::string>(millis) + "ms" +
           ", Expected Status = " + responseCode +
           ", Actual Status = " + res.getStatusCode();
      passedFlag = false;
    }
...
int main() {
  ...
  std::cout << "Server SSL Enabled - "
            << CastUtil::lexical_cast<std::string>(sslEnabled) << std::endl;
  if (sslEnabled) {
  ...
  std::cout << "Total Tests = " << total - 1 << ", Passed = " << passed
            << ", Failed = " << failed << ", Skipped = " << skipped
            << ", Time taken = " << timerc.elapsedMilliSeconds() << "ms"
            << std::endl;
  ...
  for (int var = 1; var < 2; var++) {
    ...
    if (cd == pd) {
      std::cout << "Failed Date header cache validation pd -> " << pd
                << " cd -> " << cd << std::endl;
      return 1;
    } else {
      std::cout << "Passed Date header cache validation pd -> " << pd
                << " cd -> " << cd << std::endl;
    }
  }
